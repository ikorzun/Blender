...
// ⚠️⚠️ 125, А НЕ 25 — И ЭТО РЕШЕНИЕ ВЛАДЕЛЬЦА, НЕ ЗАМЕР (2026-08-09).
// Дословно: «достаточно Б, мы сильно много перестраховываем».
// ⛔ НЕ ЧИТАТЬ КАК ПОДТВЕРЖДЁННОЕ ИЗМЕРЕНИЕМ. Константа стояла с пометкой
// «ждёт живых данных» по правилу «правка по ЗАМЕРУ, а не по опасению».
// Данных не появилось — появилось ПРОДУКТОВОЕ РЕШЕНИЕ, и это третья,
// отдельная категория. Кто придёт крутить число назад, должен видеть, что
// спорит не с замером, а со словом владельца.
// ЦЕНА НАЗВАНА ВЛАДЕЛЬЦУ И ПРИНЯТА: сдерживание в реалистичном случае
// («обогнать верхнего игрока со 100 000») падает с ~часа до ~13 минут.
// ⛔ И ГРАНИЦА ЭТОЙ ФРАЗЫ: «сильно много перестраховываем» сказано про ЭТО
// ОДНО число. Остальные защиты (частота, подпись, seq, скрытие) остаются
// на замере — мандата ослаблять их нет.
const GROW_PER_S = 125;     // было 25 (честный пик игры ~9 ед/с)
const GROW_BASE  = 2000;    // «за один присест» — уровень целиком
// ⚠️ ЧИСЛА В README ВЫВОДЯТСЯ ОТСЮДА, А НЕ ПЕРЕПИСЫВАЮТСЯ РУКАМИ: время
// догона = (баланс − GROW_BASE) / GROW_PER_S. Меняешь константу — числа
// в доке обязаны быть пересчитаны той же формулой (на копиях числа этот
// проект обжигался трижды за неделю).
