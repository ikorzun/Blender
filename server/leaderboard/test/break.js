...
const SABOTAGE = [
  ...
  { name: 'the rung is compared by the score alone again (ties across a hundredth boundary)',
    find: 'if (rs > row.s || (rs === row.s && ru <= row.u)) { base = (i + 1) * LADDER_STEP; bound = [rs, ru]; }',
    repl: 'if (rs >= row.s) { base = (i + 1) * LADDER_STEP; bound = [rs, ru]; }',
    expect: 'TIES across the hundredth boundary' },
  { name: '/me drops the snapshot time',
    find: 'rank: exactRank, exact: 1, t: snap ? snap.t : 0,',
    repl: 'rank: exactRank, exact: 1,',
    expect: '/me CARRIES t' },
  { name: '/me reads the OLD ladder key (numeric rungs served after a deploy)',
    find: "const snap = await readSnap(env, 'ladder2');\n  const ladder = (snap && snap.v) || [];",
    repl: "const snap = await readSnap(env, 'ladder');\n  const ladder = (snap && snap.v) || [];",
    expect: 'THE HOUR AFTER A DEPLOY' },
