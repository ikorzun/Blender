...
function seed(env, n) {
  ...
  st.run('gidseedhidden', KEY, 'ShouldNotAppear', 1, 999999, t0, 1, t0, 2);  // 2 = ровно то, что пишет /admin/hide
...
(async () => {
    ...
    if (u.pathname.startsWith('/_dev/')) {
      ...
      try {
        ...
        if (u.pathname === '/_dev/rewind') {
          // ⚠️ ЗАЧЕМ: окно частоты 20 с — каждая вторая отправка с одного id
          // даёт 429, и разработка экрана встаёт на ровном месте.
          // ⛔ ВТОРАЯ ПРИЧИНА УМЕРЛА ВМЕСТЕ С МЕХАНИКОЙ. Здесь стояло
          // предупреждение про возрастной потолок («свежей строке разрешено
          // ~2000, выше — прячем из общей таблицы»), и оно было самым ценным
          // текстом файла: снаружи это выглядело как «строка пропала, клиент
          // сломан». Потолок снят решением владельца 2026-08-09 — снимаю и
          // предупреждение, чтобы следующий не искал несуществующую болезнь.
          // `c` перематываем по-прежнему: пусть строка выглядит взрослой.
          ...
          return json(rs, { ok: 1, sec,
            note: 'часы строк отмотаны назад: открывает окно частоты (20 с) досрочно' });
        }
  ...
  console.log('');
  console.log('⚠️ СЧЁТ ПРИНИМАЕТСЯ КАК ПРИСЛАН: потолка доверия и авто-скрытия больше');
  console.log('     НЕТ (решение владельца 2026-08-09). Строка исчезает из общей таблицы');
  console.log('     только от ручного /admin/hide — в сиде такая одна, ShouldNotAppear');
  console.log('     со счётом 999999: увидели её в топе — сломан фильтр f = 0.');
  console.log('⚠️ Единственный ответ 429, который остался, — окно частоты 20 с.');
  });
})();
