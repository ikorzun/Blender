...
const CASES = [
  ['self-check: a comment edit', "// THE SITE WORKER — blendo.monster (2026-09-09-c).", "// THE SITE WORKER — blendo.monster (2026-09-09-c). (an empty sabotage)", 0],
  ['the www/http redirect dropped', "if (url.hostname === WWW || (url.hostname === APEX && url.protocol === 'http:')) {", "if (false) {", 3],
  ['the media not delegated to the slicer (200 to a Range)', "if (MEDIA.test(url.pathname)) return film.fetch(request, env);", "", 4],
  ['the cache policy dropped', "res.headers.set('Cache-Control', STATIC_DAY.test(url.pathname) ? 'public, max-age=86400' : 'no-cache');", "", 3],
