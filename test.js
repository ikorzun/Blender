...
`;
  ...
  // ответы сервера мы задаём сами — включая отказ по частоте.
  {
    ...
    // ===== ТОЧКА ВХОДА: ТРИ АВАТАРА ВСЕГДА (слово владельца 2026-08-10) =====
  // ⚠️ Проверяется САМЫЙ ХУДШИЙ случай — данных НЕТ (адрес ведёт в никуда):
  // раньше при пустом `top()` справа не рисовалось НИЧЕГО, и блок выглядел
  // сломанным ровно тогда, когда таблица ещё мала.
  // ⚠️⚠️ И ВТОРАЯ ПОЛОВИНА, БЕЗ КОТОРОЙ АССЕРТ ПУСТ: раньше узкие экраны
  // ПРЯТАЛИ третий и второй аватар (замер: на 320 заголовку не хватало 65px).
  // Значит мало «три в разметке» — нужно «три ВИДИМЫ и заголовок ЦЕЛИКОМ»,
  // иначе возврат `display:none` прошёл бы молча.
  const аватары = [];
  for (const w of [320, 359, 375, 393]){
    await lbPage.setViewportSize({ width:w, height:852 });
  ...
  console.log('вход/аватары:', JSON.stringify(аватары));
  expect(аватары.every(a => a.видимых === 3),
    '⚠️ ВХОД В ТАБЛИЦУ: ТРИ аватара видимы на ВСЕХ ширинах, даже когда данных нет (' +
    аватары.map(a => a.ширина + '→' + a.видимых).join(', ') + ')');
  expect(аватары.every(a => a.текстЦеликом && a.гориз === 0),
    '⚠️ ВХОД В ТАБЛИЦУ: заголовок не обрезан и нет горизонтальной прокрутки — ' +
    'три аватара помещаются СТОПКОЙ, а не ценой текста (' +
    аватары.map(a => a.ширина + '→' + (a.текстЦеликом ? 'цел' : 'ОБРЕЗАН') + '/' + a.гориз).join(', ') + ')');
    ...
    try {
      ...
      const out = { чернила:Math.round(ink.width), бокс:Math.round(box.width),
                    ...
                    видимыхАватаров:Array.prototype.filter.call(
                      document.querySelectorAll('#msLbeAvs img'),
