...
const HUD_FLOOR = { day: 1.30, night: 12.5 };   // белок глаза против неба (день — ПИН, см. выше)
  ...
  // ===== ПАРТИЯ ВЛАДЕЛЬЦА 2026-08-20: НЕТ КРАСНОГО ВЕРХА, НЕТ СУНДУКА,
  // НАСТРОЙКИ НАД ГЛАЗАМИ, КАРТОЧКА ГЛАЗ ПРОЗРАЧНА =====
  {
    ...
    // ── МЕНЮ: настройки НАД глазами, карточка глаз БЕЗ ФОНА
    await пв.click('#pauseBtn', { force: true });
    // ⚠️ ЖДЁМ ФАКТ ОТКРЫТИЯ, А НЕ ЧАСЫ: под нагрузкой полного прогона
    // фиксированная пауза меряет часы стенда, а не состояние страницы.
    await пв.waitForFunction(() => {
    ...
    const меню = await пв.evaluate(() => {
      const н = document.querySelector('.ms-settings').getBoundingClientRect();
      const г = document.querySelector('.ms-play').getBoundingClientRect();
      const cs = getComputedStyle(document.querySelector('.ms-play'));
      const чис = c => (String(c).match(/[\d.]+/g) || []).map(Number);
      const a = чис(cs.backgroundColor);
      // ⚠️ РИСУНОК ГЛАЗ ≠ ЕГО БОКС: SVG с `viewBox 240×120` вписывается в бокс
      // с сохранением пропорций, поэтому «глаза заполняют высоту» надо мерить
      // по РИСУНКУ (бокс флекс растягивает всегда, и по нему было бы зелено
      // даже при графике 240×120 посреди пустоты — ровно то, на что смотрел
      // владелец).
      const св = document.querySelector('.ms-eyes').getBoundingClientRect();
      const k = Math.min(св.width / 240, св.height / 120);
      return { настройкиВыше: н.bottom <= г.top, нТоп: Math.round(н.top), гТоп: Math.round(г.top),
               // ⚠️⚠️ САНИТАР ВЫРОЖДЕНИЯ: у скрытого элемента getBoundingClientRect
               // отдаёт НУЛИ, и `0 <= 0` истинно — «настройки выше глаз» было бы
               // зелено на сборке, где не отрисован НИ ОДИН из двух блоков.
               обаВидны: н.height > 40 && г.height > 40 && н.width > 40 && г.width > 40,
               высН: Math.round(н.height), высГ: Math.round(г.height),
               белок: (() => { const b = document.querySelector('.ms-eyes .ms-eye-w');
                 const c = b && getComputedStyle(b);
                 const st = document.querySelectorAll('#msEyeVol stop');
                 const кромка = st.length ? getComputedStyle(st[st.length - 1]).stopColor : null;
                 return c ? { заливка: c.fill, кромка, стопов: st.length,
                              обводкаНет: c.stroke === 'none' || !parseFloat(c.strokeWidth) } : null; })(),
               зрачокЗалитГрадиентом: (() => { const z = document.getElementById('msPupL');
                 const c = z && getComputedStyle(z);
                 return !!c && c.fill.indexOf('url(') === 0; })(),
               боксГлаз: [Math.round(св.width), Math.round(св.height)],
               рисунокГлаз: [Math.round(240 * k), Math.round(120 * k)],
               доляВысоты: +(120 * k / св.height).toFixed(3),
               фон: cs.backgroundColor, картинка: cs.backgroundImage, тень: cs.boxShadow };
    });
    await пв.close();
    console.log('меню:', JSON.stringify(меню));
    expect(меню.настройкиВыше && меню.обаВидны,
      '⚠️⚠️ БЛОК НАСТРОЕК СТОИТ НАД БЛОКОМ С ГЛАЗАМИ, МОБИЛЬНАЯ ПОЛОВИНА (' +
      JSON.stringify(меню) + '). Слово владельца 2026-08-20. Диверсия — вернуть ' +
      '`.ms-play { order:2 }` / `.ms-settings { order:3 }` в мобильном блоке. ' +
      '⚠️ `обаВидны` — НЕ украшение: у неотрисованного блока rect нулевой, и ' +
      'условие «низ настроек не ниже верха глаз» выродилось бы в `0 <= 0`');
    // ⛔ ПРЕЖНИЙ АССЕРТ «ФОНА НЕТ ВОВСЕ» ПЕРЕЕХАЛ, А НЕ УДАЛЁН: слово владельца
    // «100% прозрачный фон» (2026-08-20) сужено его же словом того же дня
    // «на размере 700 и меньше цвет фона блока 16% белого» (2026-08-20-в).
    // Прозрачность теперь свойство ШИРОКИХ экранов и проверяется ниже, на 701.
    expect(меню.фон === 'rgb(255, 255, 255)' && меню.картинка === 'none' &&
           меню.тень === 'none',
      '⚠️⚠️ НА УЗКИХ (≤700) У КАРТОЧКИ ГЛАЗ ПОДЛОЖКА 100% БЕЛАЯ (' +
      JSON.stringify({ фон: меню.фон, картинка: меню.картинка, тень: меню.тень }) +
      '). Слово владельца 2026-08-20-в; его же «16% белого» прожило одну правку ' +
      'и отменено им по живому экрану. ⚠️ Картинка и тень проверяются В ТОМ ЖЕ ' +
      'ассерте: градиент-заливка и `box-shadow: inset` — два других способа дать ' +
      'блоку фон, и «подложка белая» было бы истинно поверх любого из них');
    expect(!!меню.белок && /url\(/.test(меню.белок.заливка) &&
           меню.белок.кромка === 'rgb(231, 237, 248)' && меню.белок.обводкаНет &&
           !меню.зрачокЗалитГрадиентом,
      '⚠️⚠️ У БЕЛКОВ ОБЪЁМ РАДИАЛЬНЫМ ГРАДИЕНТОМ, САМЫЙ ТЁМНЫЙ ЦВЕТ #E7EDF8 (' +
      JSON.stringify(меню.белок) + ', зрачок залит градиентом: ' +
      меню.зрачокЗалитГрадиентом + '). Решение владельца 2026-08-20-д: «обводка ' +
      'у глаз плохо, добавь им простого объёма радиальным градиентом, где самый ' +
      'тёмный цвет E7EDF8». ⛔ Прежняя чёрная обводка прожила одну правку, и ' +
      '«обводки НЕТ» проверяется В ТОМ ЖЕ ассерте: иначе правка «оставим и то, ' +
      'и другое» прошла бы молча, а владелец забраковал именно обводку. ⚠️ КРОМКА АССЕРТИТСЯ ' +
      'ЦВЕТОМ, А НЕ ФАКТОМ ГРАДИЕНТА: владелец назвал КОНКРЕТНЫЙ потолок ' +
      'темноты, и «градиент есть» было бы истинно у любой другой пары цветов. ' +
      '⚠️ ВТОРАЯ ПОЛОВИНА — КОНТРОЛЬ: зрачок в том же SVG тоже `circle`, и ' +
      'селектор без класса залил бы градиентом и его');
    expect(меню.рисунокГлаз[0] === меню.боксГлаз[0] && меню.доляВысоты >= 0.8,
      '⚠️⚠️ НА УЗКИХ (≤700) ГЛАЗА ЗАПОЛНЯЮТ ОБЛАСТЬ ПО ВЫСОТЕ (' +
      JSON.stringify({ бокс: меню.боксГлаз, рисунок: меню.рисунокГлаз, доля: меню.доляВысоты }) +
      '). Слово владельца 2026-08-20-в. До правки рисунок был жёстко 240×120 в ' +
      'боксе 240×192 — 62% высоты и 72px пустоты. ⚠️ ПОРОГ 0.8, А НЕ 1.0, И ЭТО ' +
      'НЕ ПОБЛАЖКА: у конструкции глаз пропорция 2:1, и на самой узкой карточке ' +
      '(390: бокс 326×192) впритык по высоте потребовало бы ширины 384 > 326 ' +
      'либо обрезки белков. Замер: 390 -> 0.85, от ~440 и шире -> ровно 1.0. ' +
      'Диверсия — вернуть `.ms-eyes { width:240px }`: доля падает до 0.62');
    ...
    const десктоп = async (вы) => {
      ...
      const о = await д.evaluate(() => {
        ...
        return { глазаПервые: г.bottom <= пр.top + 1, профильВышеНастроек: пр.bottom <= н.top + 1,
    ...
    expect(д900.глазаПервые && д900.профильВышеНастроек && д900.всеВидны &&
           д1200.глазаПервые && д1200.профильВышеНастроек && д1200.всеВидны,
      '⚠️⚠️ НА ДЕСКТОПЕ ПОРЯДОК: ГЛАЗА -> ПРОФИЛЬ -> НАСТРОЙКИ (' +
      JSON.stringify({ '900': д900, '1200': д1200 }) + '). Слово владельца ' +
      '2026-08-20-в. ⛔ ЭТО ОТМЕНЯЕТ десктопную половину правила «настройки над ' +
      'глазами» ТОГО ЖЕ ДНЯ — на узких она в силе и проверяется выше. Диверсия — ' +
      'вернуть в `grid-template-areas` строку "prof prof chead" первой');
    ...
    // ── ГРАНИЦА 700: «700 и меньше» — значит 700 ещё узкий, 701 уже широкий
    // ⚠️⚠️ ГРАНИЦУ ДЕРЖАТ ДВА АССЕРТА ПО ОБЕ СТОРОНЫ ЧИСЛА. Одна половина
    // («на 390 фон 16%») истинна и у сборки, где фон 16% ВЕЗДЕ, включая десктоп,
    // — то есть правка «покрасил всё подряд» прошла бы молча.
    const край = async (ш) => {
      const к = await browser.newPage({ viewport: { width: ш, height: 900 } });
      к.on('pageerror', e => errors.push('PAGEERROR(край700): ' + e.message));
      await к.goto('file://' + PAGE_FILE + '?dev=1');
      await к.waitForFunction(() => window.__game && window.__game.alive() > 0, { timeout: 60000 });
      await к.evaluate(() => { window.__game.skipIntro(); });
      await к.click('#pauseBtn', { force: true });
      await к.waitForFunction(() => {
        const m = document.getElementById('mainScreen');
        return m && getComputedStyle(m).visibility !== 'hidden' &&
               document.querySelector('.ms-play').getBoundingClientRect().height > 0;
      }, { timeout: 15000 });
      const о = await к.evaluate(() => {
        const b = document.querySelector('.ms-eyes .ms-eye-w');
        const c = b && getComputedStyle(b);
        const кс = getComputedStyle(document.querySelector('.ms-play'));
        return { фон: кс.backgroundColor, выравнивание: кс.justifyContent,
                 объём: c ? (/url\(/.test(c.fill) ? 'градиент' : c.fill) : 'нет' };
      });
      await к.close();
      return о;
    };
    const к700 = await край(700), к701 = await край(701);
    console.log('граница 700:', JSON.stringify({ '700': к700, '701': к701 }));
    expect(к700.фон === 'rgb(255, 255, 255)' && /, 0\)$/.test(к701.фон) &&
           к700.объём === 'градиент' && к701.объём === 'rgb(255, 255, 255)' &&
           к700.выравнивание === 'flex-end' && к701.выравнивание === 'flex-start',
      '⚠️⚠️ ГРАНИЦА РОВНО ПО СЛОВУ ВЛАДЕЛЬЦА «700 И МЕНЬШЕ» (' +
      JSON.stringify({ '700': к700, '701': к701 }) + '): на 700 подложка белая и ' +
      'у белков объём, на 701 подложка прозрачная и белок плоско-белый. ⚠️ Ради этого ' +
      'соседнее правило подняли с `min-width:700` на `701` — один пиксель, но ' +
      'иначе на 700 действовало бы обратное сказанному. ⚠️⚠️ ВЫРАВНИВАНИЕ В ЭТОМ ' +
      'ЖЕ АССЕРТЕ — ЕДИНСТВЕННЫЙ ПРИЗНАК, КОТОРЫЙ ВИДИТ САМ ЭТОТ ПИКСЕЛЬ: фон, ' +
      'объём и размер глаз на 700 выходят одинаковыми при обеих редакциях ' +
      'порога (мой блок стоит НИЖЕ в файле и побеждает по порядку), и диверсия ' +
      '`701 -> 700` не роняла НИЧЕГО — проверено прогоном. Разводит их только ' +
      '`justify-content`: широкое правило ставит flex-start, база — flex-end. ' +
      '⚠️ ОБЪЁМ ПРОВЕРЯЕТСЯ ПО ' +
      'ОБЕ СТОРОНЫ: он нужен ТОЛЬКО на белой подложке, а шире 700 глаза стоят на ' +
      'небе — градиент там был бы правкой, которой владелец не просил');
  }
