...
page.on('response', (r) => {
  ...
  const мягкий = await page.evaluate(async () => {
    const g = window.__game, sl = ms => new Promise(r => setTimeout(r, ms));
    g.setLevel(5); g.regen(); g.skipIntro(); await sl(700);
    for (let i = 0; i < 400 && g.alive() > 10; i++){
      if (!g.autoMatch()){ g.shake(); await sl(650); }
      await sl(60);
    }
    const снять = (встрясок) => { const l = g.level();
      l.shakes = встрясок; l.adShakes = 0; g.forceRefresh();
      return +g.cfg.matchRadius.toFixed(2); };
    return { живых: g.alive(), при3: снять(3), при1: снять(1), при0: снять(0), назад3: снять(3) };
  });
  console.log('мягкий эндшпиль:', JSON.stringify(мягкий));
  expect(мягкий.живых <= 10 && мягкий.при3 < 10 && мягкий.при1 > 10 && мягкий.при0 > 10 &&
         мягкий.назад3 < 10,
...
`;
  ...
  // список, заданное место), а не наследует от соседей.
  {
    ...
    expect(деск.зазор === 4 && деск.аватаров === 3,
      '⚠️ ТОЧКА ВХОДА, ДЕСКТОП: зазор между аватарами 4 и их ТРИ (' +
