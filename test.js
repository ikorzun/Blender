...
`;
  ...
  // НАСТРОЙКИ НАД ГЛАЗАМИ, КАРТОЧКА ГЛАЗ ПРОЗРАЧНА =====
  {
    ...
    expect(дельта <= 4,
      ...
      'слой `uGrind` наливал верх красным до (217,93,91) при покое (110,134,255) — ' +
    ...
    const меню = await пв.evaluate(() => {
      ...
      return { настройкиВыше: н.bottom <= г.top, нТоп: Math.round(н.top), гТоп: Math.round(г.top),
               ...
               фонПрозрачен: cs.backgroundImage === 'none' && (a.length > 3 ? a[3] === 0 : false),
    ...
    expect(меню.фонПрозрачен && меню.тень === 'none',
      '⚠️⚠️ У КАРТОЧКИ ГЛАЗ ФОНА НЕТ ВОВСЕ (' + JSON.stringify(меню) + '). Слово ' +
      'владельца «100% прозрачный фон». ⚠️ Тень проверяется В ТОМ ЖЕ ассерте: ' +
      '`box-shadow: inset` рисует ВНУТРИ бокса, и на прозрачной карточке она ' +
      'читалась бы белой рамкой — то есть тем самым фоном, который просили убрать');

    // ── ДЕСКТОПНОЕ ПЛЕЧО ТОГО ЖЕ ПРАВИЛА
    // ⚠️⚠️ ПОРЯДОК ЗАДАН В ДВУХ МЕСТАХ: `order` в @media (max-width:1079px) и
    // `grid-template-areas` в @media (min-width:1080px). Замер на одном
    // вьюпорте закрывает ПОЛОВИНУ правила — ровно та ошибка, что была поймана
    // на мобильных свитчерах («мобильная правка неотличима от правки, снёсшей
    // ползунки везде»).
    // ⚠️⚠️ И ВТОРОЕ, БЕЗ ЧЕГО ПЛЕЧО БЫЛО БЫ ПОЛОВИНЧАТЫМ: на десктопе мало
    // ПОРЯДКА, надо ещё проверить, КОМУ ДОСТАЛСЯ ГИБКИЙ РЯД. Перестановка имён
    // в `grid-template-areas` НЕ двигает `1fr` — он привязан к позиции ряда.
    // Забыв переставить `grid-template-rows`, мы отдали весь запас высоты
    // НАСТРОЙКАМ: на 1440×1200 белая карточка раздувалась до 557px при 184px
    // содержимого, а карточка глаз садилась на свой `min-height`.
    // Признак выбран БЕЗ МАГИЧЕСКИХ ЧИСЕЛ: меряем ДВА вьюпорта разной высоты и
    // требуем, чтобы с ростом экрана рос ИМЕННО Play, а настройки стояли.
    const десктоп = async (вы) => {
      ...
      const о = await д.evaluate(() => {
        const н = document.querySelector('.ms-settings').getBoundingClientRect();
        const г = document.querySelector('.ms-play').getBoundingClientRect();
        return { настройкиВыше: н.bottom <= г.top + 1,
                 обаВидны: н.height > 40 && г.height > 40,
                 высН: Math.round(н.height), высГ: Math.round(г.height),
    ...
    expect(д900.настройкиВыше && д900.обаВидны && д1200.настройкиВыше && д1200.обаВидны,
      '⚠️⚠️ БЛОК НАСТРОЕК СТОИТ НАД БЛОКОМ С ГЛАЗАМИ И НА ДЕСКТОПЕ (' +
      JSON.stringify({ '900': д900, '1200': д1200 }) + '). Диверсия — вернуть в ' +
      '`grid-template-areas` строку "play play grid" выше "sets sets grid". ' +
      'Без этого плеча правка была бы проверена только на 390 px, то есть ' +
      'наполовину');
    ...
    expect(ростГ > 200 && Math.abs(ростН) <= 8,
      ...
      'блоком с глазами». Диверсия — вернуть `grid-template-rows:auto 1fr auto ' +
      'auto`, НЕ трогая areas: порядок останется верным, а весь запас высоты ' +
      'заберёт белая карточка настроек. ⚠️ Признак ОТНОСИТЕЛЬНЫЙ намеренно — ' +
      'абсолютные высоты зависят от кегля и содержимого настроек');
  }
