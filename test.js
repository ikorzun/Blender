...
page.on('response', (r) => {
  ...
  const мягкий = await мэ.evaluate(async () => {
    const g = window.__game, sl = ms => new Promise(r => setTimeout(r, ms));
    // ⚠️⚠️ СЦЕНА СТАВИТСЯ ПРЯМО, А НЕ ИГРОЙ, И ЭТО НЕ ЛЕНЬ. Окно, где мягкая
    // ступень вообще различима, — РОВНО ОДНО ЗНАЧЕНИЕ счётчика: 9 (жёсткая
    // ступень забирает всё <= 8, мягкая объявлена как < 10). Матч снимает
    // ПАРУ, то есть счётчик идёт через два и окно проскакивается через раз, а
    // пока бот пытается доиграть, финал успевает доесть кучу — три попытки
    // подряд приезжали к нулю живых. Поэтому `cull` ставит ровно 9.
    ...
    const итог = { счётных: счёт(), живых: g.alive(),
  ...
  expect(мягкий.счётных === 9 && мягкий.при3 < 10 && мягкий.при1 > 10 && мягкий.при0 > 10 &&
         мягкий.назад3 < 10,
    '⚠️⚠️ ЭНДШПИЛЬ: при <10 живых радиус снимается, КОГДА встрясок не осталось, и ' +
...
`;
  ...
  const bgSeams = await page.evaluate(async () => {
    ...
    // ⚠️⚠️ ПРИВОДИМ СОСТОЯНИЕ САМИ (канон): диагностика поймала на этой
    // странице ОТКРЫТЫЙ winOverlay из ранней секции — кромки 5-й редакции
    // честно темнели от него, и три ассерта краснели не на механике, а на
    // унаследованной обстановке. Закрываем БОЕВЫМ путём (те же show/hide).
    ...
    // ⚠️ ДИАГНОСТИКА В СНИМКЕ: какие .overlay видимы по computed — кромки
    // темнеют от ЛЮБОГО из них, и без имён красный тут нечитаем.
    const видимые = () => Array.prototype.filter.call(document.querySelectorAll('.overlay'),
      e => getComputedStyle(e).display !== 'none').map(e => e.id || e.className);
  ...
  // ⚠️⚠️ ПЯТАЯ РЕДАКЦИЯ (слово владельца 2026-08-12: «мне не нужны чёрные
  // полосы, мне нужен цвет каждого экрана от верхнего края до нижнего»).
  // Канал доказан ЧЁРНОЙ редакцией как экспериментом: полосы = живой семплинг
  // ФИКС-БАРОВ у кромок. Прежний «закон загрузки» был НЕВЕРЕН — полоса не
  // менялась потому, что НЕ МЕНЯЛИСЬ БАРЫ, а не потому, что Safari замер.
  // Стражи снова переезжают: каждый экран ВЕДЁТ кромки, у каждого ассерта
  // вторая половина «И НЕ чёрный» — возврат 4-й редакции не пройдёт молча.
  const ЧЁРНЫЙ = /^rgb\(0, ?0, ?0\)$/;
  expect(bgSeams.closed.html.col === bgSeams.skyTop && !ЧЁРНЫЙ.test(bgSeams.closed.html.col),
    '⚠️ КРОМКА В ИГРЕ: фон документа = ВЕРХНИЙ стоп неба (5-я редакция), не чёрный (' +
    bgSeams.closed.html.col + ', небо ' + bgSeams.skyTop + ')');
  expect(bgSeams.open.меню === true,
    'САНИТАР: меню действительно открылось кликом по паузе (иначе следующий ассерт мерит пустоту)');
  expect(bgSeams.open.html.col === bgSeams.menuRGB && bgSeams.open.html.col !== bgSeams.closed.html.col,
    '⚠️ КРОМКА В МЕНЮ: меню ВЕДЁТ кромку — фон документа стал фоном меню (' +
    JSON.stringify(bgSeams.open) + ', меню ' + bgSeams.menuRGB + ')');
  expect(bgSeams.back.html.col === bgSeams.closed.html.col,
    '⚠️ КРОМКА ПОСЛЕ МЕНЮ: вернулась к небу — тон меню не залип (' +
    bgSeams.closed.html.col + ' -> ' + bgSeams.open.html.col + ' -> ' + bgSeams.back.html.col + ')');
  ...
  expect(bgSeams.closed.мета === bgSeams.closed.html.col &&
         bgSeams.open.мета === bgSeams.open.html.col && bgSeams.open.мета !== bgSeams.closed.мета &&
         bgSeams.back.мета === bgSeams.closed.мета,
    '⚠️⚠️ КРОМКА, ВТОРОЙ КАНАЛ (мета theme-color): в ЛОКСТЕПЕ с фоном на каждом ' +
    'экране и ДВИЖЕТСЯ вместе с ним (' + bgSeams.closed.мета + ' -> ' +
  ...
  expect(кромка.игроваяRgb === кромка.верх && кромка.игроваяRgb !== '0,0,0',
    '⚠️ КРОМКА: в игре мета = ВЕРХНИЙ стоп неба (5-я редакция), не чёрный (' +
  ...
  expect(кромка.метаRgb === кромка.msRgb && кромка.вМеню.мета !== кромка.доМеню,
    '⚠️ КРОМКА: меню ВЕДЁТ мету — она стала фоном меню (' +
  ...
  expect(кромка.после.мета === кромка.доМеню,
    '⚠️ КРОМКА: после меню мета вернулась к небу — переход в обе стороны (' +
  ...
  expect(подложка.картинка === 'none' && подложка.цвет === 'rgb(' + подложка.верх.replace(/,/g, ', ') + ')',
    '⚠️ КРОМКА: подложка — плоский ВЕРХНИЙ стоп неба, без градиента (' +
    подложка.цвет + ' / верх ' + подложка.верх + ')');
  // ⚠️⚠️ БАРЫ — ДОКАЗАННЫЙ КАНАЛ ПОЛОС: верх и низ РАЗНЫЕ в игре (градиент),
  // тёмный фейд оверлея темнит оба, возврат чист. Оверлей открываем БОЕВЫМ
  // путём (winScreen ходит теми же show/hide, что checkEnd).
  const край = await chPage.evaluate(async () => {
    const g = window.__game, sl = ms => new Promise(r => setTimeout(r, ms));
    const игра = g.chromeInfo();
    g.winScreen(true); await sl(250);
    const фейд = g.chromeInfo();
    g.winScreen(false); await sl(250);
    const после = g.chromeInfo();
    return { игра, фейд, после };
  });
  console.log('кромка/бары:', JSON.stringify(край));
  expect(край.игра.верх === край.игра.небоВерх && край.игра.низ === край.игра.небоНиз &&
         край.игра.верх !== край.игра.низ,
    '⚠️⚠️ БАРЫ: в игре верхний бар = верхний стоп, нижний = нижний, и они РАЗНЫЕ ' +
    '— кромки несут ГРАДИЕНТ экрана (' + JSON.stringify(край.игра) + ')');
  expect(край.фейд.верх === '10,14,22' && край.фейд.низ === '10,14,22' &&
         край.фейд.оверлеев === 1 && край.после.верх === край.игра.верх,
    '⚠️⚠️ БАРЫ: тёмный фейд оверлея темнит ОБЕ кромки и отпускает при закрытии (' +
    JSON.stringify({ фейд: край.фейд.верх, после: край.после.верх }) + ')');
  // ⛔⛔ СКРУГЛЕНИЕ СНЯТО СЛОВОМ ВЛАДЕЛЬЦА 2026-08-12 («убирай вариант
  // скругления вью»). Страж СОХРАНЁН С ОБРАТНЫМ УТВЕРЖДЕНИЕМ, а не удалён
  // (правило канона: снос мешающего ассерта пропустил бы возврат молча).
  // Формульные ассерты радиуса умерли ВМЕСТЕ с механикой — они стерегли
  // правило, которого больше нет.
  const скругл = await chPage.evaluate(() => getComputedStyle(document.body).clipPath);
  await chPage.setViewportSize({ width: 1280, height: 800 });
