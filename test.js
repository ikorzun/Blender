...
`;
  ...
  // ⚠️ НАКЛАДОК ТЕПЕРЬ ДВЕ (слово владельца 2026-08-19 «сделай этот эффект на
  // горящем объекте, но огонь вокруг не убирай»): пламя снаружи и РАСКАЛЁННАЯ
  // КОРКА под ним. Обе живут в одном списке `fires` и гаснут вместе.
  expect(!!ign && ign.fires === 2, 'ОГОНЬ: зажёгся с коркой (' + JSON.stringify(ign) + ')');
  await new Promise(r => setTimeout(r, 300));
  const burn = await fxPage.evaluate(() => {
    const p = window.__game.fxProbe();
    return { fires: p.fires, детей: p.kidsTotal, макс: p.kidsMax };
  });
  // ⚠️⚠️ НЕСУЩЕЕ ЗДЕСЬ — «ДЕТЕЙ РОВНО У ОДНОГО ПРЕДМЕТА», а не их число: смысл
  // ассерта в том, что горячее висит НАКЛАДКАМИ и не трогает материал (иначе
  // просочится в портреты коллекции — грабля двух потребителей uVeil). Число
  // накладок переехало 1 -> 2 вместе с правилом: пламя + корка.
  expect(burn.fires === 2 && burn.детей === 1 && burn.макс === 2,
    '⚠️ ОГОНЬ — ДВЕ НАКЛАДКИ-РЕБЁНКА (пламя + раскалённая корка), А НЕ ПРАВКА ' +
    'МАТЕРИАЛА: горит ' + burn.fires + ', предметов с детьми ' + burn.детей +
    ', детей на предмете ' + burn.макс);
  // ЛЕДЯНАЯ КОРКА: тот же эффект в ПРЕСЕТНОЙ синей палитре, на замороженном
  // предмете ПОД льдом (слово владельца 2026-08-19 «замороженный объект внутри
  // с коркой как была в пресете»).
  // ⚠️⚠️ ОНА В ДРУГОМ СПИСКЕ, И ЭТО НЕСУЩЕЕ: пламя гасит `extinguishAll` по
  // таймеру горения, и попади ледяная корка туда же — она умирала бы вместе с
  // чужим огнём. Ассерт это и ловит: пока горит огонь, у льда своя жизнь.
  const холод = await fxPage.evaluate(async () => {
    const g = window.__game;
    for (let ур = 14; ур <= 22; ур++){
      g.setLevel(ур); g.regen(); g.skipIntro();
      await new Promise(r => setTimeout(r, 1200));
      const f = (g.frozenInfo() || [])[0];
      if (!f) continue;
      const p1 = g.fxProbe();
      g.ignite();                       // поджигаем ДРУГОЙ предмет
      await new Promise(r => setTimeout(r, 250));
      const p2 = g.fxProbe();
      g.extinguish();                   // гасим огонь — лёд обязан выжить
      await new Promise(r => setTimeout(r, 900));
      const p3 = g.fxProbe();
      return { глыб: 1, доОгня: p1.chills, приОгне: p2.chills, послеТушения: p3.chills,
               огнейПослеТушения: p3.fires, тон: g.heatTune('cold') };
    }
    return null;
  });
  console.log('ледяная корка:', JSON.stringify(холод));
  expect(холод && холод.доОгня >= 1 && холод.послеТушения === холод.доОгня &&
         холод.огнейПослеТушения === 0,
    '⚠️⚠️ ЛЕДЯНАЯ КОРКА ЖИВЁТ СВОЕЙ ЖИЗНЬЮ: тушение чужого огня её не снимает ' +
    '(' + JSON.stringify(холод) + '). Диверсия — положить её в список `fires`');

  // ⚠️ ПАЛИТРА КОРКИ — ВЫБОР ВЛАДЕЛЬЦА («корка в палитре пламени», 2026-08-19).
  // Ему показывали и синий вариант исходного пресета; страж утверждает именно
  // выбранный, иначе тихая замена на синий прошла бы незамеченной. Значения
  // сняты С БОЕВОЙ СБОРКИ, а не написаны по памяти: это FIRE_DEEP/HOT/CORE.
  const тонКорки = await fxPage.evaluate(() => window.__game.heatTune());
  expect(тонКорки.cool === '#ff470f' && тонКорки.mid === '#ffd83f' && тонКорки.hot === '#fff7cc',
    '⚠️ КОРКА В ПАЛИТРЕ ПЛАМЕНИ (выбор владельца): она обязана читаться как ' +
    '«предмет раскалился», а не как отдельный эффект (' + JSON.stringify(тонКорки) + ')');
