...
page.on('response', (r) => {
  ...
  expect(pill.фон === 'rgba(0, 0, 0, 0)' && pill.тень === 'none' &&
    pill.круг === 'rgb(255, 255, 255)' && pill.диаметр === 64 && pill.иконка === 32 &&
    pill.зазор === '7px' && pill.плюс === 'rgb(255, 255, 255)',
    'ПОБЕДА: у награды только белый круг с иконкой, «+1» белый, подложки и ' +
    'свечения НЕТ (' + JSON.stringify(pill) + ')');
...
const HUD_FLOOR = { day: 1.30, night: 12.5 };   // белок глаза против неба (день — ПИН, см. выше)
  ...
  // проверяется через API (`level().shakes`), а показывает число DOM.
  {
    ...
    const снятьShake = () => стр.evaluate(() => {
      ...
      const h = b.querySelector('svg.shake-hand'), hint = document.getElementById('hintBtn');
      ...
      return { тег: b.tagName, ария: b.getAttribute('aria-label'),
               ...
               естьКисть: !!h, вьюбокс: h && h.getAttribute('viewBox'),
               ...
               инк: h ? (() => { const g = h.querySelector('g'); const b2 = g.getBoundingClientRect();
                 return [+(b2.left - r.left).toFixed(2), +(b2.top - r.top).toFixed(2),
                         +b2.width.toFixed(2), +b2.height.toFixed(2)]; })() : null,
    ...
    expect(ш0.тег === 'BUTTON' && ш0.ария === null &&
           ...
           ш0.фон === 'rgba(0, 0, 0, 0)' && ш0.фонКартинка === 'none' &&
           ш0.позиция === 'relative' && ш0.события === 'auto',
      '⚠️⚠️ SHAKE — ИКОНКА-КИСТЬ 64×64 БЕЗ ПОДЛОЖКИ, А НЕ ТЁМНАЯ ПИЛЮЛЯ (' +
      JSON.stringify({ тег: ш0.тег, ария: ш0.ария, имя: ш0.имяИзСодержимого,
                       размер: [ш0.ширина, ш0.высота],
                       фон: ш0.фон, картинка: ш0.фонКартинка, позиция: ш0.позиция,
                       события: ш0.события }) + '). Макет владельца 886:3949, ' +
    ...
    expect(ш0.естьКисть && ш0.вьюбокс === '14 11 60 54.5823' && ш0.pAR === null &&
           ш0.путей === 18 && ш0.градиентовК === 8 && ш0.масокК === 1 && градЗаливок === 8 &&
           близко(ш0.кисть[0], 2) && близко(ш0.кисть[1], 4.02) &&
           близко(ш0.кисть[2], 60) && близко(ш0.кисть[3], 54.58) &&
           // рисунок обязан НАКРЫВАТЬ рамку и вылезать симметрично (внешняя
           // обводка-маска даёт законные ~2px с каждой стороны)
           обнимает(ш0.инк, ш0.кисть, 2.6) &&
           ш0.теньКисти === 'drop-shadow(rgba(0, 0, 0, 0.12) 0px 3px 12px)' &&
           ш0.цвета['rgb(255, 255, 255)'] === 1 && ш0.цвета['rgb(0, 0, 0)'] === 8 &&
           ш0.дублиId.length === 0 && ш0.битыеСсылки.length === 0,
      '⚠️⚠️ КИСТЬ — ОБЪЁМНЫЙ ИНЛАЙНОВЫЙ SVG: 18 путей, 8 ГРАДИЕНТОВ, МАСКА, ' +
      'ТЕНЬ И БЕЗ МИНЫ DEV MODE (' +
      JSON.stringify({ есть: ш0.естьКисть, вьюбокс: ш0.вьюбокс, pAR: ш0.pAR,
                       путей: ш0.путей, лист: ш0.кисть, цвета: ш0.цвета }) + '). ' +
      '⚠️⚠️ ЦВЕТА ЧИТАЮТСЯ COMPUTED-СТИЛЕМ (11 чёрных путей + 1 белый), И ЭТО ' +
      'НЕСУЩЕЕ: именно «кисть несёт СВОИ цвета из ассета» служит ОБОСНОВАНИЕМ ' +
      'вывода кнопки из системного правила суток. Перекрась её чужим ' +
      '`svg path { fill: var(--btn-fg) }` — обоснование станет ложным, а ' +
      'размеры и число путей останутся верными. ' +
      '⚠️⚠️ `preserveAspectRatio === null` — ГЛАВНАЯ ПОЛОВИНА ЭТОГО АССЕРТА: ' +
      'ассет приехал из Figma с `preserveAspectRatio="none"`, и с ним кисть ' +
      'растянулась бы под любой чужой бокс. Диверсия — вернуть атрибут: ' +
      'размеры листа останутся теми же, красным станет ровно это поле. ' +
      '⛔⛔ ТЕНЬ У КИСТИ СНЯТА 2026-08-21-ж (владелец убрал её в макете), И ' +
      'ВМЕСТЕ С НЕЙ УШЁЛ СРЕЗ ВЬЮБОКСА: поле под тень (5/1/5/9) в экспорте ' +
      'больше не оставляют, вьюбокс снова с нуля. Прежние пины `5 1 …` и ' +
      '`drop-shadow(… .08)` краснели бы на исправной сборке. ⚠️ Замер ИНКА ' +
      'ОСТАЁТСЯ несущим: при неверном вьюбоксе рамка `<svg>` осталась бы на ' +
      'месте, а рисунок внутри уехал бы и уменьшился, чего рамка не видит. ' +
      '⚠️ И `тень === none` — ТОЖЕ ПИН: вернуть её мимо макета так же легко, ' +
      'как потерять. ⚠️ ЦВЕТ КОНТУРА СМЕНИЛСЯ С ЧЁРНОГО НА `#2E3F61` — им же ' +
      'теперь нарисована лупа; обе иконки ушли с чистого чёрного. ' +
      '⚠️ 12 путей — санитар «геометрия на месте»: пустой <svg> дал бы верные ' +
      'вьюбокс и рамку при полностью невидимой кисти. ⚠️ Лист 80×72.7758 на ' +
      'y=4 — числа ноды, а не «примерно как на скрине»');

    expect(близко(ш0.бейджX, 28.5) && близко(ш0.бейджY, 40) &&
           ш0.бейджВ === 24 && ш0.бейджШ === 24 &&
           ш0.бФон === 'rgb(192, 255, 71)' && ш0.бЦвет === 'rgb(74, 113, 0)' &&
           ш0.бРадиус === '32px' && ш0.бКегль === '14px/800' &&
           ш0.бТень === 'drop-shadow(rgba(0, 0, 0, 0.16) 0px 2px 4px)' &&
           ш0.бейджX + ш0.бейджШ <= 64,
      '⚠️⚠️ ЛАЙМОВЫЙ БЕЙДЖ В ТОЧКЕ 28.5/40 ПО МАКЕТУ (' +
      JSON.stringify({ точка: [ш0.бейджX, ш0.бейджY], размер: [ш0.бейджШ, ш0.бейджВ],
      ...
      '⚠️ ШИРИНА 24 АССЕРТИТСЯ ТОЛЬКО У ОДНОЗНАЧНОГО ЧИСЛА: она ' +
      'выведена из падинга 7 и `min-width`, то есть от шрифта не зависит. ' +
    ...
    const лупа = await стр.evaluate(() => {
      ...
      const m = b.querySelector('svg.tip-mag');
    ...
    expect(лупа.тег === 'BUTTON' && лупа.ария === 'Hint' && !/iconBtn/.test(лупа.классы) &&
           ...
           лупа.фон === 'rgba(0, 0, 0, 0)' && лупа.фонКартинка === 'none' &&
           лупа.события === 'auto' && лупа.позиция === 'relative',
      '⚠️⚠️ ПОДСКАЗКА — ИКОНКА-ЛУПА 64×64 БЕЗ КРУГЛОЙ ПОДЛОЖКИ (' +
      JSON.stringify({ тег: лупа.тег, классы: лупа.классы, размер: [лупа.ширина, лупа.высота],
                       фон: лупа.фон, события: лупа.события }) + '). Макет 887:4051. ' +
    ...
    expect(лупа.естьЛупа && лупа.вьюбокс === '14.48 11.58 50.492 55' && лупа.pAR === null &&
           лупа.путей === 3 && лупа.градиентов === 2 && градЛупы === 1 &&
           блЛ(лупа.лупаБокс[0], 6.75) && блЛ(лупа.лупаБокс[1], 4) &&
           блЛ(лупа.лупаБокс[2], 50.48) && блЛ(лупа.лупаБокс[3], 55) &&
           обнимает(лупа.инк, лупа.лупаБокс, 2) &&
           лупа.тень === 'drop-shadow(rgba(0, 0, 0, 0.12) 0px 2.895px 11.579px)' &&
           лупа.цвета['rgb(255, 255, 255)'] === 2,
      '⚠️⚠️ ЛУПА — ОБЪЁМНЫЙ ИНЛАЙНОВЫЙ SVG: 3 пути, ГРАДИЕНТ СТЕКЛА, ТЕНЬ ' +
      'И БЕЗ МИНЫ DEV MODE (' +
      JSON.stringify({ вьюбокс: лупа.вьюбокс, pAR: лупа.pAR, путей: лупа.путей,
                       бокс: лупа.лупаБокс, инк: лупа.инк, тень: лупа.тень,
                       градиентов: лупа.градиентов }) + '). ' +
      '⛔⛔ ЛУПА ПЕРЕРИСОВАНА 2026-08-21-л (нода 891:4205): плоский контур ' +
      'сменился ОБЪЁМНЫМ — 3 пути, радиальный + линейный градиент; тень своя ' +
      '(dy 2.89, σ 5.79, 12%); размер 50.492×55 в точке (6.75, 4) против ' +
      'прежних 56×61 в (4, 1). Все прежние пины краснели бы на исправной ' +
      'сборке. ⚠️ ВЬЮБОКС СРЕЗАН ПО АРТУ (`14.48 11.58 …`): родной равен ' +
      'ОБЛАСТИ ФИЛЬТРА, а тень мы отдали CSS — SVG-фильтр при срезе обрезался ' +
      'бы вьюпортом. ⚠️ СЧЁТ ПЛОСКИХ ЦВЕТОВ ЗАМЕНЁН СЧЁТОМ ГРАДИЕНТОВ: заливки ' +
      'теперь ссылки `url(#…)`, проверять «3 тёмно-синих пути» больше нечего. ' +
      '⚠️ ИНК ПРОВЕРЯЕТСЯ ВЛОЖЕННОСТЬЮ С ДОПУСКОМ И СИММЕТРИЕЙ, а не ' +
      'равенством: у арта своя внешняя обводка, она законно шире рамки');

    expect(блЛ(лупа.бейджX, 4) && блЛ(лупа.бейджY, 40) &&
           лупа.бейджВ === 24 && лупа.бейджШ === 24 &&
           лупа.бФон === 'rgb(255, 165, 183)' && лупа.бЦвет === 'rgb(135, 16, 72)' &&
           лупа.бРадиус === '32px' && лупа.бКегль === '14px/800' &&
           лупа.бТень === 'drop-shadow(rgba(0, 0, 0, 0.16) 0px 2px 4px)' &&
           лупа.бейджX + лупа.бейджШ <= 64 &&
           лупа.текст === String(лупа.зарядов) && лупа.класс === '',
      '⚠️⚠️ РОЗОВЫЙ БЕЙДЖ В ТОЧКЕ 4/40 И ГОВОРИТ ПРАВДУ (' +
      ...
      '⛔⛔ ПАЛИТРА ЭТОГО БЕЙДЖА СМЕНИЛАСЬ ТРИЖДЫ ЗА СУТКИ, ДЕЙСТВУЮЩАЯ — ' +
      'РОЗОВАЯ #ffa5b7/#871048 (нода 887:4061). До неё голубая #9ce2ff/#1a6c8e ' +
      'и лаймовая #c0ff47/#4a7100 — каждый прежний пин краснел бы на исправной ' +
      'сборке, ровно поэтому цвет ассертится ЗНАЧЕНИЕМ. ⚠️ ЛАЙМ ПРИ ЭТОМ ' +
      'ОСТАЛСЯ У SHAKE (прочитана его нода 887:4038, а не предположено): пара ' +
      'СНОВА разноцветная, и это не рассинхрон. ⚠️ Оба состояния бейджа ' +
      'подсказки розовые — соседняя нода «Number» 887:4055 прочитана отдельно. ' +
      'Точка (4,40): бейдж по левому краю ЛУПЫ, а не рамки. ' +
      '⚠️⚠️ БЕЙДЖ СЛЕВА (0), А У SHAKE СПРАВА (28.5) — пара смотрит НАРУЖУ от ' +
      'центра бара. Перепутаешь стороны — обе половины останутся «в макете» ' +
      'поодиночке, а вместе будут смотреть внутрь. ⚠️ ТЕКСТ СВЕРЯЕТСЯ С ' +
    ...
    expect(наградa.есть && /tip-mag/.test(наградa.класс) &&
           ...
           наградa.позиция === 'static' && !наградa.стараяРука &&
           наградa.ширина === '32px' && наградa.высота === '32px',
