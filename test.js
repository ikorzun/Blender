...
`;
  ...
  // ⚠️ ЗАМЕР ДЕЛАЕТСЯ В ПЕРВУЮ СЕКУНДУ ПОСЛЕ ДЕЙСТВИЯ (оговорка Графики
  // 2026-07-31): красная угроза помола `uGrind` наливает ВЕРХ кадра сама по
  // себе за секунды простоя, и замер на простое ловил бы её, а не рецепт.
  {
    ...
    expect(дельта(игра.html, играК.верх) <= 2 && дельта(игра.body, играК.низ) <= 2,
      ...
      'Это прямая защита от жалобы «покраснение снизу»: краснеет только верх ' +
      'кадра, и при таком разведении красному физически некуда попасть вниз');
