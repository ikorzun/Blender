...
`;
  ...
  // ⚠️⚠️ НА ИГРОВОМ ЭКРАНЕ МЕТА НЕСЁТ НИЖНИЙ СТОП. По записи с лендинга
  // `theme-color` тонирует ТОЛЬКО НИЖНИЙ бар iOS, а верх красит фон html —
  // значит оба канала обязаны нести РАЗНЫЕ цвета. Раньше в мету шёл верхний
  // стоп, и нижняя полоса выходила тёмно-синей под малиновым низом кадра.
  expect(кромка.игроваяRgb === кромка.низ && кромка.низ !== кромка.верх,
    '⚠️ КРОМКА: в игре мета = НИЖНЕМУ стопу неба, а не верхнему (' + кромка.доМеню +
    ' → ' + кромка.низ + ', верх ' + кромка.верх + ')');
