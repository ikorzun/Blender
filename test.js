...
(async () => {
  ...
  // ⚠️⚠️ ЧЕРНИЛА ТЕПЕРЬ ШИРЕ РАМКИ, И ЭТО НОРМА (макеты 891:4205 / 891:4199).
  // До объёмных иконок рисунок ЗАПОЛНЯЛ рамку `<svg>`, и стражи требовали
  // ТОЧНОГО равенства «инк == бокс». У нового арта есть ВНЕШНЯЯ обводка-маска,
  // она законно выходит за рамку на ~2px у кисти и ~1.45 у лупы — точное
  // равенство краснело бы на исправной сборке.
  // Проверяем то, что осталось несущим: рисунок НАКРЫВАЕТ рамку (не съехал и
  // не сжался) и вылезает СИММЕТРИЧНО (перекос = уехавший вьюбокс).
  // ⚠️ `доп` — потолок вылета, а не «допуск на глазок»: больше него означает,
  // что рисунок раздулся, и страж обязан покраснеть.
  const обнимает = (инк, бокс, доп) =>
    инк && бокс &&
    инк[0] <= бокс[0] + 0.1 && инк[1] <= бокс[1] + 0.1 &&
    инк[0] + инк[2] >= бокс[0] + бокс[2] - 0.1 &&
    инк[1] + инк[3] >= бокс[1] + бокс[3] - 0.1 &&
    инк[0] >= бокс[0] - доп && инк[1] >= бокс[1] - доп &&
    инк[0] + инк[2] <= бокс[0] + бокс[2] + доп &&
    инк[1] + инк[3] <= бокс[1] + бокс[3] + доп &&
    Math.abs((бокс[0] - инк[0]) - ((инк[0] + инк[2]) - (бокс[0] + бокс[2]))) < 0.2 &&
    Math.abs((бокс[1] - инк[1]) - ((инк[1] + инк[3]) - (бокс[1] + бокс[3]))) < 0.2;
...
page.on('response', (r) => {
  ...
  // ⛔⛔ СТРАЖ ПЕРЕЕХАЛ ЗА ПРАВИЛОМ 2026-08-21-м: владелец сказал «убери
  // обводку, оставь только иконку в окружности, а +1 сделай белым». Подложка
  // пилюли и лаймовое внутреннее свечение (оба по ноде 779:1114) СНЯТЫ, «+1»
  // побелел. Прежние пины краснели бы на исправной сборке.
  // ⚠️ ПРОВЕРЯЕМ ОТСУТСТВИЕ ЯВНО, А НЕ МОЛЧАНИЕМ: «фон прозрачный И тени нет»
  // — иначе ассерт зеленел бы и у сборки, где подложку вернули другим цветом.
  // ⚠️ КРУГ, ДИАМЕТР, ИКОНКА И ЗАЗОР ОСТАЮТСЯ ПОД ПИНОМ: их он не отменял, и
  // ровно они — то самое «оставь только иконку в окружности».
  // ⛔⛔ ПИЛЮЛЯ СНОВА СО СТЕКЛОМ (нода 779:1115, слово владельца 2026-08-21-р
  // «обнови стиль блока с подсказкой»). ⛔ ОТМЕНЯЕТ его же слово 2026-08-21-м
  // «убери обводку, оставь только иконку в окружности» — оно прожило день, и
  // прежний ассерт («фон прозрачный И тени нет, круг белый 64, иконка 32»)
  // краснел бы теперь на ИСПРАВНОЙ сборке по четырём полям сразу.
  // ⚠️ БЕЛОГО КРУГА БОЛЬШЕ НЕТ ВОВСЕ — слот 54 стоит прямо на стекле, поэтому
  // `круг` обязан быть ПРОЗРАЧНЫМ: без этого поля возврат круга прошёл бы
  // зелёным, а он и есть то, что владелец снял.
  expect(pill.фон === 'rgba(255, 255, 255, 0.08)' &&
    pill.круг === 'rgba(0, 0, 0, 0)' && pill.диаметр === 54 && pill.иконка === 43 &&
    pill.плюс === 'rgb(255, 255, 255)',
    '⚠️⚠️ ПОБЕДА: награда — СТЕКЛЯННАЯ пилюля (белый 8%, радиус 64) со слотом ' +
    'лупы 54 и белым «+1», БЕЗ белого круга (' + JSON.stringify(pill) + '). ' +
    '⛔ Тот же стеклянный стиль, что у строки таблицы выше — обе ноды пришли ' +
    'одним сообщением именно поэтому');
  // ═══ ШАПКА ЭКРАНА ПОБЕДЫ ПО ПЕРЕРИСОВАННОЙ НОДЕ 778:732 (2026-08-21-н) ═══
  // ⚠️⚠️ ДО ЭТОЙ СЕКЦИИ ШАПКУ НЕ СТЕРЁГ НИКТО. В сьюте не было ни одного
  // упоминания `winCleaned`/`winLevel`/`winTime`: перерисовка прошла бы зелёной
  // В ОБЕ СТОРОНЫ — и правильная, и возврат к прежней раскладке.
  // ⚠️ ПОРЯДОК В СТРОКЕ УТВЕРЖДАЕМ ГЕОМЕТРИЕЙ (уровень ЛЕВЕЕ времени и оба на
  // одной высоте), а не разметкой: `flex-direction:row-reverse` переставил бы
  // их молча, оставив DOM прежним.
  // ⚠️ «НА ОДНОЙ ВЫСОТЕ» — ЭТО ЦЕНТРЫ, А НЕ ВЕРХА: у надписей разный набор
  // букв («Level 1» с выносным элементом, «0:01» с двоеточием), и верхние
  // кромки их чернил законно не совпадают.
  const винШапка = await page.evaluate(() => {
    const у = ид => { const e = document.getElementById(ид); if (!e) return null;
      const b = e.getBoundingClientRect(), cs = getComputedStyle(e);
      return { текст: e.textContent, кегль: cs.fontSize, залив: cs.fill, штрих: cs.stroke,
               толщ: cs.strokeWidth, лево: Math.round(b.left), право: Math.round(b.right),
               центрY: Math.round(b.top + b.height / 2) }; };
    const ур = у('winLevel'), вр = у('winTime'), ст = у('winCleaned'), сч = у('winScore');
    // ⚠️⚠️ РОВНОСТЬ СТРОКИ МЕРИМ ПО РАМКАМ SVG, А НЕ ПО ЧЕРНИЛАМ. Первая
    // редакция сравнивала центры <text> и краснела на ИСПРАВНОЙ сборке:
    // «Level 1» и «0:25» — разные наборы букв, их чернильные боксы законно не
    // совпадают по высоте (замер: 88 против 92). Ровняет строку flex по
    // рамкам, они и есть решающая величина.
    const рамка = сел => { const e = document.querySelector(сел); if (!e) return null;
      const b = e.getBoundingClientRect();
      return { центрY: Math.round(b.top + b.height / 2), в: Math.round(b.height) }; };
    // пересечение чернил стикера и времени — их коллизию ловили кадром
    const пересеч = (a, b) => { if (!a || !b) return null;
    ...
    const ряд = document.querySelector('.win-toprow');
    const рр = ряд ? ряд.getBoundingClientRect() : null;
    const кол = document.getElementById('winWrap').getBoundingClientRect();
    const рамкаСчёта = document.querySelector('.win-score').getBoundingClientRect();
    return { уровень: ур, время: вр, стикер: ст, счёт: сч,
             рядЛево: рр ? Math.round(рр.left) : null,
             рядПраво: рр ? Math.round(рр.right) : null,
             колЛево: Math.round(кол.left), колПраво: Math.round(кол.right),
             подСчётом: !!рр && Math.round(рр.top) >= Math.round(рамкаСчёта.bottom) - 2,
             точек: document.querySelectorAll('.win-dot').length,
             обводки: (() => { const о = document.getElementById('winScoreOut');
               const в = document.getElementById('winScore');
               if (!о || !в) return null;
               const co = getComputedStyle(о), cv = getComputedStyle(в);
               return { низ: [co.stroke, co.strokeWidth], верх: [cv.stroke, cv.strokeWidth],
                        текстСовпал: о.textContent === в.textContent }; })(),
             рамкаУр: рамка('.win-level'), рамкаВр: рамка('.win-time'),
             стикерХВремя: пересеч('winCleaned', 'winTime'),
             рядов: document.querySelectorAll('.win-toprow').length,
             старыхРядов: document.querySelectorAll('.win-cleanrow').length };
  });
  console.log('победа/шапка:', JSON.stringify(винШапка));
  // ⛔⛔ СТРОКА «Level N • MM:SS» ПЕРЕЕХАЛА ВНИЗ ШАПКИ И СЖАЛАСЬ ДО 24
  // (нода 891:4314 внутри перерисованного фрейма 891:4251, слово владельца
  // 2026-08-21-р). Было: строка СВЕРХУ, кегль 28, `space-between` во всю
  // ширину. Прежние пины кегля и кромок краснели бы на исправной сборке.
  // ⚠️ НИЖНЕЕ ПОЛОЖЕНИЕ — ГЛАВНОЕ НОВОЕ УТВЕРЖДЕНИЕ: строка обязана стоять
  // ПОД счётом. Без него «кегль 24 и по центру» истинно и у прежней верхней
  // раскладки, то есть переезд прошёл бы зелёным в обе стороны.
  expect(винШапка.рядов === 1 && винШапка.старыхРядов === 0 &&
    винШапка.уровень && винШапка.время &&
    винШапка.уровень.кегль === '24px' && винШапка.время.кегль === '24px' &&
    винШапка.уровень.право < винШапка.время.лево &&
    винШапка.подСчётом && винШапка.точек === 1 &&
    винШапка.рамкаУр && винШапка.рамкаВр &&
    винШапка.рамкаУр.центрY === винШапка.рамкаВр.центрY &&
    // ⚠️⚠️ КРАЯ — ГЛАВНАЯ ПОЛОВИНА, И ОНА НАПИСАНА ПО ДЫРЕ, НАЙДЕННОЙ В ПЕРВОЙ
    // РЕДАКЦИИ. Без них диверсия `justify-content:flex-start` проходила ЗЕЛЁНОЙ:
    // надписи вставали рядом слева, но «уровень левее времени», «центры равны»
    // и «кегли 28» оставались верны — то есть страж пропускал ровно ту
    // раскладку, которую перерисовка и убрала. Несущее в ноде 891:4314 —
    // `space-between` во всю ширину колонки, значит стеречь надо КРОМКИ.
    // ⚠️ Допуск 12 — под обводку (она рисуется наружу от буквы) и округление;
    // `row-reverse` ловится этой же парой даром.
    // ⛔ ПИН КРОМОК СНЯТ ВМЕСТЕ С `space-between`: строка больше не растянута
    // во всю ширину, она СОБРАНА К ЦЕНТРУ с зазором 11. Проверяем ровно это —
    // ряд УЖЕ колонки и стоит по её середине.
    винШапка.рядЛево !== null && винШапка.рядПраво - винШапка.рядЛево < 300 &&
    Math.abs((винШапка.рядЛево + винШапка.рядПраво) / 2 -
             (винШапка.колЛево + винШапка.колПраво) / 2) <= 2,
    '⚠️⚠️ ПОБЕДА/ШАПКА: уровень и время — ОДНА строка по краям, оба кеглем 28 ' +
    '(нода 891:4314, слово владельца «обнови финальный экран»). ⛔ ОТМЕНЯЕТ ' +
    'прежнюю раскладку: уровень 50px по центру + время 28px наклонённое рядом ' +
    'с надписью. Диверсия — вернуть `.win-cleanrow`: краснеет счётчик старых ' +
    'рядов (' + JSON.stringify(винШапка) + ')');
  expect(винШапка.стикер && винШапка.стикер.текст === 'SAVED' &&
    винШапка.стикер.залив === 'rgb(255, 255, 255)' &&
    винШапка.стикер.штрих === 'rgb(192, 255, 71)' &&
    винШапка.стикер.кегль === '57.2px',
    '⚠️⚠️ ПОБЕДА/СТИКЕР: слово «SAVED», заливка БЕЛАЯ, обводка лайм #c0ff47 ' +
    '(нода 891:4315). ⛔ Четвёртая редакция этой надписи: было «CLEANED» и ' +
    'заливка чёрная. ⚠️ КЕГЕЛЬ 57.2 НЕ ОТ НОДЫ, А ОТ СЛОВА ВЛАДЕЛЬЦА +30% ' +
    '(2026-07-28) — нода просит меньше и уже проигрывала этому слову однажды; ' +
    'менять его на число ноды нельзя без нового слова (' +
    JSON.stringify(винШапка.стикер) + ')');
  // ⛔⛔ ДВЕ ОБВОДКИ У ОЧКОВ И ЗВЕЗДЫ (слово владельца 2026-08-21-р: «черная
  // на 6 пикселей и белая на 12»). Прежняя одиночная 11.7 отменена.
  // ⚠️ ЧИТАЕМ ОБА СЛОЯ И ИХ ПОРЯДОК: нижний обязан быть БЕЛЫМ и ТОЛЩЕ, верхний
  // ЧЁРНЫМ и тоньше. Поменяй их местами — «две обводки есть» осталось бы
  // истинным, а видно было бы только белую.
  // ⚠️ И ТЕКСТ СЛОЁВ ОБЯЗАН СОВПАДАТЬ: count-up пишет цифру каждый кадр, и
  // второй писатель отставал бы от первого — обводка «плыла» бы за числом.
  // ⚠️ computed отдаёт УДВОЕННУЮ толщину (`--otl × 2` спрятано в `.otext`):
  // 12 и 24 при заданных 6 и 12 — это норма, а не ошибка.
  expect(винШапка.обводки &&
    винШапка.обводки.низ[0] === 'rgb(255, 255, 255)' &&
    винШапка.обводки.низ[1] === 'calc(24px)' &&
    винШапка.обводки.верх[0] === 'rgb(0, 0, 0)' &&
    винШапка.обводки.верх[1] === 'calc(12px)' &&
    винШапка.обводки.текстСовпал,
    '⚠️⚠️ ПОБЕДА/СЧЁТ: ДВЕ ОБВОДКИ — белая 12 снизу, чёрная 6 поверх, и оба ' +
    'слоя несут ОДИН текст (' + JSON.stringify(винШапка.обводки) + ')');
  expect(винШапка.счёт && винШапка.счёт.залив === 'rgb(255, 231, 48)',
    '⚠️ ПОБЕДА/СЧЁТ: заливка `#ffe730` (нода 891:4329). ⛔ Прежний `#ffc800` ' +
    'тоже был словом владельца (2026-08-12) — сменился ОТТЕНОК, а не механизм: ' +
    'градиент `#gWinScore` как был мёртв, так и остался (' +
    JSON.stringify(винШапка.счёт) + ')');
  expect(винШапка.стикерХВремя === 0,
    '⚠️⚠️ ПОБЕДА: ПОВЁРНУТЫЙ СТИКЕР НЕ НАЕЗЖАЕТ НА ВРЕМЯ (перекрытие по ' +
    'вертикали ' + винШапка.стикерХВремя + 'px, должно быть 0). Страж написан ' +
    'ПО ФАКТУ: первая редакция раскладки дала 10px на десктопе и 11 на ' +
    'телефоне — правый верхний угол надписи, поднятый поворотом −10.6°, влезал ' +
    'в цифры. Лечится отступом сверху у `.win-cleaned`, и это ОСОЗНАННОЕ ' +
    'отступление от наползания `mb:-24` в ноде: там надпись занимает 37% ' +
    'колонки, у нас 64% из-за кегля +30%');

  // ═══ СТРОКА «N place / on leaderboard» НА ЭКРАНЕ ПОБЕДЫ (нода 891:4297) ═══
  // ⚠️⚠️ ЭТО ВОЗВРАТ БЛОКА, КОТОРЫЙ ВЛАДЕЛЕЦ КОГДА-ТО СНЯЛ, И ПОТОМУ ОТДЕЛЬНЫЙ
  // СТРАЖ: рядом живут ДВА обратных утверждения — «врезки `#winLb` нет» и
  // «отправка счёта к ней не привязана» (ниже по файлу). Они остаются в силе:
  // снята была ТАБЛИЦА НА ТРИ СТРОКИ с соседями, а вернулась ОДНА компактная
  // строка, нарисованная 2026-08-21. Перепутать их — значит решить, что одно
  // из двух утверждений протухло, и снять чужого стража.
  // ⚠️ ВЫСОТА 72 ПИНУЕТСЯ НАРОЧНО: её держит значок 48 + паддинг 12×2, то есть
  // ТЕКСТ на неё не влияет. Данные приезжают по сети позже показа экрана, и
  // если высота начнёт зависеть от текста, приход ответа сдвинет кнопку Next
  // из-под пальца — ровно та болезнь, от которой лечили прежнюю врезку.
  const винЛБ = await page.evaluate(() => {
    const box = document.getElementById('winLbEntry');
    if (!box) return { нетУзла: true };
    const b = box.getBoundingClientRect(), cs = getComputedStyle(box);
    const список = document.getElementById('winTopList').getBoundingClientRect();
    const шапка = document.querySelector('.win-head').getBoundingClientRect();
    const зн = box.querySelector('.ms-lbe-badge');
    const мест = document.getElementById('winLbeRank');
    const мену = document.getElementById('msLbeRank');
    // ⚠️⚠️ ЧИТАЕМ РЕШАЮЩУЮ ВЕЛИЧИНУ, А НЕ ДОГАДЫВАЕМСЯ ПО ВИДУ: строка живёт
    // только когда фича таблицы включена (задан адрес сервиса). Без этого
    // страж краснел бы на сборке, где таблица законно выключена, — то есть
    // утверждал бы не то, что названо словом владельца.
    const фича = (() => { const lb = window.__lb;
      return !!(lb && lb.top && lb.me && (typeof lb.base !== 'function' || lb.base())); })();
    const мену0 = document.getElementById('msLbEntry');
    return { фича, скрытМеню: мену0 ? мену0.hidden : null,
             скрыт: box.hidden, высота: Math.round(b.height), фон: cs.backgroundColor,
             радиус: cs.borderRadius, курсор: cs.cursor,
             подШапкой: Math.round(b.top) >= Math.round(шапка.bottom),
             надСписком: Math.round(b.bottom) <= Math.round(список.top),
             значков: зн ? зн.querySelectorAll('svg').length : 0,
             видимыхЗначков: зн ? Array.prototype.filter.call(зн.querySelectorAll('svg'),
               s => getComputedStyle(s).display !== 'none').length : 0,
             своихКнопок: box.querySelectorAll('.ms-lbe-act').length,
             аватаров: (document.getElementById('winLbeAvs') || { children: [] }).children.length,
             тегСвоего: (() => { const h = document.getElementById('winLbeAvs');
               return h && h.firstElementChild ? h.firstElementChild.tagName : null; })(),
             // ⚠️ КУРСОР СВЕРЯЕМ С МЕНЮ, А НЕ С ЛИТЕРАЛОМ 'default': игровой
             // курсор задан всей странице картинкой (`image-set(url(data:…))`),
             // и любое литеральное имя тут врёт. Несущее — РАЗЛИЧИЕ: в меню
             // строка кликабельна и носит руку, на победе не кликабельна и
             // руки носить не смеет.
             курсорМеню: мену0 ? getComputedStyle(мену0).cursor : null,
             текстПобеды: мест ? мест.textContent : null,
             текстМеню: мену ? мену.textContent : null,
             экземпляров: document.querySelectorAll('.ms-lbentry').length };
  });
  console.log('победа/строка таблицы:', JSON.stringify(винЛБ));
  // ⚠️⚠️ ДВЕ ВЕТКИ, И НИ ОДНА НЕ МОЛЧИТ. Фича включена — стережём геометрию;
  // выключена — стережём, что ОБА экземпляра спрятаны СОГЛАСОВАННО. Так страж
  // не «уходит в тишину» на сборке без сервиса и ловит расхождение экранов.
  expect(!винЛБ.нетУзла && винЛБ.экземпляров === 2 &&
    винЛБ.скрыт === !винЛБ.фича && винЛБ.скрытМеню === !винЛБ.фича &&
    (!винЛБ.фича || (винЛБ.высота === 72 &&
      винЛБ.фон === 'rgba(255, 255, 255, 0.08)' && винЛБ.подШапкой && винЛБ.надСписком &&
      винЛБ.значков === 3 && винЛБ.видимыхЗначков === 1 && винЛБ.своихКнопок === 0 &&
      // ⚠️⚠️ СВЕРЯЕМ ТОЛЬКО СВОЙ АВАТАР, А НЕ «один против трёх». Первая
      // редакция требовала `аватаровМеню === 3` и ПОКРАСНЕЛА НА ИСПРАВНОЙ
      // СБОРКЕ: три аватара в меню — это ТОП, он приходит по сети, и на
      // странице сьюта ответа нет вовсе (`аватаровМеню: 0`). Число, зависящее
      // от сети, в этом ассерте было бы флейком, а не утверждением.
      // ⚠️ ЗАТО `тегСвоего === 'IMG'` НЕСУЩЕЕ: пустой слот рисуется `<i>`, и
      // «ровно один ребёнок» истинно и у заглушки — то есть без тега страж
      // зеленел бы на сборке, где аватар игрока не нашёлся.
      винЛБ.аватаров === 1 && винЛБ.тегСвоего === 'IMG' &&
      винЛБ.курсор !== винЛБ.курсорМеню)),
    '⚠️⚠️ ПОБЕДА: строка «N place / on leaderboard» стоит МЕЖДУ шапкой и списком ' +
    'предметов — СТЕКЛЯННАЯ пилюля 72px (белый 8%, радиус 64), РОВНО ОДИН ' +
    'аватар — ИГРОКА, а не первого из топа (слово владельца 2026-08-21-р ' +
    '«вместо трёх аватарок показываем только аватарку игрока»; его правило ' +
    '«всегда три» от 2026-08-05 осталось в силе ДЛЯ МЕНЮ и стережётся там же), ' +
    'ровно ОДИН видимый значок направления из ' +
    'трёх, без кнопки «Open» и без курсора-руки (нода 891:4297, слово владельца ' +
    '2026-08-21-н). ⚠️ `экземпляров === 2` — САНИТАР ПРОТИВ ПОДМЕНЫ: строка ' +
    'общая с меню, и если её переписать своей вёрсткой, число не сойдётся. ' +
    '⚠️ Три значка при одном видимом — это штамп из меню: разметку значка не ' +
    'дублируют, её копируют у первого экземпляра, и пустой штамп дал бы ноль. ' +
    '⛔ НЕ ПУТАТЬ С ВРЕЗКОЙ `#winLb`: та была таблицей на три строки и снята ' +
    '2026-08-10; утверждения о её отсутствии ниже по файлу ОСТАЮТСЯ В СИЛЕ (' +
    JSON.stringify(винЛБ) + ')');
  expect(винЛБ.текстПобеды !== null && винЛБ.текстМеню !== null &&
    винЛБ.текстПобеды === винЛБ.текстМеню,
    '⚠️⚠️ ПОБЕДА: место в строке — ТО ЖЕ ЧИСЛО, ЧТО В МЕНЮ, и сверяется ' +
    'РАВЕНСТВОМ двух экземпляров, а не литералом. Два экрана считают место ' +
    'ОДНОЙ формулой (`lbRankNow`); разойдись они — игрок увидел бы два разных ' +
    'места, и оба выглядели бы правдой (' + JSON.stringify({
      победа: винЛБ.текстПобеды, меню: винЛБ.текстМеню }) + ')');
  ...
  // ⛔⛔ ПЛАШКА «×N» СНОВА ТЁМНАЯ С БЕЛЫМ ТЕКСТОМ (нода 891:4251, слово
  // владельца 2026-08-21-р «досконально изучи и обнови вёрстку финального
  // экрана»). ⛔ ОТМЕНЯЕТ его же слово 2026-08-11 «зелёная ТЕМ ЖЕ цветом, что
  // полоска прогресса, текст чёрный» — оно прожило десять дней, и прежний
  // ассерт (РАВЕНСТВО с цветом полоски) краснел бы теперь на исправной сборке.
  // ⚠️ Равенство сменилось на ЗНАЧЕНИЯ не от лени: требование «как у полоски»
  // отменено, а новое требование — конкретная пара цветов из ноды.
  expect(винЛайаут.фонМульт === 'rgba(0, 0, 0, 0.4)' &&
         винЛайаут.текстМульт === 'rgb(255, 255, 255)' &&
         винЛайаут.фонМульт !== винЛайаут.фонПолоски,
    '⚠️ ПОБЕДА: плашка «×N» — ТЁМНАЯ rgba(0,0,0,.4) с БЕЛЫМ текстом (' +
...
`;
  ...
  const slot = await page.evaluate(async () => {
    ...
    // ⚠️⚠️ ПОДСВЕТКУ ПРОВЕРЯЕМ ЯВНО ПО ОТСУТСТВИЮ, А НЕ МОЛЧАНИЕМ. Слоя
    // `::before` с лаймовой заливкой и `blur(22px)` под предметом БОЛЬШЕ НЕТ
    // (слово владельца 2026-08-21-о). Снять правило легко, вернуть — ещё легче,
    // а на кадре крошечный ореол незаметен, пока предмет не вырастет: ровно
    // так он и прожил незамеченным до удвоения.
    const до = getComputedStyle(cb, '::before');
    const res = { подсветка: { содержимое: до.content, фон: до.backgroundColor,
                               фильтр: до.filter },
      фон: c.backgroundColor, кольцо: c.boxShadow,
  ...
  // ⛔⛔ И ПЕРЕЕХАЛ ВТОРОЙ РАЗ 2026-08-21-м: «увеличь в мобильном бонусный
  // предмет в 2 раза» — 83 → 166. ⛔⛔ И ТРЕТИЙ 2026-08-22-в: «уменьши размер
  // бонусной вещи в мобильном на 30%» — 166 → 116. Удвоение прожило сутки.
  // ⚠️ ОКНО ПУЛЬСА ПРИ ЭТОМ НЕ ТРОГАЛИ, И ЭТО ПРОВЕРКА САМОЙ ЕГО ФОРМЫ: оно в
  // ДОЛЯХ, поэтому смена размера его не двигает — ровно затем доля и заводилась.
  // Вьюпорт этой секции мобильный (390), так
  // что здесь проверяется именно мобильное число; десктопные 104 пинуются
  // ОТДЕЛЬНО, в десктопной секции ниже.
  // ⛔⛔ ОКНО ПУЛЬСА ПЕРЕВЕДЕНО В ДОЛЮ ОТ РАЗМЕРА, А НЕ В ПИКСЕЛИ. Пульс —
  // `transform:scale(1 -> 1.04)`, то есть ход РАСТЁТ ВМЕСТЕ СО СЛОТОМ:
  // 0.04·83 = 3.3 px, но 0.04·166 = 6.6 px. Прежнее окно `> 1 && < 6` было
  // в пикселях и покраснело бы на ИСПРАВНОЙ сборке сразу после удвоения —
  // страж бы не поймал поломку, а выдумал её. Границы 0.012..0.072 — ровно
  // прежние 1..6 px, поделённые на прежние 83: строгость не ослаблена,
  // просто снят литерал, который сносило при каждой смене размера.
  // ⛔ ЛАЙМОВОЙ ПОДСВЕТКИ ПОД ПРЕДМЕТОМ НЕТ (слово владельца 2026-08-21-о
  // «убери добавление зелёной подсветки фона снизу»). Читаем ТРИ признака:
  // псевдоэлемент не создан (`content:none`), заливка не лаймовая, размытия
  // нет — вернуть слой можно любым из них по отдельности.
  expect(slot.подсветка &&
    (slot.подсветка.содержимое === 'none' ||
     (!/192, 255, 71/.test(slot.подсветка.фон) && slot.подсветка.фильтр === 'none')),
    '⚠️⚠️ ЗАРЯД: ЛАЙМОВОЙ ПОДСВЕТКИ ПОД ПРЕДМЕТОМ НЕТ (' +
    JSON.stringify(slot.подсветка) + '). ⛔ ЭТО ОТМЕНА ПРЕЖНЕГО СЛОВА ВЛАДЕЛЬЦА ' +
    '«подсвечивается» (нода 829:1242): свечение жило под предметом с самого его ' +
    'появления и было незаметно, пока было ему по размеру. Заметным его сделало ' +
    'удвоение предмета 2026-08-21-м — `inset:0` растянул размытие вчетверо, и ' +
    'ореол стал зелёной заливкой правого нижнего угла экрана. ' +
    '⚠️ Страж утверждает ОТСУТСТВИЕ ЯВНО: без него возврат правила прошёл бы ' +
    'зелёным — ни один другой ассерт не читает `::before` у этой кнопки');
  const доля = slot.ход / slot.слот;
  expect(/rgba\(0, 0, 0, 0\)|transparent/.test(slot.фон) && slot.кольцо === 'none' &&
    slot.слот === 116 && slot.слот > slot.подсказка && доля > 0.012 && доля < 0.072,
    'ЗАРЯД: не кнопка, а модель 116 (нода 829:1242 +30%, затем ×2, затем −30% в мобильном), и она пульсирует ' +
    'долей 0.04 от своего размера (' + JSON.stringify(Object.assign({ доля: +доля.toFixed(4) }, slot)) + ')');
  ...
  // ⚠️⚠️ ТРЕТИЙ ГЕЙТ СТАВИТСЯ ПЕРЕД САМИМ ВЕРДИКТОМ, И ВОТ ПОЧЕМУ ОН НУЖЕН:
  // комментарий выше обещает, что «перед вердиктом стоит второй, добирающий
  // хвост» — но ХВОСТОВОЙ гейт стоит здесь, на 40% файла, а не перед
  // вердиктом. Всё, что случилось ПОСЛЕ этой строки (а это больше половины
  // прогона), в `failures` не попадало НИКОГДА: обещание канона было верным
  // по замыслу и неверным по факту. Найдено состязательным разбором
  // 2026-08-21, когда новая секция завела двух слушателей `pageerror`,
  // которые физически не могли ни на что повлиять.
  // ⚠️ ВОДЯНОЙ ЗНАК ДВИГАЕМ ЗДЕСЬ, а не считаем всё заново: иначе одни и те же
  // ошибки уехали бы в вердикт дважды.
  ...
  // правило цвета кнопок; ⛔ «инверсия Shake» вычеркнута 2026-08-21 — кнопка
  // стала иконкой без подложки и из html.night вышла). Раньше час был вписан в обе руками,
  ...
  // ⛔⛔ СТРАЖ ПЕРЕЕХАЛ ЗА ПРАВИЛОМ 2026-08-20: слово владельца «оставь только
  // дневную тему всегда». Список часов ТОТ ЖЕ (в нём обе прежние границы, 4/5 и
  // 19/20/23) — но теперь каждый обязан дать ДЕНЬ. Так страж стережёт НОВОЕ
  // правило и заодно доказывает, что ночная ветка недостижима НИ НА ОДНОМ часе,
  // включая полночные.
  // ⚠️ ФОРС-ХУК `?hour=` ЖИВ И ПРОВЕРЯЕТСЯ ОТДЕЛЬНО: без этого страж стал бы
  // тавтологией «сломанный хук всегда даёт день». Час обязан доехать — а решение
  // по нему обязано быть дневным.
  ...
  for (const h of [4, 5, 19, 20, 23]){
    ...
    expect(got.sky === 'day',
      '⚠️⚠️ ТОЛЬКО ДЕНЬ: небо на ' + h + ':00 дневное (' + got.sky + '). Диверсия — ' +
      'вернуть в `skyTimeNow` (10-stage) решение по часу');
    expect(got.night === false && got.html === false,
      '⚠️⚠️ ТОЛЬКО ДЕНЬ: тема кнопок на ' + h + ':00 дневная, класса `night` на ' +
      '`html` нет (' + JSON.stringify(got) + '). Диверсия — вернуть в `isNightSky` ' +
      '(85-hud) решение по часу');
  }
  expect(hoursSeen.every(x => (x.sky === 'night') === x.night),
    ...
    '). ⚠️ Ассерт оставлен КАК ЕСТЬ, хотя обе теперь константы: он стережёт не ' +
    'значение, а СОВПАДЕНИЕ двух функций в разных файлах — вернут ночь одной из ' +
    'них, и он покраснеет первым');
  ...
  // ⛔ ТРЕТЬЕ ТРЕБОВАНИЕ ПРИЕХАЛО 2026-08-20 И ПЕРЕПИСАЛО ИНВАРИАНТ: фон экрана
  // паузы стал ГРАДИЕНТОМ (тем же, что небо), а сам `#mainScreen` из-за этого
  // обязан быть прозрачным — свой слой градиента он иначе спрячет. Сравнивать
  // тон шапки с тоном меню стало НЕ С ЧЕМ. Новый источник тона у шапки —
  // ВЕРХНИЙ СТОП палитры: она стоит у верхней кромки, там теперь именно он.
  // ⚠️ Совпадение с КАДРОМ проверяет секция «ПОЛЯ SAFARI 26» ниже; здесь
  // остаётся то, что живёт только тут: альфа ~0 по спеке владельца и НЕ ноль
  // по рецепту хрома.
  const stickyBg = await menuPage.evaluate(() => {
    ...
    const в = чис('rgb(' + getComputedStyle(document.documentElement)
      .getPropertyValue('--sky-top-rgb').trim() + ')');
    return { шапка: ш, верхНеба: в,
             тонСовпал: ш[0] === в[0] && ш[1] === в[1] && ш[2] === в[2],
  ...
  expect(stickyBg.тонСовпал && stickyBg.альфа > 0 && stickyBg.альфа <= 0.02,
    ...
    'иначе Safari красит верхнюю полосу чёрным; тон — ВЕРХНИЙ СТОП НЕБА, тот же, ' +
    'что нарисован под шапкой после перевода экрана паузы на градиент (' +
  ...
  // ⛔⛔ НАДГРОБИЕ 2026-08-20-б: фраза ниже «белый HUD читается на небе только за
  // счёт яркости самого неба» была ВЕРНА для прежней палитры. На палитре
  // владельца (светло-голубой верх) белые глаза НЕ читаются за счёт неба —
  // контраст 1.505 при прежней норме 3:1. Дневная половина стража работает
  // теперь РЕГРЕССИОННЫМ ПИНОМ (см. HUD_FLOOR), а не стандартом читаемости;
  // ловит она дальнейшее осветление, а не саму читаемость.
  // ⚠️ КНОПКА ПАУЗЫ ПРИ ЭТОМ ВЫИГРАЛА: тёмный диск на светлом небе дал 9.493
  // против прежних 4.68 — её пол оставлен как был, он ловит ЗАТЕМНЕНИЕ.
  ...
  // ⛔ ПОЛЫ ОСТАВЛЕНЫ ОБА, НО НОЧНОЙ НЕДОСТИЖИМ (слово владельца 2026-08-20
  // «только дневная тема всегда»). Числа не удаляю: они замерены на устройстве и
  // понадобятся в ту же минуту, когда ночь вернут. Ниже обе пробы идут по
  // ДНЕВНОМУ полу, а час 22 остался в списке как ДОКАЗАТЕЛЬСТВО правила —
  // поздний час обязан дать то же дневное небо, что и полдень.
  // ⛔⛔ ПОЛ ГЛАЗ БОЛЬШЕ НЕ СТАНДАРТ ЧИТАЕМОСТИ, А РЕГРЕССИОННЫЙ ПИН (2026-08-20-б).
// Палитра владельца (`#85dcff` вверху вместо `#6e86ff`) СВЕТЛЕЕ вдвое по
// яркости, и белые глаза на ней почти сливаются с небом: замер той же линейкой
// 3.08 -> 1.505 при прежнем поле 3.00. Порог 3:1 на этой палитре НЕДОСТИЖИМ
// ничем, кроме правки самих глаз, а глаза владелец не просил трогать.
// ⚠️ ЧТО ПИН ПРОДОЛЖАЕТ ЛОВИТЬ: дальнейшее ТИХОЕ осветление неба. Коридор
// пустой и узкий — здоровое 1.505 против диверсионной почти-белой палитры
// 1.11; пин 1.30 стоит В СЕРЕДИНЕ, как требует правило «порог не ставится в
// доли процента от здорового значения».
// ⚠️ ЧТО ПИН БОЛЬШЕ НЕ ЛОВИТ И О ЧЁМ НАДО ГОВОРИТЬ ВСЛУХ: он НЕ утверждает,
// что глаза читаются. Вопрос читаемости открыт и передан владельцу с числом.
// ⛔ ПРЕЖНИЕ ПОЛЫ СОХРАНЕНЫ ДЛЯ ВОЗВРАТА: день 3.00, ночь 12.5 — вернётся
// прежняя палитра, вернутся и они.
const HUD_FLOOR = { day: 1.30, night: 12.5 };   // белок глаза против неба (день — ПИН, см. выше)
  ...
  const hudProbe = async (pg, sel, mode) => {
    // ⛔ ПРИШПИЛИВАНИЕ ПОКОЯ ОСТАВЛЕНО, НО ПРИЧИНА ЕГО СНЯТА 2026-08-20 вместе с
    // красным верхом (слово владельца). Держим потому, что покой — правильная
    // ОБСТАНОВКА замера сама по себе (лопасти не работают, глаза спокойны), а
    // не потому, что иначе кадр краснеет. Ниже — прежний разбор, он объясняет,
    // ОТКУДА взялось само требование:
    // ⚠️ (ИСТОРИЯ) Угроза помола (uGrind) наливала
    ...
    const px = await pg.evaluate(async ([b64, g, mode]) => {
      ...
      // ⚠️ ВЕРХ КАДРА ОБЯЗАН РОВНО СОВПАДАТЬ С ПЕРВЫМ СТОПОМ ПАЛИТРЫ — ассерт-факт
      // поверх пришпиленного покоя, а не вместо него. ⛔ Раньше он назывался
      // «детектор угрозы помола» и стерёг красный слой `uGrind`; слой снят
      // 2026-08-20, а проверка осталась — теперь она стережёт САМ РЕЦЕПТ кромок
      // (раскладку стопов по экрану), у которого потребитель тот же.
  ...
  const hudSeen = {};
  for (const [tema, h] of [['полдень', 12], ['поздний час', 22]]){
    ...
    expect(r.topDelta <= 6,
      '⚠️ ВЕРХ КАДРА = ПЕРВЫЙ СТОП ПАЛИТРЫ (' + tema + '), Δ' + r.topDelta + ' <= 6');
    expect(r.ratio >= HUD_FLOOR.day,
      'ПОЛ КОНТРАСТА ГЛАЗ (' + tema + '): ' + r.ratio + ' >= ' + HUD_FLOOR.day +
      ' (белок ' + r.own + ', небо ' + r.sky + ')');
    expect(btn.settled && btn.ratio >= BTN_FLOOR.day,
      'ПОЛ КОНТРАСТА КНОПКИ ПАУЗЫ (' + tema + '): ' + btn.ratio + ' >= ' + BTN_FLOOR.day +
      ' (диск ' + btn.own + ', небо ' + btn.sky + ', ' + btn.why + ')');
  }
  expect(Math.abs(hudSeen['полдень'].eyes.sky - hudSeen['поздний час'].eyes.sky) < 0.01,
    '⚠️⚠️ ТОЛЬКО ДЕНЬ, ЗАМЕРОМ ПО ПИКСЕЛЯМ: небо в 22:00 то же самое, что в 12:00 (' +
    hudSeen['полдень'].eyes.sky + ' против ' + hudSeen['поздний час'].eyes.sky + '). ' +
    'Это НЕ дубль стража по функциям выше: тот читает решение, а этот — сам кадр. ' +
    'Диверсия — вернуть ночную палитру: яркость неба на 22:00 рухнет, и оба ' +
    'дневных пола рядом покраснеют вместе с этим');
  await hudPage.close();
  ...
  expect(sab.ratio < HUD_FLOOR.day,
    ...
    ' < ' + HUD_FLOOR.day + ' (на исправной палитре было ' +
    hudSeen['полдень'].eyes.ratio + ')');
  await sabPage.close();
  ...
  // по ноде (shake right 16; ⚠️ подсказка — 108, а НЕ прежние 150: те
  // считались от ширины ПИЛЮЛИ 122, а с 2026-08-21 Shake это кисть 80,
  // 16+80+12 = 108; ⛔ и это число протухло ЕЩЁ РАЗ 2026-08-21-д — кластер
  // отодвинут от кромки на 8 и зазор вырос до 20, так что подсказка была в
  // 24+64+20 = 108... совпало случайно; ⛔ и ТРЕТИЙ раз 2026-08-22-в: кнопки
  // ужались до 56, зазор стал 16, и подсказка теперь в 24+56+16 = 96.
  // ⚠️ САМ АССЕРТ ОТ ЭТИХ ЧИСЕЛ НЕ ЗАВИСИТ — он про НАЕЗД на витрину, а не про
  // координату; арифметика тут справка, и протухает она молча).
  expect(zoomDesk.overVit === false,
    'ДЕСКТОП: подсказка и Shake НЕ наезжают на список предметов (' + JSON.stringify(zoomDesk) + ')');
  // ⛔ ЧИСЛО ПЕРЕЕХАЛО ЗА ПРАВИЛОМ: было 16 (падинг бара), стало 24 — слово
  // владельца 2026-08-21-д «отодвинь иконку шейка от правого края и снизу на
  // 8 px», прочитанное как ДЕЛЬТА к нынешнему полю. Прежний пин краснел бы на
  // ИСПРАВНОЙ сборке.
  expect(Math.abs(zoomDesk.shakeRight - 24) <= 4 && zoomDesk.hintRight > zoomDesk.shakeRight,
    'ДЕСКТОП: подсказка и Shake справа по ноде 741:1497 + отступ владельца ' +
  ...
  // ===== ЗВЁЗД НЕТ: ТОЛЬКО ДНЕВНАЯ ТЕМА (слово владельца 2026-08-20) =====
  // ⛔⛔ СТРАЖ ПЕРЕЕХАЛ ЗА ПРАВИЛОМ, А НЕ УДАЛЁН. Здесь стояла тяжёлая секция
  // «БОЛЬШЕ МЕЛКИХ + ПУЛЬС МЕНЬШИНСТВА»: она грузила страницу на `?hour=1`,
  // снимала по 5 кадров на руку и считала пятна ярче фона СТРОКИ. Звёзды живут
  // только на ночном небе (`uStars: skyTimeNow() === 'night' ? 1 : 0`, 10-stage),
  // а ночь теперь недостижима — мерить стало нечего.
  // ⚠️ ЧИСЛА ТОЙ КАЛИБРОВКИ СОХРАНЕНЫ ЗДЕСЬ ЖЕ, чтобы вернуть страж одним
  // движением, если владелец вернёт ночь:
  ...
  //   размах > 0.45; здоровые при доле 1.0: 37.7 / 55.8 / 57.1 / 57.5 / 61.5 /
  //   64.1 / 64.1 / 78.3 (минимум 37.7), при доле 0 — РОВНО 0 во всех восьми,
  //   порог 20 стоит в середине пустого коридора; сравнивалась РАЗНОСТЬ рук;
  //   размер — звёзд >= 25 при медиане площади <= 16.
  //   ⚠️ И урок: прежний порог 40 стоял впритык к здоровому разбросу и ОДИН РАЗ
  //   ПОКРАСНЕЛ НА ИСПРАВНОЙ СБОРКЕ (37.7 в чужом прогоне).
  // ⚠️ Полный текст прежней секции — в истории git (коммит «Только дневная тема»).
  {
    ...
    await starPage.goto('file://' + PAGE_FILE + '?hour=1');   // самый ночной час
    ...
    await new Promise(r => setTimeout(r, 900));
    // ⚠️ МЕРИМ ПИКСЕЛИ, А НЕ ЮНИФОРМУ: «uStars === 0» доказывало бы только то,
    // что ноль записан. Верхние 45% кадра ночью были усыпаны пятнами ярче фона
    // строки — если ночь вернётся тайком, они появятся здесь же.
    const звёздНет = await starPage.evaluate(async () => {
      ...
      for (let y = 0; y < h; y++){
        let сумма = 0;
        for (let i = 0; i < w; i++){ const o = (y * w + i) * 4; сумма += (d[o] + d[o+1] + d[o+2]) / 3; }
        const фон = сумма / w;
        for (let i = 0; i < w; i++){ const o = (y * w + i) * 4;
          if ((d[o] + d[o+1] + d[o+2]) / 3 - фон > 12) пятен++; }
      }
      return { пятен, час: window.__game.skyHour() };
    });
    const пульс = await starPage.evaluate(() => window.__game.starPulse());
    await starPage.close();
    console.log('звёзды в бою:', JSON.stringify({ звёздНет, пульс }));
    expect(звёздНет.час.time === 'day' && звёздНет.час.hour === 1,
      'САНИТАР: страница честно загружена на 1:00 и небо всё равно дневное (' +
      JSON.stringify(звёздНет.час) + ')');
    expect(звёздНет.пятен === 0,
      '⚠️⚠️ ЗВЁЗД НЕТ НИ НА ОДНОМ ЧАСЕ: в верхних 45% кадра на 1:00 ноль пятен ' +
      'ярче фона строки (' + звёздНет.пятен + '). Диверсия — вернуть в `uStars` ' +
      '(10-stage) решение по времени суток: ночью здесь были десятки пятен');
    // ⚠️ ЧИСЛО ВЛАДЕЛЬЦА СТЕРЕЖЁМ ДАЖЕ У НЕДОСТИЖИМОЙ ФИЧИ: «каждая пятая»
    // (2026-08-17). Вернут ночь — доля обязана быть той же, а не «какой была
    // до правки темы».
    expect(пульс && Math.abs(пульс.frac - 0.20) < 1e-6,
      'ДОЛЯ ПУЛЬСА ЗВЁЗД цела — 2 из 10, слово владельца «каждая пятая» (' +
      JSON.stringify(пульс) + ')');
  }
  

  // ===== ПОЛЯ SAFARI 26: КАНАЛ ТИНТА НА ВСЕХ ЭКРАНАХ (слово владельца
  // 2026-08-20 «возьми рабочую практику... и сразу сделай правильно, проверив
  // все экраны») =====
  // ⚠️⚠️ ЧЕГО ЭТОТ СТРАЖ НЕ МОЖЕТ И НЕ ОБЕЩАЕТ: САМУ ПОЛОСУ SAFARI headless не
  // рисует — её видно только на устройстве. Здесь проверяется ПРЕДПОСЫЛКА, и
  // она проверяема честно: цвет, который экран ОБЪЯВЛЯЕТ полосе, обязан
  // совпадать с настоящим пикселем кадра у той же кромки. Расходятся — полоса
  // получит цвет, которого на экране нет, чем бы Safari её ни красил.
  // ⚠️ ВТОРАЯ РУКА — СТРУКТУРНАЯ: `viewport-fit=cover` на месте и ни один
  // ВИДИМЫЙ фикс-элемент у кромки не имеет ПОЛНОСТЬЮ прозрачного фона. Именно
  // прозрачный фикс-элемент («прозрачный ЧЁРНЫЙ») и давал чёрные поля во всех
  // пяти прошлых редакциях — эта рука ловит весь класс разом.
  // ⛔ ОГОВОРКА «ЗАМЕР ТОЛЬКО В ПЕРВУЮ СЕКУНДУ ПОСЛЕ ДЕЙСТВИЯ» СНЯТА 2026-08-20
  // (Графика, 2026-07-31 — исторически): её причиной была красная угроза помола
  // `uGrind`, наливавшая ВЕРХ кадра за секунды простоя; слой удалён словом
  // владельца, и верх кадра больше не расходится с первым стопом палитры ни в
  // какой момент. Пришпиливание покоя ниже ОСТАВЛЕНО намеренно: оно дешёвое и
  // держит замер детерминированным, а урок — «прежде чем лечить расхождение
  // кромки, спроси, не красит ли кадр КАКОЙ-НИБУДЬ игровой слой» — в силе;
  // сегодня такой слой один, лихорадка комбо `uCombo`, и она красит НИЗ.
  {
    const кр = await browser.newPage({ viewport: { width: 390, height: 844 } });
    кр.on('pageerror', e => errors.push('PAGEERROR(край): ' + e.message));
    await кр.goto('file://' + PAGE_FILE + '?dev=1');
    await кр.waitForFunction(() => window.__game && window.__game.alive() > 0, null, { timeout: 60000 });
    await кр.evaluate(() => { window.__game.setLevel(3); window.__game.regen(); window.__game.skipIntro(); });
    await кр.waitForTimeout(1500);
    const чис = (c) => (String(c).match(/[\d.]+/g) || []).map(Number);
    // ⚠️ БЕЛЫЙ СПИСОК ИЗ ДВУХ, И У КАЖДОГО СВОЯ ПРИЧИНА:
    //  `c` — игровой канвас: фона у него нет по природе, но он САМ рисует
    //        пиксели кромки, а поверх него лежат бары со своим каналом;
    //  `mainScreen` — обязан быть прозрачным, иначе спрячет свой же слой
    //        градиента; его кромки прикрыты `#msSticky` сверху и
    //        псевдо-полоской `::after` снизу (обе проверяются отдельно).
    const бело = ['c', 'mainScreen'];
    const снять = () => кр.evaluate((бело) => {
      const прозр = c => { const m = (String(c).match(/[\d.]+/g) || []).map(Number);
        return !m.length || (m.length > 3 && m[3] === 0); };
      const из = { верх: [], низ: [], лишние: [] };
      for (const e of document.querySelectorAll('*')){
        const cs = getComputedStyle(e);
        if (cs.position !== 'fixed' || cs.display === 'none' || cs.visibility === 'hidden') continue;
        const b = e.getBoundingClientRect();
        if (!b.width || !b.height) continue;
        const имя = e.id || (e.tagName.toLowerCase() + '.' + String(e.className).split(' ')[0]);
        const z = cs.zIndex === 'auto' ? 0 : +cs.zIndex;
        const зап = { имя, bg: cs.backgroundColor, z };
        if (b.top <= 1) из.верх.push(зап);
        if (b.bottom >= innerHeight - 1) из.низ.push(зап);
        if ((b.top <= 1 || b.bottom >= innerHeight - 1) && прозр(cs.backgroundColor)
            && бело.indexOf(имя) < 0) из.лишние.push(имя);
      }
      const пс = (sel, кто) => { const e = document.querySelector(sel); if (!e) return null;
        const cs = getComputedStyle(e, кто); return cs ? cs.backgroundColor : null; };
      из.msStickyBg = (document.getElementById('msSticky') || {}) &&
        (document.getElementById('msSticky') ? getComputedStyle(document.getElementById('msSticky')).backgroundColor : null);
      из.mainAfter = пс('#mainScreen', '::after');
      из.html = getComputedStyle(document.documentElement).backgroundColor;
      из.body = getComputedStyle(document.body).backgroundColor;
      из.мета = (document.querySelector('meta[name=viewport]') || {}).content || '';
      return из;
    }, бело);
    const кадр = async () => {
      const buf = await кр.screenshot();
      return кр.evaluate(async (b64) => {
        const im = new Image();
        await new Promise(r => { im.onload = r; im.src = 'data:image/png;base64,' + b64; });
        const c = document.createElement('canvas'); c.width = im.width; c.height = im.height;
        const x = c.getContext('2d'); x.drawImage(im, 0, 0);
        const строка = y => { const d = x.getImageData(0, y, im.width, 1).data;
        ...
        return { верх: строка(0), низ: строка(im.height - 1) };
      }, buf.toString('base64'));
    };
    const дельта = (цвет, пиксель) => { const c = чис(цвет);
      return Math.max(Math.abs(c[0] - пиксель[0]), Math.abs(c[1] - пиксель[1]), Math.abs(c[2] - пиксель[2])); };

    // ── ИГРА
    const игра = await снять(), играК = await кадр();
    const топБар = игра.верх.find(x => x.имя === 'topBar');
    const низБар = игра.низ.find(x => x.имя === 'bottomBar');
    console.log('кромки игры:', JSON.stringify({ топБар, низБар, html: игра.html, body: игра.body,
      кадр: играК, лишние: игра.лишние }));
    expect(/viewport-fit=cover/.test(игра.мета),
      '⚠️⚠️ `viewport-fit=cover` НА МЕСТЕ (' + игра.мета + '). Без него страница ' +
      'не заходит под полосы, и весь канал тинта мёртв — комплект неделим');
    expect(топБар && низБар && дельта(топБар.bg, играК.верх) <= 2 && дельта(низБар.bg, играК.низ) <= 2,
      '⚠️⚠️ ИГРА: бары объявляют полосам РОВНО тот цвет, что нарисован у кромки (' +
      JSON.stringify({ топБар: топБар && топБар.bg, кадрВерх: играК.верх,
                       низБар: низБар && низБар.bg, кадрНиз: играК.низ }) + '). ' +
      'Диверсия — увести `--sky-top-rgb`/`--sky-bot-rgb` с крайних стопов палитры ' +
      'или вернуть барам `transparent`');
    expect(дельта(игра.html, играК.верх) <= 2 && дельта(игра.body, играК.низ) <= 2,
      '⚠️⚠️ ДВА КАНАЛА РАЗВЕДЕНЫ: `html` несёт ВЕРХНИЙ цвет, `body` — НИЖНИЙ (' +
      JSON.stringify({ html: игра.html, body: игра.body, кадр: играК }) + '). ' +
      'Это выросло из жалобы «покраснение снизу»: тогда красный верх (`uGrind`) ' +
      'пролезал в НИЖНЮЮ полосу Safari через фон `html`. ⛔ Слой снят 2026-08-20, ' +
      'и краснеть больше нечему — но разведение ОБЯЗАТЕЛЬНО и без него: два ' +
      'наших замера с устройства расходятся в том, откуда Safari берёт тинт, а ' +
      'палитра неба сама даёт верху и низу разные цвета');
    expect(игра.лишние.length === 0,
      '⚠️⚠️ НИ ОДИН ВИДИМЫЙ ФИКС-ЭЛЕМЕНТ У КРОМКИ НЕ ПРОЗРАЧЕН (игра): ' +
      JSON.stringify(игра.лишние) + '. Прозрачный читается Safari 26 как ' +
      '«прозрачный ЧЁРНЫЙ» — это и есть чёрные поля всех пяти прошлых редакций');

    // ── ПАУЗА (меню)
    await кр.click('#pauseBtn', { force: true });
    await кр.waitForTimeout(900);
    const меню = await снять(), менюК = await кадр();
    console.log('кромки паузы:', JSON.stringify({ msSticky: меню.msStickyBg, after: меню.mainAfter,
      кадр: менюК, лишние: меню.лишние }));
    expect(меню.msStickyBg && дельта(меню.msStickyBg, менюК.верх) <= 2,
      '⚠️⚠️ ПАУЗА, ВЕРХ: плавающая шапка объявляет цвет своей кромки (' +
      меню.msStickyBg + ' против кадра ' + JSON.stringify(менюК.верх) + '). Она лежит ' +
      'ВЫШЕ баров игры (z 31 против 5), значит канал у верхней кромки — её. ' +
      'Диверсия — вернуть ей прежний `--ms-bg`: экран паузы стал градиентом, и ' +
      'ровная заливка меню на верхней кромке больше не совпадает ни с чем');
    expect(меню.mainAfter && дельта(меню.mainAfter, менюК.низ) <= 2,
      '⚠️⚠️ ПАУЗА, НИЗ: полоска `#mainScreen::after` объявляет цвет нижней кромки (' +
      меню.mainAfter + ' против кадра ' + JSON.stringify(менюК.низ) + '). Сам ' +
      '`#mainScreen` обязан быть прозрачным (иначе спрячет свой слой градиента), ' +
      'а лежит он выше баров — без этой полоски нижняя кромка осталась бы за ' +
      'прозрачным фикс-элементом, то есть чёрной');
    expect(меню.лишние.length === 0,
      'НИ ОДИН ВИДИМЫЙ ФИКС-ЭЛЕМЕНТ У КРОМКИ НЕ ПРОЗРАЧЕН (пауза): ' +
      JSON.stringify(меню.лишние));

    // ── ТЁМНЫЕ ОВЕРЛЕИ: их собственная заливка и есть канал
    const тёмные = [];
    await кр.evaluate(() => { const b = document.querySelector('.ms-play'); if (b) b.click(); });
    await кр.waitForTimeout(600);
    for (const id of ['winOverlay', 'loseOverlay', 'museumOverlay', 'starsOverlay', 'lbOverlay', 'adOverlay']){
      await кр.evaluate(i => { document.getElementById(i).style.display = 'flex'; }, id);
      await кр.waitForTimeout(350);
      const о = await снять();
      тёмные.push({ id, лишние: о.лишние, верх: (о.верх.find(x => x.имя === id) || {}).bg });
      await кр.evaluate(i => { document.getElementById(i).style.display = 'none'; }, id);
    }
    await кр.close();
    console.log('кромки оверлеев:', JSON.stringify(тёмные));
    expect(тёмные.every(x => x.лишние.length === 0 && x.верх && чис(x.верх)[3] !== 0),
      '⚠️⚠️ ТЁМНЫЕ ОВЕРЛЕИ: у каждого своя НЕпрозрачная заливка, она же канал ' +
      'полос — отдельного элемента им не нужно (' + JSON.stringify(тёмные) + '). ' +
      'Диверсия — сделать `.overlay` прозрачным: полосы на всех шести экранах ' +
      'станут чёрными, а страж покраснеет на каждом');
  }

  // ===== ПАРТИИ ВЛАДЕЛЬЦА 2026-08-20/21: НЕТ СУНДУКА, НЕТ КРАСНОГО ВЕРХА,
  // ГРАДИЕНТ С ПОЗИЦИЯМИ, ДЕСКТОПНОЕ МЕНЮ, МОБИЛЬНАЯ КНОПКА-ПИЛЮЛЯ =====
  // ⚠️ ЗАГОЛОВОК ПЕРЕПИСАН, А НЕ ДОПИСАН: прежний («настройки над глазами,
  // карточка глаз прозрачна») называл два правила, отменённых тем же владельцем
  // в тот же день. Заголовок секции — такой же текст, как сообщение ассерта, и
  // протухает так же молча.
  {
    const пв = await browser.newPage({ viewport: { width: 390, height: 844 } });
    пв.on('pageerror', e => errors.push('PAGEERROR(партия): ' + e.message));
    await пв.goto('file://' + PAGE_FILE + '?dev=1');
    await пв.waitForFunction(() => window.__game && window.__game.alive() > 0, { timeout: 60000 });
    await пв.evaluate(() => { window.__game.setLevel(3); window.__game.regen(); window.__game.skipIntro(); });
    await пв.waitForTimeout(1200);

    // ── СУНДУК УДАЛЁН (слово владельца «сундук удали и открытый и закрытый»)
    const пул = await пв.evaluate(() => {
      const с = window.__game.accSnapshot();
      return { типов: с.length, сундук: с.some(x => x.key === 'piratechest'),
               пираты: с.filter(x => /^pirate/.test(x.key)).map(x => x.key).sort() };
    });
    console.log('пул:', JSON.stringify(пул));
    expect(!пул.сундук && пул.пираты.join() === 'piratebarrel,piratepalm',
      '⚠️⚠️ СУНДУКА В ПУЛЕ НЕТ (' + JSON.stringify(пул) + '). Слово владельца ' +
      '2026-08-20: художник прислал ОТКРЫТЫЙ сундук вместо прежнего закрытого, ' +
      'владелец не захотел ни того, ни другого. ⚠️ Вторая половина ассерта — ' +
      'КОНТРОЛЬ: без неё «сундука нет» истинно и на сборке, где пиратская пачка ' +
      'потерялась целиком');

    // ── ГРАДИЕНТ ФОНА = ПАЛИТРА ВЛАДЕЛЬЦА, С ЕГО ПОЗИЦИЯМИ
    // ⚠️⚠️ ПОЗИЦИИ ПРОВЕРЯЮТСЯ ХУКОМ, А НЕ ПИКСЕЛЯМИ, И ЭТО НЕ ЛЕНЬ. Позиции
    // владельца 0/36/65/100 отстоят от равномерных 0/33.3/66.7/100 всего на
    // 2.7% и 1.7% высоты, что даёт Δ≈2 из 255 на канал — скрин-диверсия утонула
    // бы в шуме кадра, то есть страж был бы зелен на сборке, где позиции
    // потеряны. Хук отдаёт РАЗОБРАННОЕ состояние и ГОТОВУЮ строку CSS: так
    // видно, что позиции дошли до ОБОИХ потребителей — рампы и `--sky-grad`.
    const небо = await пв.evaluate(() => window.__game.skyInfo());
    console.log('небо:', JSON.stringify(небо));
    const ждём = ['#85dcff', '#9aeafa', '#b0f4f8', '#ccfff8'];
    const позОк = небо.позиции.length === 4 &&
      [0, 0.36, 0.65, 1].every((v, i) => Math.abs(небо.позиции[i] - v) < 1e-6);
    expect(небо.стопы.join() === ждём.join() && небо.свои && позОк,
      '⚠️⚠️ ГРАДИЕНТ ФОНА — ПАЛИТРА ВЛАДЕЛЬЦА 2026-08-20-б, ЧЕТЫРЕ СТОПА С ЕГО ' +
      'ПОЗИЦИЯМИ (' + JSON.stringify({ стопы: небо.стопы, позиции: небо.позиции, свои: небо.свои }) +
      '). Диверсия — снять позиции из `SKY_STOPS.day`: разбор молча разложит ' +
      'стопы РАВНОМЕРНО (0/33.3/66.7/100), `свои` станет false, и небо ' +
      'разъедется с той же строкой в CSS, где браузер позиции уважает');
    expect(/36%/.test(небо.градиент) && /65%/.test(небо.градиент) &&
           небо.градиент.indexOf('180deg') > 0,
      '⚠️⚠️ ПОЗИЦИИ ДОШЛИ И ДО CSS (' + небо.градиент + '). ⚠️ ВТОРОЙ ПОТРЕБИТЕЛЬ ' +
      'ПРОВЕРЯЕТСЯ ОТДЕЛЬНО НАМЕРЕННО: рампа шейдера и `--sky-grad` читают одни ' +
      'и те же стопы, но РАЗНЫМ кодом, и потерять позиции можно в любом из двух — ' +
      'тогда небо в игре и фон экрана паузы разойдутся, оставаясь каждый ' +
      'по-своему правдоподобным');

    // ── КРАСНОГО ВЕРХА ПРИ ЗЛОСТИ НЕТ (слово владельца «убери вверху изменение
    // фона (покраснение) когда миксер злится»)
    // ⚠️ ЗЛОСТЬ ФОРСИРУЕМ, А НЕ ЖДЁМ: полный простой на Easy это 15 секунд, а
    // укороченный лимит доводит миксер до работы за пару секунд — тот же приём,
    // что у соседней секции про огонь.
    const строкаКадра = async (стр) => {
      const buf = await пв.screenshot();
      return пв.evaluate(async (о) => {
        const im = new Image();
        await new Promise(r => { im.onload = r; im.src = 'data:image/png;base64,' + о.b64; });
        const c = document.createElement('canvas'); c.width = im.width; c.height = im.height;
        const x = c.getContext('2d'); x.drawImage(im, 0, 0);
        const y = о.низ ? im.height - 1 : 0;
        const d = x.getImageData(0, y, im.width, 1).data;
        let r = 0, g = 0, b = 0;
        for (let i = 0; i < d.length; i += 4){ r += d[i]; g += d[i+1]; b += d[i+2]; }
        const n = im.width; return [Math.round(r/n), Math.round(g/n), Math.round(b/n)];
      }, { b64: buf.toString('base64'), низ: стр === 'низ' });
    };
    const верхПокой = await строкаКадра('верх');
    const живыхДо = await пв.evaluate(() => window.__game.alive());
    await пв.evaluate(() => { window.__game.level().idleLimit = 2;
      window.__game.stats().lastAction = performance.now() - 20000; });
    await пв.waitForTimeout(3000);
    // ⚠️⚠️ ПРИЗНАК ЗЛОСТИ — ПЕРЕКЛЮЧЕНИЕ СЛОЯ ЛИЦА И СЪЕДЕННЫЕ ПРЕДМЕТЫ.
    // Две редакции санитара до этой были ПУСТЫМИ, и обе выглядели рабочими:
    //  (1) `level().grinding` — такого поля НЕТ (`grinding` живёт локальной
    //      переменной цикла), приходил `undefined`, то есть проверялась
    //      собственная опечатка;
    //  (2) `getComputedStyle(fAngry).display !== 'none'` — ВСЕГДА ИСТИННО:
    //      слои лица гасятся не `display`, а классом `.on` и `opacity`
    //      (`#eyes .face { opacity:0 }` в shell.html), у всех пяти слоёв
    //      display остаётся `inline` при любом выражении.
    // Действующий признак читает ТОТ САМЫЙ механизм, которым `setFace`
    // переключает слои, и требует ПЕРЕХОДА: злые зажглись И спокойные погасли.
    const мелет = await пв.evaluate((до) => {
      const рд = id => { const e = document.getElementById(id);
        return e ? { on: e.classList.contains('on'),
                     op: parseFloat(getComputedStyle(e).opacity) } : null; };
      const зл = рд('fAngry'), сп = рд('fRound');
      return { злые: !!зл && zl(зл), спокойныеПогасли: !!сп && !сп.on && сп.op < 0.5,
               слои: { fAngry: зл, fRound: сп },
               съедено: до - window.__game.alive() };
      function zl(x){ return x.on && x.op > 0.5; }
    }, живыхДо);
    const верхЗлость = await строкаКадра('верх');
    const низЗлость = await строкаКадра('низ');
    const дельта = Math.max(...верхПокой.map((v, i) => Math.abs(v - верхЗлость[i])));
    console.log('верх кадра:', JSON.stringify({ верхПокой, верхЗлость, низЗлость, дельта, мелет }));
    expect(мелет.злые && мелет.спокойныеПогасли && мелет.съедено > 0,
      'САНИТАР ЗЛОСТИ: миксер ДЕЙСТВИТЕЛЬНО работает — слой злых глаз ЗАЖЁГСЯ, ' +
      'слой спокойных ПОГАС и предметы съедены (' + JSON.stringify(мелет) + '). ' +
      'Без него «красного нет» истинно и на сборке, где злость не наступила ' +
      'вовсе. ⚠️ Читать надо класс `.on` и `opacity`, а НЕ `display`: слои лица ' +
      'через display не гасятся никогда, и такой санитар был бы тавтологией');
    expect(дельта <= 4,
      '⚠️⚠️ ВЕРХ КАДРА НЕ КРАСНЕЕТ ПРИ ЗЛОСТИ МИКСЕРА (' +
      JSON.stringify({ верхПокой, верхЗлость, дельта }) + '). До правки 2026-08-20 ' +
      'слой `uGrind` наливал верх красным поверх спокойного неба — ' +
      'то есть Δ около 107. Диверсия — вернуть в шейдер неба (10-stage) строки ' +
      '`float grind = uGrind * …` и `col = mix(col, uGrindCol, grind)` вместе с ' +
      'драйвером в `loop`');

    // ⚠️ СТРАНИЦУ ЗАКРЫВАЕМ ЗДЕСЬ: её `close()` уехал вместе с вырезанным
    // блоком «настройки над глазами», и без этой строки прогон оставлял бы
    // за собой живую вкладку — ровно тот класс, о котором канон говорит
    // «склейка двух правильных блоков даёт неправильные КРАЯ».
    await пв.close();

    // ── МОБИЛЬНЫЙ БЛОК = ОДНА ПРИБИТАЯ КНОПКА (слово владельца 2026-08-21,
    // макет Figma 840:1819). ⛔ ЭТИМ СНЯТЫ ТРИ ПРЕЖНИХ АССЕРТА ЭТОГО ЖЕ ДНЯ, И
    // ОНИ УДАЛЕНЫ, А НЕ ПЕРЕПИСАНЫ: «настройки над глазами» (у прибитой кнопки
    // нет места в потоке — сравнивать не с чем), «подложка #D8BBFF» и «глаза
    // заполняют область по высоте» (карточки на мобильном больше нет вовсе).
    // Страж умирает вместе с механикой; всё три живут в десктопной половине.
    // ⚠️ КОНТЕКСТ ТАЧЕВЫЙ (`hasTouch`/`isMobile`), И ЭТО НЕСУЩЕЕ: без него
    // Chromium отдаёт `hover:hover`, глаза уходят на путь «следят за курсором»
    // и без курсора СТОЯТ — то есть проверка «анимированы» была бы зелёной на
    // сборке, где сама анимация мертва.
    const снятьКнопку = async (ш) => {
    const мк = await browser.newContext({ viewport: { width: ш, height: 844 },
      hasTouch: true, isMobile: true, deviceScaleFactor: 1 });
    const мп = await мк.newPage();
    мп.on('pageerror', e => errors.push('PAGEERROR(кнопка): ' + e.message));
    await мп.goto('file://' + PAGE_FILE + '?dev=1');
    await мп.waitForFunction(() => window.__game && window.__game.alive() > 0, { timeout: 60000 });
    await мп.evaluate(() => window.__game.skipIntro());
    await мп.click('#pauseBtn', { force: true });
    await мп.waitForFunction(() => {
    ...
    const кнопка = await мп.evaluate(async () => {
      const sleep = ms => new Promise(r => setTimeout(r, ms));
      ...
      // ⚠️ ЖИВОСТЬ ГЛАЗ — ПО РЕАЛЬНОМУ ДВИЖЕНИЮ ЗРАЧКОВ, а не по наличию узла:
      // «глаза внутри кнопки так же анимированы» — прямой пункт владельца, и
      // статичная копия из макета прошла бы любую проверку на присутствие.
      const снять = () => { const L = document.getElementById('msPupL'), R = document.getElementById('msPupR');
        return L.getAttribute('cx') + '|' + L.getAttribute('cy') + '|' + R.getAttribute('cx'); };
      const кадры = []; for (let i = 0; i < 10; i++){ кадры.push(снять()); await sleep(350); }
      // прокрутка до конца: кнопка обязана остаться на месте, контент — не под ней
      const m = document.getElementById('mainScreen');
      m.scrollTop = m.scrollHeight; await sleep(350);
      const r1 = pl.getBoundingClientRect();
      const низ = document.getElementById('msDev').getBoundingClientRect();
      return { позиция: cs.position, фон: cs.backgroundColor, радиус: cs.borderRadius,
               паддинг: cs.paddingLeft + '/' + cs.paddingRight, зазор: cs.gap,
               тень: cs.boxShadow,
               рект: [Math.round(r0.left), Math.round(r0.top), Math.round(r0.width), Math.round(r0.height)],
               доНиза: Math.round(innerHeight - r0.bottom),
               послеСкролла: [Math.round(r1.left), Math.round(r1.top), Math.round(r1.width), Math.round(r1.height)],
               прокручено: m.scrollTop > 100,
               глазаВнутри: pl.contains(sv), глаза: [Math.round(sr.width), Math.round(sr.height)],
               положенийЗрачков: new Set(кадры).size, кадров: кадры.length,
               центр: Math.round(r0.left + r0.width / 2), полЭкрана: Math.round(innerWidth / 2),
               просветДоКонтента: Math.round(r1.top - низ.bottom) };
    });
    await мк.close();
    return кнопка;
    };
    const кнопка = await снятьКнопку(390), к700 = await снятьКнопку(700);
    console.log('кнопка 390:', JSON.stringify(кнопка));
    console.log('кнопка 700:', JSON.stringify({ рект: к700.рект, центр: к700.центр, полЭкрана: к700.полЭкрана }));
    expect(кнопка.позиция === 'fixed' && кнопка.доНиза === 24 &&
           кнопка.рект[3] === 84 && кнопка.паддинг === '24px/24px' && кнопка.зазор === '12px' &&
           кнопка.фон === 'rgb(29, 28, 38)' && кнопка.радиус === '1500px',
      '⚠️⚠️ МОБИЛЬНЫЙ БЛОК СХЛОПНУТ В ОДНУ ПРИБИТУЮ КНОПКУ ПО МАКЕТУ (' +
      JSON.stringify(кнопка) + '). Слово владельца 2026-08-21 + Figma 840:1819; ' +
      'отступ снизу 24 и внутренний 24 — его уточнения, они важнее макетных 33. ' +
      '⚠️ `position:fixed` В ЭТОМ ЖЕ АССЕРТЕ НЕСУЩЕЕ: появится у `#mainScreen` ' +
      '`transform`/`filter`/`contain` — кнопка станет `absolute` и уедет со ' +
      'скроллом, а геометрия при этом останется верной');
    // ⚠️⚠️ «НЕ ТЯНЕТСЯ» ДОКАЗЫВАЕТСЯ РАВЕНСТВОМ ШИРИН НА ДВУХ ЭКРАНАХ, А НЕ
    // ЧИСЛОМ. Ширина по содержимому зависит от шрифта и текста кнопки («Resume»
    // против «Play Game»), и литерал пришлось бы править при каждой смене
    // надписи. Растянутая кнопка дала бы 342 против 652 — расхождение видно
    // сразу. Плюс центровка: кнопка обязана стоять по середине экрана.
    expect(кнопка.рект[2] === к700.рект[2] &&
           Math.abs(кнопка.центр - кнопка.полЭкрана) <= 1 &&
           Math.abs(к700.центр - к700.полЭкрана) <= 1,
      '⚠️⚠️ КНОПКА НЕ ТЯНЕТСЯ — ШИРИНА ПО СОДЕРЖИМОМУ И ПО ЦЕНТРУ (390: ' +
      кнопка.рект[2] + 'px, центр ' + кнопка.центр + '/' + кнопка.полЭкрана +
      '; 700: ' + к700.рект[2] + 'px, центр ' + к700.центр + '/' + к700.полЭкрана +
      '). Слово владельца 2026-08-21-б «кнопка не тянется, только внутренние ' +
      'отступы по бокам 24 px» — оно ОТМЕНИЛО его же растяжку `left:24/right:24` ' +
      'и внутренний отступ 32 часом раньше. Диверсия — вернуть растяжку: ширины ' +
      'разъедутся 342 против 652');
    expect(кнопка.прокручено &&
           JSON.stringify(кнопка.рект) === JSON.stringify(кнопка.послеСкролла),
      '⚠️⚠️ КНОПКА ПЕРЕЖИВАЕТ ПРОКРУТКУ (' + JSON.stringify(кнопка.рект) + ' -> ' +
      JSON.stringify(кнопка.послеСкролла) + '). ⚠️ САНИТАР `прокручено` В ТОМ ЖЕ ' +
      'АССЕРТЕ: без него «рект не изменился» истинно и на странице, которая ' +
      'вообще не прокрутилась');
    expect(кнопка.глазаВнутри && кнопка.глаза[0] === 80 && кнопка.глаза[1] === 40 &&
           кнопка.положенийЗрачков >= 5,
      '⚠️⚠️ ГЛАЗА ВНУТРИ КНОПКИ, 80×40, И ЖИВЫЕ (' +
      JSON.stringify({ внутри: кнопка.глазаВнутри, размер: кнопка.глаза,
                       положений: кнопка.положенийЗрачков + ' из ' + кнопка.кадров }) +
      '). Прямой пункт владельца «глаза внутри кнопки так же анимированы». ' +
      '⚠️ ДВИЖЕНИЕ ПРОВЕРЯЕТСЯ ЗАМЕРОМ, А НЕ ПРИСУТСТВИЕМ УЗЛА: подставь сюда ' +
      'статический экспорт глаз из макета — он прошёл бы любую проверку на ' +
      'наличие и не двигался бы. Порог 5 из 10, а не 10: два соседних сэмпла ' +
      'законно совпадают, когда зрачок в крайней точке');
    expect(кнопка.просветДоКонтента >= 0,
      '⚠️⚠️ НИЗ МЕНЮ НЕ УХОДИТ ПОД КНОПКУ: при прокрутке ДО КОНЦА между ' +
      'последним блоком и кнопкой ' + кнопка.просветДоКонтента + 'px. Без нижнего ' +
      'отступа обёртки последняя строка коллекции была бы недостижима, и ' +
      'заметить это можно только докрутив — обычный замер на верху страницы ' +
      'зелен при любом отступе');

    // ── ДЕСКТОПНОЕ ПЛЕЧО И ГРАНИЦА 700
    // ⚠️⚠️ ПОРЯДОК ЗАДАН В ДВУХ МЕСТАХ И ОНИ ТЕПЕРЬ РАЗНЫЕ: на узких — `order`
    // в @media (max-width:1079px), настройки НАД глазами (слово владельца
    // 2026-08-20); на десктопе — `grid-template-areas` в @media (min-width:1080px),
    // ГЛАЗА ПЕРВЫМИ, за ними профиль, за ним настройки (слово владельца
    // 2026-08-20-в). Замер на одном вьюпорте закрывал бы половину правила — и
    // ровно этим однажды был пропущен блокер гибкого ряда.
    // ⚠️⚠️ ГИБКИЙ РЯД ПРОВЕРЯЕТСЯ ОТДЕЛЬНО ОТ ПОРЯДКА: перестановка имён в
    // `grid-template-areas` НЕ двигает `1fr` — он привязан к позиции ряда.
    // Забыв переставить `grid-template-rows`, мы уже отдавали весь запас высоты
    // НАСТРОЙКАМ (белая карточка 797px при 184px содержимого). Признак взят
    // ОТНОСИТЕЛЬНЫЙ, без магических чисел: с ростом экрана обязан расти Play.
    const десктоп = async (вы) => {
      const д = await browser.newPage({ viewport: { width: 1440, height: вы } });
      д.on('pageerror', e => errors.push('PAGEERROR(партия-десктоп): ' + e.message));
      await д.goto('file://' + PAGE_FILE + '?dev=1');
      await д.waitForFunction(() => window.__game && window.__game.alive() > 0, { timeout: 60000 });
      await д.evaluate(() => { window.__game.skipIntro(); });
      await д.click('#pauseBtn', { force: true });
      await д.waitForFunction(() => {
        const m = document.getElementById('mainScreen');
        return m && getComputedStyle(m).visibility !== 'hidden' &&
               document.querySelector('.ms-play').getBoundingClientRect().height > 0;
      }, { timeout: 15000 });
      const о = await д.evaluate(() => {
        const R = s => document.querySelector(s).getBoundingClientRect();
        const г = R('.ms-play'), пр = R('.ms-card'), н = R('.ms-settings');
        const зг = R('.ms-coll-title'), кл = R('.ms-coll');
        const кс = getComputedStyle(document.querySelector('.ms-play'));
        const бел = document.querySelector('.ms-eyes .ms-eye-w');
        const бк = бел && getComputedStyle(бел);
        // ⚠️ ЗРАЧОК ЧИТАЕТСЯ ОТДЕЛЬНО: он в том же SVG и тоже `circle` —
        // без него «белок плоско-белый» ничего не сказало бы о том, что
        // общий селектор не залил заодно и зрачки.
        const зр = document.getElementById('msPupL');
        const зк = зр && getComputedStyle(зр);
        return { подложка: кс.backgroundColor, картинка: кс.backgroundImage, тень: кс.boxShadow,
                 объём: бк ? (/url\(/.test(бк.fill) ? 'градиент' : бк.fill) : 'нет',
                 обводка: бк ? (бк.stroke + '/' + бк.strokeWidth) : 'нет',
                 зрачок: зк ? зк.fill : 'нет',
                 градиентовВСвг: document.querySelectorAll('.ms-eyes radialGradient, .ms-eyes linearGradient').length,
                 профильВышеГлаз: пр.bottom <= г.top + 1, глазаВышеНастроек: г.bottom <= н.top + 1,
                 всеВидны: г.height > 40 && пр.height > 40 && н.height > 40,
                 гТоп: Math.round(г.top), прТоп: Math.round(пр.top), нТоп: Math.round(н.top),
                 высГ: Math.round(г.height), высН: Math.round(н.height),
                 // ⚠️ САНИТАР ПРАВОЙ КОЛОНКИ: она спанит `grid-row:1/-1`, а
                 // заголовок лежит поверх с `align-self:start`. Перенос `play`
                 // в ПЕРВЫЙ ряд — ровно тот случай, который мог оставить между
                 // заголовком и сеткой дыру в целый гибкий ряд.
                 щельПодЗаголовком: Math.round(кл.top - зг.top),
                 ряды: getComputedStyle(document.querySelector('.ms-wrap')).gridTemplateRows };
      });
      await д.close();
      return о;
    };
    const д900 = await десктоп(900), д1200 = await десктоп(1200);
    console.log('десктоп 900:', JSON.stringify(д900));
    console.log('десктоп 1200:', JSON.stringify(д1200));
    expect(д900.профильВышеГлаз && д900.глазаВышеНастроек && д900.всеВидны &&
           д1200.профильВышеГлаз && д1200.глазаВышеНастроек && д1200.всеВидны,
      '⚠️⚠️ НА ДЕСКТОПЕ ПОРЯДОК: ПРОФИЛЬ -> ГЛАЗА -> НАСТРОЙКИ (' +
      JSON.stringify({ '900': д900, '1200': д1200 }) + '). Слово владельца ' +
      '2026-08-20-к: «блок с аватаркой и лидербордами поставь над глазами, как ' +
      'это было когда-то». ⛔ ТРЕТЬЯ РЕДАКЦИЯ ДЕСКТОПНОГО ПОРЯДКА ЗА ДЕНЬ ' +
      '(prof/sets/play -> sets/play -> play/prof/sets -> prof/play/sets), и ' +
      'каждая — переезд ассерта за словом, а не починка. На узких порядок СВОЙ ' +
      '(настройки над глазами) и проверяется выше. Диверсия — вернуть "play play ' +
      'chead" первой строкой');
    expect(д900.щельПодЗаголовком === 0 && д1200.щельПодЗаголовком === 0,
      '⚠️⚠️ ПРАВАЯ КОЛОНКА ЦЕЛА: сетка коллекции начинается ОТ ВЕРХА колонки, а не ' +
      'ниже гибкого ряда (щель ' + д900.щельПодЗаголовком + '/' + д1200.щельПодЗаголовком +
      '). Без этого ассерта перенос `play` в первый ряд мог бы развести заголовок ' +
      'и карточки на всю высоту гибкого ряда, и заметно это стало бы только глазом');
    // ⛔⛔ ГРАДИЕНТ В БЕЛКАХ СНЯТ СЛОВОМ ВЛАДЕЛЬЦА 2026-08-22-в («убери градиент
    // в белках глаз») — прожил двое суток. Страж ПЕРЕЕХАЛ за правилом: было
    // `объём === 'градиент'`, стало «плоско-белый И ни одного градиента внутри
    // самого SVG». Второе плечо несущее: снять правило `fill:url(...)`, но
    // оставить `<radialGradient>` в `<defs>` — это мёртвый второй источник
    // правды о цвете, и одним лишь `fill` он не ловится.
    // ⚠️⚠️ ОБВОДКА ПРОВЕРЯЕТСЯ ЗДЕСЬ ЖЕ, И ЭТО НЕ ПЕДАНТИЗМ: градиент был
    // ЗАМЕНОЙ чёрной обводке, которую владелец забраковал ОТДЕЛЬНО
    // (2026-08-20-д «обводка у глаз плохо»). Сняв градиент, легко «вернуть
    // глазам форму» контуром — то есть вернуть ровно то, что он забраковал.
    // ⚠️ И ЗРАЧОК: он в том же SVG и тоже `circle`. Без его чтения ассерт
    // «белок плоско-белый» зелен и у сборки, где общий селектор залил всё.
    expect(д900.подложка === 'rgb(216, 187, 255)' &&
           д900.объём === 'rgb(255, 255, 255)' && д900.градиентовВСвг === 0 &&
           // ⚠️⚠️ ПРОВЕРЯЕТСЯ КРАСКА, А НЕ ТОЛЩИНА: у `stroke-width` НАЧАЛЬНОЕ
           // значение `1px`, и computed отдаёт его даже там, где обводки нет
           // вовсе. Пин `'none/0px'` покраснел бы на ИСПРАВНОЙ сборке — поймано
           // пробой ДО прогона. Ширина без краски не значит ничего.
           д900.обводка.indexOf('none/') === 0 && д900.зрачок === 'rgb(0, 0, 0)' &&
           д1200.подложка === 'rgb(216, 187, 255)' &&
           д1200.объём === 'rgb(255, 255, 255)' && д1200.градиентовВСвг === 0 &&
           д900.картинка === 'none' && д900.тень === 'none',
      '⚠️⚠️ НА ДЕСКТОПЕ ПОДЛОЖКА ПОД ГЛАЗАМИ #D8BBFF, БЕЛКИ ПЛОСКО-БЕЛЫЕ, БЕЗ ' +
      'ГРАДИЕНТА И БЕЗ ОБВОДКИ (' + JSON.stringify({ подложка: д900.подложка,
        объём: д900.объём, обводка: д900.обводка, градиентов: д900.градиентовВСвг,
        зрачок: д900.зрачок }) + '). ' +
      'Цвет подложки — слово владельца 2026-08-20-к. ⛔ ПРЕЖНИЙ ТЕКСТ ЭТОГО ' +
      'АССЕРТА («такая же, как в мобильном») СТАЛ ЛОЖНЫМ 2026-08-21: мобильный ' +
      'блок схлопнут в тёмную кнопку-пилюлю, и лиловая подложка — свойство ' +
      'ТОЛЬКО десктопа. ⛔⛔ А ТРЕБОВАНИЕ ОБЪЁМА ОТМЕНЕНО 2026-08-22-в: он ' +
      'сказал «убери градиент в белках глаз», и довод «на белом белый белок ' +
      'исчезает» снят его же словом — подложка лиловая, белок на ней читается. ' +
      'Диверсия — вернуть `fill:url(#msEyeVol)` ЛИБО оставить один только ' +
      '`<radialGradient>` в `<defs>`: первую ловит `объём`, вторую `градиентовВСвг`');
    const ростГ = д1200.высГ - д900.высГ, ростН = д1200.высН - д900.высН;
    expect(ростГ > 200 && Math.abs(ростН) <= 8,
      '⚠️⚠️ ГИБКИЙ РЯД ПРИНАДЛЕЖИТ КАРТОЧКЕ ГЛАЗ, А НЕ НАСТРОЙКАМ: при росте ' +
      'вьюпорта на 300 px растёт Play (' + ростГ + '), а настройки стоят (' +
      ростН + ') — ' + JSON.stringify({ '900': д900, '1200': д1200 }) + '. ' +
      'Это стоящее решение владельца «не оставляя дыр, заполняй пространство ' +
      'блоком с глазами». Диверсия — сдвинуть `1fr` в `grid-template-rows` на ' +
      'чужой ряд, НЕ трогая areas: порядок останется верным, а запас высоты ' +
      'заберёт другой блок');

    // ⛔⛔ ГРАНИЦА 700 СНЯТА 2026-08-21 ВМЕСТЕ С МЕХАНИКОЙ. Она различала размер
    // глаз в мобильной карточке, а карточки на мобильном больше нет — блок
    // схлопнут в прибитую кнопку. Здесь стояли четыре проверки (фон по обе
    // стороны, объём, выравнивание, сам пиксель границы); все они утверждали
    // свойства снятого правила. Страж умирает вместе с механикой, а не
    // переписывается ради счётчика PASS.
    // ⚠️ ЧТО ИЗ ТОГО РАЗБОРА ОСТАЛОСЬ ЖИТЬ (и почему это не потеря): урок
    // «диверсия ничего не уронила ≠ слепой страж» записан в канон, а
    // единственный признак, различавший тот пиксель (`justify-content`),
    // больше ничего не различает — ниже 1080 всё одинаково.
  }

  // ===== КНОПКА SHAKE = ИКОНКА-КИСТЬ С ЛАЙМОВЫМ БЕЙДЖЕМ =====
  // Слово владельца 2026-08-21 «везде замени кнопку Shake» + макеты Figma
  // 886:3949 (число) и 886:4017 («Ad»), прочитаны Dev Mode'ом.
  // ⚠️⚠️ ДО ЭТОЙ СЕКЦИИ ВИД КНОПКИ НЕ СТЕРЁГ НИКТО. В сьюте было семь обращений
  // к `#shakeBtn` — занавес, четыре клика, курсор и десктопная геометрия, — и
  // все они переживут ЛЮБОЙ внешний вид: ни один не читал ни надпись, ни фон,
  // ни бейдж. То есть подмена пилюли на иконку прошла бы молча в обе стороны.
  ...
  // проверяется через API (`level().shakes`), а показывает число DOM.
  {
    ...
    await стр.evaluate(() => { window.__game.skipIntro(); });

    const снятьShake = () => стр.evaluate(() => {
      // ⚠️⚠️ ПОМОЩНИК ЧИТАЕТ ПИКСЕЛИ PNG: у картинки нет ни `fill`, ни
      // computed-цвета, а подмена ассета на прежний чёрный оставила бы и размер,
      // и рамку прежними. Белое и полупрозрачное отбрасывается, отдаётся самый
      // частый непрозрачный НЕ-белый тон. `data:`-картинка холст не пятнает.
      const доминанта = (im) => { try {
        const c = document.createElement('canvas');
        c.width = im.naturalWidth; c.height = im.naturalHeight;
        const x = c.getContext('2d'); x.drawImage(im, 0, 0);
        const d = x.getImageData(0, 0, c.width, c.height).data; const сч = {};
        ...
        let л = null, м = 0;
        for (const k in сч) if (сч[k] > м) { м = сч[k]; л = k; }
        return л;
      } catch (e) { return 'ERR:' + e.name; } };
      const b = document.getElementById('shakeBtn'), l = document.getElementById('shakeLbl');
      // ⚠️ СЕЛЕКТОР БЕЗ `svg` — ИКОНКА ТЕПЕРЬ `<img>` (2026-08-21-р). Прежний
      //    `svg.shake-hand` отдавал null, и ВЕСЬ снимок кисти приходил пустым.
      ...
      // ⚠️ ЦВЕТА КИСТИ — ИМЕННО ОНИ ОБОСНОВЫВАЮТ ВЫХОД КНОПКИ ИЗ ПРАВИЛА СУТОК
      // («несёт СВОИ цвета из ассета»), и до разбора их не читал ни один
      // ассерт: «12 путей» ловит только пустой <svg>, а не перекрашенный.
      const цвета = {};
      if (h) h.querySelectorAll('path').forEach(pa => {
        const f = getComputedStyle(pa).fill; цвета[f] = (цвета[f] || 0) + 1; });
      return { тег: b.tagName, ария: b.getAttribute('aria-label'),
               имяИзСодержимого: (b.textContent || '').replace(/\s+/g, ' ').trim(),
               классКнопки: b.className, цвета: цвета,
               ширина: Math.round(r.width), высота: Math.round(r.height),
               фон: cs.backgroundColor, фонКартинка: cs.backgroundImage,
               радиус: cs.borderRadius, тень: cs.boxShadow,
               позиция: cs.position, события: cs.pointerEvents,
               естьКисть: !!h, тегИконки: h && h.tagName,
               натур: h ? ((h.naturalWidth|0) + 'x' + (h.naturalHeight|0)) : null,
               // ⚠️⚠️ ЦВЕТ КОНТУРА ЧИТАЕТСЯ ПО ПИКСЕЛЯМ САМОГО PNG, потому что
               // у картинки нет ни `fill`, ни computed-цвета: подмени ассет на
               // прежний чёрный — размер и рамка остались бы теми же, и оба
               // ассерта прошли бы зелёными. Белое и полупрозрачное отброшено,
               // берётся самый частый непрозрачный НЕ-белый тон.
               контур: h && h.tagName === 'IMG' ? доминанта(h) : null,
               вьюбокс: h && h.getAttribute('viewBox'),
               pAR: h && h.getAttribute('preserveAspectRatio'),
               путей: h ? h.querySelectorAll('path').length : 0,
               // ⚠️⚠️ ГРАДИЕНТЫ И МАСКА — НОВЫЕ НЕСУЩИЕ ПОЛЯ (макет 891:4205).
               // Плоские заливки ушли: 8 путей кисти залиты `url(#…)`. Считать
               // ЦВЕТА больше нельзя — computed вернёт строку `url("…")`, а не
               // rgb; считаем сами объявления.
               градиентовК: h ? h.querySelectorAll('linearGradient,radialGradient').length : 0,
               масокК: h ? h.querySelectorAll('mask').length : 0,
               // ⚠️⚠️ ДУБЛИ id ПО ВСЕЙ СТРАНИЦЕ. Figma назвала ОБЕ иконки
               // одинаково (`paint0_linear_0_4`), и на одной странице браузер
               // берёт ПЕРВОЕ объявление: вторая иконка залилась бы градиентом
               // первой МОЛЧА, без ошибки в консоли и без сдвига геометрии.
               // Лечение — префиксы `sh-`/`tip-`/`tipw-`; этот скан их стережёт.
               дублиId: (() => { const все = [...document.querySelectorAll('[id]')].map(e => e.id);
                 return [...new Set(все.filter((v, i) => все.indexOf(v) !== i))]; })(),
               // ⚠️ И ОБРАТНАЯ ПОЛОВИНА: ссылка `url(#…)`, которой некуда вести.
               // Переименуешь id, забыв ссылку, — путь зальётся ЧЁРНЫМ, а число
               // путей и рамка останутся верными.
               битыеСсылки: (() => { const сс = [];
                 document.querySelectorAll('*').forEach(e => {
                   for (const a of ['fill', 'stroke', 'mask', 'filter']) {
                     const v = e.getAttribute && e.getAttribute(a);
                     if (v && /^url\(#/.test(v)) сс.push(v.slice(5, -1)); } });
                 return [...new Set(сс.filter(id => !document.getElementById(id)))]; })(),
               кисть: hr ? [+(hr.left - r.left).toFixed(2), +(hr.top - r.top).toFixed(2),
                            +hr.width.toFixed(2), +hr.height.toFixed(2)] : null,
               // ⚠️⚠️ ИНК КИСТИ НА ЭКРАНЕ, А НЕ ТОЛЬКО РАМКА SVG. Вьюбокс у нас
               // СРЕЗАН (`5 1 …`), потому что в экспорте вокруг кисти оставлено
               // поле под тень. Рамка `<svg>` при неверном вьюбоксе осталась бы
               // на месте, а РИСУНОК внутри уехал бы и уменьшился — ловится
               // только замером самой группы путей.
               // ⚠️ У `<img>` НЕТ `<g>`, И БЕЗ ЭТОЙ ПРОВЕРКИ СНИМОК БРОСАЛ
               //    ИСКЛЮЧЕНИЕ ВНУТРИ evaluate: прогон умирал БЕЗ ВЕРДИКТА на
               //    597 зелёных (снаружи неотличимо от «оборвался сам»).
               //    Поле осталось ради копии лупы на победе — она ещё SVG.
               инк: (() => { const g = h && h.querySelector && h.querySelector('g');
               ...
               теньКисти: h ? getComputedStyle(h).filter : null,
               бейджX: +(lr.left - r.left).toFixed(1), бейджY: +(lr.top - r.top).toFixed(1),
               бейджНиз: +(r.bottom - lr.bottom).toFixed(1),
               бейджШ: Math.round(lr.width), бейджВ: Math.round(lr.height),
               текст: l.textContent, класс: l.className,
               бФон: cl.backgroundColor, бЦвет: cl.color, бРадиус: cl.borderRadius,
               бКегль: cl.fontSize + '/' + cl.fontWeight, бТень: cl.filter,
               низПодсказки: Math.round(hb.bottom), низКнопки: Math.round(r.bottom),
               центрПодсказки: +(hb.top + hb.height / 2).toFixed(1),
               центрКнопки: +(r.top + r.height / 2).toFixed(1),
               справаОтКромки: Math.round(innerWidth - r.right),
               снизуОтКромки: Math.round(innerHeight - r.bottom),
               правПодсказки: Math.round(hb.right), левКнопки: Math.round(r.left),
               // ⚠️ РЕШАЮЩАЯ ВЕЛИЧИНА, А НЕ ЛИТЕРАЛ: бейдж обязан показывать
               // РОВНО `level.shakes + purchasedShakes()`. Сверяя DOM с числом
               // «3», страж проверял бы лесенку бесплатных встрясок (её
               // владелец правил и ещё будет править) вместо своего свойства.
               своих: window.__game.level().shakes + window.__game.purchasedShakes(),
               адБезлимит: window.__game.level().adShakes === Infinity };
    });

    const ш0 = await снятьShake();
    console.log('shake-кисть 390:', JSON.stringify(ш0));
    // ⚠️⚠️ ПЛЕЧО `!/iconBtn/` ЗАВЕДЕНО 2026-08-22-в И НЕ ЯВЛЯЕТСЯ ПЕДАНТИЗМОМ:
    //    после ужатия кнопок до 56 общий класс `.iconBtn` (пауза, зум, крестики)
    //    имеет РОВНО ТОТ ЖЕ размер — и соблазн «вернуть класс, размер же
    //    совпал» стал реальным. Вернёт он вместе с собой круглую форму
    //    (`border-radius:1000px`), правило суток (`--btn-bg`) и снятую
    //    моторику `:active scale(.94)`. У подсказки такое плечо стояло с
    //    2026-08-21-е, у Shake не было ВООБЩЕ.
    expect(ш0.тег === 'BUTTON' && ш0.ария === null && !/iconBtn/.test(ш0.классКнопки || '') &&
           ш0.имяИзСодержимого === 'Shake ' + ш0.текст &&
           ш0.ширина === 56 && ш0.высота === 56 &&
           ш0.фон === 'rgba(255, 255, 255, 0.4)' && ш0.фонКартинка === 'none' &&
           ш0.радиус === '16px' &&
           ш0.тень === 'rgba(255, 255, 255, 0.7) 0px 4px 8px 0px inset' &&
           ш0.позиция === 'relative' && ш0.события === 'auto',
      '⚠️⚠️ SHAKE — 56×56 СО СТЕКЛЯННОЙ ПОДЛОЖКОЙ .40 И РАДИУСОМ 16 (' +
      JSON.stringify({ тег: ш0.тег, ария: ш0.ария, имя: ш0.имяИзСодержимого,
                       размер: [ш0.ширина, ш0.высота],
                       фон: ш0.фон, картинка: ш0.фонКартинка, радиус: ш0.радиус,
                       тень: ш0.тень, позиция: ш0.позиция,
                       события: ш0.события }) + '). ⛔⛔ ОТМЕНЯЕТ ПРЕЖНЕЕ ' +
      '«без подложки»: 2026-08-21-п владелец вернул кнопке фон и внутреннюю ' +
      'белую тень. ⛔⛔ ЕГО ЖЕ ЧИСЛА ПЕРЕЕХАЛИ 2026-08-22-в: фон .20 → .40, ' +
      'радиус 20 → 16, рамка 64 → 56, и «суперэллипс против круга» отменён — ' +
      'подсказка стала такой же. ⚠️ ФОН ПИНУЕТСЯ ЗНАЧЕНИЕМ, А НЕ «не ' +
      'прозрачный»: покой от наведения отличает ШАГ 40% → 80%, и «какая-то ' +
      'подложка» зеленела бы у залипшего ховера. ⚠️ В ПРАВИЛО СУТОК КНОПКА НЕ ВЕРНУЛАСЬ: ' +
      'белое стекло не красится ни днём, ни ночью. Прежний текст: ' +
      'макет владельца 886:3949, ' +
      'ПЕРЕРИСОВАН им же 2026-08-21-в: рамка была 80×80, стала 64×64 — числа ' +
      'здесь от второй редакции. ' +
      '⚠️⚠️ `ария === null` — НЕ ПРИДИРКА, А НЕСУЩЕЕ: `aria-label` ПЕРЕКРЫВАЕТ ' +
      'содержимое, и с ним доступное имя было бы ВСЕГДА «Shake» — незрячий ' +
      'игрок перестал бы слышать «Ad», а на «Ad» тап мгновенно крутит ролик ' +
      'без подтверждения. Имя обязано СОБИРАТЬСЯ из скрытого слова и бейджа. ' +
      'Диверсия — вернуть `aria-label`: размеры и цвета останутся верными, ' +
      'красным станет ровно это поле. ' +
      '⚠️ ПРОЗРАЧНОСТЬ ПРОВЕРЯЕТСЯ ДВУМЯ ПОЛЯМИ: `background-color` И ' +
      '`background-image` — вернуть подложку можно любым из них, и одного ' +
      'ассерта на цвет не хватило бы. ⚠️ `pointer-events:auto` В ЭТОМ ЖЕ ' +
      'АССЕРТЕ НЕСУЩЕЕ: `.bar` глушит события, без него тап уходит сквозь ' +
      'кнопку в канвас, а геометрия при этом остаётся верной. ⚠️ `position` ' +
      'тоже: без него абсолютный бейдж привяжется к `.shakeWrap`. ⚠️ Тег ' +
      'BUTTON держит курсор-руку, звук `ui` и клик `requestShake`');

    const близко = (a, b) => Math.abs(a - b) < 0.05;
    // ⛔⛔ ЧЕТВЁРТАЯ РЕДАКЦИЯ ЗА СУТКИ, И ПЕРВАЯ, ГДЕ СМЕНИЛСЯ СПОСОБ, А НЕ
    // РИСУНОК (макет 891:4205). Плоский контур стал ОБЪЁМОМ: 18 путей вместо
    // 12, ВОСЕМЬ линейных градиентов и маска внешней обводки, тень ВЕРНУЛАСЬ
    // (0 3px 12px, 12%), вьюбокс снова со срезом `14 11 …`. Прежние пины
    // (`0 0 63.9998 58.2206`, 12 путей, `тень === none`, счёт цветов `#2E3F61`)
    // краснели бы на ИСПРАВНОЙ сборке — страж переехал за макетом.
    // ⚠️⚠️ ЦВЕТА БОЛЬШЕ НЕ СЧИТАЮТСЯ ПОШТУЧНО: 8 путей залиты `url(#…)`, и
    // computed отдаёт ссылку, а не rgb. Обоснование «кисть несёт СВОИ цвета из
    // ассета» держат теперь ГРАДИЕНТЫ: перекрасишь её чужим
    // `svg path { fill: var(--btn-fg) }` — счёт градиентных заливок рухнет.
    const градЗаливок = Object.keys(ш0.цвета).filter(k => /^url\(/.test(k)).length;
    // ⛔⛔ ИКОНКА БОЛЬШЕ НЕ SVG, А PNG ВЛАДЕЛЬЦА (слово 2026-08-21-р «обнови
    // иконки лупы и руки, не меняй их форму и не модифицируй, они ровно под
    // бокс кнопки»). Здесь стояли пины на вьюбокс `14 11 60 54.5823`, 18 путей,
    // 8 градиентов, маску, тень и симметрию чернил — всего этого нет ВОВСЕ, и
    // прежний ассерт краснел бы на исправной сборке.
    // ⚠️ ЧТО СТЕРЕЖЁМ ТЕПЕРЬ: тег IMG (а не оставшийся SVG), КАРТИНКА ЖИВАЯ
    // (декодировалась, натуральные **168×168** — то есть ассет доехал, а не
    // битая ссылка) и РАМКА РОВНО ПО КНОПКЕ 64×64 без единого пикселя отступа.
    // ⛔ ЧИСЛО 192 ЗДЕСЬ ПРОТУХЛО 2026-08-22: владелец подменил оба PNG на диске
    // (168×168, контур индиго вместо чёрного). Комментарий переехал вместе с
    // пином — иначе следующий читал бы объяснение, спорящее с самим ассертом.
    // ⚠️ «РОВНО ПОД БОКС» ПРОВЕРЯЕТСЯ ЧЕТЫРЬМЯ НУЛЯМИ, А НЕ ОДНОЙ ШИРИНОЙ:
    // `object-fit` или чужой `padding` дали бы верную ширину при съехавшем
    // рисунке. ⚠️ И ФИЛЬТР: «не модифицируй» — это и про тень, её вернуть
    // проще всего.
    expect(ш0.естьКисть && ш0.тегИконки === 'IMG' && ш0.натур === '168x168' &&
           ш0.контур === '72,68,114' &&
           близко(ш0.кисть[0], 0) && близко(ш0.кисть[1], 0) &&
           близко(ш0.кисть[2], 56) && близко(ш0.кисть[3], 56) &&
           ш0.теньКисти === 'none' &&
           ш0.дублиId.length === 0 && ш0.битыеСсылки.length === 0,
      '⚠️⚠️ КИСТЬ — PNG ВЛАДЕЛЬЦА ВО ВЕСЬ БОКС КНОПКИ, БЕЗ ФИЛЬТРОВ (' +
      JSON.stringify({ тег: ш0.тегИконки, натуральные: ш0.натур, рамка: ш0.кисть,
                       фильтр: ш0.теньКисти, контур: ш0.контур }) +
      '). ⛔ ПАРТИЯ ВЛАДЕЛЬЦА 2026-08-22: 192×192 с ЧЁРНЫМ контуром заменена на ' +
      '168×168 с индиго `#484472` — он подменил файлы по тем же путям, форма ' +
      'бит-в-бит та же. ⛔ ОТМЕНЯЕТ инлайновый SVG ' +
      'на 92 КБ (18 путей, 8 градиентов, маска) — вместе с ним ушли префиксы ' +
      'id `sh-`. ⚠️ Проверка дублей id ОСТАВЛЕНА: на странице ещё живёт копия ' +
      'лупы на экране победы со своим `tipw-`, и правило «два SVG из одного ' +
      'файла Figma обязаны получить разные префиксы» в силе');

    // ⛔⛔ БЕЙДЖ ПЕРЕЕХАЛ В ЛЕВЫЙ НИЖНИЙ УГОЛ (2026-08-21-п). Было: точка
    // 28.5/40, высота 24, тень `0 2px 4px`. Стало: угол (0, низ), высота 22,
    // тень `0 2px 8px`. Прежние пины краснели бы на исправной сборке.
    // ⚠️ УГОЛ ПРОВЕРЯЕТСЯ ОТ НИЖНЕЙ КРОМКИ, А НЕ ЧЕРЕЗ `бейджY`: высота бейджа
    // менялась уже дважды, и пин по верху пришлось бы пересчитывать каждый раз —
    // а требование владельца звучит про УГОЛ.
    // ⛔⛔ И САМ УГОЛ ПЕРЕЕХАЛ 2026-08-22-в: `бейджНиз` теперь −6, потому что
    // владелец сказал «счетчик выходит на 6 пикселей ниже по нижней грани».
    // ЗНАК ОТРИЦАТЕЛЬНЫЙ ПО ПОСТРОЕНИЮ МЕТРИКИ: `бейджНиз = кнопка.bottom −
    // бейдж.bottom`, то есть выступ ВНИЗ даёт минус. Ноль здесь означал бы
    // «заподлицо» — ровно то состояние, которое он и просил изменить.
    expect(близко(ш0.бейджX, 0) && близко(ш0.бейджНиз, -6) &&
           ш0.бейджВ === 22 && ш0.бейджШ === 22 &&
           ш0.бФон === 'rgb(192, 255, 71)' && ш0.бЦвет === 'rgb(74, 113, 0)' &&
           ш0.бРадиус === '32px' && ш0.бКегль === '14px/800' &&
           ш0.бТень === 'drop-shadow(rgba(0, 0, 0, 0.16) 0px 2px 8px)' &&
           // ⚠️ ПОТОЛОК ЕДЕТ ЗА ШИРИНОЙ КНОПКИ: 64 → 56. Литерал 64 читается
           //    как «магическое число бейджа», а это ШИРИНА КНОПКИ — на 56 он
           //    остался бы истинным и перестал бы что-либо ограничивать.
           ш0.бейджX + ш0.бейджШ <= 56,
      '⚠️⚠️ ЛАЙМОВЫЙ БЕЙДЖ В ЛЕВОМ НИЖНЕМ УГЛУ КНОПКИ, 22×22 (' +
      JSON.stringify({ точка: [ш0.бейджX, ш0.бейджY], отНиза: ш0.бейджНиз,
                       размер: [ш0.бейджШ, ш0.бейджВ],
                       фон: ш0.бФон, цвет: ш0.бЦвет, радиус: ш0.бРадиус,
                       кегль: ш0.бКегль, тень: ш0.бТень }) + '). ' +
      '#c0ff47 = токен Experimental/Electric lime; ⛔ ЦВЕТ ЦИФРЫ ПЕРЕЕЗЖАЛ ' +
      'ДВАЖДЫ ЗА СУТКИ: чёрный -> #5a8605 (2026-08-21-е) -> #4a7100 ' +
      '(2026-08-21-ж). Каждый прежний пин краснел бы на исправной сборке — ' +
      'ровно поэтому цвет ассертится ЗНАЧЕНИЕМ, а не «не чёрный». ' +
      '⚠️ ШИРИНА 22 АССЕРТИТСЯ ТОЛЬКО У ОДНОЗНАЧНОГО ЧИСЛА: она ' +
      'выведена из падинга 6 и `min-width:22`, то есть от шрифта не зависит. ' +
      'Ширину «Ad» (в ноде 35) НЕ ПИНУЕМ ЧИСЛОМ — её задают метрики шрифта, а ' +
      'headless рисует не ui-rounded, и литерал краснел бы на исправной сборке. ' +
      '⚠️ ТЕНЬ СВЕРЯЕТСЯ ЗНАЧЕНИЕМ, А НЕ ПОДСТРОКОЙ: `/drop-shadow/` было бы ' +
      'зелено при любых числах, хотя текст ассерта называет макетные 0 2 4 .16. ' +
      '⚠️ И ПОТОЛОК: правый край бейджа обязан остаться ВНУТРИ кнопки — это ' +
      'ровно тот контракт, который CSS-комментарий доказывает арифметикой ' +
      '(запас справа после перерисовки 2026-08-21-в почти исчерпан: «Ad» ' +
      'встаёт правым краем в 63.5 при рамке 64), и без него ширина не ' +
      'ограничена сверху ничем');

    // ⚠️⚠️ ПЕРЕХОД ИСПОЛНЯЕТСЯ БОЕВЫМ ПУТЁМ, А НЕ ЗАПИСЬЮ В СОСТОЯНИЕ: бейдж
    // перекрашивает `updateHUD`, а он зовётся ПО СОБЫТИЯМ, не по таймеру.
    // Прямая запись `level().shakes = 0` оставляет на экране прежнее число —
    // проверено пробой, страж на ней был бы зелен при любой реализации.
    // ⚠️⚠️ ПОКУПНАЯ ПОЛОВИНА РЕШАЮЩЕЙ ВЕЛИЧИНЫ ИСПОЛНЯЕТСЯ ОТДЕЛЬНО. Бейдж
    // показывает `level.shakes + purchasedShakes()`, и у второго слагаемого
    // есть СВОЙ комментарий в 85-hud («без этого игрок с 50 оплаченными
    // встрясками видел бы No shakes»). Проверяя только бесплатные, страж
    // остался бы зелен у сборки, где покупные потерялись из формулы: игрок с
    // бандлом увидел бы «Ad» вместо своего запаса. Даём 5 — число, которого
    // при бесплатной лесенке на первом уровне (3) быть не может.
    await стр.evaluate(() => { window.__game.mergeRaw({ pe: 5 });
      window.__game.level().shakes = 1; window.__game.requestShake(); });
    await стр.waitForFunction(() => document.getElementById('shakeLbl').textContent === '5',
      { timeout: 5000 }).catch(() => {});
    const шКупл = await снятьShake();
    // гасим покупной запас (ps монотонен и мержится по max) и уходим в «Ad»
    await стр.evaluate(() => { window.__game.mergeRaw({ ps: 5 });
      window.__game.level().shakes = 1; window.__game.requestShake(); });
    await стр.waitForFunction(() => document.getElementById('shakeLbl').textContent === 'Ad',
      { timeout: 5000 }).catch(() => {});
    const шAd = await снятьShake();
    await стр.evaluate(() => { window.__game.level().shakes = 2; window.__game.requestShake(); });
    await стр.waitForFunction(() => document.getElementById('shakeLbl').textContent === '1',
      { timeout: 5000 }).catch(() => {});
    const шНазад = await снятьShake();
    console.log('shake-состояния:', JSON.stringify({ было: [ш0.текст, ш0.класс],
      купл: [шКупл.текст, шКупл.класс], ad: [шAd.текст, шAd.класс, шAd.бейджШ, шAd.бейджX + шAd.бейджШ],
      назад: [шНазад.текст, шНазад.класс, шНазад.бейджШ],
      классыКнопки: [ш0.классКнопки, шКупл.классКнопки, шAd.классКнопки, шНазад.классКнопки] }));
    expect(ш0.своих > 0 && ш0.текст === String(ш0.своих) && ш0.класс === '' &&
           шКупл.своих === 5 && шКупл.текст === String(шКупл.своих) && шКупл.класс === '' &&
           шAd.своих === 0 && шAd.текст === 'Ad' && шAd.класс === 'ad' &&
           шAd.бейджШ > шAd.бейджВ && шAd.бейджX + шAd.бейджШ <= 56 &&
           шНазад.своих > 0 && шНазад.текст === String(шНазад.своих) &&
           шНазад.класс === '' && шНазад.бейджШ === 22 &&
           // ⛔ «Ad» ТЕПЕРЬ ФИОЛЕТОВЫЙ С БЕЛЫМ ТЕКСТОМ (нода 892:2066,
           // 2026-08-21-п). Прежде оба состояния были одного цвета и
           // отличались только шириной; теперь состояние видно ЦВЕТОМ, и это
           // проверяется ПЕРЕХОДОМ — до «Ad» лайм, в «Ad» фиолет, после лайм.
           ш0.бФон === 'rgb(192, 255, 71)' &&
           шAd.бФон === 'rgb(197, 71, 255)' && шAd.бЦвет === 'rgb(255, 255, 255)' &&
           шНазад.бФон === 'rgb(192, 255, 71)' &&
           ![ш0, шКупл, шAd, шНазад].some(с => /\bad\b/.test(с.классКнопки)) &&
           ш0.адБезлимит,
      '⚠️⚠️ БЕЙДЖ ГОВОРИТ ПРАВДУ И ПЕРЕКЛЮЧАЕТСЯ В ОБЕ СТОРОНЫ (' +
      JSON.stringify({ старт: [ш0.текст, ш0.класс, ш0.своих],
                       покупные: [шКупл.текст, шКупл.класс, шКупл.своих],
                       послеТраты: [шAd.текст, шAd.класс, шAd.своих],
                       возврат: [шНазад.текст, шНазад.класс, шНазад.своих] }) + '). ' +
      '⚠️⚠️ СВЕРЯЕМСЯ С РЕШАЮЩЕЙ ВЕЛИЧИНОЙ `level.shakes + purchasedShakes()`, ' +
      'А НЕ С ЛИТЕРАЛОМ: «3» — это лесенка бесплатных встрясок, её владелец ' +
      'правил и ещё будет править, и страж на литерале краснел бы на ИСПРАВНОЙ ' +
      'сборке при первой же смене лесенки. ' +
      '⚠️ ПРОВЕРЯЕТСЯ ПЕРЕХОД, А НЕ ЗНАЧЕНИЕ: «текст === Ad» истинно и на ' +
      'сборке, где бейдж залип на «Ad» навсегда. ⚠️ КЛАСС `.ad` ЖИВЁТ НА ' +
      'БЕЙДЖЕ, А НЕ НА КНОПКЕ (зеркало `#hintCnt.ad`) — держать его в обоих ' +
      'местах значило бы завести вторую истину о состоянии. ⚠️ САНИТАР ' +
      '`адБезлимит` В ТОМ ЖЕ АССЕРТЕ: `AD_SHAKES_PER_LEVEL = Infinity`, и ' +
      'именно поэтому владелец дал ДВА макета — третье состояние («0» с ' +
      'гашением) в живой игре не наступает. Станет константа конечной — ' +
      'санитар покраснеет и напомнит, что макета на третье состояние нет. ' +
      '⚠️ ПОКУПНОЕ ПЛЕЧО («5») ИСПОЛНЯЕТСЯ ОТДЕЛЬНО: бейдж показывает СУММУ ' +
      'бесплатных и оплаченных, и без этого плеча страж был бы зелен у ' +
      'сборки, где покупные выпали из формулы — игрок с бандлом увидел бы ' +
      '«Ad» вместо своего запаса. ⚠️ КЛАСС КНОПКИ ЧИТАЕТСЯ ВО ВСЕХ ЧЕТЫРЁХ ' +
      'состояниях: заголовок этого ассерта утверждает, что `.ad` живёт на ' +
      'бейдже и НЕ на кнопке, а до разбора это утверждение не проверялось ' +
      'ничем. ⚠️ И ВЕРХНЯЯ ГРАНИЦА У «Ad»: без неё «шире, чем высота» — ' +
      'односторонний порог, зелёный и у бейджа, вылезшего за кнопку');

    // ⚠️⚠️ СОСЕД, КОТОРОГО ВЛАДЕЛЕЦ ТРОГАТЬ НЕ ПРОСИЛ. У `.grp` стоит
    // `align-items:center`; пока подсказка и Shake были обе 56, он ничего не
    // значил, а кисть 80 подняла БЫ подсказку на 12px над нижней кромкой.
    // Лечится точечным `#bottomBar .grpShake { align-items:flex-end }`.
    // ⚠️ ПЕРВАЯ РЕДАКЦИЯ ПРАВИЛА МОЛЧА ПРОИГРАЛА ПО ПОРЯДКУ В ФАЙЛЕ (у `.grp`
    // и `.grpShake` специфичность одинаковая) — поймано этим замером, не
    // чтением. Поэтому ассерт стоит НА ГЕОМЕТРИИ, а не на наличии класса.
    const дескШ = await browser.newPage({ viewport: { width: 1280, height: 832 } });
    дескШ.on('pageerror', e => errors.push('PAGEERROR(shake-десктоп): ' + e.message));
    await дескШ.goto('file://' + PAGE_FILE + '?dev=1');
    await дескШ.waitForFunction(() => window.__game && window.__game.alive() > 0, { timeout: 60000 });
    await дескШ.evaluate(() => { window.__game.skipIntro(); });
    const дШ = await дескШ.evaluate(() => {
      ...
      return { низКнопки: Math.round(b.bottom), низПодсказки: Math.round(h.bottom),
               центрПодсказки: +(h.top + h.height / 2).toFixed(1),
               центрКнопки: +(b.top + b.height / 2).toFixed(1),
               правПодсказки: Math.round(h.right), левКнопки: Math.round(b.left),
               справаОтКромки: Math.round(innerWidth - b.right),
               снизуОтКромки: Math.round(innerHeight - b.bottom) };
    });
    ...
    expect(Math.abs(ш0.центрПодсказки - ш0.центрКнопки) <= 0.6 &&
           ш0.левКнопки - ш0.правПодсказки === 16 &&
           ш0.справаОтКромки === 16 && ш0.снизуОтКромки === 16 &&
           Math.abs(дШ.центрПодсказки - дШ.центрКнопки) <= 0.6 &&
           // ⚠️⚠️ ЗАЗОР ЗАДАН ОДНИМ ПРАВИЛОМ ВНЕ МЕДИАЗАПРОСА, А ПИНОВ ДВА —
           // мобильный выше и этот. Правя один, легко забыть второй: первая
           // редакция правки 2026-08-22-в так и сделала, и десктопное плечо
           // покраснело бы на ИСПРАВНОЙ сборке. Держать их рядом по значению.
           дШ.левКнопки - дШ.правПодсказки === 16 &&
           дШ.справаОтКромки === 24 && дШ.снизуОтКромки === 24,
      '⚠️⚠️ КЛАСТЕР НИЖНЕГО БАРА ПО ТРЁМ ЧИСЛАМ ВЛАДЕЛЬЦА 2026-08-21-д (' +
      JSON.stringify({ моб: { центры: [ш0.центрПодсказки, ш0.центрКнопки],
                              зазор: ш0.левКнопки - ш0.правПодсказки,
                              отКромок: [ш0.справаОтКромки, ш0.снизуОтКромки] },
                       деск: { центры: [дШ.центрПодсказки, дШ.центрКнопки],
                               зазор: дШ.левКнопки - дШ.правПодсказки,
                               отКромок: [дШ.справаОтКромки, дШ.снизуОтКромки] } }) + '). ' +
      'Его слова: «отодвинь иконку шейка от правого края и снизу на 8 px», ' +
      '«увеличь отступ между иконками на 8 px», «иконки отцентрируй между ' +
      'друг другом». ⚠️ «НА 8» ПРОЧИТАНО КАК ДЕЛЬТА: было 8/8 на мобильном и ' +
      '16/16 на десктопе (падинги бара) — стало 16/16 и 24/24. Поэтому числа ' +
      'РАЗНЫЕ на двух вьюпортах, и одного замера не хватило бы. ' +
      '⚠️⚠️ ЦЕНТРЫ, А НЕ НИЗЫ: прежняя редакция ассерта требовала РОВНЫХ НИЗОВ ' +
      '(я держал подсказку на месте, когда кисть выросла) — владелец посмотрел ' +
      'на кадр и захотел обратного. Страж переехал за правилом, а не «починен». ' +
      '⛔⛔ ЧЕСТНАЯ ОГОВОРКА 2026-08-21-е: ПЛЕЧО ЦЕНТРОВ БОЛЬШЕ НЕ РАЗЛИЧАЕТ ' +
      'ДИВЕРСИЮ. Прежде здесь было написано «вернуть `align-items:flex-end` — ' +
      'центры разъедутся на 4», и это работало ровно потому, что подсказка была ' +
      '56, а кисть 64. Подсказка стала ЛУПОЙ 64×64 — при равных высотах `center` ' +
      'и `flex-end` дают ОДНО И ТО ЖЕ, и та диверсия теперь проходит зелёной. ' +
      'Плечо оставлено РЕГРЕССИОННЫМ ПИНОМ на день, когда размеры снова ' +
      'разъедутся; различают сегодня ЗАЗОР и ПОЛЯ, они ниже. Это ровно закон ' +
      '«страж не ломается, а ЗАМОЛКАЕТ» — и замолчал бы молча, если бы разведка ' +
      'не назвала это до правки. ' +
      '⚠️ ЗАЗОР ПИНУЕТСЯ ЧИСЛОМ, А НЕ «подсказка левее»: «левее» истинно и при ' +
      'зазоре 0, и при 100 — у горизонтали не было бы ограничителя вовсе');

    // ── ПОДСКАЗКА = ИКОНКА-ЛУПА С ГОЛУБЫМ БЕЙДЖЕМ (2026-08-21-е) ──────────
    // Макеты 887:4051 / 887:4057. ⚠️⚠️ ВИД ПОДСКАЗКИ НЕ СТЕРЁГ НИКТО — ровно
    // как было у Shake до его переделки: `#hintCnt` встречался в сьюте ОДИН
    // раз, и то внутри чужого сообщения. Замена круглой тёмной кнопки на лупу
    // прошла бы зелёной в ОБЕ стороны.
    const лупа = await стр.evaluate(() => {
      // ⚠️⚠️ ПОМОЩНИК ЧИТАЕТ ПИКСЕЛИ PNG: у картинки нет ни `fill`, ни
      // computed-цвета, а подмена ассета на прежний чёрный оставила бы и размер,
      // и рамку прежними. Белое и полупрозрачное отбрасывается, отдаётся самый
      // частый непрозрачный НЕ-белый тон. `data:`-картинка холст не пятнает.
      ...
      const m = b.querySelector('.tip-mag');   // ⚠️ без `svg`: теперь это <img>
      ...
      const cs = getComputedStyle(b), cl = getComputedStyle(l);
      const цвета = {}; if (m) m.querySelectorAll('path').forEach(pa => {
        const f = getComputedStyle(pa).fill; цвета[f] = (цвета[f] || 0) + 1; });
      return { тег: b.tagName, ария: b.getAttribute('aria-label'), классы: b.className,
        ширина: Math.round(r.width), высота: Math.round(r.height),
        фон: cs.backgroundColor, фонКартинка: cs.backgroundImage,
        события: cs.pointerEvents, позиция: cs.position,
        естьЛупа: !!m, вьюбокс: m && m.getAttribute('viewBox'),
        pAR: m && m.getAttribute('preserveAspectRatio'),
        путей: m ? m.querySelectorAll('path').length : 0, цвета,
        градиентов: m ? m.querySelectorAll('linearGradient,radialGradient').length : 0,
        лупаБокс: mr ? [+(mr.left - r.left).toFixed(2), +(mr.top - r.top).toFixed(2),
                        +mr.width.toFixed(2), +mr.height.toFixed(2)] : null,
        инк: g ? [+(g.left - r.left).toFixed(2), +(g.top - r.top).toFixed(2),
                  +g.width.toFixed(2), +g.height.toFixed(2)] : null,
        тень: m && getComputedStyle(m).filter,
        // ⚠️ ИМЯ `теньКнопки`, А НЕ `тень`: `тень` в этом же снимке — это
        // drop-shadow САМОГО SVG лупы, и одноимённое поле молча перекрыло бы
        // чужой пин двумя ассертами ниже.
        тегИконки: m && m.tagName,
        натур: m ? ((m.naturalWidth|0) + 'x' + (m.naturalHeight|0)) : null,
        // ⚠️ Тот же приём, что у кисти: цвет PNG наблюдаем только пикселями.
        контур: m && m.tagName === 'IMG' ? доминанта(m) : null,
        радиус: cs.borderRadius, теньКнопки: cs.boxShadow,
        бейджX: +(lr.left - r.left).toFixed(1), бейджY: +(lr.top - r.top).toFixed(1),
        бейджНиз: +(r.bottom - lr.bottom).toFixed(1),
        бейджШ: Math.round(lr.width), бейджВ: Math.round(lr.height),
        бФон: cl.backgroundColor, бЦвет: cl.color, бРадиус: cl.borderRadius,
        бКегль: cl.fontSize + '/' + cl.fontWeight, бТень: cl.filter,
        текст: l.textContent, класс: l.className,
        зарядов: window.__game.wallet().hints };
    });
    console.log('подсказка-лупа:', JSON.stringify(лупа));
    expect(лупа.тег === 'BUTTON' && лупа.ария === 'Hint' && !/iconBtn/.test(лупа.классы) &&
           лупа.ширина === 56 && лупа.высота === 56 &&
           лупа.фон === 'rgba(255, 255, 255, 0.4)' && лупа.фонКартинка === 'none' &&
           лупа.радиус === '16px' &&
           лупа.теньКнопки === 'rgba(255, 255, 255, 0.7) 0px 4px 8px 0px inset' &&
           лупа.события === 'auto' && лупа.позиция === 'relative',
      '⚠️⚠️ ПОДСКАЗКА — 56×56 СО СТЕКЛЯННОЙ ПОДЛОЖКОЙ .40, РАДИУС 16 (' +
      JSON.stringify({ тег: лупа.тег, классы: лупа.классы, размер: [лупа.ширина, лупа.высота],
                       фон: лупа.фон, радиус: лупа.радиус, тень: лупа.теньКнопки,
                       события: лупа.события }) + '). ' +
      '⛔⛔ ОТМЕНЯЕТ ПРЕЖНЕЕ «без круглой подложки» (2026-08-21-е): 2026-08-21-п ' +
      'владелец вернул фон и внутреннюю белую тень. ⛔⛔ А КРУГ ОТМЕНЁН ' +
      '2026-08-22-в: обе кнопки стали 56×56 с радиусом 16 и фоном .40, то есть ' +
      'ОДНОЙ формы. ⚠️ СЛЕДСТВИЕ ДЛЯ ЭТОГО ЖЕ СТРАЖА, НАЗЫВАЮ ЧЕСТНО: радиус ' +
      'был ЕДИНСТВЕННЫМ внешним различием пары, и теперь перестановка кнопок ' +
      'местами оставила бы оба вида верными. Различают их ИКОНКА (пины `натур` ' +
      'и `контур` выше) и СТОРОНА в кластере (плечо зазора), а не форма. ' +
      'Прежний текст: макет 887:4051. ' +
      '⚠️⚠️ `pointer-events:auto` ЗДЕСЬ САМОЕ ОПАСНОЕ МЕСТО ПРАВКИ: его давал ' +
      'снятый класс `.iconBtn`, а `.bar` глушит события — забудь выписать, и ' +
      'кнопка останется ВИДИМОЙ, но тап провалится в канвас и провернёт камеру. ' +
      '⚠️ `!iconBtn` в том же ассерте: класс обязан быть снят, иначе вернётся ' +
      'тёмный круг 56 поверх лупы. ⚠️ Тег BUTTON держит курсор-руку, звук `ui` ' +
      'и клик; id `hintBtn` дереференсят ТРИ чужие секции без проверки на null — ' +
      'переименуешь, получишь не красный ассерт, а падение прогона');

    const блЛ = (a, b) => Math.abs(a - b) < 0.05;
    // ⛔⛔ ЛУПА ТОЖЕ СТАЛА ОБЪЁМНОЙ (макет 891:4199): 3 пути вместо 4,
    // радиальный градиент стекла плюс линейный, тень ВЕРНУЛАСЬ
    // (0 2.895px 11.579px, 12%), вьюбокс срезан `14.48 11.58 …`. Прежние пины
    // краснели бы на исправной сборке.
    // ⚠️ ГРАДИЕНТОВ РОВНО 2, А ЗАЛИВКА `url(#…)` ОДНА: стекло градиентное,
    // ободок и ручка белые. Считаем и то, и другое — счёт объявлений ловит
    // выпиленный `<defs>`, счёт заливок ловит перекраску путей поверх.
    const градЛупы = Object.keys(лупа.цвета).filter(k => /^url\(/.test(k)).length;
    // ⛔⛔ ТО ЖЕ У ЛУПЫ: PNG вместо SVG (2026-08-21-р). Разбор — у кисти выше.
    // ⚠️ КОПИЯ ЛУПЫ НА ЭКРАНЕ ПОБЕДЫ ОСТАЛАСЬ SVG — так в ноде 891:4317, и
    // отдельный страж ниже по файлу её и стережёт. Это НЕ рассинхрон: в баре
    // иконка занимает весь бокс кнопки, на победе — слот 54 внутри пилюли.
    expect(лупа.естьЛупа && лупа.тегИконки === 'IMG' && лупа.натур === '168x168' &&
           лупа.контур === '72,68,114' &&
           блЛ(лупа.лупаБокс[0], 0) && блЛ(лупа.лупаБокс[1], 0) &&
           блЛ(лупа.лупаБокс[2], 56) && блЛ(лупа.лупаБокс[3], 56) &&
           лупа.тень === 'none',
      '⚠️⚠️ ЛУПА — PNG ВЛАДЕЛЬЦА ВО ВЕСЬ БОКС КНОПКИ, БЕЗ ФИЛЬТРОВ (' +
      JSON.stringify({ тег: лупа.тегИконки, натуральные: лупа.натур,
                       рамка: лупа.лупаБокс, фильтр: лупа.тень,
                       контур: лупа.контур }) + '). ⛔ ПАРТИЯ ВЛАДЕЛЬЦА ' +
      '2026-08-22: 192×192 чёрная заменена на 168×168 индиго `#484472`');

    // ⛔⛔ БЕЙДЖ ПОДСКАЗКИ ПЕРЕЕХАЛ И ПЕРЕКРАСИЛСЯ (2026-08-21-п). Было: точка
    // (4, 40), высота 24, РОЗОВЫЙ `#ffa5b7`/`#871048`. Стало: левый нижний угол,
    // высота 22, ЛАЙМ — тот же, что у Shake. ⚠️ ПАЛИТРА ЭТОГО БЕЙДЖА СМЕНИЛАСЬ
    // ПЯТЫЙ РАЗ ЗА СУТКИ (лайм → голубая → лаймовая → розовая → лайм), и ровно
    // поэтому цвет пинуется ЗНАЧЕНИЕМ, а не «не такой, как был».
    // ⚠️ И ТЕПЕРЬ ОБА БЕЙДЖА ОДИНАКОВЫ — сверяем их РАВЕНСТВОМ ниже, а не двумя
    // независимыми литералами: требование владельца было про ОДИН стиль.
    expect(блЛ(лупа.бейджX, 0) && блЛ(лупа.бейджНиз, -6) &&
           лупа.бейджВ === 22 && лупа.бейджШ === 22 &&
           лупа.бФон === 'rgb(192, 255, 71)' && лупа.бЦвет === 'rgb(74, 113, 0)' &&
           лупа.бРадиус === '32px' && лупа.бКегль === '14px/800' &&
           лупа.бТень === 'drop-shadow(rgba(0, 0, 0, 0.16) 0px 2px 8px)' &&
           лупа.бейджX + лупа.бейджШ <= 56 &&
           // ⚠️⚠️ РАВЕНСТВО С БЕЙДЖЕМ SHAKE — ГЛАВНАЯ ПОЛОВИНА ЭТОГО АССЕРТА.
           // Владелец задал ОДИН стиль на оба бейджа, значит требование — это
           // СОВПАДЕНИЕ двух мест, а не два одинаковых литерала. Правь кто-то
           // один из них — литералы промолчали бы наполовину.
           лупа.бФон === ш0.бФон && лупа.бЦвет === ш0.бЦвет &&
           лупа.бейджВ === ш0.бейджВ && лупа.бТень === ш0.бТень &&
           лупа.бРадиус === ш0.бРадиус && лупа.бКегль === ш0.бКегль &&
           лупа.текст === String(лупа.зарядов) && лупа.класс === '',
      '⚠️⚠️ ЛАЙМОВЫЙ БЕЙДЖ В ЛЕВОМ НИЖНЕМ УГЛУ, ТОТ ЖЕ, ЧТО У SHAKE, И ГОВОРИТ ПРАВДУ (' +
      JSON.stringify({ точка: [лупа.бейджX, лупа.бейджY], размер: [лупа.бейджШ, лупа.бейджВ],
                       фон: лупа.бФон, цвет: лупа.бЦвет, текст: лупа.текст,
                       зарядов: лупа.зарядов }) + '). Ноды 887:4055 / 887:4061: ' +
      '⛔⛔ ПАЛИТРА ЭТОГО БЕЙДЖА СМЕНИЛАСЬ ПЯТЬ РАЗ ЗА СУТКИ, ДЕЙСТВУЮЩАЯ — ' +
      'ЛАЙМ #c0ff47/#4a7100 (слово владельца 2026-08-21-п, нода 892:2069). До ' +
      'неё: лайм → голубая #9ce2ff/#1a6c8e → лаймовая → РОЗОВАЯ #ffa5b7/#871048 ' +
      '(нода 887:4061) → снова лайм. Каждый прежний пин краснел бы на исправной ' +
      'сборке, ровно поэтому цвет ассертится ЗНАЧЕНИЕМ. ' +
      '⛔ И ПАРА БОЛЬШЕ НЕ РАЗНОЦВЕТНАЯ: раньше у подсказки был свой цвет, у ' +
      'Shake свой, и это было проверено чтением обеих нод. Теперь у них ОДИН ' +
      'стиль — поэтому сверка идёт РАВЕНСТВОМ с бейджем Shake. ' +
      '⛔ И СТОРОНА ОДНА: раньше бейдж подсказки стоял слева (4,40), а у Shake ' +
      'справа (28.5,40) — пара «смотрела наружу от центра бара». Теперь оба в ' +
      'ЛЕВОМ НИЖНЕМ УГЛУ своей кнопки. ⚠️ ТЕКСТ СВЕРЯЕТСЯ С ' +
      'РЕШАЮЩЕЙ ВЕЛИЧИНОЙ `wallet().hints`, а не с литералом: заряды растут с ' +
      'уровнем, и число в страже протухло бы при первой правке лесенки');

    // ── ЧЁРНАЯ ОБВОДКА 6 px У УРОВНЯ И ОЧКОВ (слово владельца 2026-08-21-з) ──
    // ⚠️ Пишем и проверяем ТОЛЬКО `--otl`/`--otl-color` — удвоение под заливку
    // спрятано в `.otext text`, поэтому computed `stroke-width` = 12 при 6.
    const обв = async (стр2) => стр2.evaluate(() => {
      const снять = сел => { const e = document.querySelector(сел); if (!e) return null;
        const cs = getComputedStyle(e);
        return { обводка: cs.strokeWidth, цвет: cs.stroke, заливка: cs.fill,
                 кегль: cs.fontSize, пятно: cs.fill === cs.stroke }; };
      return { LV: снять('#lvlSvg text'), очки: снять('#scSvg text'),
               ширина: innerWidth };
    });
    const обвМоб = await обв(стр), обвДеск = await обв(дескШ);
    console.log('обводка:', JSON.stringify({ моб: обвМоб, деск: обвДеск }));
    // ⛔ ЦВЕТ ПЕРЕЕХАЛ 2026-08-21-м: владелец назвал `#113444` вместо чистого
    // чёрного. Страж идёт ЗА правилом — прежний `rgb(0, 0, 0)` покраснел бы.
    // ⚠️ САНИТАР «ПЯТНО» НИЖЕ ВАЖЕН ТЕПЕРЬ БОЛЬШЕ, А НЕ МЕНЬШЕ: #113444 почти
    // чёрный, и на чёрной заливке десктопного уровня слился бы так же — цвет
    // сменился, ловушка нет.
    const ОБВ = 'rgb(17, 52, 68)';   // #113444
    expect(обвМоб.очки.обводка === 'calc(8px)' && обвМоб.очки.цвет === ОБВ &&
           обвДеск.очки.обводка === 'calc(8px)' && обвДеск.очки.цвет === ОБВ &&
           обвМоб.LV.обводка === 'calc(8px)' && обвМоб.LV.цвет === ОБВ &&
           !обвМоб.LV.пятно && !обвМоб.очки.пятно && !обвДеск.LV.пятно && !обвДеск.очки.пятно,
      '⚠️⚠️ ОБВОДКА #113444 ТОЛЩИНОЙ 4 px У ОЧКОВ (обе раскладки) И У УРОВНЯ (там, где он ' +
      'светлый) — ' + JSON.stringify({ моб: обвМоб, деск: обвДеск }) + '. Слово ' +
      'владельца 2026-08-21-з, толщина уточнена им же по кадру до 4 ' +
      '(2026-08-21-к; шесть прожили одну правку). ⚠️ computed 8 при заданных 4 — НЕ ошибка: ' +
      'удвоение под заливку спрятано в `.otext text` (`--otl × 2`), снаружи ' +
      'пишется ВИДИМАЯ толщина. Руками `stroke-width` не трогать. ' +
      '⚠️⚠️ САНИТАР «ПЯТНО» (заливка !== обводка) — ГЛАВНАЯ ПОЛОВИНА ЭТОГО ' +
      'АССЕРТА, И ОН НАПИСАН ПО ФАКТУ: первая редакция правки повесила чёрную ' +
      'обводку и на ДЕСКТОПНЫЙ уровень, а он там ЧЁРНЫЙ (базовое ' +
      '`.otext text { fill:#000 }`) — «LV 3» превратилось в сплошное чёрное ' +
      'пятно, снято кадром. Поэтому десктопный уровень оставлен с белой ' +
      'обводкой 2, и это названо владельцу. Диверсия — распространить чёрную ' +
      'обводку на `#lvlSvg text` глобально: краснеет ровно санитар');

    // ── ЭКРАН ПОБЕДЫ: ГЛИФ-РУКА ЗАМЕНЁН ЛУПОЙ (слово владельца 2026-08-21-з) ──
    // ⚠️ Читаем СКРЫТЫЙ узел: оверлей победы `display:none`, но атрибуты и
    // computed-заливки резолвятся и так — открывать экран ради трёх свойств
    // значило бы гонять целую победу.
    const наградa = await стр.evaluate(() => {
      const ic = document.querySelector('.win-reward-ic');
      if (!ic) return { есть: false };
      const svg = ic.querySelector('svg');
      const цв = {}; svg.querySelectorAll('path').forEach(pa => {
        const f = getComputedStyle(pa).fill; цв[f] = (цв[f] || 0) + 1; });
      const cs = getComputedStyle(svg);
      return { есть: true, класс: svg.getAttribute('class'), вьюбокс: svg.getAttribute('viewBox'),
               путей: svg.querySelectorAll('path').length, цвета: цв,
               ширина: cs.width, высота: cs.height, позиция: cs.position,
               // ⚠️ СЛОТ ОБЯЗАН ОСТАТЬСЯ МАКЕТНЫМ 32×32: чужой страж пилюли
               // награды пинует его числом, и первая редакция уронила его на
               // 29.4, вписав элемент по РИСУНКУ вместо МЕСТА под рисунок.
               // ⚠️ прежний глиф-рука начинался с этой команды — если он всё ещё
               // здесь, значит заменили не то место
               стараяРука: /M14\.333 2\.667/.test(svg.innerHTML) };
    });
    console.log('награда-победа:', JSON.stringify(наградa));
    // ⛔ ПЕРЕЕХАЛ ЗА МАКЕТОМ 891:4199 ВМЕСТЕ С ЛУПОЙ В БАРЕ.
    // ⚠️⚠️ И ЗДЕСЬ ЖИВЁТ ТРЕТИЙ ПРЕФИКС id — `tipw-`. Копия лупы на экране
    // победы это ОТДЕЛЬНЫЙ инлайновый SVG с теми же именами градиентов; без
    // своего префикса она поделила бы объявления с лупой в баре, и правка
    // одной МОЛЧА меняла бы другую. Проверяем ссылку явно.
    const градПобеды = Object.keys(наградa.цвета).filter(k => /^url\(/.test(k));
    expect(наградa.есть && /tip-mag/.test(наградa.класс) &&
           наградa.вьюбокс === '14.48 11.58 50.492 55' &&
           наградa.путей === 3 && наградa.цвета['rgb(255, 255, 255)'] === 2 &&
           градПобеды.length === 1 && /#tipw-/.test(градПобеды[0]) &&
           // ⛔ СЛОТ ПЕРЕЕХАЛ С БЕЛОГО КРУГА 32 НА СТЕКЛЯННУЮ ПИЛЮЛЮ 54
           // (нода 891:4317): лупа 42.602×46.406 в точке (5.7, 3.38), то есть
           // абсолютная внутри слота, а не `static` по центру круга.
           наградa.позиция === 'absolute' && !наградa.стараяРука &&
           наградa.ширина === '42.602px' && наградa.высота === '46.406px',
      '⚠️⚠️ НА ЭКРАНЕ ПОБЕДЫ У НАГРАДЫ «+1 ПОДСКАЗКА» ТА ЖЕ ЛУПА, ЧТО НА КНОПКЕ (' +
      JSON.stringify(наградa) + '). Слово владельца 2026-08-21-з «на экране ' +
      'победы нужно заменить иконку» — ЭТО ЗАКРЫТИЕ РАЗВИЛКИ, которую я назвал ' +
      'ему партией раньше: тот же глиф стоял ВТОРЫМ показом одной сущности, и ' +
      'после переделки кнопки игрок видел бы два разных значка подсказки. ' +
      '⚠️⚠️ `стараяРука` В ТОМ ЖЕ АССЕРТЕ НЕСУЩЕЕ: «лупа есть» истинно и у ' +
      'сборки, где лупу ДОБАВИЛИ, а руку забыли убрать — оба значка легли бы ' +
      'друг на друга. ⚠️ `position:static` тоже: у `.tip-mag` в баре стоит ' +
      '`absolute`, и без сброса лупа выпала бы из центровки круга в его левый ' +
      'верхний угол. ⚠️⚠️ СЛОТ 32×32 — ПИН ПО ЧУЖОМУ СТРАЖУ: пилюля награды ' +
      '(нода 779:1114) требует иконку 32, и первая редакция этой правки ' +
      'уронила его на 29, вписав элемент по РИСУНКУ вместо МЕСТА под рисунок. ' +
      'Пропорцию 56:61 держит вьюбокс сам — лупа центруется внутри слота. ' +
      '⚠️ Замер идёт по СКРЫТОМУ узлу (оверлей `display:none`) — атрибуты и ' +
      'заливки резолвятся и так, открывать победу ради четырёх свойств не нужно');
    ...
    // требует «class/state reads + transitions-disabled computed styles».
      // ══ БЛОК ГЛАЗ: 120 НА МОБИЛЬНОМ, 210 НА ДЕСКТОПЕ (2026-08-22) ══
    // Слово владельца: «уменьши блок с глазами до 120 px в мобильной версии».
    // ⚠️⚠️ ДЕСКТОПНОЕ ПЛЕЧО ОБЯЗАТЕЛЬНО: «в мобильной версии» сказано прямо, и
    // без него правка, снёсшая размер ВЕЗДЕ, прошла бы зелёной.
    // ⚠️⚠️ И ТРЕТЬЕ ПЛЕЧО — УЗКИЙ ЭКРАН: `--eyeW` это `min(120, (100vw−202)×0.87)`,
    // где второе слагаемое бережёт правый стек от наезда. Замени формулу
    // литералом `120px` — на 320 глаза стали бы ШИРЕ дозволенного, и ни один
    // из первых двух ассертов этого не заметил бы. Проверяем, что на 320
    // ширина СТРОГО МЕНЬШЕ 120 и стек не задет.
    // ⛔⛔ И ЧЕТВЁРТОЕ ПЛЕЧО — РАЗВЯЗКА ЧИСЛА ОТСЧЁТА (слово владельца
    // 2026-08-22-б: «оставь глаза текущего размера, но верни прошлый размер
    // таймера»). До этого вся конструкция считалась долями `--eyeW`, и число
    // уменьшилось вместе с глазами; теперь размер числа берётся у СВОЕЙ
    // переменной `--timerW`, а посадка — по-прежнему у `--eyeW`.
    // ⚠️ ПРОВЕРЯЕМ ОБЕ СТОРОНЫ РАЗВЯЗКИ: число совпадает с долей `--timerW`
    // И НЕ совпадает с долей `--eyeW`. Без второй половины возврат к одной
    // переменной прошёл бы зелёным на десктопе, где они равны, — поэтому
    // сравнение стоит на МОБИЛЬНОМ, единственной раскладке, где они разные.
    const глазаНа = async (ш, в) => {
      const стр = await browser.newPage({ viewport: { width: ш, height: в } });
      стр.on('pageerror', e => errors.push('PAGEERROR(глаза ' + ш + '): ' + e.message));
      await стр.goto('file://' + PAGE_FILE + '?dev=1');
      await стр.waitForFunction(() => window.__game && window.__game.alive() > 0, { timeout: 60000 });
      await стр.evaluate(() => window.__game.skipIntro());
      const о = await стр.evaluate(() => {
        const gl_r = e => e.getBoundingClientRect(), gl_ш = r => r.width;
        const f = document.getElementById('face').getBoundingClientRect();
        const t = document.getElementById('mixerTimerSvg');
        const стек = document.querySelector('#topBar .grp:last-child');
        const с = стек ? стек.getBoundingClientRect() : null;
        // ⚠️⚠️ ПЕРЕМЕННЫЕ ЧИТАЮТСЯ ЧЕРЕЗ УЗЛЫ, А НЕ ЧЕРЕЗ `getPropertyValue`:
        // кастовое свойство вычисляется В СВОЁ ОБЪЯВЛЕНИЕ (строка
        // `min(120px, calc(...))`), а не в пиксели, и `parseFloat` даёт NaN.
        // Прогон 2026-08-22 напечатал `глазW:0, таймерW:0` — страж мерил
        // пустоту и краснел на исправной сборке. Используемые значения несут
        // сами носители: `#eyes` шириной `--eyeW`, `#mixerTimerSvg` — `--timerW`.
        const гл = document.getElementById('eyes');
        return { ширина: Math.round(f.width),
                 число: t ? +t.getBoundingClientRect().height.toFixed(1) : null,
                 глазW: гл ? +gl_ш(gl_r(гл)).toFixed(1) : 0,
                 таймерW: t ? +gl_ш(gl_r(t)).toFixed(1) : 0,
                 // ⚠️⚠️ ЗАЗОР МЕРЯЕТСЯ ОТ САМОГО ПРАВОГО КРАЯ КОНСТРУКЦИИ, А НЕ
                 // ОТ РАМКИ `#face`: после развязки 2026-08-22 число отсчёта
                 // ШИРЕ глаз и свисает за рамку (~22px на 390). Мерь по рамке —
                 // и страж охранял бы не тот край, показывая 55 там, где до
                 // стека на самом деле ~33.
                 зазорДоСтека: с ? Math.round(с.left - Math.max(f.right,
                   t ? gl_r(t).right : f.right)) : null };
      });
      await стр.close();
      return о;
    };
    const гМоб = await глазаНа(390, 844), гУзк = await глазаНа(320, 568),
          гДеск = await глазаНа(1280, 800);
    console.log('глаза:', JSON.stringify({ моб: гМоб, узкий: гУзк, деск: гДеск }));
    const близкоЧ = (a, b) => Math.abs(a - b) < 0.7;
    expect(гМоб.ширина === 120 && гДеск.ширина === 210 &&
           гУзк.ширина < 120 && гУзк.зазорДоСтека > 0 && гМоб.зазорДоСтека > 0 &&
           близкоЧ(гМоб.число, гМоб.таймерW * 0.325) &&
           !близкоЧ(гМоб.число, гМоб.глазW * 0.325) &&
           гМоб.таймерW > гМоб.глазW,
      '⚠️⚠️ БЛОК ГЛАЗ 120 НА МОБИЛЬНОМ, 210 НА ДЕСКТОПЕ, И НА УЗКОМ ЭКРАНЕ ' +
      'ФОРМУЛА ЕЩЁ ЖИВА (' + JSON.stringify({ моб: гМоб, узкий: гУзк,
        деск: гДеск }) + '). ⛔ Прежний мобильный кап 165 (нода 829:1242) ' +
      'отменён словом владельца 2026-08-22. ⛔ А число отсчёта РАЗВЯЗАНО с ' +
      'глазами его же словом 2026-08-22-б: размер берётся у `--timerW` (прежний ' +
      'кап 165), посадка — по-прежнему у `--eyeW`. ⚠️ Сверяются ОБЕ стороны ' +
      'развязки, и именно на мобильном: на десктопе переменные равны, и ' +
      'возврат к одной прошёл бы там зелёным');

    // ══ БЕЙДЖ МНОЖИТЕЛЯ НА КАРТОЧКЕ УМЕНЬШАЕТСЯ НА УЗКИХ (2026-08-22-в) ══
    // Слово владельца дословно: «в мобильном на экранах меньше 580 px плавно
    // уменьшай этот бейдж на 30%. Т.е. на ширине 390 px этот бейдж должен быть
    // на 30% меньше, чем на 580 px и далее не уменьшается».
    // ⚠️⚠️ ДО ЭТОЙ СЕКЦИИ РАЗМЕР `.msc-badge` НЕ СТЕРЁГ НИКТО — ни один ассерт
    // не читал ни его высоту, ни кегль (проверено грепом: упоминаний класса в
    // сьюте не было вовсе). То есть и правка, и её откат прошли бы молча.
    // ⚠️⚠️ ЧЕТЫРЕ ПЛЕЧА, И КАЖДОЕ ЗАКРЫВАЕТ СВОЙ СПОСОБ ОШИБИТЬСЯ:
    //   1. ВЕРХНЯЯ ПОЛКА (≥580 и десктоп) — 35 как было; без неё «уменьшилось»
    //      истинно и у правки, ужавшей бейдж ВЕЗДЕ;
    //   2. НИЖНЯЯ ПОЛКА (390 и 320 равны) — «и далее не уменьшается» сказано
    //      прямо; без неё формула без `clamp` съезжала бы дальше вниз;
    //   3. РОВНО 70% на 390 — это и есть «на 30% меньше»;
    //   4. ПРОМЕЖУТОЧНАЯ ТОЧКА (500) строго между полками — «плавно», а не
    //      ступенькой; ступенчатые медиазапросы роняют ровно её.
    // ⚠️ КЕГЛЬ ПРОВЕРЯЕТСЯ ВМЕСТЕ С РАМКОЙ: «бейдж на 30% меньше» — это ВЕСЬ
    // бейдж. Правка, ужавшая только коробку, оставила бы текст прежним и
    // разорвала бы пилюлю; отношение кегля к высоте обязано быть постоянным.
    {
      const бейджНа = async (ш) => {
        const стр = await browser.newPage({ viewport: { width: ш, height: 900 } });
        стр.on('pageerror', e => errors.push('PAGEERROR(бейдж ' + ш + '): ' + e.message));
        await стр.goto('file://' + PAGE_FILE + '?dev=1');
        await стр.waitForFunction(() => window.__game && window.__game.alive() > 0, { timeout: 60000 });
        await стр.evaluate(() => window.__game.skipIntro());
        // ⚠️ МЕНЮ ОТКРЫВАЕТСЯ НАСТОЯЩИМ КЛИКОМ ПО КНОПКЕ, а не хуком: сетка
        //    коллекции строится при открытии, и обход этого пути проверял бы
        //    вёрстку, которой игрок не видит.
        await стр.click('#pauseBtn');
        await стр.waitForFunction(() => !!document.querySelector('.msc-badge'), { timeout: 15000 });
        const о = await стр.evaluate(() => {
          const b = document.querySelector('.msc-badge');
          const r = b.getBoundingClientRect(), c = getComputedStyle(b);
          return { высота: +r.height.toFixed(1), кегль: +parseFloat(c.fontSize).toFixed(2),
                   падинг: +parseFloat(c.paddingLeft).toFixed(2), текст: b.textContent };
        });
        await стр.close();
        return о;
      };
      const б1280 = await бейджНа(1280), б580 = await бейджНа(580),
            б500 = await бейджНа(500), б390 = await бейджНа(390),
            б320 = await бейджНа(320);
      console.log('бейдж множителя:', JSON.stringify({ '1280': б1280, '580': б580,
        '500': б500, '390': б390, '320': б320 }));
      const бл = (a, b) => Math.abs(a - b) < 0.4;
      const доля = б390.высота / б580.высота;
      expect(бл(б1280.высота, 35) && бл(б580.высота, 35) &&
             Math.abs(доля - 0.7) < 0.02 &&
             бл(б320.высота, б390.высота) &&
             б500.высота > б390.высота + 1 && б500.высота < б580.высота - 1 &&
             Math.abs(б390.кегль / б390.высота - б580.кегль / б580.высота) < 0.02 &&
             Math.abs(б390.падинг / б390.высота - б580.падинг / б580.высота) < 0.02,
        '⚠️⚠️ БЕЙДЖ МНОЖИТЕЛЯ: 35 ПРИ 580 И ШИРЕ, 70% ОТ НЕГО ПРИ 390 И УЖЕ, ' +
        'ПЛАВНО МЕЖДУ (' + JSON.stringify({ '1280': б1280, '580': б580, '500': б500,
          '390': б390, '320': б320, доля: +доля.toFixed(3) }) + '). ' +
        'Слово владельца 2026-08-22-в. ⚠️ ЧЕТЫРЕ ПЛЕЧА: верхняя полка (иначе ' +
        '«уменьшилось» истинно и у правки, ужавшей бейдж на десктопе), нижняя ' +
        'полка (его «далее не уменьшается»), ровно 0.7 на 390 и ПРОМЕЖУТОЧНАЯ ' +
        'точка 500 строго между — её роняют ступенчатые медиазапросы, то есть ' +
        'ровно та реализация, которая выглядит как «плавно», но ею не является. ' +
        '⚠️ Кегль и падинг сверяются ОТНОШЕНИЕМ к высоте, а не литералами: ' +
        'уменьшать надо ВЕСЬ бейдж, а числа 16/35 и 8/35 при первой же правке ' +
        'размера разъедутся с кодом');
    }

    // ══ АВТОМАТИЧЕСКИЙ ДОЕЗД ПОСЛЕ ОБЛЁТА (слово владельца 2026-08-21-р) ══
    // «В анимации поворота чаши после приближения добавь автоматический
    // плавный зум равный градации одного нажатия на кнопку +».
    // ⚠️⚠️ ПРОВЕРЯЕТСЯ РАВЕНСТВО ШАГУ КНОПКИ, А НЕ ЛИТЕРАЛ 3.2: требование
    // звучит как СОВПАДЕНИЕ с градацией «+», значит и страж обязан сверять
    // два места, а не помнить число. Шаг снимается ЖИВЫМ нажатием кнопки на
    // той же странице — так проверяется та величина, которая решает бой.
    // ⚠️ И ДОЕЗД ОБЯЗАН БЫТЬ ПЛАВНЫМ: одиночный замер «стало на 3.2 ближе»
    // истинен и у мгновенного присваивания. Ловим ПРОМЕЖУТОЧНЫЙ кадр —
    // радиус между началом и целью хотя бы в одном сэмпле.
    const дзПоле = await browser.newPage({ viewport: { width: 390, height: 844 } });
    дзПоле.on('pageerror', e => errors.push('PAGEERROR(доезд): ' + e.message));
    await дзПоле.goto('file://' + PAGE_FILE + '?dev=1');
    // ⚠️ ЖИВОЕ интро, без skipIntro: доезд живёт в `finishIntro`, и обход
    //    интро проверял бы путь, которого у игрока нет.
    await дзПоле.waitForFunction(
      () => document.documentElement.classList.contains('introdone'), { timeout: 60000 });
    const дз = await дзПоле.evaluate(async () => {
      const g = window.__game, старт = g.cam().r, проба = [];
      const t0 = performance.now();
      while (performance.now() - t0 < 900){
        проба.push(+g.cam().r.toFixed(3));
        await new Promise(r => requestAnimationFrame(r));
      }
      return { старт, конец: g.cam().r, проба };
    });
    // шаг кнопки — живым нажатием, уже ПОСЛЕ доезда
    // ⚠️⚠️ ШАГ КНОПКИ МЕРИМ ПО УСТОЯВШЕМУСЯ ЗНАЧЕНИЮ, А НЕ ЧЕРЕЗ ПАУЗУ: сам
    // `zoomBy` АНИМИРОВАН, и замер через 60 мс поймал середину полёта (2.71
    // вместо 3.2) — ассерт краснел на исправной сборке. Ждём ФАКТ: радиус
    // перестал меняться два кадра подряд, потолок 1.5 с страховкой.
    const шагКнопки = await дзПоле.evaluate(async () => {
      const g = window.__game, до = g.cam().r;
      document.getElementById('zoomInBtn').click();
      let прошлый = -1, тихих = 0, t0 = performance.now();
      while (performance.now() - t0 < 1500 && тихих < 3){
        await new Promise(r => requestAnimationFrame(r));
        const сейчас = +g.cam().r.toFixed(4);
        тихих = (сейчас === прошлый) ? тихих + 1 : 0;
        прошлый = сейчас;
      }
      return +(до - g.cam().r).toFixed(3);
    });
    await дзПоле.close();
    const дзСредние = дз.проба.filter(r => r < дз.старт - 0.05 && r > дз.конец + 0.05).length;
    console.log('доезд после облёта:', JSON.stringify({ старт: дз.старт, конец: дз.конец,
      прошёл: +(дз.старт - дз.конец).toFixed(2), шагКнопки, промежуточных: дзСредние }));
    expect(Math.abs((дз.старт - дз.конец) - шагКнопки) < 0.05 && дзСредние >= 3,
      '⚠️⚠️ ПОСЛЕ ОБЛЁТА КАМЕРА САМА ДОЕЗЖАЕТ РОВНО НА ОДНО НАЖАТИЕ «+», И ' +
      'ПЛАВНО (' + JSON.stringify({ старт: дз.старт, конец: дз.конец,
        шагКнопки, промежуточных: дзСредние }) + '). ⚠️ Величина сверяется с ' +
      'ЖИВЫМ нажатием кнопки, а не с литералом: владелец сказал «равный ' +
      'градации одного нажатия», то есть требование — СОВПАДЕНИЕ двух мест. ' +
      '⚠️ `промежуточных >= 3` — та половина, что отличает плавный доезд от ' +
      'мгновенного присваивания: у второго промежуточных кадров нет вовсе');

  // ── ЗАРЯД НА ДЕСКТОПЕ НЕ УДВОЕН (слово владельца 2026-08-21-м) ──────
    // ⚠️ ЭТОТ ПИН ЗАВЕДЁН ВМЕСТЕ С САМОЙ ПРАВКОЙ, а не «на всякий случай»:
    // владелец сказал «в МОБИЛЬНОМ в 2 раза», а прошлый раз говорил «в мобиле
    // И десктопе» — раскладки он называет осознанно. Без этого пина ×2,
    // расползшееся на десктоп, прошло бы зелёным: мобильный ассерт выше
    // читает 390-й вьюпорт и десктопа не видит вовсе.
    // ⚠️ Читаем СКРЫТЫЙ узел (заряда на старте нет) — размер задан правилом
    // CSS, а не выдачей, поэтому `display:none` замеру не мешает: снимаем
    // computed `width`, а не рамку.
    const зарядД = await дескШ.evaluate(() => {
      const cb = document.getElementById('chargeBtn');
      return cb ? { ширина: getComputedStyle(cb).width, позиция: getComputedStyle(cb).position } : null;
    });
    console.log('заряд-десктоп:', JSON.stringify(зарядД));
    expect(зарядД && зарядД.ширина === '104px' && зарядД.позиция === 'fixed',
      '⚠️⚠️ ЗАРЯД НА ДЕСКТОПЕ ОСТАЛСЯ 104 И `fixed`: удвоение 2026-08-21-м ' +
      'сказано ПРО МОБИЛЬНЫЙ. Диверсия — снять `@media` с правила 116: ' +
      'краснеет ровно здесь (' + JSON.stringify(зарядД) + ')');

    // ══ ХОВЕР ЗАЛИВАЕТ ФОН, И БОЛЬШЕ НИЧЕГО НЕ ПРОИСХОДИТ (2026-08-21-п) ══
    // Слово владельца: «ховер на кнопки: фон кнопки заполняется 80% белого,
    // другие анимации удали».
    // ⛔⛔ ЭТОТ АССЕРТ ЗАМЕНИЛ ПРЕЖНИЙ «ХОВЕР ПОДНИМАЕТ ИКОНКУ» (слово
    // 2026-08-21-г, прожило сутки). Там пинались `translateY(-4px)` у
    // `.shake-art` и неподвижность рамки кнопки. Подъёма больше нет —
    // прежний ассерт краснел бы на ИСПРАВНОЙ сборке.
    // ⚠️⚠️ НЕПОДВИЖНОСТЬ РАМКИ ПЕРЕЕХАЛА СЮДА, А НЕ ПРОПАЛА, И ЭТО НЕСУЩЕЕ:
    // `getBoundingClientRect()` кнопки читают семь мест сьюта, а `:hover` в
    // headless ЗАЛИПАЕТ после `page.click`. Верни кто-нибудь моторику на саму
    // кнопку — соседи снова начали бы мерить то 836, то 832.
    // ⚠️ И `transform === 'none'` НА ОБЕИХ КНОПКАХ — тоже часть «других анимаций
    // удали»: без этого поля возврат `scale` по нажатию прошёл бы зелёным.
    const хов = await дескШ.evaluate(() => ({
      shakeФон: getComputedStyle(document.getElementById('shakeBtn')).backgroundColor,
      hintФон: getComputedStyle(document.getElementById('hintBtn')).backgroundColor,
      артTransform: getComputedStyle(document.querySelector('.shake-art')).transform,
      кнопкаTransform: getComputedStyle(document.getElementById('shakeBtn')).transform,
      переход: getComputedStyle(document.getElementById('shakeBtn')).transitionProperty,
      переходДлит: getComputedStyle(document.getElementById('shakeBtn')).transitionDuration,
    ...
    await дескШ.hover('#hintBtn');
    const хов3 = await дескШ.evaluate(() => ({
      shakeФон: getComputedStyle(document.getElementById('shakeBtn')).backgroundColor,
      hintФон: getComputedStyle(document.getElementById('hintBtn')).backgroundColor }));
    await дескШ.close();
    console.log('кнопки-ховер:', JSON.stringify({ покой: хов, наShake: хов2, наHint: хов3 }));
    // ⛔ ПОКОЙ ПЕРЕЕХАЛ .20 → .40 (слово владельца 2026-08-22-в, вместе с
    //    формой и размером). ЗАЛИТЫЙ он не называл — шаг 40 → 80 остался.
    const ПОКОЙ = 'rgba(255, 255, 255, 0.4)', ЗАЛИТ = 'rgba(255, 255, 255, 0.8)';
    expect(хов.shakeФон === ПОКОЙ && хов.hintФон === ПОКОЙ &&
           хов2.shakeФон === ЗАЛИТ && хов2.hintФон === ПОКОЙ &&
           хов3.hintФон === ЗАЛИТ && хов3.shakeФон === ПОКОЙ &&
           хов.артTransform === 'none' && хов2.артTransform === 'none' &&
           // ⚠️ ПРОВЕРЯЕМ ДЛИТЕЛЬНОСТЬ, А НЕ ИМЯ СВОЙСТВА: у кнопок стоит
           // общий `transition-property: all` (наследие кнопочных правил), и
           // пин `=== 'none'` краснел бы на исправной сборке. Анимации нет,
           // когда нет ВРЕМЕНИ — «другие анимации удали» про это.
           хов.кнопкаTransform === 'none' && хов.переходДлит === '0s' &&
           хов.рамкаДо === хов2.рамкаПосле,
      '⚠️⚠️ ХОВЕР ЗАЛИВАЕТ ФОН 40% → 80%, И ЭТО ЕДИНСТВЕННОЕ, ЧТО ОН ДЕЛАЕТ ' +
      '(слово владельца 2026-08-21-п). ⛔ ОТМЕНЯЕТ его же слово 2026-08-21-г про ' +
      'подброс иконки — оно прожило сутки. ⚠️ Наведение проверяется НА КАЖДОЙ ' +
      'кнопке ОТДЕЛЬНО: правило у них общее, и заливка ОБЕИХ разом была бы ' +
      'такой же зелёной при селекторе без `:hover`. ⚠️ `transform === none` и ' +
      '`transition-property === none` — та часть, что стережёт СНЯТИЕ моторики, ' +
      'а не появление заливки (' + JSON.stringify({ покой: хов, наShake: хов2,
        наHint: хов3 }) + ')');

    // ══ КАРТОЧКА КОЛЛЕКЦИИ: ВИД ПО НАВЕДЕНИЮ, А НЕ ПО КЛИКУ (2026-08-21-н) ══
    // Слово владельца: «сейчас это вид клика, а нужно сделать ховером, десктоп».
    // ⚠️⚠️ ДО ЭТОЙ СЕКЦИИ НИ ОДИН СТРАЖ НЕ ЧИТАЛ НИ ФОН КАРТОЧКИ, НИ ЦВЕТ
    // КНОПКИ BOOST, НИ КЛАСС `sel`: в сьюте было НОЛЬ вхождений `.msc-boost`,
    // `.msc.sel` и `classList.contains('sel')`. Правка прошла бы зелёной В ОБЕ
    // СТОРОНЫ — и как сделано, и как было.
    // ⚠️ ПРОВЕРЯЕМ ПЕРЕХОД, А НЕ СНИМОК: покой → наведение → уход. Один замер
    // «под курсором подсветка есть» был бы зелен и на сборке, где подсветка
    // стоит ВСЕГДА.
    // ⚠️ ПЕРЕХОДЫ ГАСИМ: живой замер отдал бы середину плавного перекраса.
    const мкДеск = await browser.newPage({ viewport: { width: 1280, height: 900 } });
    мкДеск.on('pageerror', e => errors.push('PAGEERROR(карточка-ховер): ' + e.message));
    await мкДеск.goto('file://' + PAGE_FILE + '?dev=1');
    await мкДеск.waitForFunction(() => window.__game && window.__game.alive() > 0, { timeout: 60000 });
    await мкДеск.evaluate(() => { window.__game.skipIntro(); window.showMainScreen(); });
    await мкДеск.waitForSelector('#msGrid .msc:not(.lock)', { timeout: 30000 });
    await мкДеск.addStyleTag({ content: '.msc,.msc-boost{transition:none !important}' });
    const мкСнять = () => мкДеск.evaluate(() => {
      const c = document.querySelector('#msGrid .msc:not(.lock)');
      const b = c && c.querySelector('.msc-boost');
      return { фон: c ? getComputedStyle(c).backgroundColor : null,
               кнопка: b ? getComputedStyle(b).backgroundColor : null,
               классы: c ? c.className : null,
               selВДоме: document.querySelectorAll('.sel').length };
    });
    const мкПокой = await мкСнять();
    await мкДеск.hover('#msGrid .msc:not(.lock)');
    const мкХовер = await мкСнять();
    await мкДеск.mouse.move(4, 4);
    const мкУшёл = await мкСнять();
    // клик по карточке НЕ обязан оставлять вид включённым — в этом вся правка
    await мкДеск.click('#msGrid .msc:not(.lock)');
    await мкДеск.mouse.move(4, 4);
    const мкПослеКлика = await мкСнять();
    await мкДеск.close();
    console.log('карточка-ховер:', JSON.stringify({ покой: мкПокой, ховер: мкХовер,
      ушёл: мкУшёл, послеКлика: мкПослеКлика }));
    const мкБаза = 'rgba(0, 0, 0, 0)', мкПодсветка = 'rgba(255, 255, 255, 0.6)';
    expect(мкПокой.фон === мкБаза && мкХовер.фон === мкПодсветка &&
      мкУшёл.фон === мкБаза &&
      мкПокой.кнопка === 'rgb(255, 255, 255)' && мкХовер.кнопка === 'rgb(156, 229, 46)' &&
      мкУшёл.кнопка === 'rgb(255, 255, 255)',
      '⚠️⚠️ КАРТОЧКА: ПОДСВЕТКА ПРИХОДИТ И УХОДИТ С КУРСОРОМ — фон ' +
      'rgba(255,255,255,.6) и кнопка Boost лаймовая только под наведением ' +
      '(слово владельца 2026-08-21-н «сейчас это вид клика, а нужно сделать ' +
      'ховером, десктоп»). ⚠️ ОБЕ ПОЛОВИНЫ ВИДА В ОДНОМ АССЕРТЕ: подсветку ' +
      'карточки и перекрас кнопки дают ДВА правила, и починить одно, забыв ' +
      'второе, — самая вероятная половинчатая правка. Диверсия — снять любое из ' +
      'них: краснеет ровно это поле (' + JSON.stringify({ покой: мкПокой, ховер: мкХовер,
        ушёл: мкУшёл }) + ')');
    expect(мкПослеКлика.фон === мкБаза && мкПослеКлика.кнопка === 'rgb(255, 255, 255)' &&
      мкПокой.selВДоме === 0 && мкХовер.selВДоме === 0 && мкПослеКлика.selВДоме === 0 &&
      !/\bsel\b/.test(мкПослеКлика.классы || ''),
      '⚠️⚠️ КАРТОЧКА: КЛИК БОЛЬШЕ НЕ ЗАЩЁЛКИВАЕТ ВИД. Курсор уведён в угол ' +
      'ПОСЛЕ клика — фон обязан вернуться к базовому, а класса `sel` не должно ' +
      'быть НИ НА ОДНОМ узле страницы. ⚠️ ЭТО ГЛАВНАЯ ПОЛОВИНА ПРАВКИ: ' +
      'добавить ховер, оставив защёлку, — ровно то состояние, на которое ' +
      'владелец пожаловался. Диверсия — вернуть строку `msSelKey = ...` в ' +
      '90-input: подсветка под курсором останется верной, красным станет ' +
      'счётчик `sel` (' + JSON.stringify(мкПослеКлика) + ')');

    // ⚠️ ТАЧ: ХОВЕРА НЕТ, И ТАП НЕ ПОДСВЕЧИВАЕТ. Без гейта `hover:hover` на
    // мобильных `:hover` ЗАЛИПАЕТ после тапа — карточка осталась бы светлой
    // навсегда. Тач ничего не теряет: он и раньше не выбирал карточку, а
    // крутил портрет (класс `spinning`), и это проверяется тем же ассертом.
    const мкТач = await browser.newPage({ viewport: { width: 390, height: 844 },
      hasTouch: true, isMobile: true });
    мкТач.on('pageerror', e => errors.push('PAGEERROR(карточка-тач): ' + e.message));
    await мкТач.goto('file://' + PAGE_FILE + '?dev=1');
    await мкТач.waitForFunction(() => window.__game && window.__game.alive() > 0, { timeout: 60000 });
    await мкТач.evaluate(() => { window.__game.skipIntro(); window.showMainScreen(); });
    await мкТач.waitForSelector('#msGrid .msc:not(.lock)', { timeout: 30000 });
    await мкТач.tap('#msGrid .msc:not(.lock)');
    await мкТач.waitForTimeout(400);
    const мкТачСнимок = await мкТач.evaluate(() => {
      const c = document.querySelector('#msGrid .msc:not(.lock)');
      return { ховерЕсть: matchMedia('(hover:hover) and (pointer:fine)').matches,
               фон: getComputedStyle(c).backgroundColor, классы: c.className,
               selВДоме: document.querySelectorAll('.sel').length };
    });
    await мкТач.close();
    console.log('карточка-тач:', JSON.stringify(мкТачСнимок));
    expect(мкТачСнимок.ховерЕсть === false &&
      мкТачСнимок.фон === 'rgba(0, 0, 0, 0)' && мкТачСнимок.selВДоме === 0 &&
      /spinning/.test(мкТачСнимок.классы),
      '⚠️⚠️ КАРТОЧКА НА ТАЧЕ: ТАП КРУТИТ ПОРТРЕТ И НЕ ПОДСВЕЧИВАЕТ. Медиазапрос ' +
      '`(hover:hover) and (pointer:fine)` тут ложен, значит ховерных правил нет ' +
      'вовсе — иначе залипший после тапа `:hover` оставил бы карточку светлой ' +
      'до касания в другом месте. `spinning` в классах подтверждает, что тач-путь ' +
      'жив и правка его не задела. Диверсия — снять медиа-гард у ховерных ' +
      'правил: краснеет фон (' + JSON.stringify(мкТачСнимок) + ')');

    // ══ НА ТАЧЕ ЗАЛИВКИ ПО НАВЕДЕНИЮ НЕТ (2026-08-21-п) ══
    // ⛔ ЗАМЕНИЛО ПРЕЖНЮЮ ПРОВЕРКУ «иконка не залипает поднятой»: подъёма
    // больше нет вовсе, а гейт остался нужен — теперь он бережёт ФОН.
    // ⚠️ Мобильные браузеры держат `:hover` после тапа: снимут гейт — кнопка
    // останется залитой белым 80% до касания в другом месте.
    // ⛔⛔ СВОЯ СТРАНИЦА, А НЕ ОБЩАЯ СО ВСЕЙ СЕКЦИЕЙ — И ЭТО ПОЧИНКА ПАДЕНИЯ
    //    БЕЗ ВЕРДИКТА (2026-08-22-в). Раньше здесь наводили на кнопку страницы
    //    `стр`, открытой в начале секции. Страница ЖИВАЯ: пока идут соседние
    //    секции, миксер на ней молотит кучу, и уровень доигрывается САМ. На
    //    прогоне 33 он и доигрался — `#winOverlay` перехватил наведение,
    //    `page.hover` отвалился по таймауту, и сьют умер на 615 зелёных БЕЗ
    //    строки вердикта, то есть снаружи выглядел как «оборвался сам».
    //    ⚠️ ТРИГГЕРОМ СТАЛА НЕ ПРАВКА ВЁРСТКИ, А ДЛИНА ПРОГОНА: новая секция
    //    бейджа множителя добавила пять загрузок страниц, и чужая вкладка
    //    прожила на минуту дольше. **Секция, вставленная в СЕРЕДИНУ файла,
    //    удлиняет жизнь ЧУЖИХ страниц — а живая игра на них продолжает идти.**
    //    Лечится не порядком секций, а тем же правилом, что и всегда: страж
    //    ПРИВОДИТ нужное состояние сам и не наследует его от соседей.
    const ктхТ2 = await browser.newContext({ viewport: { width: 390, height: 844 },
      hasTouch: true, isMobile: true, deviceScaleFactor: 1 });
    const стрТ2 = await ктхТ2.newPage();
    стрТ2.on('pageerror', e => errors.push('PAGEERROR(ховер-тач): ' + e.message));
    await стрТ2.goto('file://' + PAGE_FILE + '?dev=1');
    await стрТ2.waitForFunction(() => window.__game && window.__game.alive() > 0, { timeout: 60000 });
    await стрТ2.evaluate(() => { window.__game.skipIntro(); });
    await стрТ2.hover('#shakeBtn');
    const ховТач = await стрТ2.evaluate(() => ({
      ховерЕсть: matchMedia('(hover:hover) and (pointer:fine)').matches,
      фон: getComputedStyle(document.getElementById('shakeBtn')).backgroundColor,
      арт: getComputedStyle(document.querySelector('.shake-art')).transform }));
    await ктхТ2.close();
    console.log('кнопки-ховер(тач):', JSON.stringify(ховТач));
    // ⛔ ФОН ПОКОЯ ПЕРЕЕХАЛ .20 → .40 (2026-08-22-в). Плечо живёт в 500 строках
    //    от основной секции, и правя фон, о нём забывают первым.
    expect(ховТач.ховерЕсть === false && ховТач.фон === 'rgba(255, 255, 255, 0.4)' &&
           ховТач.арт === 'none',
      '⚠️⚠️ НА ТАЧЕ КНОПКА НЕ ЗАЛИВАЕТСЯ ПО НАВЕДЕНИЮ (' + JSON.stringify(ховТач) +
      '). Гейт `@media (hover:hover) and (pointer:fine)`. ⚠️ Плечо ОБЯЗАТЕЛЬНОЕ: ' +
      'снимут гейт — на десктопе не изменится НИЧЕГО, а на телефоне кнопка после ' +
      'каждого тапа останется белой на 80% до касания в другом месте. Диверсия — ' +
      'снять медиазапрос: краснеет ровно это поле');

    // ⛔⛔ ЗДЕСЬ СТОЯЛА СЕКЦИЯ «КЛИК ПОДБРАСЫВАЕТ ИКОНКУ» — снята 2026-08-21-п
    // словом владельца «другие анимации удали». Она читала анимацию `shakeToss`
    // через Web Animations, ставила её на паузу и мерила кадры по долям
    // длительности (280 мс), плюс ждала снятия класса `.toss` по `animationend`.
    // Ни класса, ни кадров, ни слушателя в 90-input больше нет — секция не
    // «отключена», а удалена вместе с механизмом.
    // ⚠️ ЧТО ОТ НЕЁ ОСТАЛОСЬ ЖИТЬ: проверка `transform === 'none'` у `.shake-art`
    // в ассерте ховера выше. Именно она краснеет, если моторику вернут.

    // ── ДОСЫПКА ЛЕТИТ БЫСТРЕЕ (слово владельца 2026-08-21-д) ──────────────
    // «Ускорь анимацию досыпания объектов, а то есть ощущение падения кадров».
    // ⚠️⚠️ ОЩУЩЕНИЕ ОКАЗАЛОСЬ ПРАВДОЙ, И ЭТО ЗАМЕР, А НЕ ВПЕЧАТЛЕНИЕ (GPU
    // metal, CPU ×4, 390×844, ур.20): в покое худший кадр 34.7 мс, во время
    // досыпки турбо — 61.7 при ДЕВЯТИ предметах в воздухе разом. Со стартовой
    // скоростью вниз в воздухе максимум ШЕСТЬ, худший кадр 42-49.
    // ⚠️ СТРАЖ ЧИТАЕТ МЕХАНИЗМ, А НЕ КАДРЫ: перф-число зависит от загрузки
    // стенда и краснело бы на исправной сборке. Проверяем ФАКТ — свежесыпанный
    // предмет получает скорость вниз, и она НИЖЕ терминальной (16), то есть
    // анти-туннельный предел не тронут.
    // ⛔⛔ И ЭТОМУ СТРАЖУ ТОЖЕ СВОЯ СТРАНИЦА (2026-08-22-в) — ТА ЖЕ БОЛЕЗНЬ, ЧТО
    //    У ТАЧ-ХОВЕРА ВЫШЕ, ДРУГОЙ СИМПТОМ. Он работал на странице `стр`,
    //    открытой в начале секции. Пока идут соседние секции, миксер там
    //    молотит кучу, и уровень доигрывается САМ: замер прогона 34 —
    //    `{летящих: 0, живых: 0}`, то есть `leaveSingles()` было нечего
    //    оставлять и досыпать. Красный не про механику досыпки, а про
    //    ВОЗРАСТ чужой вкладки.
    //    ⚠️ ПОЧЕМУ ЭТО НЕ ЛОВИЛОСЬ РАНЬШЕ: страж жил на удаче тайминга —
    //    прогон был короче, и уровень не успевал кончиться. Новая секция
    //    бейджа множителя добавила пять загрузок, и удача кончилась.
    //    Симптом у двух стражей РАЗНЫЙ (там перехват наведения оверлеем, тут
    //    пустая куча), а корень один — поэтому и лечение одно.
    const ктхД = await browser.newContext({ viewport: { width: 390, height: 844 },
      hasTouch: true, isMobile: true, deviceScaleFactor: 1 });
    const стрД = await ктхД.newPage();
    стрД.on('pageerror', e => errors.push('PAGEERROR(досыпка): ' + e.message));
    await стрД.goto('file://' + PAGE_FILE + '?dev=1');
    await стрД.waitForFunction(() => window.__game && window.__game.alive() > 0, { timeout: 60000 });
    await стрД.evaluate(() => { window.__game.skipIntro(); });
    const досып = await стрД.evaluate(async () => {
      const g = window.__game, sleep = ms => new Promise(r => setTimeout(r, ms));
      const былоN = g.alive();
      g.leaveSingles();                 // БОЕВОЙ путь: остаются сироты -> цикл сам зовёт докидку пар
      // ⚠️ Ждём ФАКТ появления летящих НАД КРОМКОЙ, а не часы: только
      // свежесыпанные могут быть выше FUNNEL.H, куча туда не достаёт.
      let проб = 0, летящих = 0, самаяБыстрая = 0, сталоN = былоN;
      while (проб < 120){
        const над = g.itemsGeo().filter(o => o.y > 9.2 && o.vy != null);
        if (над.length){ летящих = над.length;
          самаяБыстрая = +Math.min(...над.map(o => o.vy)).toFixed(2);
          сталоN = g.alive(); break; }
        await sleep(40); проб++;
      }
      return { v0: g.cfg.dropV0, летящих, самаяБыстрая, живых: сталоN };
    });
    await ктхД.close();
    console.log('shake-досыпка:', JSON.stringify(досып));
    expect(досып.v0 === 8 && досып.летящих > 0 && досып.самаяБыстрая <= -7,
      '⚠️⚠️ ДОСЫПАЕМЫЕ ПРЕДМЕТЫ СТАРТУЮТ СО СКОРОСТЬЮ ВНИЗ, А НЕ ПАДАЮТ С НУЛЯ (' +
      JSON.stringify(досып) + '). Слово владельца 2026-08-21-д «ускорь анимацию ' +
      'досыпания объектов, а то есть ощущение падения кадров». ⚠️⚠️ ОЩУЩЕНИЕ ' +
      'ПОДТВЕРДИЛОСЬ ЗАМЕРОМ (GPU metal, CPU ×4, 390×844, ур.20): в покое ' +
      'худший кадр 34.7 мс, при досыпке турбо 61.7 при ДЕВЯТИ предметах в ' +
      'воздухе разом; со стартовой скоростью — шесть и 42-49. ' +
      '⚠️⚠️ СТРАЖ ЧИТАЕТ МЕХАНИЗМ, А НЕ КАДРЫ: перф-число зависит от загрузки ' +
      'стенда и краснело бы на исправной сборке. Ловим скорость ПЕРВОГО ЖЕ ' +
      'сэмпла, где над кромкой появились летящие: свободное падение набрало бы ' +
      '7 только за ~0.32 с, а опрос идёт каждые 40 мс. ⚠️ ФИЛЬТР «выше кромки» ' +
      'несущий — куча туда не достаёт, значит меряем именно свежесыпанных. ' +
      '⚠️ 8 НИЖЕ ТЕРМИНАЛЬНОЙ 16 — анти-туннельный предел физики не тронут, и ' +
      'поднимать его ради скорости НЕЛЬЗЯ (v>20 пробивала компаунды). ' +
      '⚠️ 12 ПРОБОВАЛИ: в воздухе те же шесть, выигрыша нет — не крутить назад');
    ...
    // ⛔⛔ СМЫСЛ ЭТОЙ СЕКЦИИ СМЕНИЛСЯ 2026-08-21-п. Раньше она стерегла, что при
    // `prefers-reduced-motion` ГАСНЕТ моторика иконки (подъём, переход, кадры
    // подброса). Моторики больше нет ни у кого — и проверять «выключено ли
    // выключенное» бессмысленно. Теперь секция стережёт ДРУГОЕ и ЖИВОЕ: у
    // кнопок остался ровно один эффект — заливка фона по наведению, и он НЕ
    // анимация, а мгновенная смена цвета. Значит в режиме «меньше движения» он
    // обязан работать ТАК ЖЕ, а движения не появиться ни на одном узле.
    // ⚠️ ПОЧЕМУ СЕКЦИЯ НЕ УДАЛЕНА ВМЕСТЕ С ПОДБРОСОМ: контекст `reducedMotion`
    // создаётся только здесь, и это единственное место, где вообще можно
    // поймать «вернули анимацию, но забыли про режим меньшего движения».
    ...
    const рм = await стрRM.evaluate(() => {
      ...
      return { заливка: getComputedStyle(b).backgroundColor,
               арт: getComputedStyle(art).transform,
               кнопка: getComputedStyle(b).transform,
               переход: getComputedStyle(b).transitionDuration,
               переходАрта: getComputedStyle(art).transitionDuration };
    });
    await стрRM.click('#shakeBtn');
    const рмКадров = await стрRM.evaluate(() => {
    ...
    console.log('кнопки-reduced:', JSON.stringify({ ...рм, кадров: рмКадров }));
    expect(рм.заливка === 'rgba(255, 255, 255, 0.8)' && рм.арт === 'none' &&
           рм.кнопка === 'none' && рм.переход === '0s' && рм.переходАрта === '0s' &&
           рмКадров === 0,
      '⚠️⚠️ ПРИ `prefers-reduced-motion` ЗАЛИВКА ПО НАВЕДЕНИЮ РАБОТАЕТ, А ДВИЖЕНИЯ ' +
      'НЕТ НИ НА ОДНОМ УЗЛЕ (' + JSON.stringify({ ...рм, кадров: рмКадров }) + '). ' +
      '⛔ ОТМЕНЯЕТ прежнюю редакцию секции, которая гасила подъём/переход/кадры ' +
      'подброса: моторику сняли целиком 2026-08-21-п. ⚠️⚠️ ЗАЛИВКА ЗДЕСЬ — ' +
      'ГЛАВНОЕ ПОЛЕ, А НЕ ФОРМАЛЬНОСТЬ: «меньше движения» не значит «меньше ' +
      'состояний», и погасить заодно и её — самая вероятная ошибка следующей ' +
      'правки этого режима. ⚠️ Клик РЕАЛЬНЫЙ: счёт анимаций после него ловит ' +
      'возврат любых кадров, а не только тех, что назывались `shakeToss`');
  }
  ...
  // ⚠️ СПИСОК ПЕРЕЕХАЛ ЗА ПРАВИЛОМ: с 2026-08-19 к машинам и еде добавились
  // ЗВЕРИ («тёплый сатин», выбор владельца). Перечисление поимённо оставлено
  // намеренно — оно и есть проверка ЧИСЛА пачек с картинкой: молчаливое
  // появление четвёртой (или пропажа одной) обязано краснеть.
  const сКарт = 'animal,car,food';
  expect(пмДо.сКартинкой.slice().sort().join() === сКарт &&
         скарт(пмДо).sort().join() === сКарт &&
         ['car', 'food', 'animal'].every(k => пмДо.пачки[k].наКартинке === пмДо.пачки[k].предметов),
    '⚠️⚠️ КАРТИНКИ ПАЧЕК В БОЮ: у машин, еды и зверей матчеп — их СОБСТВЕННАЯ ' +
    'картинка на ВСЕХ предметах, у прочих пачек — общий пресет (' + JSON.stringify(пмДо.пачки) +
  ...
  // ⚠️⚠️ ГДЕ СВЕЖИЙ КЛЮЧ, А ГДЕ ТОТ ЖЕ — ВЫБРАНО НАМЕРЕННО, ЭТО НЕ РАЗНОБОЙ.
  // Портреты кэшируются ДВАЖДЫ: `thumbItemForKey` держит предмет ВМЕСТЕ С
  // МАТЕРИАЛОМ (`thumbItemCache`), `itemThumb` — готовый PNG (`thumbCache`).
  // До правки 2026-08-19 замер «до/после» на ОДНОМ ключе показывал старую
  // картинку и врал «ничего не изменилось» — на этом я и споткнулся.
  // ⚠️ ТЕПЕРЬ ОДИН КЛЮЧ — ЭТО САМ СТРАЖ ТОЙ ПРАВКИ: пары `предБи`/`примБи` и
  // `котДо`/`котПосле` мерят ровно то, что портреты ОБНОВЛЯЮТСЯ, и обязаны
  // покраснеть, если правку снимут.
  // ⚠️ А СВЕЖИЕ КЛЮЧИ ОСТАВЛЕНЫ ТАМ, ГДЕ СТРАЖ НЕ ДОЛЖЕН ЗАВИСЕТЬ ОТ ЭТОЙ
  // ПРАВКИ: `свинБез`/`свинМаз` (живо ли «применять сразу») меряются свежими и
  // на РАЗНЫХ страницах, поэтому снятие сброса снимков их не гасит — проверено
  // диверсией: пара кота замерзает на 54.5, свинья остаётся 120.7.
  ...
  // ⚠️ ЧИСЛА ПЕРЕСНЯТЫ 2026-08-19 ПОСЛЕ ВЛИВАНИЯ ЗВЕРИНОЙ КАРТИНКИ («тёплый
  // сатин»): у зверей теперь свой матчеп, и все портреты пачки сдвинулись.
  //   чистая страница     animalcow 198.2/0.119   animalbee 135.0/0.418
  //   после ОТКРЫТИЯ      animalcow 198.2/0.119  (до правки — 114.3/0.239)
  //   после «Применить»   animalbee  64.4/0.772  (до правки альфы — 255.0/0)
  //   свинья чистая 172.2/0.425 → впервые ПОСЛЕ «Применить» 96.6/0.454
  //   свинья без мазка 96.6 → с мазком 120.7 (кисть белая, ширина 200)
  //   пират чистый 156.7/0.440 → после смены «фона» 81.1/0.823
  //   кот после «Применить» 54.5/0.443 → после мазка 74.1/0.371
  //     (диверсия «снять сброс снимков» морозит его на 54.5/0.443)
  //     (диверсия альфы там же — 255.0/0; диверсия обёртки — 156.7/0.440)
  // ⚠️⚠️ ВЕТКИ `mceApply` ОБЕ, И ЭТО НЕ ИЗБЫТОК. Первая («рождение своей
  // текстуры пачки») щупается страницей Б, вторая (запись внутрь существующей
  // `image.data`) — страницами В и Г. Разбор 2026-08-19 показал, что стражи
  // ТОЛЬКО по первой ветке оставляли сьют зелёным при откате `dst.set(src)`,
  // хотя выбеливание владельца шло именно по второй.
  {
    ...
    // ⚠️⚠️ ДЛЯ СТРАНИЦЫ Г НУЖНА ПАЧКА БЕЗ СВОЕЙ КАРТИНКИ. Там правится ОБЩИЙ
    // пресет `tex`, а пачка со своей картинкой его не видит вовсе — замер
    // просто не сдвинется, и страж покраснеет НЕ ПО ДЕЛУ. На звериной картинке
    // («тёплый сатин», 2026-08-19) это ровно и случилось с прежним `animalfox`:
    // 136.9/0.748 до и после, один в один. Пиратская пачка картинки не имеет —
    // и на это ниже стоит санитар, чтобы четвёртая картинка не сломала молча.
    const базПир = await снять(a, 'piratebarrel');
    // ⚠️ ЧИСТАЯ ОПОРА ДЛЯ СВЕЖЕГО КЛЮЧА. Страж «применение дошло до пачки» ниже
    // обязан мерить СВЕЖИМ ключом (он проверяет ярусы `itemMaterial`), а страж
    // «портреты не протухают» — КЭШИРОВАННЫМ (он проверяет перевод материалов).
    // Пока оба сравнивали `animalbee`, это было ОДНО И ТО ЖЕ сравнение, и моя
    // приписка про независимость врала — поймано разбором 2026-08-19.
    const базСвин = await снять(a, 'animalpig');
    await a.close();

    // ── страница Б: открыть (ничего не трогая!) → применить к одной пачке
    const b = await открыть();
    // ⚠️⚠️ СТРОИМ ПОРТРЕТ ДО ВСЕГО — он осядет в обоих кэшах (85-hud), и дальше
    // тот же ключ покажет, ОБНОВЛЯЮТСЯ ли карточки после смены матчепа. Это и
    // есть страж дефекта «протухающие портреты» (слово владельца 2026-08-19).
    const предБи = await снять(b, 'animalbee');
    ...
    // ⚠️ ВТОРАЯ ПАРА НА ОДНОМ КЛЮЧЕ — про ПРАВКУ НА МЕСТЕ. После «Применить» у
    // пачки уже есть своя текстура, и мазок пишет в ЕЁ БАЙТЫ: объект тот же,
    // `setPackMatcap` не зовётся, а значит снимок обязан сбросить сам `mceApply`.
    // Ключ ДРУГОЙ, не свиной: иначе этот страж и страж «применять сразу» ниже
    // гасли бы одной диверсией и перестали быть независимыми.
    const котДо = await снять(c, 'animalcat');
    ...
    const котПосле = await снять(c, 'animalcat');
    const свинМаз = await снять(c, 'animalpig');
    await c.close();

    // ── страница Г: ПУТЬ ВЛАДЕЛЬЦА ПО УМОЛЧАНИЮ. Ничего в списке целей не
    // трогаем: отмечена №0 «все текстурные разом», «применять сразу» включено.
    // ⚠️⚠️ ЭТА СТРАНИЦА ЗАКРЫВАЕТ ДЫРУ, НАЙДЕННУЮ РАЗБОРОМ 2026-08-19: страж
    // `примБи` выше щупает только ПЕРВУЮ ветку `mceApply` («рождение своей
    // текстуры пачки»), а выбеливание владельца шло по ВТОРОЙ — записью внутрь
    // существующей `image.data` общего пресета `tex`. Откат `dst.set(src)`
    // оставлял весь сьют зелёным. Здесь цель — именно `tex`, ветка вторая.
    // ⚠️ Правим ЦВЕТ ФОНА, а не рисуем: это заодно единственный замер обёртки
    // `() => репост()` у цветовых полей (голым обработчиком в `тихо` прилетел
    // бы Event, и применение цветов молча умерло бы).
    const г = await открыть();
    await г.evaluate(() => window.__game.matcapEdit());
    await г.waitForTimeout(600);
    const цветПрим = await г.evaluate(() => {
      const c = document.querySelector('#matcapEdit input[type=color]');   // «фон»
      if (!c) return false;
      c.value = '#3ba0ff'; c.dispatchEvent(new Event('input')); return true;
    });
    await г.waitForTimeout(500);
    const пирПосле = await снять(г, 'piratebarrel');
    const сКартинкойГ = await г.evaluate(() => window.__game.packMatcapInfo().сКартинкой);
    await г.close();

    console.log('редактор:', JSON.stringify({ базКоу, открКоу, отм, нажато, реестр, базБи, примБи, свинБез, свинМаз, цветПрим, сКартинкойГ, базПир, пирПосле, предБи, котДо, котПосле }));

    expect(отм === 1 && нажато && реестр.join() === 'animal'
        && базКоу && базКоу.n > 500 && базБи && базБи.n > 500 && свинБез && свинМаз
        && предБи && предБи.n > 500 && котДо && котДо.n > 500 && котПосле && базСвин,
    ...
    expect(свинБез && базСвин && Math.abs(свинБез.я - базСвин.я) > 5,
      'ПРИМЕНЕНИЕ ВООБЩЕ ДОШЛО ДО ПАЧКИ: портрет, построенный ВПЕРВЫЕ ПОСЛЕ ' +
      '«Применить», отличается от чистого (' + JSON.stringify({ базСвин, свинБез }) +
      '). Без этой строки страж выше зеленел бы и на редакторе, который не делает ' +
      'НИЧЕГО. ⚠️ КЛЮЧ ЗДЕСЬ СВЕЖИЙ НАМЕРЕННО: он проверяет ЯРУСЫ `itemMaterial` ' +
      '(реестр → картинка пачки → общий пресет), а не кэш портретов. На ' +
      '`animalbee` этот страж дублировал бы соседа снизу и краснел бы с ним ' +
      'одной диверсией');

    expect(свинМаз.я > свинБез.я + 8 && свинМаз.я < 250 && свинМаз.нас > 0.1,
      ...
      '⚠️ Верхняя граница здесь НЕ УКРАШЕНИЕ: этот мазок идёт по ВТОРОЙ ветке ' +
      '`mceApply` (у пачки уже есть своя текстура), и без неё откат альфы к ' +
      '`dst.set(src)` делал бы цель БЕЛОЙ — то есть «ещё ярче», и страж на одном ' +
      'только «стало светлее» ЗЕЛЕНЕЛ БЫ НА ДЕФЕКТЕ');

    expect(цветПрим && базПир && базПир.n > 500 && пирПосле
        && сКартинкойГ.indexOf('pirate') < 0,
      'САНИТАР ЦЕЛИ ПО УМОЛЧАНИЮ: поле «фон» найдено и получило событие, оба ' +
      'портрета построились, а пиратская пачка НЕ ОБЗАВЕЛАСЬ своей картинкой ' +
      '(иначе она перестала бы видеть общий пресет и два стража ниже покраснели ' +
      'бы не по делу — бери другую пачку из тех, что без картинки) (' +
      JSON.stringify({ цветПрим, сКартинкойГ, базПир, пирПосле }) + ')');

    expect(пирПосле && пирПосле.я < 250 && пирПосле.нас > 0.1,
      '⚠️⚠️ ВТОРАЯ ВЕТКА `mceApply` НЕ ВЫБЕЛИВАЕТ: правка общего пресета `tex` ' +
      '(цель по умолчанию) сохраняет цвет (' + JSON.stringify(пирПосле) + '). ' +
      'Диверсия — вернуть `dst.set(src)` вместо покомпонентного копирования RGB; ' +
      'замер диверсии здесь 255.0/0, здоровой сборки 81.1/0.823');

    expect(пирПосле && Math.abs(пирПосле.я - базПир.я) > 5,
      '⚠️ ЦВЕТОВЫЕ ПОЛЯ ПРИМЕНЯЮТСЯ СРАЗУ: смена «фона» меняет цель без нажатия ' +
      '«Применить» (' + JSON.stringify({ базПир, пирПосле }) + ', замер 156.7 → 81.1). ' +
      'Диверсия — вернуть `addEventListener(\'input\', репост)` без обёртки: в ' +
      '`тихо` прилетит Event, применение цветов умрёт МОЛЧА, и замер сойдётся с ' +
      'базовым один-в-один (156.7/0.440)');

    expect(Math.abs(примБи.я - предБи.я) > 5,
      '⚠️⚠️ ПОРТРЕТЫ НЕ ПРОТУХАЮТ: карточка, СНЯТАЯ ДО правки матчепа, после ' +
      '«Применить» показывает новый материал (' + JSON.stringify({ предБи, примБи }) +
      ', замер 135.0 → 64.4). Дефект был в том, что `thumbCacheDrop` чистит ' +
      'только готовые PNG, а портретный предмет со СТАРЫМ материалом остаётся в ' +
      '`thumbItemCache` — снимок переснимали тем же материалом. Диверсия — убрать ' +
      'из `packMatcapRepoint` (10-stage) список `thumbItemsOfPack`: замер замрёт ' +
      'на 135.0/0.418. ⚠️ Эта же диверсия НЕ гасит соседей: «применение дошло» ' +
      'мерит СВЕЖИМ ключом (его строит уже новый материал), «применять сразу» — ' +
      'тоже свежими и на другой странице');

    expect(котДо && котПосле && Math.abs(котПосле.я - котДо.я) > 8,
      '⚠️⚠️ ПРАВКА НА МЕСТЕ ТОЖЕ ОБНОВЛЯЕТ КАРТОЧКУ: мазок по УЖЕ СВОЕЙ текстуре ' +
      'пачки виден в портрете (' + JSON.stringify({ котДо, котПосле }) + ', замер ' +
      '54.5 → 74.1). Здесь объект текстуры не меняется, `setPackMatcap` не ' +
      'зовётся вовсе — снимки обязан сбросить сам `mceApply`. Диверсия — убрать ' +
  ...
  // Читаем СБОРКУ, а не исходник: в игрока едет именно она.
  {
    ...
    // ⚠️⚠️ НИЧЕГО НЕ ПЕРЕЧИСЛЯЕМ — ИЩЕМ ПО ФОРМЕ. Первая редакция держала список
    // из 16 имён при 67 снятых, и это ровно канонная ловушка «перечисляются
    // случаи, которые ВСПОМНИЛИ, а не те, которые есть»: откати кто-нибудь ОДИН
    // файл из ветки ради чужой правки — вернулись бы `bonusRefill` и три
    // константы, а страж остался бы зелёным. Списка больше нет.
    // ⚠️ ИЩЕМ КОД, А НЕ ПРОЗУ, и это тоже оплачено: редакция, искавшая голую
    // подстроку, ПОКРАСНЕЛА НА ИСПРАВНОЙ СБОРКЕ — надгробие в 00-config
    // перечисляет снятые символы поимённо, а комментарии едут в сборку. Поэтому
    // функции ловим только как ВЫЗОВ `имя(`, константы — по префиксу `BONUS_`
    // (у надгробия там маска `BONUS_*`, звёздочка под регулярку не попадает).
    // ⛔ ГРАНИЦА, НАЗЫВАЮ ЧЕСТНО: прозу этот страж не проверяет и не может —
    // слово «витрина» в игре носит и ЖИВАЯ панель множителей. Комментарии
    // чистятся руками, гарантии на них нет.
    {
      const вызовы = new Set((сборка.match(/\b[A-Za-z_$][A-Za-z0-9_$]*\s*\(/g) || [])
        .map(t => t.replace(/\s*\($/, ''))
        .filter(n => /bonus/i.test(n)));
      const конст = new Set(сборка.match(/\bBONUS_[A-Z0-9_]+/g) || []);
      // ЧУЖИЕ МЕХАНИКИ ТОГО ЖЕ КОРНЯ — их вырезать было НЕЛЬЗЯ, и они же
      // работают КОНТРОЛЕМ: без них «ноль совпадений» истинно и у сборки,
      // которая вообще не собралась.
      const бело = new Set(['shakeBonus']);          // level.shakeBonus — награда за рекламу
      const остались = [...вызовы].filter(n => !бело.has(n)).concat([...конст]);
      const чужие = ['SURPRISE_BONUS', 'SURPRISE_LEVEL_BONUS', 'STAR_LEVEL_BONUS',
                     'FIRE_BONUS_MULT'].filter(n => сборка.indexOf(n) >= 0);
      expect(остались.length === 0 && чужие.length === 4,
        '⛔ ПОСТАВКА БЕЗ БОНУСНОГО УРОВНЯ: в сборке нет НИ ОДНОГО вызова `*bonus*(` ' +
        'и ни одной константы `BONUS_*` (осталось ' + JSON.stringify(остались) + '), ' +
        'И чужие механики с тем же корнем целы (' + JSON.stringify(чужие) + ') — ' +
        'уборка хирургическая, а не «вырезали всё, что похоже на слово»');
    }
  }
  ...
  const промер = async () => lbPage.evaluate(async () => {
    ...
    try {
      ...
      // ⚠️ ТОТ ЖЕ ДОВОД, ЧТО У ЗНАЧКА: берём узлы ИЗ МЕНЮ (`#msLbEntry`), а не
      // первые попавшиеся в документе — второй экземпляр живёт на победе.
      const менюСтрока = document.getElementById('msLbEntry');
      const txt = менюСтрока.querySelector('.ms-lbe-txt').getBoundingClientRect();
      const right = менюСтрока.querySelector('.ms-lbe-right').getBoundingClientRect();
      const ms = document.getElementById('mainScreen');
      const av0 = document.querySelector('#msLbeAvs > *');
      const стрелка = менюСтрока.querySelector('.ms-lbe-act');
  ...
  // ===== ГОЛОСА МАТЕРИАЛОВ (звук совмещения) =====
  {
    ...
    expect(['juicy','metal','glass','plastic','plush'].every(v => мат.голоса.indexOf(v) >= 0),
  ...
  // зелена и на сборке, где варьируется ВЕСЬ звук.
  {
    ...
    await ap.addInitScript(() => {
      ...
      // ⚠️⚠️ ПИКИ ОГИБАЮЩЕЙ — ЧЕРЕЗ `linearRampToValueAtTime`, И ЭТО НЕСУЩЕЕ.
      // Громкость процедурного «буля» решает ШЕСТОЙ АРГУМЕНТ вызова `tone(...)`,
      // а не константа рядом: подмени вызов литералом, и `procPeak()` продолжит
      // отдавать правильное число при неправильном звуке. Здесь видно ровно то,
      // что доехало до огибающей.
      window.__sfx.ramp = [];
    ...
    // ═══ ГРОМКОСТЬ ГОЛОСОВ ВЫРОВНЕНА (слово владельца 2026-08-20-ж) ═══
    // ⚠️⚠️ СТРАЖ МЕРИТ САМИ ЗАПИСИ, А НЕ СВЕРЯЕТ ЧИСЛА С КОПИЕЙ ТАБЛИЦЫ. Берём
    // декодированный буфер каждого голоса, считаем макс. краткосрочный RMS в
    // окне 200 мс и умножаем на ТОТ ЖЕ трим, что применяет бой (`trimTable()`).
    // Литералы в тесте разошлись бы с кодом при первой перезаписи звука, а
    // сверка «таблица равна таблице» была бы тавтологией.
    // ⚠️ ОКНО 200 мс, А НЕ ПИК И НЕ RMS ФАЙЛА: пик не описывает громкость
    // (щелчок и шорох с равным пиком слышны по-разному), RMS файла наказал бы
    // запись за тишину в хвосте, а окно близко к временнОй интеграции слуха.
    const гром = await ap.evaluate(async () => {
      ...
      const из = {}, гейны = {}; let нет = 0;
      for (const k in trim){
        const buf = S.bufferOf(k);
        if (!buf){ нет++; continue; }
        из[k] = +(20 * Math.log10(кратк(buf) * trim[k])).toFixed(2);
        // ⚠️ И ОТДЕЛЬНО — ЧТО ТРИМ ДОЕХАЛ ДО ТРАКТА: читаем `gain.value` у
        // РЕАЛЬНОЙ ноды. Без этого страж повторял бы арифметику боя у себя и
        // остался бы зелёным на сборке, где множитель из `playBuf` вырезан.
        window.__sfx.gain.length = 0;
        S.play('match', { n: 3, m: k.replace(/^mat_/, ''), r: 0.62, pan: 0 });
        await sleep(60);
        гейны[k] = window.__sfx.gain.length
          ? +window.__sfx.gain[0].gain.value.toFixed(4) : null;
      }
      // ⚠️ ПИК ПРОЦЕДУРНОГО — ИЗ САМОЙ ФОРМУЛЫ, а не из константы рядом.
      window.__sfx.ramp.length = 0;
      S.play('match', { n: 3 });                 // без `m` — процедурная ветка
      await sleep(80);
      const пикиОгибающей = window.__sfx.ramp.slice().filter(v => v > 0);
      // ⚠️ ОБРАТНАЯ СТОРОНА САНИТАРА: голос, у которого ЕСТЬ запись, но НЕТ
      // строки в таблице, `playBuf` молча берёт с тримом 1 — то есть он
      // выпадает из выравнивания, а страж его не видит вовсе.
      const записи = S.loaded().filter(k => k.indexOf('mat_') === 0);
      const безТрима = записи.filter(k => !(k in trim));
      return { уровни: из, гейны, уровниТрим: trim, безБуфера: нет,
               голосов: Object.keys(trim).length,
               безТрима, процПик: S.procPeak(), пикиОгибающей };
    });
    await ap.close();
    console.log('разнообразие звука:', JSON.stringify(звук));
    console.log('громкость голосов:', JSON.stringify(гром));
    const урв = Object.values(гром.уровни);
    const разбр = урв.length ? Math.max.apply(null, урв) - Math.min.apply(null, урв) : 99;
    // САНИТАР ДВУСТОРОННИЙ: (1) у каждого ключа таблицы есть буфер — иначе
    // замер меряет пустоту и «разброс 0» истинен на сборке вообще без звука;
    // (2) у каждой ЗАПИСИ есть строка в таблице — иначе голос молча идёт с
    // тримом 1, выпадает из выравнивания, и множество голосов задаёт САМА
    // таблица, то есть страж не видит того, чего в ней нет.
    expect(гром.безБуфера === 0 && урв.length === гром.голосов && урв.length >= 5 &&
           гром.безТрима.length === 0,
      'САНИТАР ГРОМКОСТИ: у каждого голоса таблицы есть буфер И у каждой записи ' +
      'есть трим (' + урв.length + ' из ' + гром.голосов + ', без буфера ' +
      гром.безБуфера + ', без трима ' + JSON.stringify(гром.безТрима) + ')');
    // ⚠️⚠️ ТРИМ ДОЕХАЛ ДО ТРАКТА — ОТДЕЛЬНЫЙ АССЕРТ, И ОН НЕСУЩИЙ. Выше страж
    // ПОВТОРЯЕТ арифметику боя у себя (буфер × таблица); один он остался бы
    // зелёным на сборке, где множитель из `playBuf` вырезан вовсе. Здесь
    // читается `gain.value` РЕАЛЬНОЙ ноды: база группы 3 это (0.5+0.06·3)·√2.
    const базаГр = 0.68 * Math.SQRT2;
    const трактОк = Object.keys(гром.гейны).every(k =>
      гром.гейны[k] !== null &&
      Math.abs(гром.гейны[k] - базаГр * гром.уровниТрим[k]) < 0.002);
    expect(трактОк,
      '⚠️⚠️ ТРИМ ПРИМЕНЯЕТСЯ В ТРАКТЕ, А НЕ ТОЛЬКО ЛЕЖИТ В ТАБЛИЦЕ: у каждой ноды ' +
      'gain = (0.5+0.06n)·√2 · трим (' + JSON.stringify(гром.гейны) + '). Диверсия — ' +
      'убрать множитель `* (VOICE_TRIM[name] || 1)` из `playBuf`: таблица останется ' +
      'верной, соседний ассерт — зелёным, а звук разъедется как прежде');
    // ⚠️⚠️ АБСОЛЮТНЫЙ УРОВЕНЬ, А НЕ ТОЛЬКО РАЗБРОС. `max−min` инвариантен к
    // ОБЩЕМУ множителю: умножь всю таблицу на 0.25 — разброс останется нулём,
    // а игра станет тише на 12 дБ. Цель −22.8 дБ выбрана так, чтобы общая
    // громкость не поехала, и без этого ассерта она ничем не защищена.
    expect(урв.every(v => v >= -24.3 && v <= -21.3),
      '⚠️⚠️ АБСОЛЮТНЫЙ УРОВЕНЬ ГОЛОСОВ В КОРИДОРЕ −24.3…−21.3 дБ (' +
      JSON.stringify(гром.уровни) + '). Цель −22.8 ± 1.5. Диверсия — умножить ВСЮ ' +
      'таблицу на общий множитель: разброс останется нулём, а уровень уедет');
    expect(разбр <= 1.0,
      '⚠️⚠️ ГРОМКОСТЬ ГОЛОСОВ ВЫРОВНЕНА: разброс ' + разбр.toFixed(2) + ' дБ <= 1.0 (' +
      JSON.stringify(гром.уровни) + '). Слово владельца 2026-08-20-ж «выровняй ' +
      'громкость звуков». ДО правки записи расходились на 16.6 дБ по RMS и 20.7 ' +
      'по пику. Диверсия — сдвинуть ЛЮБОЙ трим в `VOICE_TRIM` (75-audio): разброс ' +
      'вырастет ровно на сдвиг. ⚠️ Порог 1.0 дБ, а не 0: он оставляет место ' +
      'округлению трима до трёх знаков, но втрое меньше едва слышимой разницы');
    // ⚠️⚠️ ПРОЦЕДУРНЫЙ «БУЛЬ» — ДВУСТОРОННИЙ КОРИДОР И ПРОВЕРКА ПО ФОРМУЛЕ.
    // Он играет у 20 живых типов без записи и БЫЛ САМЫМ ГРОМКИМ звуком
    // совмещения. ⛔ ПЕРВАЯ РЕДАКЦИЯ ЭТОГО ЧИСЛА (0.19) БЫЛА НЕВЕРНА: множитель
    // тракта записей взяли как `(0.5+0.06n)·√2`, а √2 там — ВОЗВРАТ съеденного
    // панорамой, и он сокращается тождественно. Верное — 0.131.
    // ⛔ И ПЕРВАЯ РЕДАКЦИЯ СТРАЖА БЫЛА ОДНОСТОРОННЕЙ (`<= 0.25`): она одинаково
    // зелена при 0.19, при верных 0.131 и при полной тишине 0.001 — то есть
    // НАПРАВЛЕНИЕ ошибки не ловила вовсе и мой промах на 3 дБ пропустила.
    expect(гром.процПик >= 0.115 && гром.процПик <= 0.150,
      '⚠️⚠️ ПРОЦЕДУРНЫЙ «БУЛЬ» В КОРИДОРЕ 0.115…0.150 (пик ' + гром.процПик + '). ' +
      'Было 0.45 — самый громкий звук игры; 0.19 — моя ошибка с двойным счётом √2. ' +
      'Коридор ДВУСТОРОННИЙ намеренно: верхняя половина ловит возврат громкого ' +
      'звука, нижняя — «на всякий случай потише», от которого 20 типов онемели бы');
    expect(гром.пикиОгибающей.length >= 2 &&
           гром.пикиОгибающей.every(v => Math.abs(v - гром.процПик) < 1e-4),
      '⚠️⚠️ КОНСТАНТА РЕАЛЬНО ДОЕЗЖАЕТ ДО ОГИБАЮЩЕЙ (' +
      JSON.stringify(гром.пикиОгибающей) + ' против ' + гром.процПик + '). ' +
      'Громкость решает ШЕСТОЙ АРГУМЕНТ вызова `tone(...)`, а не константа рядом: ' +
      'подмени вызов литералом 0.45 — `procPeak()` продолжит отдавать 0.131 при ' +
      'громком звуке, и пин выше остался бы зелёным. Здесь читается то, что ' +
      'доехало до `linearRampToValueAtTime`');
    // САНИТАР: без декодированных записей вся секция мерила бы пустоту —
    // `playBuf` возвращает false и до нод дело не доходит вовсе.
    expect(['mat_juicy','mat_metal','mat_glass','mat_plastic','mat_plush'].every(k => звук.декодировано.indexOf(k) >= 0) &&
           звук.одинРазмер.length === 12,
      'САНИТАР ЗВУКА: ПЯТЬ записей декодированы и сыграли 12 раз (' +
  ...
  // список, заданное место), а не наследует от соседей.
  {
    ...
    const деск = await up.evaluate(() => {
      ...
      return { зазор: a.length >= 2 ? Math.round(a[1].getBoundingClientRect().left - a[0].getBoundingClientRect().right) : null,
        ...
        // ⚠️⚠️ СЕЛЕКТОР ПРИБИТ К `#msLbeBadge`, А НЕ К КЛАССУ ПО ВСЕМУ ДОКУМЕНТУ.
        // С 2026-08-21-н у компонента ДВА экземпляра (меню и экран победы), и
        // `document.querySelectorAll('.ms-lbe-badge svg')` стал находить ШЕСТЬ
        // значков вместо трёх: страж отдал «видимых 2, ширина 0» — ширину брал
        // у СКРЫТОГО экземпляра. ⛔ Это не «страж сломался», а завёлся второй
        // носитель того же класса: **добавил второй экземпляр компонента —
        // прогрепай стражи на селекторы по его классам.**
        значок: (function (){ const бдж = document.getElementById('msLbeBadge');
          const в = бдж ? Array.prototype.filter.call(бдж.querySelectorAll('svg'),
        ...
        // ⚠️⚠️ ЯКОРЬ — СВОЯ КОЛОНКА, А НЕ СОСЕД СЛЕВА. Прежде мерилось против
        // `.ms-card` (профиль), и это было верно ровно пока профиль стоял ПЕРВЫМ
        // блоком левой колонки. Владелец переставил порядок 2026-08-20-в (глаза
        // -> профиль -> настройки), профиль уехал во второй ряд, и страж дал
        // −504px на ЗДОРОВОЙ сборке. Свойство, которое он стережёт, про левую
        // колонку не знает вовсе: «заголовок прижат к верху СВОЕЙ колонки, над
        // ним нет полосы». `.ms-coll` спанит колонку целиком (`grid-row:1/-1`),
        // поэтому её верх и есть верх колонки — якорь, переживающий любые
        // будущие перестановки соседей.
    ...
    expect(деск.заголовокСмещение === 0,
      '⚠️ КОЛЛЕКЦИЯ: верх заголовка вровень с верхом СВОЕЙ колонки, полосы ' +
      'сверху нет (смещение ' + деск.заголовокСмещение + 'px). ⛔ Прежде якорем ' +
      'была карточка профиля — она перестала быть верхним блоком левой колонки ' +
      '2026-08-20-в, и страж покраснел на ЗДОРОВОЙ сборке (−504px). Свойство ' +
      '«заголовок без острова» про левую колонку не знает: якорь обязан быть ' +
      'в правой');
  ...
  // ⛔ ПРЕЖДЕ ЗДЕСЬ СТОЯЛО: «ночью звёзды ЕСТЬ и они РИСУЮТСЯ, днём слоя нет».
  // С 2026-08-20 ночи нет вовсе (слово владельца «только дневная тема всегда»),
  // и обе руки требуют ОДНОГО: слоя нет ни в 23:00, ни в 12:00. Поздний час в
  // списке ОСТАВЛЕН — он и есть доказательство правила.
  ...
  // темовые фичи; подменять Date не нужно.
  {
    ...
    const поздно = await проба(23);
    const днём = await проба(12);
    console.log('звёзды на Play:', JSON.stringify({ поздно, днём }));
    // ⛔⛔ СТРАЖ ПЕРЕЕХАЛ ЗА ПРАВИЛОМ 2026-08-20 («только дневная тема всегда»).
    // Прежняя рука требовала НОЧЬЮ ярких точек > 50 — теперь ночи нет, и обе
    // пробы обязаны дать одно: слоя нет. Час 23 в списке ОСТАВЛЕН намеренно: он
    // и есть доказательство, что поздний час больше ничего не включает.
    expect(поздно.есть && !поздно.показан && поздно.ночь === false,
      '⚠️⚠️ КАРТОЧКА PLAY: в 23:00 слоя звёзд НЕТ и темы `night` нет (' +
      JSON.stringify({ показан:поздно.показан, ночь:поздно.ночь }) + '). Диверсия — ' +
      'вернуть решение по часу в `isNightSky` (85-hud): слой поднимется и ' +
      'нарисует за полсотни ярких точек, как было до правки');
    ...
    expect(поздно.послеЗакрытия === 'none',
      'КАРТОЧКА PLAY: при закрытии меню слой погашен — цикл не живёт весь ' +
      'геймплей (' + поздно.послеЗакрытия + ')');
  }
  ...
  // ===== КОНТРОЛИ, ОСИРОТЕВШИЕ ПОСЛЕ СНЯТИЯ БОНУСНОГО УРОВНЯ (2026-08-18) =====
  // ⚠️⚠️ ЗАЧЕМ ЭТА СЕКЦИЯ СУЩЕСТВУЕТ. Вырезая фичу, я снял её секции сьюта
  // ЦЕЛИКОМ по канонному правилу «страж умирает вместе с механикой». Правило
  // верное, но применено слишком широко: внутри тех секций жили КОНТРОЛЬНЫЕ
  // ПЛЕЧИ, утверждающие свойства ОБЫЧНОЙ игры — они стояли рядом только потому,
  // что без контроля соседний ассерт был бы тавтологией. Разбор чужим глазом
  // нашёл шесть таких; здесь они восстановлены БЕЗ бонусного плеча.
  // ⛔ ПРАВИЛО НА БУДУЩЕЕ: снимая секцию целиком, выпиши её ассерты по одному и
  // спроси про каждый — он про СНЯТУЮ механику или про ту, что остаётся?
  {
    const кп = await browser.newPage({ viewport: { width: 1280, height: 800 } });
    const сон = ms => new Promise(r => setTimeout(r, ms));
    // ⚠️ СТРАНИЦА СВЕЖАЯ, ЗНАЧИТ ИГРОК НОВЫЙ — А НОВОМУ ИГРАЕТСЯ ПРОЛОГ-КОМИКС
    // (`storyPrologueDue`: `Save.st === 0`), и он полноэкранный. Драг настоящей
    // мышью ушёл бы в него, а не в канвас. Соседние секции этого не знают,
    // потому что живут на общей странице, где пролог давно закрыт. Помечаем
    // сюжет виденным ДО загрузки — тот же приём, что у `mixer_level`.
    await кп.addInitScript(() => {
      try { localStorage.setItem('mixer_save_v1', JSON.stringify({ st: 0xffffffff })); } catch(e){}
    });
    await кп.goto('file://' + PAGE_FILE);
    await кп.waitForFunction(() => window.__game && window.__game.alive() > 0, null, { timeout: 60000 });
    await кп.evaluate(() => window.__game.skipIntro());
    await сон(900);

    // (1) ПАНЕЛЬ МНОЖИТЕЛЕЙ ДЕЙСТВИТЕЛЬНО СТРОИТСЯ, А НЕ ПРОСТО «display:block».
    // ⚠️ Соседний страж (правило ширины 2/3) читает только `display`, а он
    // остаётся 'block' и у ПУСТОЙ прозрачной карточки — то есть один он зелен и
    // на сборке, где панель не собирает ни одной строки.
    const пан = await кп.evaluate(() => {
      const v = document.getElementById('vitrine');
      const cs = v ? getComputedStyle(v) : null;
      return { строк: v ? v.querySelectorAll('.vcell').length : -1,
               прозрачность: cs ? +cs.opacity : -1,
               display: cs ? cs.display : null };
    });
    console.log('панель множителей:', JSON.stringify(пан));
    expect(пан.строк > 0 && пан.прозрачность > 0.5 && пан.display !== 'none',
      'ПАНЕЛЬ МНОЖИТЕЛЕЙ СОБРАНА: на десктопе есть строки и она видна (' +
      JSON.stringify(пан) + ') — display один этого не доказывает');

    // (2) ДРАГ НАСТОЯЩЕЙ МЫШЬЮ ВРАЩАЕТ КАМЕРУ.
    // ⚠️ Оставшаяся проверка драга читает CSS-класс `grabbing`, то есть только
    // то, что обработчик ЗАПУСТИЛСЯ. А сам `pointermove` этой же правкой был
    // ПЕРЕПИСАН (снята обёртка по бонусу) — значит проверять надо СЛЕДСТВИЕ.
    // ⚠️ ДРАГ — НАСТОЯЩЕЙ МЫШЬЮ PLAYWRIGHT, а не событиями из `evaluate`: канон
    // проекта требует этого для любого ввода по координатам, и повод тот же —
    // синтетика ходит мимо конвейера ввода браузера (захват указателя,
    // объединение событий), то есть проверяла бы не тот путь, которым играет
    // игрок. Заодно закрываем оверлеи: они перехватывают события у канваса.
    const доКам = await кп.evaluate(() => window.__game.cam());
    await кп.mouse.move(640, 420);
    await кп.mouse.down();
    await кп.mouse.move(760, 440, { steps: 6 });
    const рукаСжата = await кп.evaluate(() => document.documentElement.classList.contains('grabbing'));
    await кп.mouse.up();
    const рукаРазжата = await кп.evaluate(() => !document.documentElement.classList.contains('grabbing'));
    const послеКам = await кп.evaluate(() => window.__game.cam());
    const драг = { доAz: +доКам.az.toFixed(3), послеAz: +послеКам.az.toFixed(3),
                   дельта: +Math.abs(послеКам.az - доКам.az).toFixed(3),
                   рукаСжата, рукаРазжата };
    console.log('драг вращает:', JSON.stringify(драг));
    // ⚠️ ПОРОГ С ЗАПАСОМ: сдвиг 120 px при коэффициенте 0.006 даёт |dAz| ≈ 0.72,
    // то есть 0.05 проходит уверенно и не спорит с округлением.
    expect(драг.дельта > 0.05 && драг.рукаСжата && драг.рукаРазжата,
      'ДРАГ ВРАЩАЕТ КАМЕРУ: горизонтальный драг двигает азимут (' +
      JSON.stringify(драг) + '). Класс `grabbing` этого НЕ утверждает — он ' +
      'вешается ДО ветки поворота (90-input), поэтому немая поломка внутри неё ' +
      'оставила бы его зелёным');

    // (3) СПЕЦПРЕДМЕТЫ НА ОБЫЧНЫХ УРОВНЯХ СПАВНЯТСЯ ВООБЩЕ.
    // ⚠️ Контроль-санитар: без него любой ассерт «спецпредметов где-то нет»
    // истинен и на сборке, где их нет НИГДЕ.
    const спец = await кп.evaluate(async () => {
      const g = window.__game, sl = ms => new Promise(r => setTimeout(r, ms));
      let сумма = 0, где = [];
      for (let lv = 11; lv <= 16; lv++){
        g.setLevel(lv); g.regen(); g.skipIntro(); await sl(320);
        const c = g.specialsCount();
        сумма += c.всего; if (c.всего) где.push(lv + ':' + c.всего);
      }
      return { сумма, где: где.join(',') };
    });
    console.log('спецпредметы:', JSON.stringify(спец));
    expect(спец.сумма > 0,
      'СПЕЦПРЕДМЕТЫ ЕСТЬ на обычных уровнях 11..16 (' + JSON.stringify(спец) + ')');

    // (4) ФАЗА ПАДЕНИЯ В ИНТРО СЛУЧАЕТСЯ.
    // ⚠️ Ходим ЖИВЫМ интро (skipIntro его и съедает), фазы копим опросом ИЗ
    // СТРАНИЦЫ: одиночное чтение снаружи под нагрузкой опаздывает.
    const фазы = await кп.evaluate(async () => {
      const g = window.__game, sl = ms => new Promise(r => setTimeout(r, ms));
      window.__ф = [];
      g.setLevel(12); g.regen();
      const t = setInterval(() => { const f = g.introPhase(); if (f) window.__ф.push(f); }, 16);
      for (let i = 0; i < 200 && g.introPhase(); i++) await sl(50);
      clearInterval(t);
      return Array.from(new Set(window.__ф));
    });
    console.log('фазы интро:', JSON.stringify(фазы));
    expect(фазы.includes('drop') && фазы.includes('orbit'),
      'ИНТРО ПРОХОДИТ ФАЗЫ ПАДЕНИЯ И ОБЛЁТА (' + JSON.stringify(фазы) + ')');

    // (5) ЧАША ДЕРЖИТ КУЧУ ПОСЛЕ ВСТРЯСКИ — единственный в сьюте замер выступа
    // за стену. ⚠️ После выреза `maxWallExcess` не встречался в test.js НИ РАЗУ:
    // инвариант держал только 12-минутный соак, то есть на каждом прогоне его
    // никто не проверял. Порог 0.45 — соаковая норма (канон 2026-08-12).
    const стена = await кп.evaluate(async () => {
      const g = window.__game, sl = ms => new Promise(r => setTimeout(r, ms));
      g.setLevel(11); g.regen(); g.skipIntro(); await sl(700);
      g.shake(); await sl(2500);
      return g.maxWallExcess();
    });
    console.log('выступ за стену после встряски:', JSON.stringify(стена));
    expect(стена.walled === true && стена.excess < 0.45,
      'ЧАША ДЕРЖИТ ПОСЛЕ ВСТРЯСКИ: выступ за стену в норме (' +
      JSON.stringify(стена) + ', порог 0.45 — соаковая норма)');
    // ⚠️ ПРИБИРАЕМ ПОМЕТКУ СЮЖЕТА: localStorage у страниц сьюта общий.
    await кп.evaluate(() => { try { localStorage.removeItem('mixer_save_v1'); } catch(e){} });
    await кп.close();
  }

  // (6) ⛔⛔ ХОЛОДНЫЙ СТАРТ НЕ ЛОМАЕТ ЧАШУ НА ВСЮ СЕССИЮ — САНИТАР ИНЦИДЕНТА
  // 2026-08-17. Тогда `radiusAt` сделали зависящим от уровня, а его читала
  // одноразовая `initPhysicsWorld`: чаша собиралась ЦИЛИНДРОМ и оставалась
  // такой ДО КОНЦА СЕССИИ. Сегодня ветки нет, `initPhysicsWorld` инлайнит конус
  // из констант, а `funnelRadiusAt`/`radiusAt` разведены НАМЕРЕННО — и вот
  // ровно этот инвариант после снятия бонусного уровня остался БЕЗ СТРАЖА:
  // оба его ассерта жили в снятой секции.
  // ⚠️ ЕДИНСТВЕННЫЙ ПУТЬ К ДЕФЕКТУ — ХОЛОДНЫЙ СТАРТ на сохранённом уровне:
  // все прочие стражи грузят страницу на 1-м и ходят по уровням ХУКОМ, то есть
  // у них `initPhysicsWorld` всегда отрабатывает одинаково.
  // ⚠️ Мерим ГЕОМЕТРИЕЙ ЧАШИ, а не хуками физики: конус — это спека
  // (FUNNEL 2.4→4.1), и предмет за ним виден игроку как «вывалился за стекло».
  // Формулу инлайним — хук `funnelR` уехал вместе с бонусом, и заводить его
  // обратно ради одного замера значило бы держать ручку без второго читателя.
  {
    const sleep = ms => new Promise(r => setTimeout(r, ms));
    const холодный = async (стартУр, уйтиНа) => {
      const стр = await browser.newPage({ viewport: { width: 390, height: 844 } });
      await стр.addInitScript(l => {
        try { localStorage.setItem('mixer_level', String(l)); } catch(e){}
        // сюжет виден — пролог новому игроку тут только удлиняет ожидание
        try { localStorage.setItem('mixer_save_v1', JSON.stringify({ st: 0xffffffff })); } catch(e){}
      }, стартУр);
      await стр.goto('file://' + PAGE_FILE);
      await стр.waitForFunction(() => window.__game && window.__game.alive() > 0, null, { timeout: 60000 });
      await стр.evaluate(l => { window.__game.setLevel(l); window.__game.regen(); window.__game.skipIntro(); }, уйтиНа);
      await sleep(1200);
      const r = await стр.evaluate(() => {
        ...
        return { живых: window.__game.itemsGeo().length, вне, ниже, старт: window.__game.levelNum() };
      });
      // ⚠️ ПРИБИРАЕМ ЗА СОБОЙ: localStorage у страниц сьюта общий, и оставленный
      // `mixer_level` увёл бы следующую секцию на чужой уровень.
      await стр.evaluate(() => { try { localStorage.removeItem('mixer_level'); localStorage.removeItem('mixer_save_v1'); } catch(e){} });
      await стр.close();
      return r;
    };
    // ⚠️ ДВЕ РУКИ, И ОБЕ НУЖНЫ: старт на ур.21 — санитар (мерим ли мы вообще
    // здоровую базу), старт на ур.20 — тот самый кратный десяти, на котором
    // инцидент и случился. Одна рука не различила бы «чаша цела» и «мы мерим
    // не чашу».
    const с21 = await холодный(21, 21);
    const с20 = await холодный(20, 21);
    console.log('чаша после холодного старта:', JSON.stringify({ с21, с20 }));
    expect(с21.живых > 100 && с21.вне === 0 && с21.ниже === 0,
      'САНИТАР: холодный старт с ОБЫЧНОГО уровня — чаша держит всё (' +
      JSON.stringify(с21) + '), иначе мерили бы сломанную базу');
    expect(с20.живых > 100 && с20.вне === 0 && с20.ниже === 0,
  ...
  // ⚠️⚠️ ХВОСТ ХВОСТА: ошибки страницы, случившиеся после гейта на 40% файла.
  // Фильтры те же (синтетический креш сьюта и шум новичка), чтобы гейт не
  // начал краснеть на том, что два предыдущих сознательно пропускают.
  const finalErrors = errors.slice(errorsReported)
    .filter(e => !/reading 'boom'/.test(e) && !шумНовичка(e));
  if (finalErrors.length) failures.push('runtime errors В ХВОСТЕ ПРОГОНА: ' + finalErrors.join(' | '));
  console.log('ERRORS(хвост):', finalErrors.length ? finalErrors.join('\n') : 'none');
