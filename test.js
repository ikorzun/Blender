...
`;
  ...
  // ⛔ СТРАЖА НА ОТНОСИТЕЛЬНЫЙ ПОРОГ ПРОСАДКИ ЗДЕСЬ НЕТ, И ЭТО РЕШЕНИЕ
  // (ФИЗИКА 2026-08-07, задача #32). Пробовала два, оба выброшены, а не
  // подогнаны:
  ...
  //   (2) «у плашки порог ниже абсолютного» — ФЛЕЙК ПО ПОЗЕ: порог считается
  //       от ВЕРТИКАЛЬНОЙ толщины в текущей позе (так и задумано), а на живой
  //       куче плашка чаще лежит наклонно; в прогоне у всех 177 предметов
  //       порог вышел ровно 0.12, и ассерт покраснел на ИСПРАВНОЙ сборке.
  // Детерминированным этот страж не сделать без хука на позу, а заводить хук
  // ради него — дороже пользы. Правка подтверждена СОАКОМ (подъёмы 24 -> 8,
  // просадки 3 -> 0 при том же сиде) и лесенкой в комментарии у FLOOR_PEN_FRAC.
  // Диагностика доступна: `itemsBrief().penLim` — тот же порог, что у спасателя.
