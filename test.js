...
const HUD_FLOOR = { day: 1.30, night: 12.5 };   // белок глаза против неба (день — ПИН, см. выше)
  ...
  // 16+80+12 = 108), и факт «не наезжает».
  ...
  // проверяется через API (`level().shakes`), а показывает число DOM.
  {
    ...
    expect(Math.abs(ш0.низПодсказки - ш0.низКнопки) <= 1 &&
           ш0.левКнопки - ш0.правПодсказки === 12 &&
           Math.abs(дШ.низПодсказки - дШ.низКнопки) <= 1 &&
           дШ.левКнопки - дШ.правПодсказки === 12 &&
           Math.abs(дШ.справаОтКромки - 16) <= 4,
      '⚠️⚠️ ПОДСКАЗКА НЕ УЕХАЛА ВСЛЕД ЗА ВЫРОСШЕЙ КИСТЬЮ: обе стоят НА НИЖНЕЙ ' +
      'КРОМКЕ и подсказка ЛЕВЕЕ (' + JSON.stringify({ моб: [ш0.низПодсказки, ш0.низКнопки],
      деск: [дШ.низПодсказки, дШ.низКнопки] }) + '). Кнопка выросла 56 -> 80, ' +
      'а сосед обязан остаться там, где стоял, — владелец просил заменить ' +
      'Shake, а не двигать подсказку. ⚠️ ДВА ВЬЮПОРТА, потому что раскладка ' +
      'бара разная (поля 8 против 16 и `justify-content` на десктопе). ' +
      '⚠️ Диверсия — снять `#bottomBar .grpShake` ИЛИ понизить его ' +
      'специфичность до голого класса: подсказка всплывёт на 12px, и это ' +
      'ровно то, что уже случилось при первой редакции правила. ' +
      '⚠️ ЗАЗОР 12 ПИНУЕТСЯ ЧИСЛОМ, А НЕ «подсказка левее»: «левее» истинно и ' +
      'при зазоре 0, и при 100 — то есть у горизонтали не было ограничителя ' +
      'вовсе. 12 — это `gap` группы (прямая спека владельца «отступы в ' +
