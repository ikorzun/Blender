...
`;
  ...
  // ⚠️⚠️ ПРАВИЛО СМЕНИЛОСЬ ТРЕТИЙ РАЗ ЗА ТРОЕ СУТОК (слово владельца 2026-08-12:
  // «сделаем скругление вью по 40 px для мобильного. Фон при этом под островом и
  // строкой поиска будет зависеть от темы устройства»). Нейтраль по теме →
  // подбор под палитру (e962931) → ТЕМА УСТРОЙСТВА. Стражи ПЕРЕЕЗЖАЮТ за
  // правилом, а не «чинятся».
  // ⚠️ У КАЖДОГО ВТОРАЯ ПОЛОВИНА — «И НЕ тон неба / НЕ тон меню»: без неё
  // возврат подбора под палитру прошёл бы молча, а мы его только что снимали.
  // Прежние ассерты этим и были — с обратным утверждением; переворот стоит
  // одной строки и оставляет дверь под наблюдением.
  const БЕЛЫЙ = /^rgb\(255, ?255, ?255\)$/;
  expect(БЕЛЫЙ.test(bgSeams.closed.html.col) && bgSeams.closed.html.col !== bgSeams.skyTop,
    '⚠️ КРОМКА В ИГРЕ: фон документа = ТЕМА УСТРОЙСТВА (в прогоне светлая), а НЕ ' +
    'тон неба (' + bgSeams.closed.html.col + ', небо ' + bgSeams.skyTop + ')');
  ...
  expect(bgSeams.open.мета === bgSeams.closed.мета && bgSeams.back.мета === bgSeams.closed.мета &&
  ...
  expect(кромка.игроваяRgb !== кромка.низ && кромка.игроваяRgb !== кромка.верх &&
    ...
    '⚠️ КРОМКА: в игре мета = ТЕМА УСТРОЙСТВА (светлая в прогоне), а НЕ стоп неба (' +
  ...
  expect(подложка.картинка === 'none' && подложка.цвет !== 'rgb(' + подложка.верх.replace(/,/g, ', ') + ')' &&
         подложка.цвет === 'rgb(255, 255, 255)',
    '⚠️ КРОМКА: подложка — ТЕМА УСТРОЙСТВА, а не стоп неба (' +
    подложка.цвет + ' / верх ' + подложка.верх + ')');
  // ⚠️⚠️ СКРУГЛЕНИЕ ВЬЮ — ВТОРАЯ ПОЛОВИНА ТОГО ЖЕ РЕШЕНИЯ, и без него полосы по
  // теме устройства читались бы как обрезка кадра. Утверждаем ОБЕ стороны:
  // мобильный скруглён, десктоп НЕТ (правило `max-width:1079`).
