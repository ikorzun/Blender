...
const HUD_FLOOR = { day: 1.30, night: 12.5 };   // белок глаза против неба (день — ПИН, см. выше)
  ...
  // проверяется через API (`level().shakes`), а показывает число DOM.
  {
    ...
    expect(близко(ш0.бейджX, 28.5) && близко(ш0.бейджY, 40) &&
           ...
           ш0.бФон === 'rgb(192, 255, 71)' && ш0.бЦвет === 'rgb(0, 0, 0)' &&
      ...
      '#c0ff47 = токен Experimental/Electric lime, #000 = BW/Black — оба из ' +
      'ноды. ⚠️ ШИРИНА 24 АССЕРТИТСЯ ТОЛЬКО У ОДНОЗНАЧНОГО ЧИСЛА: она ' +
    ...
    expect(Math.abs(ш0.центрПодсказки - ш0.центрКнопки) <= 0.6 &&
      ...
      'Диверсия — вернуть `align-items:flex-end` группе: центры разъедутся на 4. ' +
