...
const HUD_FLOOR = { day: 1.30, night: 12.5 };   // белок глаза против неба (день — ПИН, см. выше)
  ...
  // проверяется через API (`level().shakes`), а показывает число DOM.
  {
    ...
    expect(ш0.естьКисть && ш0.вьюбокс === '5 1 63.9997 58.2206' && ш0.pAR === null &&
           ш0.путей === 12 &&
           близко(ш0.кисть[0], 0) && близко(ш0.кисть[1], 3.2) &&
           близко(ш0.кисть[2], 63.9997) && близко(ш0.кисть[3], 58.2206) &&
           // рисунок обязан ЗАПОЛНЯТЬ рамку, а не съехать внутри неё
           близко(ш0.инк[0], ш0.кисть[0]) && близко(ш0.инк[1], ш0.кисть[1]) &&
           близко(ш0.инк[2], ш0.кисть[2]) && близко(ш0.инк[3], ш0.кисть[3]) &&
           ш0.теньКисти === 'drop-shadow(rgba(0, 0, 0, 0.08) 0px 4px 5px)' &&
           ш0.цвета['rgb(0, 0, 0)'] === 11 && ш0.цвета['rgb(255, 255, 255)'] === 1,
      '⚠️⚠️ КИСТЬ — ИНЛАЙНОВЫЙ SVG С ЖИВОЙ ГЕОМЕТРИЕЙ, ТЕНЬЮ И БЕЗ МИНЫ DEV MODE (' +
      ...
      '⚠️⚠️ ВЬЮБОКС СРЕЗАН (`5 1 …`), ПОТОМУ ЧТО В ЭКСПОРТЕ ВОКРУГ КИСТИ ' +
      'ОСТАВЛЕНО ПОЛЕ ПОД ТЕНЬ — и рядом стоит замер ИНКА: при неверном ' +
      'вьюбоксе рамка `<svg>` осталась бы на месте, а рисунок внутри уехал бы ' +
      'и уменьшился, чего рамка не видит. ⚠️ ТЕНЬ СВЕРЯЕТСЯ ЗНАЧЕНИЕМ: в ноде ' +
      'это SVG-фильтр (dy 4, σ 2.5, чёрный 8%), у нас — тот же эффект ' +
      'CSS-ом; потеряется — кисть станет плоской, а все размеры верными. ' +
    ...
    expect(близко(ш0.бейджX, 28.5) && близко(ш0.бейджY, 40) &&
           ...
           ш0.бФон === 'rgb(192, 255, 71)' && ш0.бЦвет === 'rgb(90, 134, 5)' &&
      ...
      '#c0ff47 = токен Experimental/Electric lime; ⛔ ЦВЕТ ЦИФРЫ ПЕРЕЕХАЛ ' +
      '2026-08-21-е с чёрного на #5a8605 (оливковый) — владелец перерисовал ' +
      'бейджи, и прежний пин на BW/Black краснел бы на исправной сборке. ' +
    ...
    expect(лупа.естьЛупа && лупа.вьюбокс === '5 1 56 61' && лупа.pAR === null &&
           лупа.путей === 5 &&
           блЛ(лупа.лупаБокс[0], 4) && блЛ(лупа.лупаБокс[1], 1) &&
           блЛ(лупа.лупаБокс[2], 56) && блЛ(лупа.лупаБокс[3], 61) &&
           блЛ(лупа.инк[0], лупа.лупаБокс[0]) && блЛ(лупа.инк[1], лупа.лупаБокс[1]) &&
           блЛ(лупа.инк[2], лупа.лупаБокс[2]) && блЛ(лупа.инк[3], лупа.лупаБокс[3]) &&
           лупа.тень === 'drop-shadow(rgba(0, 0, 0, 0.08) 0px 4px 5px)' &&
           лупа.цвета['rgb(0, 0, 0)'] === 3 && лупа.цвета['rgb(255, 255, 255)'] === 2,
      '⚠️⚠️ ЛУПА — ИНЛАЙНОВЫЙ SVG С ЖИВОЙ ГЕОМЕТРИЕЙ, ТЕНЬЮ И БЕЗ МИНЫ DEV MODE (' +
      JSON.stringify({ вьюбокс: лупа.вьюбокс, pAR: лупа.pAR, путей: лупа.путей,
                       бокс: лупа.лупаБокс, инк: лупа.инк, тень: лупа.тень,
                       цвета: лупа.цвета }) + '). ⚠️ ВЬЮБОКС СРЕЗАН (`5 1 …`) — ' +
      'родной равен ОБЛАСТИ ФИЛЬТРА (66×71, поле под тень 5/1/5/9), лупа лежит ' +
      'в нём со смещением 5/1; рядом замер ИНКА, потому что при неверном ' +
      'вьюбоксе рамка `<svg>` остаётся на месте, а рисунок внутри уезжает. ' +
      '⚠️ ЦВЕТА ЧИТАЮТСЯ COMPUTED-СТИЛЕМ (3 чёрных пути + 2 белых): именно ' +
      '«лупа несёт СВОИ цвета из ассета» обосновывает выход подсказки из ' +
      'системного правила суток. Перекрась её чужим `svg path { fill:… }` — ' +
      'обоснование станет ложным, а размеры останутся верными. ⚠️ ТЕНЬ ' +
      'СВЕРЯЕТСЯ ЗНАЧЕНИЕМ: в ноде это SVG-фильтр (dy 4, σ 2.5, чёрный 8%)');

    expect(блЛ(лупа.бейджX, 0) && блЛ(лупа.бейджY, 40) &&
           лупа.бейджВ === 24 && лупа.бейджШ === 24 &&
           лупа.бФон === 'rgb(156, 226, 255)' && лупа.бЦвет === 'rgb(26, 108, 142)' &&
      ...
      '⚠️⚠️ ГОЛУБОЙ БЕЙДЖ В ТОЧКЕ 0/40 И ГОВОРИТ ПРАВДУ (' +
      ...
      '#9ce2ff фон, #1a6c8e текст — СВОЯ пара, не лаймовая как у Shake. ' +
