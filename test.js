...
`;
  ...
  // ⚠️⚠️ ЧЕТВЁРТАЯ РЕДАКЦИЯ ПРАВИЛА (слово владельца 2026-08-12 «МНЕ НУЖНО
  // РЕШЕНИЕ» + «убирай скругление»): КРОМКИ ЧЁРНЫЕ ВСЕГДА, СТАТИЧЕСКИ.
  // Основание — закон «Safari фиксирует цвет полос на ЗАГРУЗКЕ» (доказан
  // скриншотом владельца против кода a771543). Стражи ПЕРЕЕЗЖАЮТ за правилом.
  // ⚠️ У КАЖДОГО ВТОРАЯ ПОЛОВИНА — «И НЕ тон неба / НЕ тон меню»: без неё
  // возврат ЛЮБОГО из трёх прежних правил прошёл бы молча.
  const ЧЁРНЫЙ = /^rgb\(0, ?0, ?0\)$/;
  expect(ЧЁРНЫЙ.test(bgSeams.closed.html.col) && bgSeams.closed.html.col !== bgSeams.skyTop,
    '⚠️ КРОМКА В ИГРЕ: фон документа ЧЁРНЫЙ (4-я редакция), а НЕ тон неба (' +
  ...
  expect(bgSeams.open.html.col === bgSeams.closed.html.col &&
         bgSeams.open.html.col !== bgSeams.menuRGB,
    '⚠️ КРОМКА В МЕНЮ: меню кромку НЕ КРАСИТ — фон документа тот же и НЕ равен ' +
    'фону меню (' + JSON.stringify(bgSeams.open) + ', меню ' + bgSeams.menuRGB + ')');
  expect(bgSeams.back.html.col === bgSeams.closed.html.col,
    '⚠️ КРОМКА ПОСЛЕ МЕНЮ: фон документа не сдвинулся ни на шаг (' +
  ...
  expect(ЧЁРНЫЙ.test(bgSeams.closed.мета.replace(/ /g,'').replace('rgb(','rgb(')) &&
         bgSeams.open.мета === bgSeams.closed.мета && bgSeams.back.мета === bgSeams.closed.мета &&
         bgSeams.back.мета !== bgSeams.menuRGB && bgSeams.back.мета !== bgSeams.skyBot,
    '⚠️⚠️ КРОМКА, ВТОРОЙ КАНАЛ (мета theme-color): не зависит НИ от меню, НИ от ' +
    'неба — её держит тема устройства (' + bgSeams.closed.мета + ' -> ' +
  ...
  expect(кромка.игроваяRgb !== кромка.низ && кромка.игроваяRgb !== кромка.верх &&
         кромка.игроваяRgb === '0,0,0',
    '⚠️ КРОМКА: в игре мета ЧЁРНАЯ (4-я редакция), а НЕ стоп неба и не нейтраль темы (' +
  ...
  expect(кромка.метаRgb !== кромка.msRgb && кромка.вМеню.мета === кромка.доМеню,
    '⚠️ КРОМКА: меню мету НЕ КРАСИТ — она та же и НЕ равна фону меню (' +
  ...
  expect(кромка.после.мета === кромка.доМеню,
    '⚠️ КРОМКА: мета не сдвинулась ни на шаг — её держит система, а не игра (' +
  ...
  expect(подложка.картинка === 'none' && подложка.цвет !== 'rgb(' + подложка.верх.replace(/,/g, ', ') + ')' &&
         подложка.цвет === 'rgb(0, 0, 0)',
    '⚠️ КРОМКА: подложка ЧЁРНАЯ (4-я редакция), а не стоп неба и не нейтраль темы (' +
