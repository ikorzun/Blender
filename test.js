...
`;
  ...
  // ⚠️ 2 ИЗ 10 (слово владельца 2026-08-17 «каждая пятая»); история числа —
  // 1 из 10 → 3 из 10 → 2 из 10, то есть последняя правка это СНИЖЕНИЕ.
  ...
  expect(ручка && Math.abs(ручка.frac - 0.20) < 1e-6,
    'ЗВЁЗДЫ: боевая доля пульса — 2 из 10, как просил владелец (' + JSON.stringify(ручка) + ')');
