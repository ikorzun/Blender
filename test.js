  ...
  // это пятью красными у бомбы и огня).
  {
    ...
    const бомба = await бп.evaluate(async () => {
      ...
      for (let lv = 5; lv <= 16; lv++){
        ...
        if ((g.level() || {}).bonus) бонусные.push(lv);
        else if (g.bombRule().вКуче) были.push(lv);
      }
      // ⚠️⚠️ РАЗРЫВ СЧИТАЕТСЯ ПО УРОВНЯМ-КАНДИДАТАМ, А НЕ ПО НОМЕРАМ. С
      // включением периода (2026-08-17-и) бонусные уровни выпадают из подачи
      // вовсе: спецпредметов там нет по спеке пункта 5 (сток уносит ПАРАМИ, а
      // спецпредмет пары не имеет). Считая номера, страж видел разрыв 4 между
      // 7-м и 11-м — хотя 10-й для бомбы не кандидат, и настоящий разрыв 3.
      const разрывы = были.slice(1).map((v, i) => {
        let d = 0;
        for (let x = были[i] + 1; x <= v; x++) if (бонусные.indexOf(x) < 0) d++;
        return d;
      });
      ...
      return { ранние, были, бонусные, разрывы, до, после2, после3, после4 };
    });
    ...
    // ⚠️⚠️ И ПРЯМОЙ АССЕРТ, ЧТО НА ВИТРИНЕ БОМБЫ НЕТ. Без него «разрывы по
    // кандидатам» выглядели бы подгонкой под красный: сначала утверждаем, что
    // бонусные уровни ДЕЙСТВИТЕЛЬНО выпадают из подачи, и только потом считаем
    // разрывы, их пропуская.
    expect(бомба.бонусные.length > 0 && бомба.бонусные.every(l => бомба.были.indexOf(l) < 0),
      '⚠️⚠️ БОМБЫ НА ВИТРИНЕ НЕТ: бонусные уровни в окне 5..16 (' +
      JSON.stringify(бомба.бонусные) + ') не выдают бомбу вовсе');
...
`;
  ...
  const bowlLadder = await bowlPage.evaluate(async () => {
    ...
    g.bowlSetN(0);
    const out = [];
    // ⚠️⚠️ ДЕСЯТЫЙ ЗАМЕНЁН НА ОДИННАДЦАТЫЙ: с 2026-08-17-и он БОНУСНЫЙ, а на
    // витрине чаши нет и трещины выключены — страж мерил бы механику, которой
    // там не существует. Ожидаемое число не поехало: лесенка считает ⌊ур/10⌋,
    // и на 11-м она даёт те же 6. Канон: «вводя правило „каждый N-й особый“,
    // грепай сьют на кратные N» — это как раз оно.
  ...
  // ═══ НА ВИТРИНЕ НЕТ ПАНЕЛИ МНОЖИТЕЛЕЙ (слово владельца 2026-08-17-ж) ═══
  // «на бонусном уровне не нужна плашка с множителями объектов в левом нижнем
  // углу». ⚠️ СВОЯ СТРАНИЦА И ДЕСКТОПНАЯ ШИРИНА: на телефоне панели нет и так
  // (правило 2/3, порог 813), и страж на 390 был бы ЗЕЛЁН при любой реализации —
  // тавтология, которую видно только если спросить, что он вообще исполняет.
  // ⚠️⚠️ ПРОВЕРЯЮТСЯ ТРИ СОСТОЯНИЯ, А НЕ ОДНО: панель ЕСТЬ на обычном (иначе
  // «на витрине пусто» истинно и у сборки, где панели нет вовсе), НЕТ на
  // витрине, и ВЕРНУЛАСЬ после неё — обратимость тоже утверждение. Ровно этот
  // третий замер и ловит первую редакцию гейта: она не строила панель заново,
  // потому что тик строит только при СМЕНЕ уровня, а ссылку он потерял.
  const вп = await browser.newPage({ viewport: { width: 1280, height: 800 } });
  await вп.goto('file://' + path.join(__dirname, 'index.html') + '?dev=1');
  await вп.waitForFunction(() => window.__game && window.__game.alive() > 0, { timeout: 60000 });
  const снятьВит = async () => {
    await вп.evaluate(() => window.__game.skipIntro());
    await вп.waitForTimeout(900);
    return вп.evaluate(() => {
      const v = document.getElementById('vitrine');
      const cs = v ? getComputedStyle(v) : null;
      return { бонус: !!(window.__game.level() || {}).bonus,
        строк: v ? v.querySelectorAll('.vcell').length : -1,
        прозрачность: cs ? +cs.opacity : -1 };
    });
  };
  const витОбычный = await снятьВит();
  await вп.evaluate(() => { window.__game.setLevel(11); window.__game.bonus(); });
  await вп.waitForFunction(() => (window.__game.level() || {}).bonus === true, { timeout: 30000 });
  const витБонус = await снятьВит();
  await вп.evaluate(() => { window.__game.setLevel(12); window.__game.regen(); });
  await вп.waitForFunction(() => (window.__game.level() || {}).bonus === false, { timeout: 30000 });
  const витНазад = await снятьВит();
  await вп.close();

  // ═══ ОТСТУП СНИЗУ НА ВИТРИНЕ (слово владельца 2026-08-17-з) ═══
  // «нужно сделать отступ снизу около 100 px, иначе неудобно мерджить объекты в
  // самом низу». ⚠️⚠️ ДВА ВЬЮПОРТА С РАЗНОЙ ВЫСОТОЙ — ЭТО И ЕСТЬ ПРОВЕРКА
  // ЕДИНИЦЫ: отступ задан в ПИКСЕЛЯХ, значит на 844 и на 800 он обязан быть
  // ОДИНАКОВЫМ. Реализация «в юнитах мира» дала бы здесь разные числа, а на
  // ОДНОМ вьюпорте была бы неотличима от правильной.
  // ⚠️ Якорь — ПРОЕКЦИЯ дна ящика на экран (`bonusFrame().дноОтНизаPx`), а не
  // моя формула: первая редакция пересчитывала пиксели по ЗАДНЕЙ грани ящика и
  // печатала честные «100» при фактических 22 на телефоне и −59 на десктопе.
  const отступы = [];
  for (const [ш, в] of [[390, 844], [1280, 800]]){
    const оп = await browser.newPage({ viewport: { width: ш, height: в } });
    await оп.goto('file://' + path.join(__dirname, 'index.html') + '?dev=1');
    await оп.waitForFunction(() => window.__game && window.__game.alive() > 0, { timeout: 60000 });
    await оп.evaluate(() => { window.__game.setLevel(11); window.__game.bonus(); });
    await оп.waitForFunction(() => (window.__game.level() || {}).bonus === true, { timeout: 30000 });
    await оп.evaluate(() => window.__game.skipIntro());
    await оп.waitForTimeout(900);
    отступы.push(Object.assign({ вьюпорт: ш + 'x' + в }, await оп.evaluate(() => window.__game.bonusFrame())));
    await оп.close();
  }
  console.log('отступ снизу:', JSON.stringify(отступы));
  expect(Math.abs(отступы[0].дноОтНизаPx - 100) <= 8 && Math.abs(отступы[1].дноОтНизаPx - 100) <= 8,
    '⚠️⚠️ ВИТРИНА: дно ящика на 100 px выше нижней кромки НА ОБОИХ вьюпортах — ' +
    'отступ задан пикселями, а не юнитами (' + JSON.stringify(отступы) + ')');
  // ⚠️ ВТОРОЙ АССЕРТ ПРО ЦЕНТР, А НЕ ПРО НИЗ СФЕРЫ (см. надгробие у хука):
  // охватный радиус уходит ниже видимых пикселей, и на десктопе с близкой
  // камерой это давало 22 px при дне на 100 — метрика правдоподобно врала.
  // Центр нижнего ряда — величина определённая, и она честно говорит, что ряд
  // стоит НАД полосой, а не свисает в неё.
  expect(отступы[0].центрНижнегоОтНизаPx > 60 && отступы[1].центрНижнегоОтНизаPx > 60,
    '⚠️ ВИТРИНА: центр нижнего ряда стоит выше полосы отступа (' +
    отступы[0].центрНижнегоОтНизаPx + ' / ' + отступы[1].центрНижнегоОтНизаPx + ' px)');
  ...
  expect(витБонус.бонус && витБонус.строк === 0 && витБонус.прозрачность < 0.05,
    '⚠️⚠️ ВИТРИНА: панели множителей НЕТ — строки сняты и панель прозрачна (' +
    JSON.stringify(витБонус) + ')');
  expect(!витНазад.бонус && витНазад.строк > 0 && витНазад.прозрачность > 0.5,
    '⚠️⚠️ ПАНЕЛЬ ВЕРНУЛАСЬ после витрины — гейт гасит, а не убивает насовсем (' +
    JSON.stringify(витНазад) + ')');
  ...
  // держался, пока сьют не подрос; (3) сейчас лёг АБСОЛЮТНЫЙ порог возврата
  // (`кап60б.мин <= 30`): при новых секциях витрины даже самый короткий кадр в
  // окне возврата вышел 32.3 против 16-18 в прошлых прогонах.
  ...
  // сенсорный дефект: три зелёные пробы подряд не трогали сломанную ветку.
  {
    ...
    // ⚠️⚠️ ВИТРИНА ТЕПЕРЬ ВЗВОДИТСЯ ЯВНО (слово владельца 2026-08-17: уровень
    // СКРЫТ и по прогрессии не наступает). Раньше помощник просто ставил
    // уровень 10/20 и получал витрину «сама собой» — на новом правиле он
    // получал бы обычный уровень, и вся секция проверяла бы не то.
    const наУровень = async (l, витрина) => {
      await page.evaluate(([l, v]) => {
        window.__game.setLevel(l);
        if (v) window.__game.bonus(); else window.__game.regen();
        window.__game.skipIntro();
      }, [l, !!витрина]);
    ...
    // (1) ⚠️⚠️ СТРАЖ ПЕРЕЕХАЛ ВТОРОЙ РАЗ, И СНОВА ЗА СЛОВОМ ВЛАДЕЛЬЦА, А НЕ
    // «ПОЧИНЕН». История одного ассерта: сперва он держал ПЕРИОДИЧНОСТЬ (10 и 20
    // бонусные), потом СКРЫТОСТЬ (не наступает нигде, только по взводу — после
    // инцидента 2026-08-17), теперь снова периодичность (слово владельца
    // 2026-08-17-и: «включай бонусный уровень каждым десятым, если только он
    // ничего не ломает и анимация насыпания сверху не тормозит»).
    // ⚠️⚠️ ЧЕТЫРЕ СОСТОЯНИЯ, И КАЖДОЕ ЗАКРЫВАЕТ СВОЮ ПОЛОМКУ:
    //   • ур.10 без взвода → витрина ЕСТЬ  (иначе период выключен);
    //   • ур.11 без взвода → витрины НЕТ   (иначе «бонус всегда», а не каждый 10-й);
    //   • ур.11 со взводом → витрина ЕСТЬ  (иначе умер ручной вход из панели);
    //   • ур.12 после взвода → НЕТ         (иначе взвод залип, и игрок остался
    //     бы в витрине навсегда — а сохранённый бонусный уровень это ровно тот
    //     холодный старт, что ломал геометрию чаши).
    await наУровень(10);       const б10 = await инфо();   // период
    await наУровень(11);       const б11 = await инфо();   // контроль периода
    await наУровень(11, true); const бВзвод = await инфо(); // ручной вход поверх
    await наУровень(12);       const бПосле = await инфо(); // взвод потрачен
    expect(б10.бонус === true && б11.бонус === false &&
           бВзвод.бонус === true && бПосле.бонус === false,
      '⚠️⚠️ ВИТРИНА КАЖДЫМ ДЕСЯТЫМ: на 10-м наступает сама, на 11-м нет, взвод ' +
      'работает поверх периода и тратится (10:' + б10.бонус + ' 11:' + б11.бонус +
      ' взвод:' + бВзвод.бонус + ' после:' + бПосле.бонус + ')');
    ...
    // ⚠️⚠️ КОНТРОЛЬ ПЕРЕБИРАЕТ УРОВНИ, А НЕ БЕРЁТ ОДИН. Раньше он смотрел на
    // 21-й и был зелен ПОТОМУ, ЧТО ТАМ ВСЕГДА БЫЛИ КАМНИ. Камни удалены
    // 2026-08-17, и оставшиеся спецпредметы на конкретном уровне ЗАКОННО могут
    // отсутствовать все разом: клад требует купленного буста
    // (`anyBoostBought`), бомба идёт с разрывом 1-3, глыба тоже. Одиночный
    // уровень стал лотереей — контроль обязан искать там, где найти МОЖНО.
    let спОбычный = { всего: 0 }, гдеНашёл = 0;
    for (let l = 11; l <= 25 && спОбычный.всего === 0; l++){
      await наУровень(l);
      спОбычный = await page.evaluate(() => window.__game.specialsCount());
      if (спОбычный.всего > 0) гдеНашёл = l;
    }
    expect(спОбычный.всего > 0,
      'КОНТРОЛЬ: на ОБЫЧНЫХ уровнях спецпредметы есть (нашлись на ' + гдеНашёл +
      '-м: ' + JSON.stringify(спОбычный) + ') — значит ассерт выше проверяет ' +
      'гейт витрины, а не пустой пул');
    ...
    await наУровень(10, true); const в10 = await инфо();
    ...
    await наУровень(10, true); const дрБонус = await драг();
    ...
    {
      const снять = async (l, витрина) => {
        await page.evaluate(() => { window.__фазы = []; });
        await page.evaluate(([l, v]) => {
          const g = window.__game;
          window.__t = setInterval(() => { const f = g.introPhase(); if (f) window.__фазы.push(f); }, 16);
          g.setLevel(l);
          // ⚠️ ВИТРИНА ТЕПЕРЬ ВЗВОДИТСЯ ЯВНО: `regen()` на 10-м даёт ОБЫЧНЫЙ
          // уровень (владелец скрыл витрину 2026-08-17), и страж мерил бы фазы
          // не того уровня — то есть был бы зелен при любой поломке анимации.
          if (v) g.bonus(); else g.regen();
        }, [l, !!витрина]);
      ...
      const ф10 = await снять(10, true);   // ВИТРИНА
      const ф11 = await снять(11);         // контроль: обычный
    ...
    // (3-новое-б) ⚠️⚠️ «БЕЗ АНИМАЦИИ» — ЭТО НУЛЕВОЕ СМЕЩЕНИЕ ПОСЛЕ КОНЦА ИНТРО,
    // А НЕ СКОРОСТЬ В СЕРЕДИНЕ РУКОПОЖАТИЯ. Прежняя редакция читала `awake().maxV`
    // сразу после генерации и краснела на 1.66 — а это ТРАНЗИЕНТ фазы `wait`
    // (тела только что включены, сон ещё не наложен), игрок его не видит вовсе.
    // Замер по факту: смещение КАЖДОГО предмета за 800 мс после `introdone` —
    // на витрине РОВНО НОЛЬ у всех 136, на обычном уровне метры (там ливень).
    // ⚠️ КОНТРОЛЬ ОБЯЗАТЕЛЕН: без него «ноль смещения» истинно и на сборке, где
    // физика мертва вовсе.
    // ⚠️ И СЧИТАТЬ ПО ИНДЕКСУ, А НЕ ПО `it.key`: ключ — это ТИП, и первая версия
    // замера свернула 136 предметов в 5 ключей, то есть мерила пять последних
    // (канон, «метрика правдоподобна, но меряет не то, что названо вслух»).
    const смещение = async (l) => page.evaluate(async (lv) => {
      const g = window.__game, sl = ms => new Promise(r => setTimeout(r, ms));
      g.setLevel(lv); g.regen();
      for (let i = 0; i < 400 && !document.documentElement.classList.contains('introdone'); i++) await sl(50);
      const снимок = () => g.itemsGeo().map(it => [it.x, it.y, it.z]);
      const a = снимок();
      await sl(800);
      const c = снимок();
      let макс = 0; const n = Math.min(a.length, c.length);
      for (let i = 0; i < n; i++)
        макс = Math.max(макс, Math.hypot(c[i][0]-a[i][0], c[i][1]-a[i][1], c[i][2]-a[i][2]));
      return { бонус: !!(g.level()||{}).бонус || !!(g.level()||{}).bonus, предметов: n,
        смещение: +макс.toFixed(3), шагов: g.bonusInfo().осадкаШагов };
    }, l);
    const движВитрина = await смещение(10);
    const движОбычный = await смещение(11);
    console.log('движение после интро:', JSON.stringify({ движВитрина, движОбычный }));
    // ⚠️⚠️ ПОРОГ 3.0 — ПО РАСПРЕДЕЛЕНИЮ, И ПЕРВАЯ ЕГО РЕДАКЦИЯ (0.05) БЫЛА
    // ПОСТАВЛЕНА ПО ОДНОМУ ПРОГОНУ, где смещение вышло РОВНО НОЛЬ. Три прогона
    // подряд дали 0.86 / 1.10 / 1.75 у ХУДШЕГО предмета при среднем 0.12 — то
    // есть куча доседает после интро, и это нормально: решётка упакована с
    // вертикальным поджатием, солвер её распускает. Под полной нагрузкой сьюта
    // тот же замер даёт 0.15 — кадров в окне меньше, значит и шагов физики
    // меньше. Здоровый диапазон 0.15…1.75, ливень на обычном уровне — 11.0;
    // порог стоит в пустом коридоре между ними.
    // ⛔ И ФОРМУЛИРОВКА ИСПРАВЛЕНА ВМЕСТЕ С ЧИСЛОМ: ассерт утверждает «куча НЕ
    // ЛЬЁТСЯ сверху», а не «ни один предмет не сдвинулся» — второе неправда, и
    // страж с таким сообщением врал бы о том, что проверяет.
    expect(движВитрина.бонус === true && движВитрина.предметов > 100 &&
           движВитрина.смещение < 3.0 && движВитрина.шагов > 0,
      '⚠️⚠️ ВИТРИНА НЕ ЛЬЁТСЯ СВЕРХУ: после интро куча только доседает (худший ' +
      'предмет < 3 юнитов против 11 у ливня), укладка сошлась синхронно (' +
      JSON.stringify(движВитрина) + ')');
    expect(движОбычный.смещение > 1.0,
      'КОНТРОЛЬ: на обычном уровне в том же окне куча ЕДЕТ (' +
      движОбычный.смещение + ') — значит ноль выше про витрину, а не про мёртвую физику');
    ...
    await наУровень(10, true); const кр10 = await page.evaluate(() => window.__game.boxInfo());
    ...
    {
      // взводит витрину на ТОЙ ЖЕ странице и отдаёт её часы: свойство «часы
      // заводятся после занавеса» живёт у витрины, а не у холодного старта
      const хс2Взвод = async (стр) => {
        await стр.evaluate(() => window.__game.bonus());
        await стр.waitForFunction(() => document.documentElement.classList.contains('introdone'),
          null, { timeout: 60000 });
        return стр.evaluate(() => ({ бонус: window.__game.bonusInfo().бонус,
      ...
      // ⚠️⚠️ ВЗВОД СНИМАЕТСЯ ДО ЗАКРЫТИЯ СТРАНИЦЫ. Здесь он стоял ПОСЛЕ
      // `close()`, и прогон умирал без вердикта на 720 — третий за день случай
      // одного семейства («склейка двух правильных блоков»): вставленный код
      // пережил ресурс, который закрывает соседний. Правило себе: вставляя блок
      // в чужой, СНАЧАЛА грепнуть `.close()` в этом блоке.
      const живВитрина = await хс2Взвод(хс);
      // ⚠️⚠️ ВТОРАЯ ПОЛОВИНА, КОТОРОЙ НЕ БЫЛО В МОМЕНТ ИНЦИДЕНТА 2026-08-17 И
      // РАДИ КОТОРОЙ ОН И СЛУЧИЛСЯ: игра обязана быть ЦЕЛА ПОСЛЕ витрины, в ТОЙ
      // ЖЕ сессии. Тогда чаша, собранная цилиндром на загрузке бонусного уровня,
      // оставалась такой до конца сессии, и владелец увидел предметы снаружи
      // чаши на 201-м. Страж повторяет ровно тот путь: холодный старт на 10-м →
      // уход на далёкий ОБЫЧНЫЙ уровень → предметов вне чаши обязано быть НОЛЬ.
      // ⚠️ Мерим ЧИСТОЙ геометрией (`funnelR`), а не игровым `radiusAt`: на
      // витрине тот отдаёт описанную окружность ящика, и «предмет внутри» стало
      // бы истинным всегда — страж молчал бы ровно про свой дефект.
      const цела = await хс.evaluate(async () => {
        const g = window.__game, sl = ms => new Promise(r => setTimeout(r, ms));
        g.setLevel(201); g.regen();
        for (let i = 0; i < 400 && (window.__game.level()||{}).bonus !== false; i++) await sl(50);
        g.skipIntro(); await sl(1200);
        let снаружи = 0, нижеДна = 0, худший = 0;
        for (const it of g.itemsGeo()){
          const d = Math.hypot(it.x, it.z) - it.r, R = g.funnelR(it.y);
          if (d > R){ снаружи++; худший = Math.max(худший, +(d - R).toFixed(3)); }
          if (it.y < -0.5) нижеДна++;
        }
        return { уровень: g.levelNum(), живых: g.alive(), снаружи, нижеДна, худший };
      });
      console.log('игра после витрины:', JSON.stringify(цела));
      expect(цела.живых > 100 && цела.снаружи === 0 && цела.нижеДна === 0,
        '⚠️⚠️ ИГРА ЦЕЛА ПОСЛЕ ВИТРИНЫ: после холодного старта на 10-м обычный ' +
        'уровень той же сессии держит кучу в чаше — ноль снаружи, ноль под дном ' +
        '(это и есть инцидент 2026-08-17: ' + JSON.stringify(цела) + ')');
      await хс.evaluate(() => { try { localStorage.removeItem('mixer_level'); } catch(e){} });
      await хс.close();
      // ⛔⛔ СТРАЖ ПЕРЕВЁРНУТ ТРЕТИЙ РАЗ, И КАЖДЫЙ — ЗА СЛОВОМ ВЛАДЕЛЬЦА.
      // Хронология одного ассерта: (1) «холодный старт на 10-м ПОДНИМАЕТ витрину»
      // — период; (2) «НЕ поднимает никогда» — уровень скрыт после инцидента
      // 2026-08-17; (3) снова ПОДНИМАЕТ — период включён 2026-08-17-и.
      // ⚠️⚠️ И ИМЕННО ЭТОТ ПУТЬ ЛОМАЛ ИГРУ: `initPhysicsWorld` строит геометрию
      // чаши РОВНО ОДИН РАЗ ЗА ЗАГРУЗКУ, и загрузка НА бонусном уровне собирала
      // её цилиндром — на всех последующих обычных уровнях сессии. Теперь путь
      // разрешён, поэтому рядом стоит второй ассерт: игра ЦЕЛА ПОСЛЕ витрины
      // (канон: «проверять не только поднялся ли особый уровень, но и цела ли
      // игра ПОСЛЕ него»). Прежняя формулировка требовала:
      // «холодный старт на 10-м ПОДНИМАЕТ витрину» — правило владельца
      // 2026-08-17 сделало уровень СКРЫТЫМ, и теперь верно ОБРАТНОЕ: холодный
      // старт витрину НЕ поднимает НИКОГДА.
      // ⚠️⚠️ ЭТО НЕ ОСЛАБЛЕНИЕ ПРОВЕРКИ, А ИМЕННО ТОТ ИНВАРИАНТ, КОТОРЫЙ БЫ
      // ПРЕДОТВРАТИЛ ИНЦИДЕНТ 2026-08-17: `initPhysicsWorld` строит геометрию
      // чаши РОВНО ОДИН РАЗ ЗА ЗАГРУЗКУ, и единственный путь к сломанной чаше
      // был — загрузиться на бонусном уровне. Пока этот ассерт зелен, такого
      // пути не существует.
      console.log('холодный старт на 10-м:', JSON.stringify(хол), '| после взвода:',
        JSON.stringify(живВитрина), 'ошибок', беды.length);
      expect(хол.бонус === true && хол.уровень === 10 && хол.живых > 100,
        '⚠️⚠️ ХОЛОДНЫЙ СТАРТ НА 10-м ПОДНИМАЕТ ВИТРИНУ (период включён ' +
        '2026-08-17-и): уровень ' + хол.уровень + ', живых ' + хол.живых +
        ', бонус ' + хол.бонус);
      expect(беды.length === 0,
        'и без ошибок страницы на новой ветке загрузки (' + JSON.stringify(беды.slice(0,2)) + ')');
      // ⚠️ ЧАСЫ РАУНДА проверяем на ВЗВЕДЁННОЙ витрине — свойство осталось
      // прежним (часы заводятся ПОСЛЕ занавеса, а не под ним), сменился только
      // способ в неё попасть.
      expect(живВитрина.бонус === true && живВитрина.осталось !== null && живВитрина.осталось > 50,
        '⚠️ ЧАСЫ РАУНДА ЗАВЕДЕНЫ ПОСЛЕ ЗАНАВЕСА, А НЕ ПОД НИМ: осталось ' +
        живВитрина.осталось + ' с из 60 (сгорели бы — было бы заметно меньше)');
    }
