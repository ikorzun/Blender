...
`;
  ...
  const прикидка = await lbPage.evaluate(async () => {
    ...
    const чтение = () => ({
      ...
      заголовок:(document.getElementById('msLbeDot') || {}).textContent || '' });
  ...
  expect(прикидка.неточное.м.rank === 845 && прикидка.неточное.м.exact === false &&
         прикидка.неточное.вид.первая === '' && прикидка.неточное.вид.вторая === '' &&
         прикидка.неточное.вид.заголовок === '',
  ...
  expect(узкий.зазорАв === 4 && тесно.зазорАв === 4,
    '⚠️ ТОЧКА ВХОДА: зазор между аватарками 4px на ОБЕИХ ширинах (390 → ' +
    узкий.зазорАв + ', 320 → ' + тесно.зазорАв + ')');
  // ⚠️ И СТРЕЛКА: на 390 видна, на 320 спрятана (слово владельца «на 320 убери
  // стрелку»). Пара обязательна — «спрятана везде» тоже прошло бы одиночный.
  expect(узкий.стрелка > 8 && тесно.стрелка <= 2,
    '⚠️ ТОЧКА ВХОДА: стрелка есть на 390 и спрятана на 320 (' + узкий.стрелка +
    ' против ' + тесно.стрелка + ')');
  // ⚠️ ЗАЗОР МЕЖДУ АВАТАРКАМИ 4 (слово владельца 2026-08-11, отменяет его же 8
  // и 16 — числа он правил живьём три раза). На 390 стрелка ЕСТЬ, на 320 её
  // НЕТ по его же слову, поэтому «зазор до стрелки» проверяем только там, где
  // стрелка есть; на узкой утверждаем ОТСУТСТВИЕ.
  expect(узкий.аватар === 32 && тесно.аватар === 32 && узкий.доСтрелки === 10,
    '⚠️ ТОЧКА ВХОДА: аватар 32 на обеих ширинах, зазор до стрелки 10 на 390 (' +
    JSON.stringify({ ав390:узкий.аватар, ав320:тесно.аватар, доСтрелки:узкий.доСтрелки }) + ')');
  ...
  // список, заданное место), а не наследует от соседей.
  {
    ...
    // сломанной сборке.
    const полоски = await up.evaluate(async () => {
      const g = window.__game, sl = ms => new Promise(r => setTimeout(r, ms));
      const k = g.accSnapshot()[0].key;
      const дробь = () => { const s = g.accSnapshot().find(x => x.key === k);
        return { счёт:s.count, множ:+s.mult.toFixed(2), след:s.next,
                 доля: s.next ? +(((s.count - (s.tier > 0 ? 100 * (Math.pow(2, 0) - 1) : 0)) / 1) >= 0 ? 1 : 0) : 0 }; };
      const ширина = () => { const c = document.querySelector('#vGrid .vcell[data-key="' + k + '"] .vbar i');
        return c ? parseFloat(c.style.width) : null; };
      g.accGrant(k, 150); await sl(400);
      const до = { множ: дробь().множ, полоска: ширина() };
      g.starGrant(200000); g.buyBoost(k); g.buyBoost(k); await sl(400);
      const после = { множ: дробь().множ, полоска: ширина() };
      g.accGrant(k, 60); await sl(400);
      const рост = { полоска: ширина() };
      return { до, после, рост };
    });
    console.log('полоска прогресса:', JSON.stringify(полоски));
    expect(полоски.до.полоска === 25 && полоски.после.полоска === 25 &&
           полоски.после.множ > полоски.до.множ && полоски.рост.полоска === 55,
