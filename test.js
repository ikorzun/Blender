...
const HUD_FLOOR = { day: 1.30, night: 12.5 };   // белок глаза против неба (день — ПИН, см. выше)
  ...
  // проверяется через API (`level().shakes`), а показывает число DOM.
  {
    ...
    // (декодировалась, натуральные 192×192 — то есть ассет доехал, а не битая
    // ссылка) и РАМКА РОВНО ПО КНОПКЕ 64×64 без единого пикселя отступа.
    ...
    // проще всего.
    expect(ш0.естьКисть && ш0.тегИконки === 'IMG' && ш0.натур === '192x192' &&
      ...
      JSON.stringify({ тег: ш0.тегИконки, натуральные: ш0.натур, рамка: ш0.кисть,
                       фильтр: ш0.теньКисти }) + '). ⛔ ОТМЕНЯЕТ инлайновый SVG ' +
    ...
    expect(лупа.естьЛупа && лупа.тегИконки === 'IMG' && лупа.натур === '192x192' &&
