...
`;
  ...
  // предмет в 2 раза» — 83 → 166. Вьюпорт этой секции мобильный (390), так
  ...
  expect(/rgba\(0, 0, 0, 0\)|transparent/.test(slot.фон) && slot.кольцо === 'none' &&
    slot.слот === 166 && slot.слот > slot.подсказка && доля > 0.012 && доля < 0.072,
    'ЗАРЯД: не кнопка, а модель 166 (нода 829:1242 +30%, затем ×2 в мобильном), и она пульсирует ' +
...
const HUD_FLOOR = { day: 1.30, night: 12.5 };   // белок глаза против неба (день — ПИН, см. выше)
  ...
  // отодвинут от кромки на 8 и зазор вырос до 20, так что подсказка теперь в
  // 24+64+20 = 108... совпало случайно, а вот САМ Shake уехал: 16 -> 24).
  ...
  // протухает так же молча.
  {
    ...
    expect(д900.подложка === 'rgb(216, 187, 255)' && д900.объём === 'градиент' &&
           д1200.подложка === 'rgb(216, 187, 255)' && д1200.объём === 'градиент' &&
           д900.картинка === 'none' && д900.тень === 'none',
      '⚠️⚠️ НА ДЕСКТОПЕ ПОДЛОЖКА ПОД ГЛАЗАМИ #D8BBFF, У БЕЛКОВ ОБЪЁМ (' +
      JSON.stringify({ подложка: д900.подложка, объём: д900.объём }) + '). ' +
      'Цвет — слово владельца 2026-08-20-к. ⛔ ПРЕЖНИЙ ТЕКСТ ЭТОГО АССЕРТА ' +
      '(«такая же, как в мобильном») СТАЛ ЛОЖНЫМ 2026-08-21: мобильный блок ' +
      'схлопнут в тёмную кнопку-пилюлю, и лиловая подложка — свойство ТОЛЬКО ' +
      'десктопа. Ровно та болезнь, которую разбор ловил накануне: сообщение ' +
      'ассерта обязано переезжать вместе с предикатом. ⚠️ ОБЪЁМ ПРОВЕРЯЕТСЯ ЗДЕСЬ ЖЕ И ЭТО НЕСУЩЕЕ: ' +
      'на белом белый белок исчезает, и подложка без градиента оставила бы на ' +
      'десктопе два чёрных зрачка в пустоте — ровно тот кадр, который владелец ' +
      'уже забраковал на телефоне');
  ...
  // проверяется через API (`level().shakes`), а показывает число DOM.
  {
    ...
    expect(ш0.тег === 'BUTTON' && ш0.ария === null &&
           ш0.имяИзСодержимого === 'Shake ' + ш0.текст &&
           ш0.ширина === 64 && ш0.высота === 64 &&
           ш0.фон === 'rgba(255, 255, 255, 0.2)' && ш0.фонКартинка === 'none' &&
           ш0.радиус === '20px' &&
           ш0.тень === 'rgba(255, 255, 255, 0.7) 0px 4px 8px 0px inset' &&
           ш0.позиция === 'relative' && ш0.события === 'auto',
      '⚠️⚠️ SHAKE — 64×64 СО СТЕКЛЯННОЙ ПОДЛОЖКОЙ И РАДИУСОМ 20 (' +
      ...
      '«без подложки»: 2026-08-21-п владелец вернул кнопке фон ' +
      '`rgba(255,255,255,.20)` и внутреннюю белую тень, форма — «суперэллипс» ' +
      '(радиус 20 его числом). ⚠️ ФОН ПИНУЕТСЯ ЗНАЧЕНИЕМ, А НЕ «не прозрачный»: ' +
      'именно 20% отличают покой от наведения (80%), и «какая-то подложка» ' +
      'зеленело бы у залипшего ховера. ⚠️ В ПРАВИЛО СУТОК КНОПКА НЕ ВЕРНУЛАСЬ: ' +
    ...
    expect(ш0.естьКисть && ш0.тегИконки === 'IMG' && ш0.натур === '168x168' &&
           ...
           близко(ш0.кисть[2], 64) && близко(ш0.кисть[3], 64) &&
           ш0.теньКисти === 'none' &&
    ...
    // ⚠️ УГОЛ ПРОВЕРЯЕТСЯ ОТ НИЖНЕЙ КРОМКИ (`бейджНиз === 0`), А НЕ ЧЕРЕЗ
    // `бейджY`: высота бейджа менялась уже дважды, и пин по верху пришлось бы
    // пересчитывать каждый раз — а требование владельца звучит про УГОЛ.
    expect(близко(ш0.бейджX, 0) && близко(ш0.бейджНиз, 0) &&
    ...
    expect(ш0.своих > 0 && ш0.текст === String(ш0.своих) && ш0.класс === '' &&
           ...
           шAd.бейджШ > шAd.бейджВ && шAd.бейджX + шAd.бейджШ <= 64 &&
    ...
    expect(лупа.тег === 'BUTTON' && лупа.ария === 'Hint' && !/iconBtn/.test(лупа.классы) &&
           лупа.ширина === 64 && лупа.высота === 64 &&
           лупа.фон === 'rgba(255, 255, 255, 0.2)' && лупа.фонКартинка === 'none' &&
           лупа.радиус === '100px' &&
           лупа.теньКнопки === 'rgba(255, 255, 255, 0.7) 0px 4px 8px 0px inset' &&
           лупа.события === 'auto' && лупа.позиция === 'relative',
      '⚠️⚠️ ПОДСКАЗКА — КРУГ 64×64 СО СТЕКЛЯННОЙ ПОДЛОЖКОЙ (' +
      ...
      'владелец вернул фон `rgba(255,255,255,.20)` и внутреннюю белую тень, ' +
      'форма — КРУГ (радиус 100 его числом), в отличие от «суперэллипса» 20 у ' +
      'Shake. ⚠️ РАДИУС — ЕДИНСТВЕННОЕ, ЧЕМ ЭТИ ДВЕ КНОПКИ ТЕПЕРЬ ОТЛИЧАЮТСЯ ' +
      'ВНЕШНЕ: перепутай их местами — всё остальное осталось бы верным. ' +
    ...
    expect(лупа.естьЛупа && лупа.тегИконки === 'IMG' && лупа.натур === '168x168' &&
           ...
           блЛ(лупа.лупаБокс[2], 64) && блЛ(лупа.лупаБокс[3], 64) &&
           лупа.тень === 'none',
    ...
    expect(блЛ(лупа.бейджX, 0) && блЛ(лупа.бейджНиз, 0) &&
  ...
  // ── ЗАРЯД НА ДЕСКТОПЕ НЕ УДВОЕН (слово владельца 2026-08-21-м) ──────
    ...
    expect(зарядД && зарядД.ширина === '104px' && зарядД.позиция === 'fixed',
      ...
      'сказано ПРО МОБИЛЬНЫЙ. Диверсия — снять `@media` с правила 166: ' +
    ...
    const ПОКОЙ = 'rgba(255, 255, 255, 0.2)', ЗАЛИТ = 'rgba(255, 255, 255, 0.8)';
    expect(хов.shakeФон === ПОКОЙ && хов.hintФон === ПОКОЙ &&
      ...
      '⚠️⚠️ ХОВЕР ЗАЛИВАЕТ ФОН 20% → 80%, И ЭТО ЕДИНСТВЕННОЕ, ЧТО ОН ДЕЛАЕТ ' +
    ...
    expect(ховТач.ховерЕсть === false && ховТач.фон === 'rgba(255, 255, 255, 0.2)' &&
           ховТач.арт === 'none',
    ...
    const досып = await стр.evaluate(async () => {
