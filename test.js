...
`;
  ...
  // с каноном.
  {
    // (1) ПО ВСЕМУ ПУЛУ: цвет выводится из предмета, поэтому проверяем ВСЕ типы,
    // а не тот, что попался. Читаем ТУ ЖЕ функцию, что и бой (хук popOutlines).
    ...
    const кон = пул.map(c => кБел(c.цвет));
    const мин = Math.min(...кон), разных = new Set(пул.map(c => c.цвет)).size;
    const худший = пул[кон.indexOf(мин)];
    // ⚠️⚠️ ДВА ПРИЗНАКА В ОДНОМ АССЕРТЕ И ОБА НЕСУЩИЕ: «разные» без «читаемо»
    // прошло бы с жёлтой обводкой под белой цифрой (замер первой версии: 1.64:1
    // у лимона), а «читаемо» без «разных» — с одной чёрной на всех.
    expect(разных >= 40 && мин >= 3.0,
      '⚠️⚠️ ОБВОДКА ПОПА: у ' + пул.length + ' типов ' + разных + ' РАЗНЫХ обводок, ' +
      'и худшая отделяет белую цифру на ' + мин.toFixed(2) + ':1 при поле 3 (' +
      худший.имя + ' ' + худший.цвет + ')');
    ...
    expect(!!живой && живой.вычисл && живой.вычисл !== '#000' &&
           живой.вычисл !== 'rgb(0, 0, 0)' &&
           живой.заливка === 'rgb(255, 255, 255)',
      '⚠️⚠️ ОБВОДКА ПОПА НА БОЕВОМ ПУТИ: цифра белая, обводка НЕ чёрная (' +
      JSON.stringify(живой) + ')');
  }
