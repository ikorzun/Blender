...
`;
  ...
  const БАЗА_ЛБ = { state:'ok', rank:25, score:11200, sent:11200, server:11200,
  ...
  const врезка = (ответ) => lbPage.evaluate(async (ответ) => {
    ...
    g.winScreen(false);
    g.winLbStub(() => Promise.resolve(ответ));
    g.winScreen(true);
    const t0 = Date.now(); let info = g.winLbInfo();
    while (Date.now() - t0 < 1500){
      info = g.winLbInfo();
      if (info && (info.on || (ответ && ответ.state !== 'ok'))) break;
      await sleep(30);
    }
    return info;
  }, ответ);
  ...
  // ЗАНИЖЕННОЕ ЧИСЛО ПОКАЗЫВАЕМ (решение владельца 2026-08-09: вариант «не
  // показывать, пока сервер не догнал» ОТКЛОНЁН). В игре 50 000 — в таблице
  // 2 000, и на экране обязано стоять СЕРВЕРНОЕ число плюс объяснение.
  const лбКлип = await врезка(Object.assign({}, БАЗА_ЛБ, { score:2000, sent:50000, server:2000 }));
  console.log('врезка/клип:', JSON.stringify(лбКлип && { s:лбКлип.rows[1].score, note:лбКлип.note }));
  expect(!!лбКлип && лбКлип.rows[1].score === '2000',
    '⚠️ ВРЕЗКА: в своей строке стоит то, что записал СЕРВЕР (2000), а не то, что мы послали (50000): ' +
    (лбКлип && лбКлип.rows[1].score));
  expect(!!лбКлип && лбКлип.note.length > 0 && лбОк.note.length === 0,
    '⚠️ ВРЕЗКА: расхождение ОБЪЯСНЕНО словами — примечание есть при заниженном и' +
    ' отсутствует при обычном (проверен ПЕРЕХОД, а не наличие): «' + (лбКлип && лбКлип.note) + '»');

  // НЕТ ДАННЫХ — МОЛЧИМ. Таблица украшение: текста ошибки на экране победы нет.
  const лбНет = await врезка({ state:'offline' });
  console.log('врезка/нет данных:', JSON.stringify(лбНет));
  expect(!!лбНет && лбНет.on === false && лбНет.rows.length === 0 && лбНет.note === '',
    '⚠️ ВРЕЗКА: нет данных — врезка МОЛЧИТ (ни строк, ни текста ошибки): ' + JSON.stringify(лбНет));

  // ⚠️⚠️ ВЫСОТА СЛОТА ОДИНАКОВА ВО ВСЕХ СОСТОЯНИЯХ. Над врезкой стоит Next, а
  // у карточки margin:auto — «то две строки, то три, то пусто» двигало бы
  // кнопку из-под пальца ровно в тот момент, когда игрок её жмёт.
  expect(лбОк.h > 0 && лбОк.h === лбНет.h && лбОк.h === лбКлип.h,
    '⚠️ ВРЕЗКА: слот держит высоту в любом состоянии — ок/заниженное/нет данных (' +
    лбОк.h + '/' + лбКлип.h + '/' + лбНет.h + ' px)');
