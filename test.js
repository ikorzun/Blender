...
`;
  ...
  expect(кап12.мин >= 55 && кап60б.мин !== null && кап60б.мин <= 30 &&
  ...
  // сенсорный дефект: три зелёные пробы подряд не трогали сломанную ветку.
  {
    ...
    // (1) ⚠️⚠️ СТРАЖ ПЕРЕЕХАЛ ЗА СМЕНОЙ ПРАВИЛА, А НЕ «ПОЧИНЕН». Прежде он
    // утверждал «10 и 20 бонусные, 11 — обычный» (периодичность). Владелец
    // 2026-08-17 сделал уровень СКРЫТЫМ: по прогрессии он не наступает НИ НА
    // КАКОМ уровне, только по явному взводу.
    // ⚠️ ОБЕ ПОЛОВИНЫ НЕСУЩИЕ: без «на 10-м САМ ПО СЕБЕ витрины нет» возврат
    // периодичности прошёл бы молча, а без «по взводу витрина есть» зелёным
    // было бы и полное отсутствие фичи.
    // ⚠️⚠️ И ТРЕТЬЯ ПОЛОВИНА — ПРО БЕЗОПАСНОСТЬ: взвод обязан ТРАТИТЬСЯ. Если
    // он залипнет, игрок останется в витрине навсегда, а сохранённый бонусный
    // уровень — это ровно тот холодный старт, что ломал геометрию чаши.
    await наУровень(10);       const б10 = await инфо();   // БЕЗ взвода — контроль
    await наУровень(20);       const б20 = await инфо();   // БЕЗ взвода — контроль
    await наУровень(10, true); const бВзвод = await инфо();
    await наУровень(11); const бПосле = await инфо();
    expect(б10.бонус === false && б20.бонус === false &&
           бВзвод.бонус === true && бПосле.бонус === false,
      '⚠️⚠️ ВИТРИНА СКРЫТА: сама по себе не наступает ни на 10, ни на 20; по ' +
      'взводу наступает; взвод тратится — следующий уровень обычный (10:' +
      б10.бонус + ' 20:' + б20.бонус + ' взвод:' + бВзвод.бонус + ' после:' +
      бПосле.бонус + ')');
    ...
    // ⚠️ Своя страница: холодный старт нельзя изобразить на уже живой.
    {
      ...
      await хс.close();
      // ⛔⛔ СТРАЖ ПЕРЕВЁРНУТ, И ЭТО ГЛАВНОЕ УТВЕРЖДЕНИЕ ФИЧИ. Он требовал
      ...
      expect(хол.бонус === false && хол.уровень === 10 && хол.живых > 100,
        '⚠️⚠️ ХОЛОДНЫЙ СТАРТ НА 10-м НЕ ПОДНИМАЕТ ВИТРИНУ — она скрыта и в ' +
        'сейв не попадает (уровень ' + хол.уровень + ', живых ' + хол.живых +
        ', бонус ' + хол.бонус + ')');
