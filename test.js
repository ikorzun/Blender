...
`;
  ...
  //   лиса чистая 147.0/0.691 → после смены «фона» 62.3/0.650
  //     (диверсия альфы там же — 255.0/0; диверсия обёртки — 147.0/0.691)
  ...
  // хотя выбеливание владельца шло именно по второй.
  {
    ...
    const базЛис = await снять(a, 'animalfox');
    await a.close();
    ...
    const лисПосле = await снять(г, 'animalfox');
    await г.close();

    console.log('редактор:', JSON.stringify({ базКоу, открКоу, отм, нажато, реестр, базБи, примБи, свинБез, свинМаз, цветПрим, базЛис, лисПосле }));
    ...
    expect(цветПрим && базЛис && базЛис.n > 500 && лисПосле,
      'САНИТАР ЦЕЛИ ПО УМОЛЧАНИЮ: поле «фон» найдено и получило событие, оба ' +
      'портрета построились (' + JSON.stringify({ цветПрим, базЛис, лисПосле }) + ')');

    expect(лисПосле && лисПосле.я < 250 && лисПосле.нас > 0.1,
      '⚠️⚠️ ВТОРАЯ ВЕТКА `mceApply` НЕ ВЫБЕЛИВАЕТ: правка общего пресета `tex` ' +
      '(цель по умолчанию) сохраняет цвет (' + JSON.stringify(лисПосле) + '). ' +
      'Диверсия — вернуть `dst.set(src)` вместо покомпонентного копирования RGB; ' +
      'замер диверсии здесь 255.0/0, здоровой сборки 62.3/0.650');

    expect(лисПосле && Math.abs(лисПосле.я - базЛис.я) > 5,
      '⚠️ ЦВЕТОВЫЕ ПОЛЯ ПРИМЕНЯЮТСЯ СРАЗУ: смена «фона» меняет цель без нажатия ' +
      '«Применить» (' + JSON.stringify({ базЛис, лисПосле }) + ', замер 147.0 → 62.3). ' +
      ...
      'базовым один-в-один (147.0/0.691)');
  }
