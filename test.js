...
`;
  ...
  // ⚠️⚠️ ПРАВИЛО СМЕНИЛОСЬ 2026-08-10 (слово владельца «на игровом тоже сделай
  // нейтральной»): кромка БОЛЬШЕ НЕ подбирается под тон неба и под тон меню —
  // она ЧИСТАЯ НЕЙТРАЛЬ по теме. Прежние три ассерта стерегли ОТМЕНЁННУЮ
  // механику и краснели на исправной сборке; они не «починены», а ПЕРЕПИСАНЫ
  // на новый инвариант. Тон меню при этом остаётся нужен как КОНТРОЛЬ: цена
  // ошибки — вернувшийся подбор под палитру, и он обязан краснеть.
  const НЕЙТРАЛЬ = /^rgb\(255, ?255, ?255\)$/;   // день; ночью здесь чёрный
  expect(НЕЙТРАЛЬ.test(bgSeams.closed.body.col) && bgSeams.closed.body.col !== bgSeams.skyTop,
    '⚠️ КРОМКА В ИГРЕ: чистая НЕЙТРАЛЬ, а НЕ тон неба (' + bgSeams.closed.body.col +
    ', небо ' + bgSeams.skyTop + ')');
  expect(bgSeams.open.меню === true,
    'САНИТАР: меню действительно открылось кликом по паузе (иначе следующий ассерт мерит пустоту)');
  expect(НЕЙТРАЛЬ.test(bgSeams.open.body.col) && НЕЙТРАЛЬ.test(bgSeams.open.html.col) &&
         bgSeams.open.body.col !== bgSeams.menuRGB,
    '⚠️ КРОМКА В МЕНЮ: та же НЕЙТРАЛЬ, а НЕ тон меню — попасть в тон нельзя, ' +
    'меню прокручивается (' + JSON.stringify(bgSeams.open) + ', фон меню ' + bgSeams.menuRGB + ')');
  expect(bgSeams.back.body.col === bgSeams.closed.body.col,
    '⚠️ КРОМКА ПОСЛЕ МЕНЮ: та же, что была до него — тон меню не залипает и небо не возвращается (' +
  ...
  expect(безПульса.глубоких <= 10 && всеПульсируют.глубоких >= 40,
    '⚠️ ЗВЁЗДЫ: пульс ДОЕЗЖАЕТ ДО ПИКСЕЛЕЙ — доля 1.0 против 0 (глубоко гуляющих ' +
    безПульса.глубоких + '% -> ' + всеПульсируют.глубоких + '%; лесенка на боевой 0.10 даёт 2.5%)');
  ...
  // ⚠️ СВОЯ СТРАНИЦА И КОНЕЦ ФАЙЛА: секция ставит ДОЛГОЖИВУЩЕЕ состояние —
  // подменённый источник данных врезки. Оставь я его в общей странице, соседи
  // получили бы чужую врезку на своих экранах победы. Приём тот же, что у
  // камней, меню и звёзд.
  ...
  await lbPage.evaluate(() => window.__game.skipIntro());
  const БАЗА_ЛБ = { state:'ok', rank:25, exact:1, score:11200, sent:11200, server:11200,
    up:[{ name:'Otter', av:7, score:12480 }, { name:'Lynx', av:9, score:19000 }],
    dn:[{ name:'Bear', av:3, score:10900 }, { name:'Mole', av:5, score:9000 }] };
  // ОТКРЫТЬ ПОБЕДУ БОЕВЫМ ПУТЁМ с заданным ответом источника.
  // ⚠️ ЖДЁМ ФАКТ (появление строк) ОПРОСОМ, а не фикс-паузой: пауза меряет
  // часы стенда, а не состояние страницы (канон «поймал момент, а не состояние»).
  const врезка = (ответ) => lbPage.evaluate(async (ответ) => {
    const g = window.__game, sleep = ms => new Promise(r => setTimeout(r, ms));
    g.winScreen(false);
    // ⚠️ ЖДЁМ ДВА ФАКТА, А НЕ ЧАСЫ: (1) источник СПРОСИЛИ, (2) врезка успела
    // отрисоваться. Ждать только появления строк нельзя — половина состояний
    // МОЛЧИТ по замыслу, и такое ожидание всегда упиралось бы в потолок,
    // то есть меряло бы часы стенда (канон «поймал момент, а не состояние»).
    window.__lbAsked = false;
    g.winLbStub(() => { window.__lbAsked = true; return Promise.resolve(ответ); });
    g.winScreen(true);
    const t0 = Date.now(); let info = g.winLbInfo();
    while (Date.now() - t0 < 1500){
      await sleep(20);
      info = g.winLbInfo();
      if (window.__lbAsked && (info.on || Date.now() - t0 > 300)) break;
    }
    return info;
  }, ответ);

  const лбОк = await врезка(БАЗА_ЛБ);
  console.log('врезка/ок:', JSON.stringify(лбОк));
  expect(!!лбОк && лбОк.on === true && лбОк.rows.length === 3,
    'САНИТАР ВРЕЗКИ: пришёл ответ — врезка видна и в ней ТРИ строки (иначе' +
    ' следующие ассерты мерят пустоту): ' + JSON.stringify(лбОк));
  expect(!!лбОк && лбОк.rows[1] && лбОк.rows[1].me === true &&
         лбОк.rows[1].pos === '#25' && лбОк.rows[1].name === 'You',
    '⚠️ ВРЕЗКА: своя строка ВЫДЕЛЕНА и стоит между соседями (место #25)');
  expect(!!лбОк && лбОк.rows[0].pos === '#24' && лбОк.rows[2].pos === '#26',
    '⚠️ ВРЕЗКА: места соседей выведены из своего места, а не выдуманы (#24/#26)');
  expect(!!лбОк && /Avatar07\.png$/.test(лбОк.rows[0].av || ''),
    '⚠️ ВРЕЗКА: аватар соседа — файл владельца по номеру из ответа (' +
    (лбОк && лбОк.rows[0].av) + ')');

  // ⚠️ ПОРЯДОК СОСЕДЕЙ В МАССИВЕ — ЧУЖОЙ КОНТРАКТ, И ОН МОЖЕТ БЫТЬ ЛЮБЫМ.
  // Тот же ответ с ПЕРЕВЁРНУТЫМ up: ближайшим сверху обязан остаться тот, у
  // кого счёт МЕНЬШЕ (Otter 12480), а не тот, кто первый в массиве (Lynx).
  // Без этого ошибка была бы тихой: врезка показала бы не тех соседей и
  // выглядела бы совершенно правдоподобно.
  const лбПорядок = await врезка(Object.assign({}, БАЗА_ЛБ, {
    up:[{ name:'Lynx', av:9, score:19000 }, { name:'Otter', av:7, score:12480 }],
    dn:[{ name:'Mole', av:5, score:9000 }, { name:'Bear', av:3, score:10900 }] }));
  console.log('врезка/порядок:', JSON.stringify(лбПорядок && лбПорядок.rows));
  expect(!!лбПорядок && лбПорядок.rows[0].name === 'Otter' && лбПорядок.rows[2].name === 'Bear',
    '⚠️ ВРЕЗКА: соседи выбраны ПО СЧЁТУ, а не по порядку в массиве (перевёрнутый ответ' +
    ' дал тех же Otter/Bear): ' + JSON.stringify(лбПорядок && лбПорядок.rows.map(r => r.name)));

  // В СВОЕЙ СТРОКЕ — ТО, ЧТО ЗАПИСАЛ СЕРВЕР, А НЕ ТО, ЧТО МЫ ПОСЛАЛИ.
  // ⚠️ Отдельного состояния под расхождение больше НЕТ (владелец 2026-08-09 снял
  // потолок доверия целиком), но разделение осталось несущим: сервер отказывает
  // по частоте и по подписи и мягко деградирует на упавшей базе, а «тело
  // разобралось» успехом не является — рисуя посланное, мы показывали бы
  // несуществующий результат в каждом таком случае.
  const лбСервер = await врезка(Object.assign({}, БАЗА_ЛБ, { sent:50000, server:11200 }));
  console.log('врезка/серверное число:', JSON.stringify(лбСервер && лбСервер.rows[1]));
  expect(!!лбСервер && лбСервер.rows[1].score === '11.2k',
    '⚠️ ВРЕЗКА: в своей строке стоит ЗАПИСАННОЕ сервером (11.2k), а не ПОСЛАННОЕ нами (50000): ' +
    (лбСервер && лбСервер.rows[1].score));

  // ⚠️⚠️ ПРИКИДКУ ЗА МЕСТО НЕ ВЫДАЁМ. Быстрый ответ на отправку несёт `rank`,
  // но это оценка по лесенке снимка: лесенка строится по каждому сотому игроку,
  // и пока в таблице меньше сотни строк, она отвечает «место 1» КАЖДОМУ (замер
  // диспетчера 2026-08-09) — то есть врала бы всем в первые недели после
  // запуска. Экран держит это сам, независимо от правок на сервере.
  const лбПрикидка = await врезка(Object.assign({}, БАЗА_ЛБ, { exact:false }));
  console.log('врезка/прикидка:', JSON.stringify(лбПрикидка));
  expect(!!лбПрикидка && лбПрикидка.on === false && лбПрикидка.rows.length === 0,
    '⚠️ ВРЕЗКА: НЕточное место (прикидка) не показывается вовсе: ' + JSON.stringify(лбПрикидка));

  // ⚠️⚠️ ОТКАЗ НА ВОПРОСЕ ДОВЕРИЯ — ЗАКРЫТЫЙ: поля `exact` НЕТ ВОВСЕ = не
  // показываем. Цена молчания — пустой виджет; цена ложного «можно» —
  // показанное неверное место, тот самый класс, который мы весь день
  // вычищали. Тихое молчание при переименовании поля лечится стражем
  // контракта ниже, а НЕ доверием по умолчанию (указание диспетчера).
  const безПоля = Object.assign({}, БАЗА_ЛБ); delete безПоля.exact;
  const лбБезПоля = await врезка(безПоля);
  console.log('врезка/без поля exact:', JSON.stringify(лбБезПоля));
  expect(!!лбБезПоля && лбБезПоля.on === false && лбБезПоля.rows.length === 0,
    '⚠️ ВРЕЗКА: поля `exact` нет вовсе — молчим (отказ ЗАКРЫТЫЙ, а не «раз не сказано — значит можно»)');

  // МОЕЙ СТРОКИ В ВЫДАЧЕ НЕТ (ещё не отправлялся / строку скрыли) — молчим.
  const лбБезМеня = await врезка(Object.assign({}, БАЗА_ЛБ, { rank:null }));
  console.log('врезка/меня нет:', JSON.stringify(лбБезМеня));
  expect(!!лбБезМеня && лбБезМеня.on === false && лбБезМеня.rows.length === 0,
    '⚠️ ВРЕЗКА: моей строки в таблице нет — врезка молчит, а не рисует место из ниоткуда');

  // НЕТ СЕТИ / МУСОР В ОТВЕТЕ — МОЛЧИМ. Таблица украшение: текста ошибки нет.
  const лбНет = await врезка({ state:'offline' });
  console.log('врезка/нет сети:', JSON.stringify(лбНет));
  expect(!!лбНет && лбНет.on === false && лбНет.rows.length === 0,
    '⚠️ ВРЕЗКА: нет данных — врезка МОЛЧИТ (ни строк, ни текста ошибки): ' + JSON.stringify(лбНет));

  // ПУСТО, НО РАНО: снимок ещё не построен — это НЕ поломка и тоже молчит.
  const лбРано = await врезка({ state:'early' });
  console.log('врезка/рано:', JSON.stringify(лбРано));
  expect(!!лбРано && лбРано.on === false && лбРано.rows.length === 0,
    '⚠️ ВРЕЗКА: снимок ещё не построен — молчим, как и при отсутствии сети');

  // ⚠️⚠️ ВЫСОТА СЛОТА ОДИНАКОВА ВО ВСЕХ ЧЕТЫРЁХ СОСТОЯНИЯХ. Над врезкой стоит
  // Next, а у карточки margin:auto — «то три строки, то пусто» двигало бы
  // кнопку из-под пальца ровно в тот момент, когда игрок её жмёт.
  expect(лбОк.h > 0 && лбОк.h === лбНет.h && лбОк.h === лбРано.h && лбОк.h === лбБезМеня.h,
    '⚠️ ВРЕЗКА: слот держит высоту во всех состояниях — список/нет сети/рано/меня нет (' +
    [лбОк.h, лбНет.h, лбРано.h, лбБезМеня.h].join('/') + ' px)');

  // ⚠️⚠️ ФИЧА ВЫКЛЮЧЕНА (модуля таблицы нет или он не настроен) — ВРЕЗКИ НЕТ
  // В РАСКЛАДКЕ ВОВСЕ. Резерв высоты оправдан, только пока данные ОЖИДАЮТСЯ;
  // пустой слот «на будущее» отодвигал бы TOP ITEMS у игрока, которому таблица
  // не показывается никогда. Проверяется ПЕРЕХОД: тот же экран с включённой
  ...
  expect(лбВыкл.h === 0 && лбВыкл.поток === 'none' && лбОк.h > 0,
    '⚠️ ВРЕЗКА: таблицы нет вовсе — слот НЕ занимает места (' + лбВыкл.h +
    ' px против ' + лбОк.h + ' с включённой фичей)');

  // СТРАЖ КОНТРАКТА: `/me` обязан нести `exact`. ⚠️ ВЗВОДИТСЯ ВМЕСТЕ С МОДУЛЕМ
  // таблицы (его пишет ИНТЕГРАЦИЯ, в сборке его пока нет) — и это ЕДИНСТВЕННАЯ
  // причина, по которой ассерта здесь сейчас нет. Он нужен потому, что отказ по
  ...
  // ЭПОХА: поздний ответ прошлого экрана не смеет дорисоваться в свежий.
  // ⚠️ ПРОВЕРЯЕТСЯ ПЕРЕХОД, А НЕ ЗНАЧЕНИЕ: сперва показываем, что МЕДЛЕННЫЙ
  // ответ на СВОЁМ экране доезжает (иначе «не нарисовалось» было бы истинно и
  // на сборке, где врезки нет вовсе), и только потом — что чужой отбрасывается.
  const лбЭпоха = await lbPage.evaluate(async (БАЗА) => {
    const g = window.__game, sleep = ms => new Promise(r => setTimeout(r, ms));
    g.winScreen(false);
    g.winLbStub(() => new Promise(r => setTimeout(() => r(БАЗА), 300)));
    g.winScreen(true);
    await sleep(800);
    const своя = g.winLbInfo();
    // тот же медленный ответ, но экран закрывают и открывают ДО его прихода;
    // второй запрос не отвечает никогда — рисовать может только опоздавший
    let n = 0;
    g.winScreen(false);
    g.winLbStub(() => (++n === 1)
      ? new Promise(r => setTimeout(() => r(БАЗА), 400))
      : new Promise(() => {}));
    g.winScreen(true);          // запрос №1, медленный
    await sleep(60);
    g.winScreen(false);         // закрыли ДО ответа — эпоха сдвинулась
    g.winScreen(true);          // открыли снова; №2 молчит навсегда
    await sleep(900);           // ответ №1 давно пришёл
    const чужая = g.winLbInfo();
    g.winScreen(false); g.winLbStub(null);
    return { своя, чужая };
  }, БАЗА_ЛБ);
  console.log('врезка/эпоха:', JSON.stringify(лбЭпоха));
  expect(!!лбЭпоха.своя && лбЭпоха.своя.on === true && лбЭпоха.своя.rows.length === 3,
    'САНИТАР ЭПОХИ: медленный ответ на СВОЁМ экране доезжает (иначе следующий' +
    ' ассерт истинен и на сборке без врезки вовсе)');
  ...
  // ⚠️⚠️ ОБА КАНАЛА НЕСУТ ОДНО ЗНАЧЕНИЕ, И ЭТО СУТЬ ПРАВКИ 2026-08-10. Мы не
  // знаем наверняка, какой из них Safari читает в каком случае, — при
  // одинаковой нейтрали ответ перестаёт иметь значение. ⚠️ Проверяется И
  // равенство каналов, И то, что это НЕ цвет неба: без второй половины ассерт
  // был бы зелен и при возврате подбора под палитру (оба канала = стоп неба).
  expect(кромка.игроваяRgb === '255,255,255' && кромка.игроваяRgb !== кромка.верх &&
         кромка.игроваяRgb !== кромка.низ,
    '⚠️ КРОМКА: в игре мета — чистая НЕЙТРАЛЬ, а не стоп неба (' + кромка.доМеню +
    ', верх ' + кромка.верх + ', низ ' + кромка.низ + ')');
  // ⚠️⚠️ В МЕНЮ — ЧИСТАЯ НЕЙТРАЛЬ, А НЕ ТОН МЕНЮ (слово владельца 2026-08-10
  // «убери заливку цветом под островом»). Попасть в тон там нельзя по
  // построению: меню ПРОКРУЧИВАЕТСЯ, под островом оказывается то шапка, то
  // карточка, а цвет один. ⚠️ Проверяется И то, что нейтраль, И то, что это
  // УЖЕ НЕ `--ms-bg`: без второй половины ассерт был бы зелен и на прежней
  // сборке, если бы фон меню однажды сделали белым.
  expect(кромка.метаRgb === '255,255,255' && кромка.метаRgb !== кромка.msRgb,
    '⚠️ КРОМКА: в меню мета — ЧИСТАЯ НЕЙТРАЛЬ, а не тон меню (' +
    кромка.вМеню.мета + ', фон меню ' + кромка.вМеню.msbg + ')');
  // ⚠️ ПЕРЕХОД «небо -> меню -> небо» БОЛЬШЕ НЕ СУЩЕСТВУЕТ: нейтраль одна на
  // оба экрана, и ассерт на смену значения был бы тавтологией (белый = белый).
  // Проверяем то, что теперь несущее: мета и фон подложки — ОДНО значение.
  expect(кромка.после.мета === кромка.доМеню,
    '⚠️ КРОМКА: нейтраль одна на игру и меню — после закрытия ничего не «залипло» (' +
    кромка.доМеню + ' -> ' + кромка.вМеню.мета + ' -> ' + кромка.после.мета + ')');

  // ⚠️ ГРАДИЕНТ ПОДЛОЖКИ СНЯТ вместе с подбором под палитру: он существовал
  // ради попадания в тон кромки, нейтрали попадать не во что. Страж остаётся,
  // но с ОБРАТНЫМ утверждением — иначе возврат градиента прошёл бы молча.
  ...
  expect(подложка.картинка === 'none' && /^rgb\(255, ?255, ?255\)$/.test(подложка.цвет),
    '⚠️ КРОМКА: подложка — ПЛОСКАЯ нейтраль без градиента неба (' +
    подложка.цвет + ' / ' + подложка.картинка + ')');
