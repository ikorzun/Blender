...
`;
  ...
  // список, заданное место), а не наследует от соседей.
  {
    ...
    const лб = await up.evaluate(async () => {
      ...
      return { строк: строки.length,
        ...
        имена: (function (){
          ...
          const n = я ? я.querySelector('.lb-name') : null;
          return { разныхКраёв: new Set(лев).size,
                   свояОбрезана: n ? (n.scrollWidth > n.clientWidth + 1) : null };
        })(),
    ...
    expect(лб.имена && лб.имена.свояОбрезана === false && лб.имена.разныхКраёв === 1,
      '⚠️⚠️ ТАБЛИЦА: имя не сокращается, пока есть место, И колонка чисел ' +
      'осталась выровненной (' + JSON.stringify(лб.имена) + ')');
