...
`;
  ...
  // ⚠️⚠️ ПРАВИЛО ПЕРЕВЕРНУЛОСЬ (слово владельца «всегда показывай 3 аватарки»):
  // прежний ассерт утверждал «на тесной их МЕНЬШЕ трёх» — то есть ровно
  // отменённое поведение. Он ПЕРЕЕХАЛ за правилом, а не «починен».
  // ⚠️ Вторая половина НЕ потеряна: соседние ассерты выше держат «заголовок
  // влезает и не наезжает», поэтому «три» нельзя получить ценой обрезанного
  // текста — а это и был смысл прежнего уступания.
  // ⚠️ Считаем ВСЕ слоты, а не `img`: при пустой таблице слот — нейтральный
  // кружок, и счёт по картинкам дал бы ноль на ИСПРАВНОЙ сборке.
  ...
  // ⚠️⚠️ ШИРИНУ МЕРИМ НА ДВУХ ВЬЮПОРТАХ, И ЭТО НЕ ПЕДАНТИЗМ: страница сьюта
  // мобильная (393), там потолок 560 недостижим ПО ПОСТРОЕНИЮ — `min(560,
  // 100vw − 32)` даёт 361. Односторонний ассерт «>=520» краснел бы на
  // ИСПРАВНОЙ сборке. Страж САМ создаёт себе широкий экран.
  const ширины = await (async () => {
    const снять = async () => await lbPage.evaluate(async () => {
      ...
      const w = document.querySelector('#lbOverlay .lb-card');
      const x = document.getElementById('lbClose');
      const из = { ширина: w ? Math.round(w.getBoundingClientRect().width) : 0,
    ...
    await lbPage.waitForTimeout(250);
    const узкий2 = await снять();
    return { широкий, узкий: узкий2 };
  })();
  console.log('экран/ширина:', JSON.stringify(ширины));
  expect(ширины.широкий.ширина === 560,
    '⚠️ ЭКРАН ТАБЛИЦЫ: на широком экране ширина РОВНО 560 — потолок владельца достигнут (' +
    ширины.широкий.ширина + 'px при вьюпорте 1280)');
  expect(ширины.узкий.ширина > 0 && ширины.узкий.ширина <= 361 && ширины.узкий.ширина >= 330,
    '⚠️ ЭКРАН ТАБЛИЦЫ: на узком ширина идёт по вьюпорту, а не залипла на потолке (' +
    ширины.узкий.ширина + 'px при вьюпорте 393)');
  // ⚠️ Стиль попапа ОБЩИЙ с More — проверяем ИМЕННО общие классы, а не «похоже
  // выглядит»: копия стилей прошла бы ассерт и разъехалась при правке More.
  expect(/\biconBtn\b/.test(ширины.широкий.крестКлассы) &&
         /\bst-close\b/.test(ширины.широкий.крестКлассы),
    '⚠️ ЭКРАН ТАБЛИЦЫ: крестик несёт ТЕ ЖЕ классы, что у More Stars (' +
    ширины.широкий.крестКлассы + ')');
