...
`;
  ...
  // Читаем СБОРКУ, а не исходник: в игрока едет именно она.
  {
    ...
    expect(!!адрес && адрес.indexOf('workers.dev') < 0,
      ...
      'отказом (адрес «' + адрес + '»)');
    // ⚠️⚠️ ГЕЙТ `file://` — ЗАЩИТА БОЕВОЙ ТАБЛИЦЫ ОТ НАШИХ ЖЕ ПРОГОНОВ. Сьют и
    // соак открывают сборку именно так и доигрывают до победы: без гейта каждый
    // прогон пяти направлений заводил бы в таблице настоящую строку (гость
    // полноправен, CORS открыт, `localStorage.clear()` даёт новый id). Поймано
    // на себе в первые минуты после объявления константы — и заметить это можно
    // было только по чужому серверу, ни один ассерт не покраснел бы.
    expect(/file:/.test(выражение),
      '⚠️ ПОСТАВКА: адрес таблицы ЗАГЕЙЧЕН на file:// — прогоны сьюта и соака не ' +
      'пишут в боевую таблицу (выражение: ' + выражение + ')');
  }
