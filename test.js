...
`;
  ...
  const экран = await lbPage.evaluate(async () => {
    ...
    const вид = () => {
      ...
      const w = o.querySelector('.lb-wrap'), x = document.getElementById('lbClose');
      return { открыт:getComputedStyle(o).display !== 'none',
               ширина:w ? Math.round(w.getBoundingClientRect().width) : 0,
               // ⚠️ Стиль попапа ОБЩИЙ с More Stars — проверяем ИМЕННО общие
               // классы, а не «похоже выглядит»: копия стилей прошла бы ассерт
               // и разъехалась бы при первой правке More.
               крестКлассы:x ? x.className : '',
  ...
  // ⚠️⚠️ ПРАВИЛО ПЕРЕВЕРНУЛОСЬ (слово владельца 2026-08-10: «всегда показывай
  // 3 аватарки»). Прежний ассерт утверждал «на тесной их МЕНЬШЕ трёх» — то есть
  // ровно то поведение, которое владелец отменил; он переехал за правилом, а не
  // «починен». ⚠️ Вторая половина при этом НЕ ПОТЕРЯНА: соседние ассерты выше
  // держат «заголовок влезает и не наезжает», поэтому «три» нельзя получить
  // ценой обрезанного текста — а это и был смысл прежнего уступания.
  // ⚠️ Считаем ВСЕ слоты (`> *`), а не `img`: при пустой таблице слот — это
  // нейтральный кружок, и счёт по `img` дал бы ноль на исправной сборке.
  expect(узкий.видимыхАватаров === 3 && тесно.видимыхАватаров === 3,
    '⚠️ ТОЧКА ВХОДА: ТРИ аватара на ОБЕИХ ширинах — стопка, а не скрытие (390 → ' + узкий.видимыхАватаров +
    ', 320 → ' + тесно.видимыхАватаров + ')');
  // ⚠️⚠️ ВКЛАДКИ ОТМЕНЕНЫ ВЛАДЕЛЬЦЕМ («только наша таблица») — ассерт про
  // переключение УМЕР ВМЕСТЕ С МЕХАНИКОЙ, а не переписан.
  // ⚠️⚠️ ШИРИНУ МЕРИМ НА ДВУХ ВЬЮПОРТАХ, И ЭТО НЕ ПЕДАНТИЗМ: первая версия
  // требовала «>=520» на МОБИЛЬНОЙ странице сьюта (393px), где потолок 560
  // недостижим по построению — `min(560, 100vw − 32)` даёт 361. Ассерт
  // краснел на ИСПРАВНОЙ сборке, потому что я не СОЗДАЛ предпосылку «широкий
  // экран», а молча её предположил. Тот же класс, что «страж приводит
  // состояние сам».
  const ширины = await (async () => {
    const снять = async () => await lbPage.evaluate(async () => {
      const sleep = ms => new Promise(r => setTimeout(r, ms));
      document.getElementById('pauseBtn').click(); await sleep(300);
      document.getElementById('msLbEntry').click(); await sleep(400);
      const w = document.querySelector('#lbOverlay .lb-wrap');
      const из = w ? Math.round(w.getBoundingClientRect().width) : 0;
      document.getElementById('lbClose').click(); await sleep(150);
      { const p = document.querySelector('.ms-play'); if (p) p.click(); }
      await sleep(200);
      return из;
    });
    ...
    await lbPage.waitForTimeout(250);
    const широкий = await снять();
    await lbPage.setViewportSize({ width: 393, height: 852 });
    await lbPage.waitForTimeout(250);
    const узкий = await снять();
    return { широкий, узкий };
  })();
  console.log('экран/ширина:', JSON.stringify(ширины));
  // ⚠️ Потолок 560 — слово владельца. Проверяем И что он ДОСТИГНУТ на широком
  // (иначе ассерт зелен у экрана шириной 200), И что на узком ширина идёт по
  // экрану, а не залипла на 560 (иначе попап вылезал бы за край).
  expect(ширины.широкий === 560,
    '⚠️ ЭКРАН ТАБЛИЦЫ: на широком экране ширина РОВНО 560 — потолок владельца достигнут (' +
    ширины.широкий + 'px при вьюпорте 1280)');
  expect(ширины.узкий > 0 && ширины.узкий <= 361 && ширины.узкий >= 330,
    '⚠️ ЭКРАН ТАБЛИЦЫ: на узком экране ширина идёт по вьюпорту, а не залипла на потолке (' +
    ширины.узкий + 'px при вьюпорте 393)');
  expect(/\biconBtn\b/.test(экран.открыт.крестКлассы) && /\bst-close\b/.test(экран.открыт.крестКлассы),
    '⚠️ ЭКРАН ТАБЛИЦЫ: крестик несёт ТЕ ЖЕ классы, что у More Stars, — общий ' +
    'стиль попапов, а не копия (' + экран.открыт.крестКлассы + ')');
