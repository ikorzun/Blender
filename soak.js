...
// ⚠️⚠️ ОБА ПОРОГА ПЕРЕСТАВЛЕНЫ ПО РАСПРЕДЕЛЕНИЮ 2026-08-07 (5 сидов × 12 мин).
// ЛИНЕЙКА: headless Chromium на настоящем GPU (--use-angle=metal), БЕЗ
// CPU-троттлинга, Hard, idle 0.25, бот autoMatch + встряски; обёрнута ПОЛНАЯ
// сессия с уровня 1. Разлёт чаши исключён, выступ считается ТОЧНЫМ охватом и
// только там, где стена есть.
// ⛔ СО СТАРЫМИ СОАК-ЧИСЛАМИ НОВЫЕ НЕ СРАВНИВАТЬ: сменилась ЛИНЕЙКА, а не
// поведение — диагностика переведена с оценки сверху на точный охват (у банана
// запас доходил до 0.36 при пороге 0.20), плюс прогрессия размера уровня и
// фикс lastAction. Проект на этом обжигался дважды; пишу заранее.
//
// ПОДЪЁМЫ ПОЛА — порог остаётся СКОРОСТЬЮ (он должен ехать за длительностью
// прогона), но частота измерена: здоровые прогоны дали 3, 16, 17, 18, 21 за
// 12 мин, то есть максимум 1.75/мин; ДЕФЕКТНЫЕ варианты порога просадки —
// 36 (3.0/мин) и 115 (9.6/мин). Между 1.75 и 3.0 ПУСТОЙ КОРИДОР, туда и
// ставим 2.5: ловит оба известных дефекта, молчит на всех ШЕСТИ здоровых
// (пять, по которым порог считался, плюс проверочный 606 — см. ниже).
// ⚠️ ПРОВЕРЕНО НА ШЕСТОМ, НЕ УЧАСТВОВАВШЕМ В ВЫБОРЕ СИДЕ (606): 23 подъёма,
// тревог ноль. Здоровый диапазон от этого расширился (3..23, то есть 1.9/мин) —
// именно поэтому порог и проверяют на постороннем прогоне, а не только на тех,
// по которым его считали.
// Прежняя единица в минуту срабатывала на ЧЕТЫРЁХ здоровых из пяти.
const FLOOR_LIFT_PER_MIN = 2.5;
// WALLEXCESS: 616 сэмплов, p50 −0.151, p90 −0.087, p99 0.033, максимум 0.382.
...
const WALL_EXCESS_NORM = 0.45;
  ...
  if (floorLifts > MINUTES * FLOOR_LIFT_PER_MIN)
    problems.push(`FLOOR LIFT STORM: ${floorLifts} подъёмов за ${MINUTES} мин `
      + `(норма <= ${Math.round(MINUTES * FLOOR_LIFT_PER_MIN)}, то есть ${FLOOR_LIFT_PER_MIN}/мин)`);
