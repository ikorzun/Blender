...
## ⚠️ Механика «как в Форбс» может не заработать вовсе — и это решает сервер

Твоя модель держится на том, что **место может падать**: потратил очки —
опустился. Но хранит ли таблица площадки **лучший** результат или **последний**,
из кода не видно — это поведение их сервера.

| Как хранит сервер | Что будет с твоей механикой |
|---|---|
| последний присланный | работает как задумано: потратил — упал |
| лучший за всё время | **не работает вообще**: падения не будет никогда |

⚠️ То есть от ответа сервера зависит не код, а **смысл фичи**. Это первое, что
проверяем на живом прогоне с токеном — до всякой вёрстки.

⚠️ И зеркальная половина находки про ответ: пустой ответ сервера приезжает как
**ошибка**, хотя запись прошла. Вместе с уже известным «ошибка приезжает как
успех» это значит, что по самому факту успеха или ошибки **нельзя судить ни в
какую сторону** — разбирать нужно только тело ответа.
