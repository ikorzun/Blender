...
| **The portal package is not built automatically** | The real package is assembled by hand from four files (index.html + both Bridge files + music.mp3), and this is a typical place where the config gets forgotten. ⛔ The clause about an archive in `release/` is struck 2026-09-01-p: the folder was deleted on the owner's word, and the archive it named had not existed for some time — the risk itself is unchanged and is why this row stays | hours |
