...
|---|---|---|
| 1 | **Музыка** (ты помнишь): новый трек + лицензия. Экспортируй сразу в **m4a, 96–112 кбит/с** — иначе переделывать. Файл просто заменяет `music.mp3` рядом с игрой | до среды 5.08 |
...
- **Subscribe $1.99 → «без рекламы навсегда» за $4.90** — решено тобой, кнопку
  переделает Интерфейс. Подтверди только, заводить ли товар в кабинете сейчас
  или после запуска.
- **28 новых типов** (влиты сегодня) открываются с 86-го уровня — та же болезнь,
  что была у стейка. Перемешать ближе к началу? Одна правка, меняет прогрессию.
- **Баннерная реклама** — спрашивал дважды, ответа нет. Без твоего слова не завожу.
...
**Интерфейс:**
- Вернуть вход в Музей (сейчас целый экран коллекции физически недоступен — кнопка
  лежит в мёртвом слое).
- Показать рейтинг 1★–3★ на экране победы (считается, но спрятан — у уровня нет
  видимой цели).
- Разморозить чип баланса (с 15-го уровня цифра перестаёт отвечать на действия).
