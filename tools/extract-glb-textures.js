// Extract PNGs from the GLB and determine their nature:
// a palette/trim ramp (can be baked into vertex colors) or a detail map (cannot).
...
console.log('PNG extracted:', manifest.length);
for (const m of manifest) console.log(`  ${m.glb.padEnd(22)} ${m.file.padEnd(46)} ${m.w}×${m.h}  ${m.kb} KB`);
...
console.log('\nIDENTICAL textures in different models:', dups.length ? '' : 'none');
