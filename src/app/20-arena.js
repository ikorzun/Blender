...
// ОТСТУП СНИЗУ: ПИКСЕЛИ ВЛАДЕЛЬЦА → ЮНИТЫ МИРА.
// ⚠️⚠️ ПЕРЕСЧЁТ ИДЁТ ПО ПЕРЕДНЕЙ ГРАНИ ЯЩИКА, А НЕ ПО ЗАДНЕЙ, И ЭТО НЕ
// ПРИДИРКА — ПЕРВАЯ ВЕРСИЯ ОШИБАЛАСЬ ВЧЕТВЕРО. `bonusVisHalfH` намеренно
// считает половину кадра на ЗАДНЕЙ грани (её и надо закрыть шириной), а
// предметы, по которым игрок целится пальцем, стоят у ПЕРЕДНЕЙ — ближе к
// камере, то есть КРУПНЕЕ. Пересчёт по задней давал «расчётные 100 px» при
// фактических 12-56, а на десктопе дно вовсе уезжало ЗА кромку (−59).
// Поймано проекцией дна на экран, а не чтением формулы: число выглядело
// совершенно нормальным (канон, «метрика меряет не то, что названо вслух»).
// ⚠️ Слагаемое `BONUS_D * tg` — это и есть разница масштабов двух граней:
// без него отступ съедала сама глубина ящика.
function bonusBottomPad(){
  const tg = Math.tan(camera.fov * Math.PI / 360);
  const разницаГраней = BONUS_D * tg;
  const h = (typeof renderer !== 'undefined' && renderer.domElement &&
             renderer.domElement.clientHeight) || (typeof window !== 'undefined' && window.innerHeight) || 0;
  if (!(h > 0)) return разницаГраней + 0.5;
  // ⚠️ ПОТОЛОК В ДОЛЕ ВЫСОТЫ КАДРА: сто пикселей на низком экране это заметная
  // часть столба, ради которого уровень и сделан. Доля, а не юниты, — по той же
  // причине, что и сам отступ в пикселях.
  const px = Math.min(BONUS_BOTTOM_PAD_PX, h * BONUS_BOTTOM_PAD_MAX);
  const передняя = Math.max(0.5, bonusCamR() - BONUS_D / 2);
  return разницаГраней + px * (2 * передняя * tg / h);
}
function bonusCamTY(){ return BONUS_FLOOR - bonusBottomPad() + bonusVisHalfH(); }
