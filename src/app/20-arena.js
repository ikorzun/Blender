...
function radiusAt(y){
  // ⚠️⚠️ БОНУСНЫЙ УРОВЕНЬ — ЦИЛИНДР ВО ВЕСЬ ЭКРАН, А НЕ КОНУС ЧАШИ (спека
  // владельца «вместо чаши всё пространство окна заполняется объектами»).
  // Правка сделана ЗДЕСЬ И ТОЛЬКО ЗДЕСЬ намеренно: `radiusAt` — единственная
  // точка, из которой ширину читают ВСЕ (стены Rapier, спавн, спасатель,
  // динамический радиус совпадения). Развести их значило бы гарантировать
  // расхождение: спасатель считал бы вылетом то, что стена держит.
  if (typeof levelNum !== 'undefined' && isBonusLevel(levelNum)) return BONUS_R;
