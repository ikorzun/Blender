...
// ⚠️⚠️ THE OVERRIDES ARE HONOURED ON A LOCAL HOST ONLY (2026-09-06-e). On a real host a crafted link
// `?lb=https://evil` would have made a player who has not registered yet send his id, his score AND
// his signing key to a foreign address on the first win — and the key is what owns the row. The sign
// of locality is the same pure function the send-gate's table of hosts is proven on (`lbHostIsLocal`,
// 00-config): `file:`, localhost, 127/8, `.local`, RFC1918 — every path the suite, the stands and the
// owner's own `localhost:8781/?lb=` link take. Off a local host the constant wins, whatever the URL
// or localStorage say.
...
// ⚠️ THE SIGNING KEY IS ACCEPTED BY THE SERVER ONLY WHEN THE ROW IS CREATED (trust-on-first-use);
// against an EXISTING row it is not even read (`postScore` looks at `body.k` only when there is no
// row) — the signature is checked with the key the server stored. So the key must survive a game
// restart: losing it, the player also loses the ability to update THEIR OWN row.
// ⚠️⚠️ SINCE 2026-09-06-e THE KEY LIVES IN THE SAVE (`Save.lk`, 77-save) AND TRAVELS WITH `Save.gid`.
// Before that it sat in localStorage alone while the id went to the cloud: on a second device the
// merge brought the OLD id, a NEW key was generated for it, the server answered 401 to every
// submission and the row froze forever, silently — the August review's #3 «door A». Whichever gid
// wins a merge, its key wins with it (`pickLk` in 77-save). The old localStorage key is adopted into
// the save once, as the key of the current gid; if that guess is wrong the server says so (401 `sig`)
// and the key is dropped so that the next cloud sync can bring the right one — see lbKeyReject.
// ⚠️ The fix is NOT retroactive: a row frozen before it heals only when the device that owns the
// key runs this build once and its copy reaches the cloud.
const LB_KEY_LS = 'mixer_lb_key';
const lbKeyValid = (k) => typeof k === 'string' && /^[0-9a-f]{64}$/.test(k);

function lbKey() {
  try {
    const saved = (typeof signingKey === 'function') ? signingKey() : '';
    if (lbKeyValid(saved)) return saved;
    let k = localStorage.getItem(LB_KEY_LS);          // the legacy home — adopted once
    if (!lbKeyValid(k)) {
      const b = new Uint8Array(32);
      crypto.getRandomValues(b);
      k = [...b].map((x) => x.toString(16).padStart(2, '0')).join('');
    }
    if (typeof setSigningKey === 'function') setSigningKey(k);
    try { localStorage.setItem(LB_KEY_LS, k); } catch (e) {}   // a copy for a build rolled back
    return k;
  } catch (e) { return ''; }
}
// The server refused our signature for this id (401 `sig`): the key is not the row's. Dropping it
// is the ONLY way out — a fresh key registers a row that does not exist yet, and a row that does
// exist gets its real key from the other device's copy at the next cloud sync. Keeping a wrong key
// means the freeze this batch exists to end. The «registered» mark goes with it: it was earned by
// that key.
function lbKeyReject() {
  try { if (typeof setSigningKey === 'function') setSigningKey(''); } catch (e) {}
  try { localStorage.removeItem(LB_KEY_LS); } catch (e) {}
  lbClearRegistered();
  lbSentScore = null;   // what sits on the server under another key is not our memory to keep
}

// Whether the row has already been created (the key is sent in EXACTLY the first submission).
// ⚠️ THE MARK NAMES THE ID IT WAS EARNED FOR (2026-09-06-e): after a merge switches `Save.gid` to an
// older id, «registered» is not true of the NEW id — the row the other device created wants its own
// key, sent once, and this is how `k` gets attached again. The legacy value `1` is read as «this
// id»: one device, one id, that is what it meant when it was written.
...
function lbRegistered(id) {
  try { const v = localStorage.getItem(LB_REG_LS); return v === '1' || (!!id && v === id); } catch (e) { return false; }
}
function lbMarkRegistered(id) { try { localStorage.setItem(LB_REG_LS, id || '1'); } catch (e) {} }
...
async function lbMe() {
  ...
  if (r.state === 'refused' && r.err === 'none') {
    out = { state: 'ok', me: null, rank: null, exact: false, up: [], dn: [], at: 0 };
  } else if (r.state === 'refused' && r.err === 'sig') {
    lbKeyReject();   // our key is not this row's — see lbKeyReject; the screen sees a refusal
    out = { state: r.state, err: r.err };
    ...
    // ⚠️ `at` — THE TIME OF THE LADDER SNAPSHOT BEHIND THE PLACE (the server's `t`, 2026-09-06-e),
    // 0 when there is none or the worker predates the field. Above the hundredth place the rank is
    // exact against the table AS IT STOOD AT `at` — a screen may say «as of»; it is not an error.
...
let lbSentGid = (function () { try { return localStorage.getItem(LB_SENT_GID_LS) || ''; } catch (e) { return ''; } })();
let lbNokeyRetried = false;   // one immediate retry with the key per `nokey` refusal — see lbSubmit
...
async function lbSubmit() {
  ...
  // ⚠️ THE MEMORY OF THE LAST SUBMISSION BELONGS TO THE ID IT WAS MADE FOR (2026-09-06-e): after a
  // merge switched `Save.gid`, that number is another row's and must not suppress the first
  // submission for this one. A legacy memory without an id is this id's — one device, one id.
  if (lbSentGid && lbSentGid !== id) { lbSentScore = null; lbSentGid = id; }
  ...
  try {
    ...
    // The key leaves in EXACTLY the first submission for THIS id (row creation); against an
    // existing row the server does not read it at all, but a key on the wire is a secret on the
    // wire — it goes out only when it has to. `lbRegistered(id)`: the mark names the id.
    if (!lbRegistered(id)) payload.k = lbKey();
    ...
    // ⚠️ `nokey` — THE SERVER HAS NO ROW FOR THIS ID while our mark said «registered»: the row was
    // deleted by the 180-day retention (a player back after half a year), or the mark is another
    // id's. The August review's #3 «door B»: without this branch every submission was refused for
    // ever, silently. The mark is dropped, the memory of the last send with it (there is no row to
    // remember), and ONE immediate retry goes out carrying the key — the row is created anew.
    if (r.state === 'refused' && r.err === 'nokey') {
      lbClearRegistered();
      lbSentScore = null;
      if (!lbNokeyRetried) { lbNokeyRetried = true; lbSchedule(0); return { state: 'deferred', retryIn: 0, nokey: true }; }
      return { state: r.state, err: r.err };
    }
    // ⚠️ `sig` — THE KEY IS NOT THIS ROW'S: it is dropped (see lbKeyReject), and the next cloud sync
    // may bring the right one. Nothing else can be done with a wrong key — keeping it is the freeze.
    if (r.state === 'refused' && r.err === 'sig') { lbKeyReject(); return { state: r.state, err: r.err }; }
    if (r.state !== 'ok') return { state: r.state };
    if (r.body.ok) { lbMarkRegistered(id); lbNokeyRetried = false; }
    lbSentScore = s; lbSentGid = id;
    try { localStorage.setItem(LB_SENT_LS, String(s)); localStorage.setItem(LB_SENT_GID_LS, id); } catch (e) {}
    lbInvalidate();
