...
function syncMeshes(){
  for (const it of items){
    ...
    // ⚡ THE RIVAL'S ROTATION IS NOT THE BODY'S — IT FACES THE PLAYER (the glass bubble, 2026-09-24-g):
    // `tickRivalFace` (40-items) gives it the camera's orientation every frame, right before the render.
    // ONE WRITER: writing the body's rotation here as well would fight it on every awake frame. Its
    // collider is a SPHERE, so the render rotation carries no physics — the tap still lands on the collider.
    // ⛔ THE DARUMA THAT STOOD HERE IS CANCELLED WITH THE STICKER BALL (the owner's «like a Japanese daruma»,
    // 2026-09-10: the yaw integrated from the body's own angvel.y about a fixed up axis, `RIVAL_UP_AXIS`,
    // `it.rvYaw`, gated on `RIVAL_UPRIGHT`). A picture that always faces the camera has no up to keep.
    if (it.rival) continue;
