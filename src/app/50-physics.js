...
// ⚠️⚠️ 55, А НЕ 80 (диспетчер, приёмка #51, 2026-08-13). Физика выбрала 80 по
// перфу ВНУТРИ интро (шаг 11.3 / 9.6 / 8.5 при 50 / 65 / 80) — и это верно для
// того окна. Но при 80 очередь длится 23 слоя × 80 = 1.84 с, а окно подачи
// (drop 0.8 + облёт 1.0) — 1.8 с: остаток вываливался `waveReleaseAll` уже
// ПОСЛЕ интро, и время до полного штиля росло 5.5 -> 6.0-6.6 с. При 55 очередь
// укладывается в 1.27 с и целиком помещается в интро с запасом.
// ⛔ НЕ ставить обратно 80 без пересчёта ЭТОЙ арифметики: число слоёв = живых/8
// и растёт с уровнем, окно подачи фиксировано.
let WAVE_MS = 55;                      // ручка --waveMs только для A/B
