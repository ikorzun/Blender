...
const WALL_GAP = 0.12;
const WALL_SEG = 32;
// ПОЛУТОЛЩИНА ПЛИТЫ ПОЛА. Верх плиты стоит РОВНО на FLOOR_REST при любом
// значении — вниз она уходит в пустоту под лопастями, где нет ни тел, ни
// других фикс-коллайдеров (лопасти — чистая графика, фикс с фиксом в Rapier
// не взаимодействует), поэтому толщина не стоит ничего.
// ⚠️ Было 0.3: плита [0.55..1.15] и НИЧЕГО под ней. Продавленный ГЛУБЖЕ
// СЕРЕДИНЫ предмет получил бы от узкой фазы нормаль ВНИЗ (снизу выходить
// ближе) и был бы вытолкнут в пустоту — на лопасти, навсегда.
// ⚠️ ЭТО СТРАХОВКА, А НЕ НАЙДЕННЫЙ КОРЕНЬ (корень — у FLOOR_PEN_MAX), но
// страховка НЕ умозрительная: замеренный максимум просадки 0.28 при
// полутолщине 0.30 — до переворота нормали оставалось 7%. Сквозного прохода
// в стрессе (56 циклов) не случилось ни разу, а владелец «до лопастей» видел;
// эти 7% и объясняют, почему у него случилось, а у меня нет.
// Замер перфа: шаг физики на взрыве p95 7.9-10.9 против 7.7-9.1 у тонкой —
// в шуме (бюджет 25).
const FLOOR_HALF = 2.4;
...
// Жертвы — почти всегда плоские модели (стейк 13 случаев из 17, посох и
// пряничный человек — остальные), и взрыв для этого НЕ НУЖЕН: худший
...
function initPhysicsWorld(){
  ...
  const floorB = world.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(0, FLOOR_REST - FLOOR_HALF, 0));
  floorCol = world.createCollider(RAPIER.ColliderDesc.cylinder(FLOOR_HALF, radiusAt(FLOOR_REST) + 0.2).setFriction(FRICTION), floorB);
}
...
// вниз 0.07. ⚠️ min(r, obb) — по той же причине, что в radialReach.
...
function rescueSweep(beforeSleep){
  ...
  for (const it of items){
    ...
    // интегратор, и утонувший стейк висел под кучей до конца уровня.
    ...
    // не «спокоен», гейт его отсекал, и стейк просидел в плите 30 с подряд на
