...
function tintChrome(){
  ...
  try {
    ...
    // ⚠️⚠️ В МЕТУ — НИЖНИЙ СТОП, А НЕ ВЕРХНИЙ. По записи с лендинга About Us
    // `theme-color` тонирует ТОЛЬКО НИЖНИЙ бар iOS, а верхнюю полосу красит
    // фон `html`. У нас оба канала были направлены в ОДИН цвет (верхний стоп) —
    // отсюда тёмно-синяя полоса под малиновым низом кадра (замер 2026-08-10:
    // отдавали rgb(3,29,131) при низе кадра rgb(215,38,185)).
    // ⚠️ Это ЛУЧШЕ полноцветных полос: ничего нового не рисуем, значит шва нет
    // ПО ПОСТРОЕНИЮ (двухстоповое приближение полос давало шов 21 при контроле 2),
    // и не нужно накрывать кромку элементом — то есть просьба владельца
    // «под островом подложки быть не должно» остаётся выполнимой.
    const hex = (t)=>{ const p = String(t).split(',').map(n => Math.max(0, Math.min(255, parseInt(n, 10) | 0)));
      return p.length === 3 ? '#' + p.map(n => n.toString(16).padStart(2, '0')).join('') : null; };
    chromeMeta(hex(b) || col);
  } catch(e){}
}
tintChrome();
