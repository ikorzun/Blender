...
const CHROME_SWAP = false;
function tintChrome(){
  try {
    // ⚠️⚠️ ФОН ДОХОДИТ ДО ОБЕИХ КРОМОК (слово владельца 2026-08-10: «если у нас
    // под островом ничего нет, то фоновый градиент должен заполнять всё от
    // самого верха до самого низа»). ОТМЕНЯЕТ нейтраль по теме, прожившую день.
    // ⚠️ КАНАЛОВ ДВА, И ОНИ РАБОТАЮТ — доказано снимком владельца: полоса в
    // меню сменилась после правки ТОЛЬКО меты. Что именно красит верх, а что
    // низ, мы так и не выяснили: игровой экран после той правки не снимали.
    // ⚠️⚠️ ПОЭТОМУ ОБМЕН КАНАЛОВ — ОДНА СТРОКА. Окажется наоборот (мета красит
    // ВЕРХ) — переставить `CHROME_SWAP`, и всё; переписывать нечего.
    const cs = getComputedStyle(document.documentElement);
    const верх = cs.getPropertyValue('--sky-top-rgb').trim();
    const низ  = cs.getPropertyValue('--sky-bot-rgb').trim();
    const hex = (t)=>{ const q = String(t).split(',').map(n => Math.max(0, Math.min(255, parseInt(n, 10) | 0)));
      return q.length === 3 ? '#' + q.map(n => n.toString(16).padStart(2, '0')).join('') : null; };
    const фонRGB  = CHROME_SWAP ? низ : верх;
    const метаRGB = CHROME_SWAP ? верх : низ;
    const d = document.documentElement;
    if (фонRGB){ const c = 'rgb(' + фонRGB + ')';
      d.style.backgroundColor = c; document.body.style.backgroundColor = c; }
    d.style.backgroundImage = '';
    chromeMeta(hex(метаRGB) || skyChromeCSS);
  } catch(e){}
}
tintChrome();
