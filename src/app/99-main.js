...
// ⛔⛔ «DECIDE ONCE PER SESSION» WAS THE HOLE, REMOVED 2026-08-30 ON THE OWNER'S REAL DATA
// («na medlennykh telefonakh ili pri rezhime ekonomii batarei na 17 iphone igra nachinaet
...
// (a menu frame is cheap and would read as «fast»), and the cost of watching forever is one
// array push per frame plus a ~100-element sort every 2.5 s — nothing.
// ⚠️ `perfDecided` KEEPS ITS NAME AND ITS TRUE MEANING — «the tier question is settled» — and
// is now set ONLY when it really is: on going low. The debug forcePerfLow path still works.
function tickPerfTier(ms){
  ...
  if (med > PERF_SLOW_FRAME_MS){
    perfDecided = true;                     // settled: we are low, nothing further to decide
    applyPerfTier('low');
    ...
    perfWinStart = 0;                       // green verdict -> RE-ARM: keep watching
  }
  perfWin = [];
}
...
window.__game = {
  ...
  // ⚠️ TEST HOOKS for the perf window (2026-08-30). The window machinery had ZERO guards, which
  // is how «decide once per session» shipped and sat unnoticed until the owner measured the lag
  // on a real phone. tickPerfTier is the real function (fed with synthetic frame times);
  // perfTierReset exists ONLY so the harness can undo the one-way tier — going low mid-suite
  // changes fxScale and would repaint every later particle guard. Production has no reset path
  // and must not grow one: one tier, only downwards, is the design.
  tickPerfTier,
  perfTierReset(){ perfDecided = false; perfWin = []; perfWinStart = 0;
    CFG.perfTier = 'high'; CFG.fxScale = 1;
    try { renderer.setPixelRatio(Math.min(devicePixelRatio || 1, 2)); resize(); } catch(e){}
  },
  bestTapTarget(mode){
    refreshAccessibility();
