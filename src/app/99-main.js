...
function tintChrome(){
  try {
    // ⚠️⚠️ ПОЛОСЫ — ЧИСТАЯ НЕЙТРАЛЬ ПО ТЕМЕ (слово владельца 2026-08-10: «на
    // игровом тоже сделай нейтральной»). ОТМЕНЯЕТ подбор под цвет неба целиком.
    // ⚠️ ОБА КАНАЛА НЕСУТ ОДНО ЗНАЧЕНИЕ намеренно: мы так и не знаем наверняка,
    // какой из них Safari читает в каком случае, — а при одинаковой нейтрали
    // ответ перестаёт иметь значение. Это и была цель правки, а не экономия.
    // ⚠️ ТЕМА — ЧЕРЕЗ `isNightSky()`, А НЕ ЧЕРЕЗ КЛАСС `html.night`: класс
    // ставит `updateHUD`, а `tintChrome` зовётся РАНЬШЕ (99-main:306 против
    // первого тика HUD) — по классу ночью вышел бы БЕЛЫЙ. Ровно тот случай,
    // где копия признака расходится с источником.
    // ⛔ КРАСНАЯ УГРОЗА ПОМОЛА ПОЛОС БОЛЬШЕ НЕ КАСАЕТСЯ — нейтраль от палитры
    // не зависит вовсе. Прежняя оговорка про мигание снята; не «чинить» связь,
    // которой нет.
    const тёмная = (typeof isNightSky === 'function') ? isNightSky() : false;
    const col = тёмная ? '#000000' : '#ffffff';
    const d = document.documentElement;
    d.style.backgroundColor = col;
    document.body.style.backgroundColor = col;
    // ⚠️ ГРАДИЕНТ НЕБА НА ПОДЛОЖКЕ СНЯТ вместе с подбором под палитру: он
    // существовал ради попадания в тон кромки, а нейтраль в этом не нуждается.
    d.style.backgroundImage = '';
    // ОБА КАНАЛА — ОДНО ЗНАЧЕНИЕ (см. верх функции).
    chromeMeta(col);
  } catch(e){}
}
tintChrome();
