...
window.__game = {
  ...
  bonusFrame(){
    ...
    // самый нижний ЖИВОЙ предмет — его НИЗ на экране
    let низПред = -1;
    for (const it of items){
      if (!it.alive || !it.mesh) continue;
      const c = it.mesh.position, r = it.r || 0.6;
      const q = new THREE.Vector3(c.x, c.y - r, c.z).project(camera);
    ...
    return { высотаPx: hpx, отступЮнитов: +pad.toFixed(3),
      ...
      нижнийПредметОтНизаPx: низПред < 0 ? -1 : Math.round(hpx - низПред) };
  },
