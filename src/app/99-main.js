...
// ⛔⛔ `CHROME_SWAP` УДАЛЁН ВМЕСТЕ С ПОДБОРОМ ПОД НЕБО (решение владельца
// 2026-08-12): полосы Safari берут ТЕМУ УСТРОЙСТВА, а вью отделён от них
// скруглением 40px. Ручка распределения каналов больше не нужна — каналов в
// нашем распоряжении не осталось вовсе, оба отданы системе.
// ⚠️ ЭТО ТРЕТЬЯ РЕДАКЦИЯ ОДНОГО РЕШЕНИЯ ЗА ТРОЕ СУТОК: нейтраль по теме →
// подбор под палитру (e962931) → тема устройства. История в CLAUDE.md; здесь
// важно одно — НЕ ВОЗВРАЩАТЬ покраску html/body и меты из игры.
function tintChrome(){
  // ⚠️ ФУНКЦИЯ ЖИВА НАМЕРЕННО И НАМЕРЕННО ПУСТА. Её зовут три места (старт,
  // смена времени суток, закрытие меню), и снести её значило бы править их все
  // ради нулевого эффекта. Пустое тело — надгробие: следующий увидит, что
  // покраска СНЯТА решением, а не потеряна при правке.
  // ⛔ Фон документа теперь задаёт CSS (`html` + `prefers-color-scheme`), мету —
  // две статические `<meta media=...>`. Оба переключаются САМИ при смене темы
  // устройства, в том числе с открытой игрой; JS-путь этого не умел.
}
tintChrome();
