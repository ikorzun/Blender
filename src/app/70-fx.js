...
function igniteItem(it, ms){
  ...
  fireSilhouetteFX(it);
  heatShellFX(it);                       // раскалённая корка ПОД пламенем
  burningItem = it;
...
// ===== РАСКАЛЁННАЯ КОРКА НА ГОРЯЩЕМ (слово владельца 2026-08-19) =====
// Перенос вида «heatmap» из thrine.app. ⚠️⚠️ ТАМ ЭТО ЭФФЕКТ ПО КАРТИНКЕ: модель
// рендерят в закадровую цель, размывают кромку и красят ПОЛНОЭКРАННЫМ шейдером,
// а сам меш из кадра исключают (`replacesGeometry`). Нам такой конвейер не
// годится: у них в кадре ОДНА модель, у нас 130-180 предметов, и обработка по
// картинке накрывает кадр целиком — поштучно это цель рендера на предмет, чего
// мобильный бюджет не выдержит (мы за то же вырезали `transmission`, он съедал
// 55% кадра).
// ⚠️ ПЕРЕНОСИТСЯ НЕ КОНВЕЙЕР, А ВЕЛИЧИНА. У них «тепло» — расстояние до кромки
// силуэта; в 3D ровно это даёт `dot(нормаль, взгляд)`: фронт горячий, скользящий
// край холодный. Изолинии, свечение, зерно и бегущая полоса считаются от неё же,
// внутри материала накладки — без единого лишнего прохода.
// ⚠️⚠️ ЭТО ВТОРАЯ НАКЛАДКА-РЕБЁНОК, А НЕ ПРАВКА МАТЕРИАЛА ПРЕДМЕТА. Канон
// запрещает трогать материал: портреты коллекции рендерятся ТЕМ ЖЕ классом
// материала, и «горячее» протекло бы в музей.
// ⚠️ Палитра по умолчанию — ОБЩАЯ С ПЛАМЕНЕМ (FIRE_*): корка обязана читаться
// как «этот предмет раскалился», а не как отдельный эффект. Синий вариант из
...
function _heatShellFX_impl(item, набор, порядок){
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    vertexShader: [
      ...
      'void main(){',
      // корка сидит на предмете, а не раздувается: 0.012 только чтобы не спорить
      // с его же поверхностью по глубине
      '  vN = normalize(normalMatrix*normal); vP = position;',
    ...
    fragmentShader: [
      ...
      'varying vec3 vN; varying vec3 vV; varying vec3 vP;',
      'void main(){',
      // ЭТО И ЕСТЬ ИХ «расстояние до кромки»: 1 в центре, 0 на скользящем крае
      '  float ndv = clamp(dot(normalize(vN), normalize(-vV)), 0.0, 1.0);',
      '  float heat = pow(ndv, 1.0/max(0.15, uInner));',
      // бегущие тёмные пятна — аналог их shadowShape, по объектной координате,
      // чтобы полоса ехала по предмету, а не по экрану
      '  float band = sin(vP.y*uScale*6.0 - t*uSpeed*4.0 + sin(vP.x*uScale*4.0)*1.5);',
      '  heat *= 0.78 + 0.22*band;',
      // контурные изолинии той же величины
      ...
      // холодные края ПРОЗРАЧНЫ — силуэт и цвет предмета остаются читаемыми,
      // раскаляется середина
