...
// ⚠️⚠️ ДЕРЖАТЬ РАВНЫМ MATCAP_LIGHT (10-stage). Осколки предметов и черепки
// чаши несут объём ЗАПЕЧЁННЫМ тинтом по этому направлению, а сами предметы —
// matcap по MATCAP_LIGHT. Разъедутся — куча будет освещена одним светом, а её
// обломки другим, и в кадре, где они рядом, это видно сразу.
// ⚙️ Lx 0 с 2026-08-03: владелец выставил свет по центру в ?matcap=1 (панель
// правит только MATCAP_LIGHT, про эту константу она не знает — синхронизирую
// руками, иначе правка владельца тихо развела бы две половины освещения).
const SHARD_LIGHT = new THREE.Vector3(0, 0.60, 0.72).normalize();
