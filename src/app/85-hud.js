...
async function lbLoadOurs(my){
  ...
  if (m && m.state === 'ok' && m.exact && m.rank > 0){
    ...
    // ⚠️ Найдя себя в снимке, оставляем ЕГО позицию: тогда весь список
    // нумерован из ОДНОГО источника и внутренне честен, а живое точное место
    // игрок и так видит в точке входа меню.
    ...
    // с чужим возможно (пул имён конечен), поэтому ниже стоит ещё и защита от
    // дубля — она снимает последствие, даже если опознание промахнулось.
    let i = -1;
    for (let k = 0; k < строки.length; k++){
      if (строки[k].name === своя.name && (строки[k].av | 0) === (своя.av | 0)){ i = k; break; }
    }
    if (i >= 0){ своя.pos = строки[i].pos; строки[i] = своя; }
    else {
      const j = m.rank - 1;
      if (j >= 0 && j < строки.length) строки[j] = своя; else строки.push(своя);
    }
    // ⚠️ ЗАЩИТА ОТ ДУБЛЯ — ПОСЛЕ замещения, а не до: удаление до сдвинуло бы
    // массив, и замещение по индексу перезаписало бы уже другого игрока.
    for (let k = строки.length - 1; k >= 0; k--){
      if (строки[k] !== своя && строки[k].name === своя.name &&
          (строки[k].av | 0) === (своя.av | 0)) строки.splice(k, 1);
    }
  }
  lbRowsRender(строки);
}
...
function newObjShow(key, done){
  ...
  const ЧЁРНЫЙ_L = 0.40;   // светлота Rec.709 по сырым каналам sRGB
  const ЧЁРНЫЙ_S = 0.14;   // насыщенность как (max−min)/255
  let rgb = '203,255,104';
  if (typeof тон === 'number'){
    const r = (тон >> 16) & 255, g = (тон >> 8) & 255, b = тон & 255;
    const св = (0.2126 * r + 0.7152 * g + 0.0722 * b) / 255;
    const нас = (Math.max(r, g, b) - Math.min(r, g, b)) / 255;
