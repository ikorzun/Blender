...
function tickVitrine(now){
  ...
  // выходила первой строкой — и панель, собранная на ПРЕДЫДУЩЕМ (обычном)
  // уровне, оставалась висеть на витрине во всей красе: три строки, opacity 1
  // (замер владельца глазами, мой зонд подтвердил бит-в-бит). Классическая
  // ошибка гейта: он закрывает БУДУЩЕЕ действие, а состояние уже создано.
  ...
  if (!vitrineOn()){
    ...
    // ТОЛЬКО если та была построена (`if (vitSlots || vitLevelRef)`) — и на
    // ХОЛОДНОМ СТАРТЕ ПРЯМО НА ВИТРИНЕ (сохранён уровень, кратный десяти) её
    // никто не строил, значит и не гасил: на десктопе оставалась пустая
    // карточка 24×24 с фоном `rgba(255,255,255,.16)`. Замер поймал это на
    // ЖИВОЙ ссылке, уже после заливки.
    // ⚠️ Тот же класс, что был у самого гейта: условие закрывало ПУТЬ, а не
    // утверждало СОСТОЯНИЕ. Состояние «панели нет» обязано выполняться при любом
    // способе попасть на витрину, включая тот, которым игрок попадает после
    // перезапуска игры.
