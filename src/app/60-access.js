...
// ═══ ДОСТУПНОСТЬ НА ВИТРИНЕ — ПЕРЕКРЫТИЕ СПЕРЕДИ, А НЕ ЛУЧИ К НЕБУ ═══
// ⚠️⚠️ ВЕЕР «К НЕБУ» НА ВИТРИНЕ БЕССМЫСЛЕН, И ЭТО НЕ ВКУСОВЩИНА. Столб вещей
// стоит стеной в 12 рядов: небо видит только ВЕРХНИЙ ряд, всё остальное ушло бы
// под вуаль — владелец играет на Hard и увидел бы серую витрину.
// ⛔ И ОЧЕВИДНАЯ ЗАМЕНА «пустить веер вперёд, к камере» САМА СЕБЯ УБИВАЕТ:
// у ящика есть СВОЯ ПЕРЕДНЯЯ СТЕНА, луч упирается в неё, и недоступным снова
// становится всё. Поэтому здесь не луч, а ГЕОМЕТРИЯ: предмет доступен, если его
// не загораживает сосед, стоящий БЛИЖЕ К ПЕРЕДНЕЙ ГРАНИ и накрывающий его в
// плоскости экрана. Ровно то, что видит игрок.
// ⚠️ ПРИЗНАК ОСТАЁТСЯ КАМЕРОНЕЗАВИСИМЫМ (канон, три бага владельца на этом):
// «перёд» — это ВНЕШНЯЯ НОРМАЛЬ ВИТРИНЫ (+Z), свойство контейнера, а не
// текущего ракурса. Камера на бонусе и так заперта, но правило важнее случая.
const BONUS_OCC_K = 0.62;   // доля суммы радиусов: насколько плотно надо накрыть
function bonusAccessible(item){
  const n = items.length;
  const rx = item.p.x, ry = item.p.y, rz = item.p.z, rr = item.r;
  for (let i = 0; i < n; i++){
    const o = items[i];
    if (o === item || !o.alive || o.animating) continue;
    if (o.p.z <= rz + 0.05) continue;             // сосед не ближе к игроку — не мешает
    const lim = (rr + o.r) * BONUS_OCC_K;
    const dx = o.p.x - rx; if (dx > lim || dx < -lim) continue;
    const dy = o.p.y - ry; if (dy > lim || dy < -lim) continue;
    if (dx*dx + dy*dy <= lim*lim) return false;   // накрыт в плоскости экрана
  }
  return true;
}
function isAccessible(item){
  ...
  if (level && level.bonus) return bonusAccessible(item);
  ...
  for (let k = 0; k < n; k++){
    ...
    for (let d = 0; d < SKY_DIRS.length; d++){
      ...
      // НЕБО». Rapier по умолчанию отдаёт попадания в сенсоры, а у нас
      // сенсором становится ВЕСЬ неактивный контейнер (бонусный на обычном
      // уровне и наоборот) плюс чаша во время разлёта. Бонусное дно — диск
      // радиуса 7.8 на высоте 6.1, то есть купол ровно над кучей: каждый луч
      // вверх упирался в него, `isAccessible` возвращал false ВСЕМУ, и на Hard
      // куча целиком уходила под вуаль (замер: ур.3 — 100 из 100 против 55).
...
function refreshAccessibilityNear(p, доп){
  // ⚠️⚠️ НА ВИТРИНЕ ЛОКАЛЬНЫЙ ПЕРЕСЧЁТ НЕВЕРЕН ПО ПОСТРОЕНИЮ. Его геометрия
  // («цилиндр по XZ вокруг точки, всё что ниже — не затронуто») выведена из
  // ВЕРТИКАЛЬНЫХ лучей: убрали группу — открылось небо тем, кто под ней. На
  // витрине открывается тот, кто ЗА снятой группой по Z, а он может быть где
  // угодно по высоте, в том числе НИЖЕ точки — гейт `it.p.y < p.y - 2` его бы
  // и отбросил. Полный обход тут дёшев (перекрытие — арифметика, а не касты).
  if (level && level.bonus) return refreshAccessibility();
  updateMatchRadius();
  accFlips = 0;
