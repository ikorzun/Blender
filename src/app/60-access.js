...
// ⚠️⚠️ СТРОГОСТЬ ПОДОБРАНА ЗАМЕРОМ, И 0.62 БЫЛО НЕ ПРОСТО «СЛОЖНО», А НЕЧЕСТНО.
// Тап ходит рейкастом по МЕШАМ: предмет, накрытый наполовину, игрок прекрасно
// видит и попадает по нему — а строгий порог объявлял его недоступным, то есть
// наказывал штрафом за попадание по видимому. Ящик глубиной в два слоя даёт
// «спереди» примерно половину кучи, и доля доступных обязана быть того же
// порядка. ЗАМЕР (Hard, ур.10/20/30, доля доступных | доступных пар):
//   0.62 → 22/24/23% | 13/16/15   (как на обычном уровне — витрина перестаёт
//                                  быть бонусом, ради которого владелец её просил)
//   0.50 → 32/32/37% | 27/21/33
//   0.42 → 43/43/36% | 47/34/24   ← взято
//   0.34 → 51/49/57% | 59/50/68   (уже почти «доступно всё»)
const BONUS_OCC_K = 0.42;   // доля суммы радиусов: насколько плотно надо накрыть
