...
function itemMaterial(t){
  ...
  if (CFG.matcap){
    ...
    mat = new THREE.MeshMatcapMaterial({
      ...
      // Пачке без своей картинки `packMatcap` отдаёт ТУ ЖЕ общую текстуру, что
      // и раньше, — по умолчанию не меняется ничего.
      matcap: (t.tex && !t.paint) ? packMatcap(t.tex, makeMatcap('tex'))
