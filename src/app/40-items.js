...
function itemMaterial(t){
  ...
  if (CFG.matcap){
    ...
    mat = new THREE.MeshMatcapMaterial({
      ...
      // ⚠️ У ТЕКСТУРНЫХ МАТЧЕП БЕРЁТСЯ ПО ПАЧКЕ (слово владельца 2026-08-17-к),
      // а сами картинки пачек — слово владельца 2026-08-18 «картинки берём».
      // ⚠️⚠️ ТРИ ЯРУСА, И ПОРЯДОК НЕСУЩИЙ, А ДВИЖОК ОДИН. Правка РЕДАКТОРА
      // (реестр `packMatcaps`, 10-stage) перебивает КАРТИНКУ ПАЧКИ
      // (`packMatcapTex`, 08-matcap-packs), а та — общий почти белый пресет
      // 'tex'. Пачке без картинки `packMatcapTex` честно отдаёт `null`
      // (08:151) и не бросает НИКОГДА — ни на незнакомой пачке, ни при
      // `t.tex === undefined`; пачке без правки редактора реестр отдаёт то,
      // что ему передали вторым аргументом. То есть по умолчанию не меняется
      // ничего, а каждый ярус включается только там, где для него есть данные.
      // ⛔ ЭТО БЫЛА ТОЧКА ВИЛКИ ДВУХ ПАРАЛЛЕЛЬНЫХ РЕАЛИЗАЦИЙ (реестр
      // диспетчера и слой Графики делались одновременно, каждая без другой).
      // Они оказались НЕ КОНКУРЕНТАМИ: реестр — рантайм-подмена для
      // редактора, слой — контент с числами владельца. Сращены здесь, ОДНОЙ
      // строкой; второго движка в игре нет.
      // ⚠️ `typeof packMatcapTex === 'function'` из веточной редакции СНЯТ
      // намеренно: это function declaration, в TDZ она не попадает никогда,
      // и проверка лишь маскировала бы настоящую поломку порядка модулей.
...
function dropOneFromSky(k, forcedTypeIdx){
  ...
  // ⚠️⚠️ СТАРТОВАЯ СКОРОСТЬ ВНИЗ — ЛЕЧЕНИЕ «ОЩУЩЕНИЯ ПАДЕНИЯ КАДРОВ» (слово
  // владельца 2026-08-21-д). Она НЕ косметика: замер показал, что кадр
  // проседает 34.7 -> 64.3 мс именно тогда, когда в воздухе висит до девяти
  // предметов разом. Вдвое более короткий полёт снимает и время, и наложение.
  // ⚠️ ОДНА ТОЧКА НА ВСЕ ТРИ ДОСЫПКИ (турбо, финальные пары, continue) — они
  // все ходят сюда; отдельная скорость у каждой разъехалась бы при первой правке.
  // ⚠️ Ниже терминальной `MAX_FALL` (16) — анти-туннельный предел не тронут.
...
function genLevel(){
  ...
  const pairsCnt = pairsForLevel(levelNum);   // прогрессия 40 -> 90 пар пар (00-config)
