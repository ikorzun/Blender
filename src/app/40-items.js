...
function genLevel(){
  ...
  const pairs = [];
  // ⚠️ КОГДА ХВОСТ ВООБЩЕ ОТКРЫВАЕТСЯ (сначала это, иначе будешь искать баг не
  // там, как искал я): `typesCount = min(TYPES.length, 9 + (levelNum − 1))`,
  // то есть 8 + номер уровня. Индекс 90 доступен с 82-го уровня, 91 — с 83-го,
  // 92 (`steak`) — только с 85-го. Замер на 84-м уровне НИЧЕГО не докажет:
  // там typesCount = 92 и стейка нет по прогрессии, а не по формуле.
  // ⚠️ ХВОСТ TYPES БЫЛ СТРУКТУРНО МЁРТВ (найдено ревью 2026-07-30): при
  // `i % typesCount` индекс i пробегает 0..pairsCnt−1, а pairsCnt ≤ PAIRS = 90,
  // поэтому типы с индексами ≥ 90 не спавнились НИКОГДА — включая `steak`,
  // собственную модель владельца. Правка по его спеке 2026-07-30.
  // ⚠️ НИЖЕ 82-ГО УРОВНЯ ПОВЕДЕНИЕ НЕ МЕНЯЕТСЯ БИТ-В-БИТ. Пока типов не больше
  // числа пар, работает прежняя ветка `i % typesCount` — там она и раздаёт
  // поровну, и ЧЕРЕДУЕТ типы. Это важно: чередование определяет ПОРЯДОК спавна,
  // а он определяет, что падает рядом.
  // ⚠️ ПОЧЕМУ НЕ `floor(i * typesCount / pairsCnt)` НА ВСЕХ УРОВНЯХ (первое,
  // что просится): при typesCount ≤ pairsCnt эта формула даёт СЕРИИ подряд
  // (тип 0 k раз, потом тип 1 k раз…). Одинаковые предметы спавнились бы
  // пачкой и ложились рядом — куча раздавала бы лёгкие матчи с первой секунды.
  // Это изменение баланса, которого никто не просил.
  // При typesCount > pairsCnt серий не бывает по построению: шаг больше
  // единицы, каждый i даёт СВОЙ тип. Поэтому равномерная развёртка безопасна
  // ровно там, где она и нужна.
  // ⚠️ ПЕРЕВЁРНУТУЮ РАЗВЁРТКУ (`typesCount−1−round(…)`) Я ПРОБОВАЛ И ОТВЕРГ ПО
  // ЗАМЕРУ — не изобретать заново. Гипотеза была: последний тип спавнится
  // последним, ложится наверх и его срезает trimOverfill. A/B по 6 раскладок
  // на ур.85/90/93: перевёрнутая 17/18, прямая 17/18 — РАЗНИЦЫ НЕТ, гипотеза
  // не подтвердилась. Оставлена простая прямая формула.
  for (let i=0;i<pairsCnt;i++){
    const type = typesCount <= pairsCnt
      ? i % typesCount                                             // ур. 1..81 — как было
      : Math.round(i * (typesCount - 1) / (pairsCnt - 1));         // ур. 82+
    pairs.push({ type, size: levelSize() });
  }
