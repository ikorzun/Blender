...
function setPackMatcap(pack, tex){
  ...
  try {
    for (const it of items){
      if (!it || !it.type || it.type.tex !== pack || !it.mesh) continue;
      const m = it.mesh.material;
      if (!m || !('matcap' in m)) continue;
      m.matcap = tex || базовая; тронуто++;
    }
  } catch (e) {}
  // ⚠️⚠️ ПОРТРЕТЫ ПАЧКИ — ОТДЕЛЬНЫЕ ПРЕДМЕТЫ СО СВОИМ МАТЕРИАЛОМ, И БЕЗ ЭТОГО
  // ЦИКЛА СБРОС СНИМКОВ НИЖЕ БЕССМЫСЛЕН. `thumbItemForKey` (85-hud) держит
  // готовый портретный предмет вечно, а `itemMaterial` дал ему СВОЙ материал —
  // в `items` его нет, цикл выше до него не достаёт. Пересняли бы тем же старым
  // материалом: карточка коллекции врала бы до перезагрузки (дефект
  // «протухающие портреты», слово владельца 2026-08-19 «почини»).
  // ⚠️ Правило то же, что у живых: `tex` или БАЗОВАЯ ПАЧКИ, не общий пресет.
  try {
    const портреты = (typeof thumbItemsOfPack === 'function') ? thumbItemsOfPack(pack) : [];
    for (const it of портреты){
      const m = it.mesh && it.mesh.material;
      if (!m || !('matcap' in m)) continue;
      m.matcap = tex || базовая;
    }
  } catch (e) {}
