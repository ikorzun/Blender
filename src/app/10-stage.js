...
(function buildSky(){
  ...
  const skyM = new THREE.ShaderMaterial({
    ...
    fragmentShader: [
      ...
      // ПОМОЛ: зеркальный красный у ВЕРХНЕЙ кромки (координата 1−sy), лесенка
      // угрозы. Наложен ДО затемнения верха — красный только темнит полосу,
      // поэтому контраст белого HUD растёт, а не падает.
      ...
      // не читаются на светлом небе (замер: фон 204, контраст 1.61:1 при
      // пороге 3:1). Экранная полоса, не всё небо — низ кадра не трогаем.
      '  col *= mix(1.0, ' + SKY_TOP_DIM.toFixed(3) + ', smoothstep(' + SKY_TOP_FROM.toFixed(3) + ', 1.0, sy));',
