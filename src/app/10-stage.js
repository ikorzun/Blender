...
// в тёмное время (ночь 18..5, как выбор панорамы в skyForNow), зелёный днём.
...
function feverColorNow(){
  ...
  const c = (h < 5 || h >= 18) ? FEVER_NIGHT : FEVER_DAY;
...
(function buildSky(){
  const skyM = new THREE.ShaderMaterial({
    ...
    fragmentShader: [
      ...
      // (драйв в 99-main). Наложен ДО затемнения верха — красный только темнит
