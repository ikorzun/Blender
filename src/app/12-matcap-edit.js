...
function mceAlphaИзДвижка(данные, S, база){
  ...
  const BS = (база.image.width || S) | 0;
  if (BS === S){
    for (let i = 3; i < данные.length && i < b.length; i += 4) данные[i] = b[i];
    return 'один-в-один';
  }
  ...
  for (let y = 0; y < S; y++){
    const by = Math.min(BS - 1, (y * BS / S) | 0);
    for (let x = 0; x < S; x++){
      const bx = Math.min(BS - 1, (x * BS / S) | 0);
      данные[(y * S + x) * 4 + 3] = b[(by * BS + bx) * 4 + 3];
    }
  }
  return 'ближайший ' + BS + '->' + S;
}
