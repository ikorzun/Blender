...
const TYPES = [
  ...
  // 24 зверя + 30 фруктов-овощей + 8 машин, замес 2:2:1 (спека владельца
  // 2026-07-21: «меньше машин, больше фруктов и животных»). Зверей в пачке
  // ровно 24 — все уже в игре, поэтому «больше» решается ДОЛЕЙ, а не числом.
  // Пицца убрана по просьбе владельца.
  // ⚠️ ПРОПОРЦИЯ И ПОРЯДОК ЗНАЧИМЫ: genLevel берёт ПЕРВЫЕ 9+уровень−1 типов,
  // значит стартовая девятка — это то, что игрок видит в первую минуту.
  // Сейчас в ней 4 зверя, 4 фрукта и 1 машина. Внутри пачек впереди стоят
  // самые РАЗЛИЧИМЫЕ: матч идёт ПО ТИПУ, и путать их на старте нельзя
  // (бурые лохматики — бобёр, олень, заяц, кабан — уведены в хвост).
  // Процедурные формы ниже всплывают только с 63-го уровня.
