...
const Ads = (function(){
  ...
  let lbLastRaw = null;             // сырой ответ сервера — для разбора, когда узнаем форму
  ...
  function submitLeaderboardScore(){
    ...
    try {
      window.bridge.leaderboards.setScore(lbBoardId, score).then((res)=>{
        ...
        // Поэтому здесь НЕЛЬЗЯ писать «сохранено».
        lbLastRaw = res;
        // [?] ТОЧКА РАЗБОРА: форму успешного ответа Bridge не нормализует и до
        // первого прогона с токеном она неизвестна. Как увидим — здесь появится
        // проверка признака приёма; до тех пор только сохраняем сырое.
        Telemetry.ev('lb', { s: score, r: res && typeof res === 'object' ? 1 : 0 });
      }).catch(()=>{
  ...
  return {
    ...
    get lbRaw(){ return lbLastRaw; },        // [?] форму узнаем на живом прогоне
